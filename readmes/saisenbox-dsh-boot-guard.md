...
它不会删除插件，也不会碰插件的数据。所谓“跳过”，只是向当前 DSH profile 的 `cordis.patch.yml` 追加一个带 Boot Guard 标记的 `disabled: true` 配置块；恢复时也只会删除自己写入的块。
...
- 写接口只接受同源 `POST` + JSON 请求
- 校验请求体大小、条目数量和 ID 格式
- Boot Guard 不能把自己设为跳过
- 配置修改串行执行，并通过同目录临时文件原子替换
