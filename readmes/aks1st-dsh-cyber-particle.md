...
内置「设置 → 粒子背景」页：可实时调节粒子数量、半径、线条粗细、连线距离、移动速度，用调色板改粒子/线条颜色，一键重置为默认；调整即时生效并持久化到 `$DSH_HOME/cyber-particle.json`，重启后自动恢复。设置页文案跟随 DSH 语言设置（中文 / English）。
...
设置写入 `$DSH_HOME/cyber-particle.json`，重启 dsh web 后自动恢复。
