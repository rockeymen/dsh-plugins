...
| 端 | 位置 | 作用 |
|---|---|---|
| Host（Node half） | `host/lib/index.js`（`main` / `exports["."]`） | 注册 `POST /api/filetree` HTTP 路由（list/read/write，基于 fs 服务） |
| Client（UI half） | `client/lib/client.js`（`exports["./client"]` + `dsh.client.platform`） | 会话视图"文件"页签：目录树 + VSCode 风格编辑器 |
