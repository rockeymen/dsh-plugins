...
![DSH](https://img.shields.io/badge/DSH-DeepSeek%20Harness-blue) ![plugins](https://img.shields.io/badge/plugins-700%2B-green) ![protocol](https://img.shields.io/badge/protocol-%40deepseek--ai%2Fcordis-orange)
...
├── README.md              ← 本文件
├── AGENTS.md              ← 仓库操作手册
├── GAP-LEDGER.md          ← 差距总账（状态机）
├── swarm/
│   └── ledger/            ← 差距条目 GAP-xxxx.md（含对照项目证据）
├── plugins/               ← DSH 插件实现（按包组织，每个差距一个包）
├── e2e/                   ← 端到端验证（挂载插件 + 注册校验 + LLM 冒烟）
└── docs/
    ├── architecture.md    ← 插件设计规范（接缝三件套）
    └── performance.md     ← 性能基线
```
