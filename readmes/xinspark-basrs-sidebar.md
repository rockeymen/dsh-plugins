...
| Level 1 | Panels |
| --- | --- |
| Sessions (会话管理) | Overview |
| Functions (功能区) | Overview · Trajectory |
...
| Option | Description |
| --- | --- |
| Open right sidebar by default | Auto-expand the sidebar on load; manual collapse is never overridden. |
| Hide duplicated session topbar elements | Master switch; when on, choose which of the following to hide: **session log · session mode · subagents · background jobs · chat/trajectory tab bar**. |
| Show workspace/session breadcrumb | The breadcrumb on the left of the session topbar; off restores the native topbar. |
