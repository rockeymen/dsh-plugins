...
```
dsh-ocr-plugin/
├── lib/index.js            # Cordis 插件本体（@deepseek-ai/dsh-ocr，提供 ocr 服务）
├── tools/ocr_image.py      # 快速 OCR 脚本：版面理解 + 阅读顺序 + 表格还原
├── tools/paste-image-plugin.json  # [可选] 粘贴图片自动保存插件（动态插件参考实现）
├── scripts/install.sh      # 一键安装到 DSH profile
├── scripts/download-models.sh    # 下载深度 OCR 模型（GGUF）
└── docs/adapter-seam.md    # 适配器 OCR 缝的接口契约（自打补丁用）
```
...
| 组件 | 必需 | 说明 |
| --- | --- | --- |
| DeepSeek Harness | ✅ | 部署含 `dsh-llm-deepseek` 适配器 |
| 适配器 OCR 缝 | ✅ | 见 [docs/adapter-seam.md](docs/adapter-seam.md)（约 30 行本地改动） |
| Node.js ≥ 18 | ✅ | 运行 harness |
| Python 3.10+ | ✅ | 快速 OCR 通道（`pip install rapidocr_onnxruntime onnxruntime pillow numpy opencv-python`） |
| llama.cpp（`llama-mtmd-cli`） | ⭕ 深度通道 | 需支持 `--jinja`（llama.cpp 较新版本）；也可只装快速通道 |
| DeepSeek-OCR-2 模型 | ⭕ 深度通道 | GGUF 约 1.6GB + mmproj 约 0.5GB |
