...
- **Package**: [dsh-lan-gateway](packages/dsh-lan-access) · **npm**: `dsh-lan-gateway` · **What it does**: **LAN / remote access gateway** for the Web UI: 0.0.0.0 binding, `crypto.randomUUID` polyfill, **token gate** (401 login page + WebSocket interception, loopback exempt), privileged-fence + settings-persistence exemptions, idempotent installer + `reapply-lan-patches.sh` recovery. Trusted networks only.
- **Package**: [dsh-vision-tool](packages/dsh-vision) · **npm**: `dsh-vision-tool` · **What it does**: Registers a `vision` tool for agents: describe local images via **any OpenAI-compatible vision endpoint you configure** — no built-in keys, optional multi-model `cross_check` to guard against hallucinations.
- **Package**: [dsh-mobile-ui](packages/dsh-mobile-ui) · **npm**: `dsh-mobile-ui` · **What it does**: Mobile UI for the Web GUI (≤768px): full-width responsive layout, overlay session drawer, 44px touch targets, safe-area support, reading enhancements — zero desktop impact.
