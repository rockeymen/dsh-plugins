...
### File · Responsibility
- **File**: `src/index.ts` · **Responsibility**: host half: `VisionService extends TypertRemoteService` — `@Remote` `queue`/`config`/`status` + `agent/pre-step` & `llm/stream` listeners + GLM call
- **File**: `src/client/index.ts` · **Responsibility**: browser half: `conversation.input.dock` slot — paste capture, submit interception, model/API-key settings menu
- **File**: `src/patch.ts` + `scripts/apply-dsh-patch.mjs` · **Responsibility**: self-healing dsh attachment-authorization patch
- **File**: `src/types.ts` · **Responsibility**: cross-face wire types (lossless JSON)
- **File**: `cordis.patch.yml` · **Responsibility**: bundle composition layer
- **File**: `lib/` · **Responsibility**: build output (published)
