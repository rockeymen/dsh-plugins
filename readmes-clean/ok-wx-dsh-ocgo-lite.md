...
聊天输入框下方（composer dock）**常驻展开**显示 OpenCode Go 套餐余量、token 消耗与花费，零外部依赖。
  ...
  ![常驻条总览](screenshots/overview.png)
...
点击弹出账户卡片：登录状态、套餐、配额概览、API Key 掩码 + **一键复制**（复制成功有 toast 提示）。
  ...
  ![GO 账户卡片](screenshots/go-account.png)

### 滚动 / 周 / 月：套餐配额

官方配额百分比圆环（健康色：蓝/黄/红），点击弹出三窗口进度条 + 重置倒计时。

  ![配额详情](screenshots/quota.png)
...
### 数据 · 来源 · 口径
- **数据**: **配额余量** · **来源**: 官方 `https://opencode.ai/zen/go/v1/usage`（Bearer auth.json key） · **口径**: **账户级**（含其他设备/软件），不受范围切换影响
- **数据**: **token / 花费** · **来源**: DSH 会话事件（`assistant/message` 的 usage，按 provider 分组统计） · **口径**: **DSH 会话全部 provider**（opencode-go / deepseek-official / 其他套餐，自动识别切换），范围=全部/本次会话/最近对话
- **数据**: **金额** · **来源**: 按官方定价表估算（per 1M tokens） · **口径**: 输入 $0.14 / 输出 $0.28 / 缓存读 $0.0028（deepseek-v4-flash）；**无官方定价的模型金额不计入，显示「定价未知」**
