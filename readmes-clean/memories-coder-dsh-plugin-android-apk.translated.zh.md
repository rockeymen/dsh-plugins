...
### 参数 · 说明 · 默认
- **参数**: `project` · **说明**: 安卓工程文件夹路径（相对路径基于会话工作目录） · **默认**: 必填
- **参数**: `variant` · **说明**: Gradle 变体（debug / release） · **默认**: `debug`
- **参数**: `clean` · **说明**: 是否先 clean 全量重建 · **默认**: `false`
- **参数**: `downloadDir` · **说明**: 工具链下载目录 · **默认**: `<workspace>/.android-build`
- **参数**: `apkOutputDir` · **说明**: APK 复制目录 · **默认**: `<workspace>/apk`
- **参数**: `gradleVersion` · **说明**: 无可用 Wrapper 时下载的 Gradle 版本 · **默认**: `8.9`
- **参数**: `compileSdk` · **说明**: 覆盖自动检测的 compileSdk · **默认**: 自动检测，未知时 34
...
### 组件 · 默认源 · 回退镜像
- **组件**: JDK (Temurin) · **默认源**: api.adoptium.net · **回退镜像**: mirrors.tuna.tsinghua.edu.cn/Adoptium
- **组件**: Gradle · **默认源**: services.gradle.org · **回退镜像**: mirrors.cloud.tencent.com/gradle、mirrors.aliyun.com
- **组件**: Android cmdline-tools · **默认源**: dl.google.com · **回退镜像**: mirrors.cloud.tencent.com/AndroidSDK
