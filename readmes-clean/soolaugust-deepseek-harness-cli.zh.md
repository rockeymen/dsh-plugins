...
### 操作 · 效果
- **操作**: 输入文字 + `Enter` · **效果**: 发送给 agent
- **操作**: `↑` / `↓` · **效果**: 历史导航（跨会话持久）
- **操作**: `Shift+Tab` · **效果**: 循环权限 preset
- **操作**: `Ctrl+O` · **效果**: 展开/收起工具组
- **操作**: `Ctrl+C` · **效果**: 运行中取消当前回合；提示符下退出
- **操作**: `/exit` `/help` `/clear` `/session` `/model` `/permission` · **效果**: slash 命令
