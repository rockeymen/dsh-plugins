![Reasonix](docs/logo-ghost-wave-effect.svg)
...
开源 · MIT · 单个 Go 二进制
### 可以一直开着跑的编码 Agent。
一套本地引擎,四个入口——终端、桌面端、浏览器,或通过 ACP 接入你的编辑器。计划模式、权限、工作区沙箱与逐轮 checkpoint,让长时间自治运行始终可读、可撤销。
...
### 平台 · 安装包 · 架构
- **平台**: macOS · **安装包**: 通用 `.dmg` 或 `.zip` · **架构**: Apple Silicon / Intel
- **平台**: Windows · **安装包**: 安装器 `.exe` 或便携 `.zip` · **架构**: x64 / ARM64
- **平台**: Linux · **安装包**: `.deb` 或 `.tar.gz` · **架构**: x64
...
## Star 趋势
   ...
   ![](https://raw.githubusercontent.com/esengine/DeepSeek-Reasonix/star-history/assets/star-history/star-history-light.svg)
