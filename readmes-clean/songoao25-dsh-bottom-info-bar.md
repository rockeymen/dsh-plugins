...
### Symptom · Fix
- **Symptom**: Bar does not appear after a page refresh · **Fix**: **Restart** `dsh web` (the host process loads plugins)
- **Symptom**: Balance shows "DEEPSEEK_API_KEY not configured" · **Fix**: Add the key under Settings → Models
- **Symptom**: Balance shows "⚠ refresh failed, showing last snapshot" · **Fix**: Transient network/key issue; retries automatically after 60 s. The last successful data is kept so the bar never goes blank. **Hover over the warning** for a detailed explanation and retry timing.
- **Symptom**: Shows "OpenCode Go not configured" · **Fix**: Add `OPENCODE_GO_API_KEY` under Settings → Models, or configure OpenCode Go in the opencode CLI
- **Symptom**: How do I bind my ChatGPT subscription? · **Fix**: Install the companion plugin **dsh-chatgpt-subscription** and authorize on the official page — it maintains the token this bar reads for quota display
- **Symptom**: ChatGPT quotas look wrong or empty · **Fix**: The wham endpoint is undocumented and may change; failures keep the last snapshot and retry every 60 s. Hover over "⚠ refresh failed" to see the retry explanation.
- **Symptom**: Why does compact mode show a different window? · **Fix**: Compact mode prioritizes the shortest-duration window (5-hour > weekly > monthly) because it refreshes fastest. If the 5-hour window is unavailable, it falls back to weekly or monthly. **Quota and countdown always match** — both come from the same window.
- **Symptom**: Why is the model's reasoning process not shown? · **Fix**: DSH does not render the model's internal reasoning in the UI — a DSH interface-layer limitation, not the plugin's
- **Symptom**: Want the original stats row back · **Fix**: Uninstall the plugin and restart
