...
### 插件 · 功能
- **插件**: **dsh-task-board** · **功能**: 侧边栏任务看板：多列 kanban、真实执行（驱动 agent 会话）、定时执行（cron）
- **插件**: **dsh-ssh** · **功能**: SSH 运维：主机管理、远程执行、SFTP 传输、隧道、集群并发、Web 终端
- **插件**: **dsh-aionui-panel** · **功能**: 右侧「预览 / 文件 / 变更」面板：文件树、多格式预览、git 操作、文件拖拽
- **插件**: **dsh-git-graph** · **功能**: 会话头部 git 分支选择器 + 提交图
- **插件**: **dsh-remote-web-ui** · **功能**: 移动端远程控制：扫码配对、设备限额、cloudflared 隧道、手机端 SPA
- **插件**: **dsh-live-stats** · **功能**: 实时 token 估算与生成速度
- **插件**: **dsh-full-stats** · **功能**: 完整统计行：覆盖官方统计（不省略）+ 运行状态指示 + 自定义状态文本
- **插件**: **dsh-web-ui-settings** · **功能**: 设置页「Web UI 插件」配置组 + 「人格设定」+「关于」版权页
- **插件**: **dsh-web-ui-all** · **功能**: 聚合包：一键安装全部插件
...
- 构建：`pnpm -r build`（或 `pnpm --filter  build`）
