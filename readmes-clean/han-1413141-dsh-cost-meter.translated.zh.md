...
### 功能 · 位置 · 说明
- **功能**: 本会话费用 · **位置**: 输入区下方 / 会话标题栏 · **说明**: 实时累计费用 + 输入/缓存/输出 token,位置可配
- **功能**: 官方余额 · **位置**: 侧边栏顶部 / 设置页(可配) · **说明**: 总余额 / 赠送 / 充值,自动刷新 + 手动刷新
- **功能**: 当日费用 · **位置**: 侧边栏底部(设置按钮上方) · **说明**: 「今日 ¥x」,悬停见调用次数与 token 明细
- **功能**: 预算图框 · **位置**: 侧边栏底部(余额行与设置按钮之间) · **说明**: 圆角方形图框:预算、已用%、进度条、今日费用与占预算%、已用/额度,≥80% 预警、≥100% 超支
- **功能**: 汇总卡片 · **位置**: 设置页 · **说明**: 今日 / 本月 / 累计费用与调用次数
- **功能**: 今日会话明细 · **位置**: 设置页 · **说明**: 每个会话的调用次数、输入/缓存/输出 token 与费用
- **功能**: 历史记录 · **位置**: 设置页 · **说明**: 按天汇总,保留天数可配(默认 180 天)
- **功能**: 预算设置 · **位置**: 设置页顶部 · **说明**: 额度、周期(今日/本月/累计/自定义日期区间)、已用%
- **功能**: 价格表 · **位置**: 设置页 · **说明**: 每模型 基础/谷时/峰时 三档价格,增删改自由
- **功能**: 峰谷计价 · **位置**: 设置页 · **说明**: DeepSeek 官方峰谷方案,带生效时间门控与当前档位状态
- **功能**: 官方价格同步 · **位置**: 设置页 · **说明**: 抓取解析官方定价页,一键应用
- **功能**: 界面语言 · **位置**: 设置页 → 显示设置 · **说明**: 简体中文 / English / 跟随浏览器(自动);切换即时生效并自动保存
- **功能**: AI 价格同步 · **位置**: [提示词](docs/AI-PRICE-SYNC-PROMPT.md) · **说明**: 交给任意 AI 自主同步多模型、分时价格
...
### 输入区下方 · 会话标题栏
- **输入区下方**: ![会话 dock](docs/screenshot-session-dock.png) · **会话标题栏**: ![会话标题栏](docs/screenshot-session-header.png)
