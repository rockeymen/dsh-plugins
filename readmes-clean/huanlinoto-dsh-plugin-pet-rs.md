...
### 操作 · 效果
- **操作**: **单击鲸鱼** · **效果**: 延迟 250ms 后打开 DSH GUI（浏览器）
- **操作**: **双击鲸鱼** · **效果**: 打开内嵌设置面板（取消单击）
- **操作**: **按住鲸鱼拖拽** · **效果**: 移动桌宠窗口位置
- **操作**: **滚轮（气泡上）** · **效果**: 滚动会话列表
...
### 操作 · 效果
- **操作**: 点击输入框 · **效果**: 获得焦点，可编辑文本
- **操作**: 输入字符 · **效果**: 实时显示（支持中文 IME）
- **操作**: `Enter` · **效果**: 提交地址 → 热切换网络连接
- **操作**: `Esc` · **效果**: 恢复原值并失焦（不关闭面板）
- **操作**: 点击输入框外 · **效果**: 提交地址并失焦
- **操作**: `←` / `→` · **效果**: 移动光标
- **操作**: `Home` / `End` · **效果**: 跳到行首 / 行尾
- **操作**: `Backspace` · **效果**: 删除光标前字符
- **操作**: `Delete` · **效果**: 删除光标后字符
- **操作**: `Ctrl+A` · **效果**: 全选
- **操作**: `Ctrl+C` / `Ctrl+X` · **效果**: 复制 / 剪切选中文本
- **操作**: `Ctrl+V` · **效果**: 粘贴剪贴板内容
- **操作**: `Ctrl+Z` · **效果**: 撤销上次编辑
...
### 菜单项 · 效果
- **菜单项**: 打开 DSH GUI · **效果**: 在浏览器打开 DSH
- **菜单项**: 隐藏/显示气泡 · **效果**: 切换状态气泡可见性
- **菜单项**: 状态提示音 · **效果**: 勾选框，切换声音并持久化
- **菜单项**: 测试提示音 · **效果**: 播放 done 音效
- **菜单项**: 放大 / 缩小 / 重置 · **效果**: 调节鲸鱼大小（50%–110%，默认 67%）
- **菜单项**: 退出桌宠 · **效果**: 保存配置并退出
...
### 平台 · 路径
- **平台**: Windows · **路径**: `%APPDATA%\dsh\pet\config.json`
- **平台**: macOS · **路径**: `~/Library/Application Support/dsh/pet/config.json`
- **平台**: Linux · **路径**: `~/.config/dsh/pet/config.json`
...
### 平台 · 状态 · 备注
- **平台**: Windows · **状态**: ✅ 完整 · **备注**: DWM per-pixel alpha 透明窗
- **平台**: macOS · **状态**: ✅ 完整 · **备注**: NSWindow 透明 + Dock 隐藏
- **平台**: Linux (X11) · **状态**: ✅ 完整 · **备注**: 透明窗 + 置顶 + skip-taskbar
- **平台**: Linux (Wayland) · **状态**: ⚠️ 降级 · **备注**: 不置顶、不跨全屏；建议用 XWayland
