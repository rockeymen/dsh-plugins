...
- **Catalog entry**: **Automatic install** · **What the scanner proved**: Every install target (entry files, patch) exists in the pinned commit's git tree · **What happens on install**: `pnpm add --ignore-scripts` — third-party lifecycle scripts never run
- **Catalog entry**: **Needs script review** · **What the scanner proved**: Targets are absent (build output not shipped), but the package declares lifecycle scripts · **What happens on install**: Scripts are shown **verbatim** in the review step; after your explicit consent, the Host installs once with `--allow-build=<name>` instead of `--ignore-scripts`, scoping script execution to exactly the reviewed package within that single invocation. Nothing is written to `allowBuilds`; consent is never persisted
- **Catalog entry**: **Manual install** · **What the scanner proved**: Neither of the above · **What happens on install**: Repository link only; the marketplace never runs anything
