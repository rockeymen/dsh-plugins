...
### 识 · 梵名 · 在 harness 中
- **识**: 眼识 · **梵名**: cakṣur-vijñāna · **在 harness 中**: 它去看的：文件内容、检索结果、渲染输出
- **识**: 耳识 · **梵名**: śrotra-vijñāna · **在 harness 中**: 别人告诉它的：用户消息、评审意见
- **识**: 鼻识 · **梵名**: ghrāṇa-vijñāna · **在 harness 中**: 没人说但它嗅到的：代码坏味道、过期配置、漂移的状态
- **识**: 舌识 · **梵名**: jihvā-vijñāna · **在 harness 中**: 尝自己做出来的东西：测试、构建、回读自己的 diff
- **识**: 身识 · **梵名**: kāya-vijñāna · **在 harness 中**: 世界的直接抵抗：非零退出、写入失败、超时
- **识**: 意识 · **梵名**: mano-vijñāna · **在 harness 中**: 分别本身：规划、判断、决定一个结果意味着什么
- **识**: 末那识 · **梵名**: kliṣṭa-manas · **在 harness 中**: 恒审思量的我执，量化为四种偏差
- **识**: 阿赖耶识 · **梵名**: ālaya-vijñāna · **在 harness 中**: 持久的种子藏，被熏习、又现行
...
###  · 观测代理 · 对治动作
- 我痴 · **观测代理**: 未经查验就断言的主张占比 · **对治动作**: 去跑那个能证伪它的检查，再重述
- 我见 · **观测代理**: 证据已否定的做法仍在重试 · **对治动作**: 说出这个立场、说出否定它的证据，换个形状
- 我慢 · **观测代理**: 连胜未断；被纠正时先辩解 · **对治动作**: 先找反例再汇报；先承认纠正
- 我爱 · **观测代理**: 拿自己先前的输出当独立证据 · **对治动作**: 引一手来源，不引自己写的摘要
...
### 工具 · 用途
- **工具**: `self_reflect` · **用途**: 读当前状态，以及指定情境下的种子
- **工具**: `self_appraise` · **用途**: 记录"这件事在我这里怎么落地"——harness 观测不到的那一半
- **工具**: `self_recall` · **用途**: 重复某个做法之前，先在藏识里查前例
- **工具**: `self_transform` · **用途**: 转依：把烦恼转成对应的智，并落到一个承诺动作
