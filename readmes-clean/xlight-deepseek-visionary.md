...
graph TD
    ...
    subgraph DSH[DeepSeek Harness]
        DP["@xlight-oss/visionary-dsh 插件deepseek_vision 等 4 个原生工具"]
        DP -->|宿主进程 spawn| SRV
    end
    subgraph visionary-server 原生二进制
        SRV["CLI + MCP stdio 服务vision / status / login / logout / skill / init / doctormcp-stdio CLI"]
        CFG["~/.deepseek-visionary/config.jsontoken + smidV2 + cf_clearance + 会话"]
        SRV --> CFG
    end
    SRV -->|HTTPS| DS["DeepSeek 网页后端"]
    SRV -->|CDP 启动 + 监听| BRO["Chrome 系浏览器仅登录时出现"]
```
...
### Agent · 文档 · 一键命令
- **Agent**: Zed · **文档**: [zed.md](docs/integrations/zed.md) · **一键命令**: 扩展市场安装（见下）
- **Agent**: OpenCode · **文档**: [opencode.md](docs/integrations/opencode.md) · **一键命令**: `visionary-server init opencode`
- **Agent**: Codex · **文档**: [codex.md](docs/integrations/codex.md) · **一键命令**: `visionary-server init codex`
- **Agent**: Claude Code · **文档**: [claude-code.md](docs/integrations/claude-code.md) · **一键命令**: `visionary-server init claude`
- **Agent**: Cursor · **文档**: [cursor.md](docs/integrations/cursor.md) · **一键命令**: `visionary-server init cursor`
- **Agent**: Claude Desktop · **文档**: [claude-desktop.md](docs/integrations/claude-desktop.md) · **一键命令**: `visionary-server init claude-desktop`
- **Agent**: DeepSeek Harness · **文档**: [deepseek-harness.md](docs/integrations/deepseek-harness.md) · **一键命令**: 原生插件 `dsh plugin --profile web add @xlight-oss/visionary-dsh`（推荐）或 `visionary-server init dsh`（skill + CLI 轻量接入）
...
- **CLI**：`visionary-server vision ` 识图（详见下文「CLI 工具」）
...
### 命令 · 说明
- **命令**: `visionary-server`（无参数） · **说明**: 输出 help 用法信息并退出码 2（不进入任何模式）
- **命令**: `visionary-server --version` · **说明**: 输出版本号
- **命令**: `visionary-server mcp-stdio` · **说明**: 显式启动 MCP stdio 服务（MCP 模式入口，所有 agent 配置均以此启动）
- **命令**: `visionary-server vision ` · **说明**: 用视觉模型分析图片（CLI 版 `deepseek_vision`）。`image` 支持路径 / base64 / data URI / `-`（stdin）；`--prompt` / `--thinking` / `--continue` / `--session-id` / `--json` / `--stream` / `--no-stream`
- **命令**: `visionary-server status` · **说明**: 轻量鉴权状态检查（CLI 版 `deepseek_vision_status`），`--json` 输出结构化状态
- **命令**: `visionary-server login` · **说明**: 浏览器自动登录（CLI 版 `deepseek_vision_login`）
- **命令**: `visionary-server logout` · **说明**: 清除保存的凭据（CLI 版 `deepseek_vision_logout`）
- **命令**: `visionary-server skill install` · **说明**: 安装 agent 调用契约 skill 到 `~/.agents/skills/`（内嵌于二进制）
- **命令**: `visionary-server doctor` · **说明**: 诊断环境：config 路径/权限、浏览器、token 有效性、平台
- **命令**: `visionary-server init [agent]` · **说明**: 检测并接入已安装的 AI agent（`--dry-run` / `--yes` / 多选 flags，含 `dsh`）
...
### 场景 · 默认行为 · 消费方
- **场景**: 终端（TTY） · **默认行为**: 流式打印回答文本 · **消费方**: 人
- **场景**: 管道/脚本（非 TTY） · **默认行为**: 一次性输出完整文本 · **消费方**: 脚本兑底
- **场景**: `visionary-server vision img.png --json` · **默认行为**: 原子 JSON：`{"text", "session_id", "parent_message_id"}`（失败为 `{"error"}`） · **消费方**: 脚本 / AI agent（推荐）
...
### 工具 · 说明
- **工具**: `deepseek_vision` · **说明**: 上传本地图片（路径 / base64 / data URI）并用 DeepSeek 视觉模型分析。参数：`image`（必填）、`prompt`、`thinking`、`continue_conversation`、`session_id`
- **工具**: `deepseek_vision_status` · **说明**: 检查登录状态与 token 有效性（含真实校验探针）
- **工具**: `deepseek_vision_login` · **说明**: 浏览器自动登录并抓取凭据（阻塞，超时可配）
- **工具**: `deepseek_vision_logout` · **说明**: 清除保存的凭据
...
### 变量 · 说明
- **变量**: `DEEPSEEK_USER_TOKEN` · **说明**: 覆盖 config.json 中的 token（可选）
- **变量**: `DEEPSEEK_SMIDV2` / `DEEPSEEK_CF_CLEARANCE` · **说明**: 覆盖对应 cookie（可选）
- **变量**: `DEEPSEEK_BASE_URL` · **说明**: API 基地址（默认 `https://chat.deepseek.com`）
- **变量**: `DEEPSEEK_LOGIN_TIMEOUT` · **说明**: 登录等待超时秒数（默认 600）
- **变量**: `DEEPSEEK_VISIONARY_BIN` · **说明**: DSH 插件解析二进制路径（`Config.binaryPath` → 此变量 → PATH）
