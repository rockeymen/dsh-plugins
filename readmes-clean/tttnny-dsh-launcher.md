...
### 菜单项 · 作用
- **菜单项**: 状态行 · **作用**: 服务是否运行（launchd 托管 / 外部实例占用 3080）
- **菜单项**: 打开 Web UI (⌘O) · **作用**: 浏览器打开 http://127.0.0.1:3080
- **菜单项**: 安装 dsh（仅未安装时显示） · **作用**: 首次使用点击安装，弹窗实时显示下载进度，装完自动启动服务
- **菜单项**: 重启服务 · **作用**: 永远可用：运行中=重启；启动失败/未运行=直接启动；端口被外部实例占用=弹窗说明占用者
- **菜单项**: 登录时自动启动本 App · **作用**: 登录后自动出现菜单栏图标并拉起服务
- **菜单项**: 打开数据目录 · **作用**: `~/.dsh`
- **菜单项**: 退出 · **作用**: 退出 App **并停止服务**（`launchctl bootout`，数据已落盘，下次打开 App 即恢复）
