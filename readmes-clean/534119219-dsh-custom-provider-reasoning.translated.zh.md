...
### 场景 · 是否注入
- **场景**: 自定义路由（`declared`）上的模型，无 `reasoningEfforts` · **是否注入**: ✅ 注入
- **场景**: 目录路由上、pi-ai 目录未收录的模型（`scope: all`） · **是否注入**: ✅ 注入
- **场景**: 已有 `reasoningEfforts` 的模型（手调字典或显式 `false`） · **是否注入**: ❌ 不覆盖 reasoningEfforts，但仍补缺失的 maxTokens/contextWindow
- **场景**: 缺 `maxTokens` 的模型 · **是否注入**: ✅ 补 `maxTokens`（默认 384000，可配）
- **场景**: 缺 `contextWindow` 的模型 · **是否注入**: ✅ 补 `contextWindow`（默认 1000000，可配）
- **场景**: pi-ai 目录已收录的模型（目录自带推理元数据） · **是否注入**: ❌ 交给目录
