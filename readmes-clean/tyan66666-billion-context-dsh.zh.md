...
Billion-Context for [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)
...
### ACP 概念 · DSH 实现
- **ACP 概念**: `compress` 工具遮蔽一段范围 · **DSH 实现**: 持久化 `surfaceOp: { op: 'replace' }`——模型书写的摘要成为 checkpoint 节点；原文保留在日志中
- **ACP 概念**: refs（`m00001` 标签） · **DSH 实现**: surface seq，由 nudge 的可压缩范围表携带
- **ACP 概念**: nudge（"考虑压缩一下"） · **DSH 实现**: 由内核的压力决策在 `agent/pre-step` 注入——简短建议，绝非命令
- **ACP 概念**: `decompress` · **DSH 实现**: 从日志只读恢复被遮蔽的原文
- **ACP 概念**: `search_context` · **DSH 实现**: 对从日志重建的块摘要与原文打分
- **ACP 概念**: `acp_status` · **DSH 实现**: 块账本与上下文压力
- **ACP 概念**: 块状态 · **DSH 实现**: 内存内核状态 + **日志重建账本**（无旁车文件）
- **ACP 概念**: 分层蒸馏（T2/T3） · **DSH 实现**: 再次压缩某块的摘要节点 = 蒸馏该块（tier 2），蒸馏 tier-2 块得 tier 3；tier 与内核块 id 持久化进日志，重启后内核状态从日志再水合、可继续蒸馏
...
### 工具 · 作用
- **工具**: `compress` · **作用**: 用你书写的紧凑摘要替换 seq 范围（边界自动平衡到 tool-call/result 配对点）；对某块的摘要节点再次压缩 = 分层蒸馏（tier 2/3）
- **工具**: `decompress` · **作用**: 恢复已压缩块的原始内容（只读）
- **工具**: `search_context` · **作用**: 按关键词搜索压缩块摘要与原文
- **工具**: `acp_status` · **作用**: 上下文占用、压缩块、可压缩范围
- **工具**: `/acp` · **作用**: 从命令栏执行 status / compress / decompress
...
### 上游项目 · 作者 · 角色
- **上游项目**: **[billion-context-pi](https://github.com/ranxianglei/billion-context-pi)** · **作者**: [ranxianglei](https://github.com/ranxianglei) · **角色**: 本项目移植的 Pi 编码代理适配器；适配器设计、工具语义与本项目默认配置的来源
- **上游项目**: **[acp-kernel](https://github.com/ranxianglei/acp-kernel)** · **作者**: [ranxianglei](https://github.com/ranxianglei) · **角色**: 框架无关的上下文压缩引擎——**原样复用**（refs、blocks、tiers、nudge 决策、search、status）
- **上游项目**: **[opencode-acp](https://github.com/ranxianglei/opencode-acp)** · **作者**: [ranxianglei](https://github.com/ranxianglei) · **角色**: ACP（"模型决定何时压缩、压缩什么"）设计的源头
- **上游项目**: **[DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)** · **作者**: DeepSeek AI · **角色**: 本项目所扩展的宿主平台（compaction 能力接缝、agent preset、持久化会话日志）
...
### 键 · 默认值 · 含义
- **键**: `modelContextLimit` · **默认值**: 自动探测（回退 `128000`） · **含义**: 用于内核压力决策的上下文窗口；显式配置时优先且跳过探测
- **键**: `autoModelContextLimit` · **默认值**: `true` · **含义**: 从模型 API 自动探测真实窗口（`agent.ctx.llm.resolveModelInfo`）；探测失败回退默认值，`acp_status` 展示窗口来源
- **键**: `nudgeMinContextLimitPct` · **默认值**: 内核默认 `0.45` · **含义**: Nudge 窗口下界（用量占比）——仅作配置校验，增长路径的触发没有百分比下限——与 billion-context-pi 相同的默认值
- **键**: `nudgeMaxContextLimitPct` · **默认值**: engine 默认 `0.70`（内核/pi 默认 `0.75`） · **含义**: 过限线：超过此值则无论增长与否都触发 nudge——刻意低于宿主 compaction-basic 的 80% 自动压缩线，保证强制 nudge 先触发；显式配置优先
- **键**: `nudgeEmergencyThresholdPct` · **默认值**: engine 默认 `0.85`（内核/pi 默认 `0.95`） · **含义**: 紧急 nudge（绕过每轮去重）——从 `0.95` 下调：95% 时模型已无操作空间且会被 80% 自动压缩线遮蔽；显式配置优先
- **键**: `coreOverrides` · **默认值**: — · **含义**: 任何其他 acp-kernel `Config` 覆盖（billion-context-pi 的 `coreOverrides` 逃生口）
- **键**: `autoTools` · **默认值**: `true` · **含义**: 在 `ctx.tools` 注册四个模型工具
- **键**: `autoCommand` · **默认值**: `true` · **含义**: 在 `ctx.commands` 注册 `/acp` 命令
- **键**: `autoNudge` · **默认值**: `true` · **含义**: 当内核建议时向 `agent/pre-step` 注入 nudge
