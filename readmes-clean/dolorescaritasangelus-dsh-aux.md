...
### 特性 · 说明
- **特性**: **统一辅助 LLM 路由** · **说明**: 每类任务可配独立模型/超时/并发;失败自动降级主模型;连续失败进入冷却;每次调用写入会话事件,可审计
- **特性**: **三个开箱即用工具** · **说明**: `vision_analyze`(图像分析)、`web_extract`(网页提取+摘要)、`compress_text`(长文本压缩)
- **特性**: **会话压缩桥接** · **说明**: 配置 `compaction` 任务后,原生 DSH 自动/手动压缩会改走 AUX 辅助模型;含图会话图片缺失/纯文本路由时自动降级,压缩不失败
- **特性**: **`/aux` 命令** · **说明**: 状态查看、模型切换、图片回收、视觉自检、图片记忆
- **特性**: **Web 设置页 + 状态 chip** · **说明**: 每任务模型下拉配置;composer 实时显示最近一次辅助调用
- **特性**: **会话图片生命周期** · **说明**: 删除会话自动清理无引用图片;共享保留、归档不误删;图片记忆跨重启可查
- **特性**: **零配置可用** · **说明**: 不配任何模型也能跑——辅助任务自动使用会话主模型

### 三个工具

### 工具 · 干什么 · 典型场景
- **工具**: `vision_analyze` · **干什么**: 图像分析(支持多图并行) · **典型场景**: “这张图里是什么?” “读出图表数值” “对比两张图”
- **工具**: `web_extract` · **干什么**: 网页抓取 + 摘要 · **典型场景**: “总结这个页面” “回答某网页里的问题”
- **工具**: `compress_text` · **干什么**: 长文本压缩(保数字/路径/标识符) · **典型场景**: 压日志、压文档、喂给上下文
...
3. 想让视觉走专用模型?`/aux model vision /mimo-v2.5`。
...
### 命令 · 作用
- **命令**: `/aux status` · **作用**: 查看各任务路由与最近调用
- **命令**: `/aux model <task> [provider/model]` · **作用**: 查看/设置某任务的辅助模型
- **命令**: `/aux vision  <question...>` · **作用**: 命令行直接看图
- **命令**: `/aux test <task>` · **作用**: 自检某任务路由
- **命令**: `/aux memory [n]` · **作用**: 查看最近图片分析记忆
- **命令**: `/aux gc-images [days]` · **作用**: 手动回收旧附件图片
...
A:不需要。dsh-aux 是**零配置**的:不配任何模型也能跑,辅助任务会自动回退到会话主模型。你可以随时通过设置页或 `/aux model <task> ` 为某个任务指定专用模型。
...
### 文档 · 内容
- **文档**: [AI.md](./dsh-aux/AI.md) · **内容**: 给 AI 代理的安装指南
- **文档**: [PRD.md](./PRD.md) · **内容**: 需求规格与设计决策
- **文档**: [CHANGELOG.md](./CHANGELOG.md) · **内容**: 版本历史
- **文档**: [COMPARISON.md](./COMPARISON.md) · **内容**: 与社区视觉插件的架构对比
- **文档**: [VISION-AGENT.md](./VISION-AGENT.md) · **内容**: 视觉子代理策略与记忆架构
- **文档**: [SESSION-ATTACHMENT-GC.md](./SESSION-ATTACHMENT-GC.md) · **内容**: 会话删除时图片清理设计
- **文档**: [CONTRIBUTIONS.md](./CONTRIBUTIONS.md) · **内容**: 致谢与借鉴说明
