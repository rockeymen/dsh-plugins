...
### 文件 · 职责
- **文件**: `src/store.ts` · **职责**: 接缝 1: 用户全局指令文件的读写语义 (被单测覆盖)
- **文件**: `src/index.ts` · **职责**: Host 插件: 注册 `GET/PUT /api/global-prompt` 路由
- **文件**: `src/client/GlobalPromptEditor.tsx` · **职责**: 接缝 2: 编辑器组件 (被单测覆盖)
- **文件**: `src/client/api.ts` · **职责**: 编辑器的 HTTP 实现 (由安装验证覆盖)
- **文件**: `src/client/index.tsx` · **职责**: Client 插件: 注册 `settings.section` 分区
- **文件**: `cordis.patch.yml` · **职责**: bundle 层: 插入 `global-prompt` 行