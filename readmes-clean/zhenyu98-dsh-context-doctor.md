# Context Doctor
  ...
  ![Context Doctor native composer panel preview](docs/assets/context-doctor-preview.png)
...
### 之前 · 之后
- **之前**: 只能靠上下文计量条猜个大概，说不清是谁在消耗 · **之后**: 指令链 / 技能 catalog / 工具 schema / MCP 四项逐项给出 token 估算
- **之前**: 重复指令、重复技能描述散落在各层文件里，无人察觉 · **之后**: 自动检测跨文件完全相同的重复段落、描述完全相同的冗余技能
- **之前**: 同名技能多来源并存时被静默遮蔽，模型用的是哪个要靠猜 · **之后**: 报告冲突胜出者与被遮蔽者（rank shadow）
- **之前**: 看到告警只能手工翻文件找线索 · **之后**: 模型可直接调用 `context_audit` 拿到分节报告与按严重度排序的裁剪建议
...
### 注入物 · 审计内容 · 成本性质
- **注入物**: **指令链** · **审计内容**: 从 git 根到当前工作目录每一层的 `AGENTS.md` / `CLAUDE.md`：文件数、token 估算、**跨文件完全相同的重复段落** · **成本性质**: 每请求常驻
- **注入物**: **技能目录（catalog）** · **审计内容**: `ctx.skills` 中所有技能的 `name + description`（模型每请求看到 `<available_skills>`）、按来源分组统计、**描述完全相同的冗余技能** · **成本性质**: 每请求常驻
- **注入物**: **工具 schema** · **审计内容**: 当前 agent 可见的全部工具（`ctx.tools.schemas`）：数量、schema token 估算、原生工具与 MCP 工具分组 · **成本性质**: 每请求常驻
- **注入物**: **MCP 工具面** · **审计内容**: 按服务器分组的 MCP 工具数与 schema token（`mcp__<server>__<tool>` 命名解析），识别工具面膨胀 · **成本性质**: 每请求常驻
- **注入物**: **技能正文**（可选） · **审计内容**: 前 N 个技能的正文总 token（按需加载，不常驻请求，用于对比"常驻 vs 按需"成本） · **成本性质**: 按需加载
