...
### 设置 · 默认值 · 可选值
- **设置**: 启用 · **默认值**: `true` · **可选值**: 开 / 关
- **设置**: 模式 · **默认值**: `live` · **可选值**: `live`、`cached`、`indexed`
- **设置**: 上下文大小 · **默认值**: `medium` · **可选值**: `low`、`medium`、`high`
- **设置**: 备用模型 · **默认值**: `gpt-5.4` · **可选值**: Codex 模型 ID
- **设置**: 最大输出 Token · **默认值**: `2048` · **可选值**: 正整数
...
### 设置 · 默认值 · 可选值
- **设置**: 启用 · **默认值**: `true` · **可选值**: 开 / 关
- **设置**: 图片模型 · **默认值**: `gpt-image-2` · **可选值**: 图片模型 ID
- **设置**: 图片数量 · **默认值**: `1` · **可选值**: 1–10
- **设置**: 尺寸 · **默认值**: `auto` · **可选值**: `auto`、`1024x1024`、`1536x1024`、`1024x1536`
- **设置**: 质量 · **默认值**: `auto` · **可选值**: `auto`、`low`、`medium`、`high`
- **设置**: 背景 · **默认值**: `auto` · **可选值**: `auto`、`opaque`、`transparent`
...
### 行 · Export · 作用
- **行**: `llm-codex-auth` · **Export**: `dsh-codex-auth` · **作用**: 共享认证协调器与 LLM 路由
- **行**: `codex-search` · **Export**: `dsh-codex-auth/search` · **作用**: 全局搜索提供方
- **行**: `codex-image` · **Export**: `dsh-codex-auth/image` · **作用**: Agent scope 图片工具

认证 / LLM 行字段全部可选。设置 `llmEnabled: false` 后仍保留供搜索/图片能力使用的
共享登录状态协调器，但不再注册 LLM 路由：

### 字段 · 默认值 · 含义
- **字段**: `llmEnabled` · **默认值**: `true` · **含义**: 注册 `openai-codex` LLM 路由
- **字段**: `authJsonPath` · **默认值**: `''` → `$CODEX_HOME`/`~/.codex/auth.json` · **含义**: Codex 登录文件
- **字段**: `credentialRef` · **默认值**: `CODEX_CHATGPT_TOKEN` · **含义**: 状态卡展示的非敏感引用
- **字段**: `refreshLeadMs` · **默认值**: `300000` · **含义**: token 到期前的刷新提前量（毫秒）
- **字段**: `codexCommand` · **默认值**: `codex` · **含义**: 登录和版本探测使用的 CLI 命令
- **字段**: `displayName` · **默认值**: `OpenAI Codex (chatgpt)` · **含义**: 模型选择器中的 provider 名称
- **字段**: `transport` · **默认值**: `sse` · **含义**: 流式传输方式：`sse`、`websocket` 或 `auto`（优先 WebSocket、失败回退 SSE）。默认 SSE：WebSocket 升级在常见 HTTP 代理下不稳定，且 `auto` 模式下每个新对话都要先付出连接超时才会回退
- **字段**: `websocketConnectTimeoutMs` · **默认值**: `5000` · **含义**: WebSocket 连接超时（毫秒，仅当 `transport` 不是 `sse` 时生效；`0` 表示禁用）
- **字段**: `timeoutMs` · **默认值**: `120000` · **含义**: 请求超时（毫秒，SSE 响应头阶段；同时作为 WebSocket 消息空闲间隔；`0` 表示禁用）
