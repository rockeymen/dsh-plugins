...
[🌐 Official website — deepseekflow.kanghelyu.org](https://deepseekflow.kanghelyu.org/)
...
It is intentionally an editor—not a workflow runtime. DeepSeek Flow helps you design, inspect, and improve a workflow; execution remains in the current Session.
  ...
  ![DeepSeek Flow in dark mode](docs/images/engdark.png)
  ![DeepSeek Flow in light mode](docs/images/englight.png)
...
### Tool · Purpose
- **Tool**: `flow_create` · **Purpose**: Create a documented linear or branched workflow and save it in the current Session.
- **Tool**: `flow_list` / `flow_read` · **Purpose**: Discover workflows and read the master document, step documents, revision, graph, and logic contract.
- **Tool**: `flow_put` · **Purpose**: Import or atomically update a complete flow definition. A successful call is already persisted.
- **Tool**: `flow_evaluate` · **Purpose**: Evaluate Boolean gates from upstream values without running Agent steps.
- **Tool**: `flow_finalize_canvas` · **Purpose**: After direct file edits, queue Studio's invisible deterministic finalize action and skip a redundant main-Session review.
- **Tool**: `flow_delete` · **Purpose**: Delete a Session flow or shared template; managed workspaces are moved to trash.
...
- **Gate**: **IF / ELSE** · **Connection behavior**: One **Yes** and one **No** branch at most. · **Boolean result**: Selects exactly the branch matching the condition result.
- **Gate**: **AND / NAND** · **Connection behavior**: Multiple distinct targets; labels are automatic. · **Boolean result**: Evaluates all known operands, with NAND negating AND.
- **Gate**: **OR / NOR** · **Connection behavior**: Multiple distinct targets; labels are automatic. · **Boolean result**: Evaluates all known operands, with NOR negating OR.
- **Gate**: **XOR / XNOR** · **Connection behavior**: Multiple distinct targets; labels are automatic. · **Boolean result**: Evaluates parity, with XNOR negating XOR.
- **Gate**: **NOT** · **Connection behavior**: Exactly one automatically labeled outgoing arrow. · **Boolean result**: Negates its single input.
...
- **Action**: **Logic validation** · **Scope**: All workflow documents and arrow relationships · **What happens before files change**: The Agent returns clickable errors and warnings; no file is changed.
- **Action**: **Optimize current document** · **Scope**: The selected `WORKFLOW.md` or `STEP.md` only · **What happens before files change**: A complete proposal appears in the preview. You must **Accept** or **Reject** it.
- **Action**: **Optimize entire workflow** · **Scope**: `WORKFLOW.md` and every `STEP.md` · **What happens before files change**: A warning is shown first. After confirmation, the Agent rewrites and saves the complete set directly. There is no per-document review or built-in undo.
