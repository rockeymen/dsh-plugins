# dsh-docs
  ...
  ![dsh-docs — local document intelligence for DeepSeek Harness: PDF, DOCX, XLSX, PPTX, Markdown, HTML, CSV, OCR, and text](docs/header.png)
...
### Tool · Purpose
- **Tool**: `dshdoc_health` · **Purpose**: Report readiness of the selected local engine.
- **Tool**: `dshdoc_convert_file` · **Purpose**: Parse an allowlisted local file.
- **Tool**: `dshdoc_extract` · **Purpose**: Preferred local-file convenience tool.
- **Tool**: `dshdoc_convert_url` · **Purpose**: Compatibility stub that returns `UNSUPPORTED_URL`.
...
- **Field**: `engine` · **Default**: `auto` · **Meaning**: `node`, `python`, or `auto`; auto selects configured embedded Python, otherwise Node Xberg.
- **Field**: `runtimeDir` · **Default**: unset · **Meaning**: Absolute embedded-runtime directory.
- **Field**: `pythonCommand` · **Default**: unset · **Meaning**: Trusted Python executable for a managed runtime.
- **Field**: `pythonWorkerPath` · **Default**: shipped worker · **Meaning**: Absolute Python worker override.
- **Field**: `tessdataPath` · **Default**: runtime `ocr/tessdata` · **Meaning**: Absolute bundled Tesseract language-data directory.
- **Field**: `ocrBackend` · **Default**: `auto` · **Meaning**: `auto` or `tesseract`; both select the pinned local Tesseract backend.
- **Field**: `ocrLanguages` · **Default**: every bundled pack · **Meaning**: Ordered local OCR language packs. Unset uses every `.traineddata` pack in the configured runtime.
- **Field**: `timeoutMs` · **Default**: `120000` · **Meaning**: Per-conversion deadline.
- **Field**: `maxFileBytes` · **Default**: `52428800` · **Meaning**: Authorized input-size cap.
- **Field**: `allowedLocalRoots` · **Default**: `[]` · **Meaning**: Extra absolute non-root directories the model may read, beyond the session workspace.
- **Field**: `allowWorkspaceFiles` · **Default**: `true` · **Meaning**: Implicitly authorize the session workspace (session cwd) as a readable root; set `false` for allowlist-only lockdown.
- **Field**: `defaultOcr` · **Default**: `false` · **Meaning**: OCR default for images and scans; enable it only with a configured local tessdata runtime.
- **Field**: `defaultTableMode` · **Default**: `accurate` · **Meaning**: `fast` or `accurate` PDF table behavior.
- **Field**: `defaultOutputFormat` · **Default**: `md` · **Meaning**: `md`, `text`, or `json`.
- **Field**: `maxOutputChars` · **Default**: `32000` · **Meaning**: Maximum result returned to the model.
- **Field**: `debug` · **Default**: `false` · **Meaning**: Logs safe engine metadata only.
