...
`127.0.0.1:`（默认 8443）监听，未认证访问显示内置登录页，校验令牌后签发 HttpOnly
...
- 局域网地址可正常编辑 web-ui 设置（主题、语言、插件配置等）：dsh web 客户端按页面源判定回环，
  ...
  鉴权仍由本插件令牌墙把关。
- 状态可视：宿主终端直接打印 `dsh-auth-proxy: listening on http://<host>:`（console.log 镜像，
...
### 字段 · 默认 · 说明
- **字段**: `enabled` · **默认**: `true` · **说明**: 总开关
- **字段**: `host` · **默认**: `127.0.0.1` · **说明**: 监听地址（仅回环与内网；无 TLS，禁止 `0.0.0.0`/`::`/公网 IP）
- **字段**: `port` · **默认**: `8443` · **说明**: 对外监听端口
- **字段**: `targetHost` · **默认**: `127.0.0.1` · **说明**: 回环转发目标
- **字段**: `targetPort` · **默认**: `3080` · **说明**: 回环转发端口
- **字段**: `token` · **默认**: 见下 · **说明**: 共享访问令牌
- **字段**: `banner` · **默认**: `''` · **说明**: 登录页横幅文案
- **字段**: `allowedIps` · **默认**: `[]` · **说明**: IP 白名单（如 `["127.0.0.1", "10.0.0.0/8"]`），空 = 一律要令牌
- **字段**: `accessUrls` · **默认**: `[]` · **说明**: 对外访问地址（可含 HTTPS 域名，多域名逗号分隔），仅用于展示
- **字段**: `maxFailures` · **默认**: `0` · **说明**: 失败锁定阈值（0 = 关闭锁定）
- **字段**: `lockoutMinutes` · **默认**: `15` · **说明**: 锁定时长（分钟）
