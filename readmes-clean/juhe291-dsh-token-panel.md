...
- **💰 **按模型分别计价****: ⏱️ **auto 峰谷价** · **flash / pro 各自套官方价表，混用模型的会话也估得准**: 2026-08-17 调价零时**自动切换**峰谷价，无需改配置
- **💰 **按模型分别计价****: 🫧 **悬停即见数值** · **flash / pro 各自套官方价表，混用模型的会话也估得准**: 实时曲线 Steam 式跟随气泡，统计曲线悬停数据点弹值
- **💰 **按模型分别计价****: 🧊 **毛玻璃面板** · **flash / pro 各自套官方价表，混用模型的会话也估得准**: 半透明磨砂玻璃，深浅色主题统一，胶囊最透
- **💰 **按模型分别计价****: 🎛️ **时间拖动条** · **flash / pro 各自套官方价表，混用模型的会话也估得准**: 实时回看历史曲线，统计翻查更早日期段
- **💰 **按模型分别计价****: ✏️ **点击即改预算 / 余额** · **flash / pro 各自套官方价表，混用模型的会话也估得准**: 统计视图行内编辑，余额随 token 消耗本地递减
- **💰 **按模型分别计价****: 🖱️ **拖拽 + 长按位置菜单** · **flash / pro 各自套官方价表，混用模型的会话也估得准**: 四角预设 + 自定义默认位置，位置跨刷新记忆
- **💰 **按模型分别计价****: 📊 **按日 / 按月持久化统计** · **flash / pro 各自套官方价表，混用模型的会话也估得准**: JSONL 落盘，重启不丢，越用越完整
...
### 能力 · 说明
- **能力**: 会话列表 · **说明**: 每个会话一行：**标题 + 当前上下文压力 + 累计消耗 + 会话花费**，标题来自 DSH 会话标题服务
- **能力**: 会话详情 · **说明**: 点击展开：输入 / 输出 / 缓存读 / 缓存写、压力 / 预计 / 容量、估算成本、上下文占用进度条（>85% 变红）
- **能力**: 实时曲线 · **说明**: 每会话独立 SVG 面积曲线，带 Y 轴自动刻度（1/2/2.5/5×10ⁿ 取整 + 滞回，空闲归零，单位标签常驻）、X 轴时间刻度、最新点虚线引导线和浮动数值标签，支持 **2m / 5m / 15m** 范围切换；鼠标悬停曲线上任意位置，跟随气泡显示该处 token 值（Steam 风格）；曲线下方**时间拖动条**可回看更早时间段
- **能力**: 峰值速率 · **说明**: 曲线区顶部显示窗口内峰值消耗速率（t/s）
- **能力**: 跟随当前会话 · **说明**: 默认只显示当前打开的对话；历史会话折叠在「展开全部」后面（重启后仍保留）
- **能力**: 空会话过滤 · **说明**: 新开但 0 token 的对话完全不显示
- **能力**: TPS · **说明**: 底部栏实时显示生成速度（t/s）

### 📊 统计视图

### 能力 · 说明
- **能力**: 按日 / 按月 · **说明**: 独立切换，趋势曲线 + 明细列表（默认收起，点「展开全部」查看全部日期 / 月份）
- **能力**: 趋势曲线 · **说明**: 每日 / 每月消耗的 SVG 曲线，刻度显示日期（M/D），含 Y 轴刻度；数据点**悬停弹气泡**显示该日 / 该月 token 值；下方**时间拖动条**可查看更早日期段（默认最近 14 天 / 6 个月）
- **能力**: 累计消耗 · **说明**: 顶部大字单行汇总：累计 token 总数 + ≈¥ 估算成本
- **能力**: 预算与余额 · **说明**: **点击数值行内编辑**（回车保存 / Esc 取消）；预算显示本月已用 / 总额进度条（超支变红）；余额随 token 消耗本地估算递减，未设置时回退 API 拉取的官网余额
- **能力**: 持久化 · **说明**: 数据按天写入磁盘（JSONL），**重启不丢**，越用越完整
...
### 键 · 默认 · 说明
- **键**: `pollInterval` · **默认**: `1500` · **说明**: 浏览器实时轮询间隔（毫秒）
- **键**: `priceMode` · **默认**: `auto` · **说明**: 计价模式：`auto` 在 2026-08-17 零时自动从 flat 切到峰谷价；`flat` / `peak-offpeak` 固定
- **键**: `pricePerM*` · **默认**: `1 / 0.02 / 2` · **说明**: 全局兜底价格（每百万 token，CNY，仅展示）
- **键**: `pricePeak*` · **默认**: `3 / 0.1 / 9` · **说明**: 高峰时段兜底价格（北京时间 9-12、14-18）
- **键**: `priceOffpeak*` · **默认**: `1.5 / 0.05 / 4.5` · **说明**: 空闲时段兜底价格
- **键**: `modelPrices` · **默认**: 内置 flash + pro · **说明**: 按模型价表（flat / peak / offpeak 三档），覆盖或新增模型
- **键**: `budgetMonthly` · **默认**: `0` · **说明**: 月预算（CNY），>0 显示预算进度条；也可在统计视图点击数值直接设置
- **键**: `dataDir` · **默认**: `~/.dsh/cache/dsh-token-panel` · **说明**: 持久化用量日志目录
