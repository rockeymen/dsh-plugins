...
- **能力**: 实时曲线 · **说明**: 每会话独立 SVG 面积曲线，带 Y 轴自动刻度（1/2/2.5/5×10ⁿ 取整 + 滞回，空闲归零，单位标签常驻）、X 轴时间刻度、最新点虚线引导线和浮动数值标签，支持 **2m / 5m / 15m** 范围切换；鼠标悬停曲线上任意位置，跟随气泡显示该处 token 值（Steam 风格）
...
- **能力**: 趋势曲线 · **说明**: 每日 / 每月消耗的 SVG 曲线，刻度显示日期（M/D），含 Y 轴刻度；数据点**悬停弹气泡**显示该日 / 该月 token 值
