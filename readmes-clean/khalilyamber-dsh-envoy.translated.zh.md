...
### 情况 · 特征 · 您的路径
- **情况**: **A. 已在跑 DSH** · **特征**: 您的 DSH（Web UI 或桌面版）正在运行，浏览器能打开 `http://127.0.0.1:3080` · **您的路径**: 装完插件即可用，**零配置**（外接模式）
- **情况**: **B. 装了 DSH 没在跑** · **特征**: 本机有 DSH 安装，但服务没启动 · **您的路径**: 二选一：启动 DSH 走外接；或不启动，用插件内置模式（需填 apiKey）
- **情况**: **C. 还没装 DSH** · **特征**: 本机没有 DSH · **您的路径**: 先按 [DSH 官方文档](https://github.com/deepseek-ai/deepseek-harness) 安装（CLI 或桌面版均可，请以官方说明为准），装完回到 A 或 B
...
### 配置项 · 何时需要
- **配置项**: `mode` · **何时需要**: 默认 `auto`（探测到 3080 有 DSH 就走外接，否则内置）。想固定走某一种再改
- **配置项**: `apiKey` · **何时需要**: **仅内置模式需要**（外接模式凭证由 DSH 自己管理，不填）。填 DeepSeek API Key，只经环境变量传给任务进程，不落盘
- **配置项**: `defaultCwd` · **何时需要**: 可留空。留空时外接模式任务落进 DSH 的「协助Hana」工作区
- **配置项**: `dshInstallDir` · **何时需要**: 可留空。内置模式找不到 DSH 安装时，填它的安装根目录
...
### 模式 · 越界操作的行为
- **模式**: 外接（external） · **越界操作的行为**: DSH 挂起等审批。Hana 侧内联问询您；180 秒无人应答自动拒绝；DSH 界面原弹窗也可点
- **模式**: 内置（headless） · **越界操作的行为**: 立即拒绝（fail closed），DSH 在报告里说明被拒原因。您明确授权后，Hana 可带授权重派（danger-full-access）
