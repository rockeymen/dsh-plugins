...
### 工具 · 作用
- **工具**: `gemini_send_message` · **作用**: 发消息 / 多轮续聊（可附图片、视频）
- **工具**: `gemini_analyze_media` · **作用**: 上传图片/视频让 Gemini 识别——**用户传图时应优先调用**
- **工具**: `gemini_generate_image` · **作用**: 文生图 / 图生图（Imagen），自动下载到本地
- **工具**: `gemini_generate_video` · **作用**: 文生视频（Veo），异步渲染并轮询下载
- **工具**: `gemini_list_conversations` · **作用**: 列出最近的历史对话
- **工具**: `gemini_read_conversation` · **作用**: 读取某个对话的轮次
- **工具**: `gemini_delete_conversation` · **作用**: 删除对话（不可恢复）
- **工具**: `gemini_download_media` · **作用**: 用登录会话下载 googleusercontent 媒体
- **工具**: `gemini_status` · **作用**: 诊断：Cookie 完整性、令牌可换性、续期状态
...
### 现象 · 原因 / 解决
- **现象**: 返回 401 / `SNlM0e` 解析失败 · **原因 / 解决**: 会话失效：重新导出 cookie；服务常驻期间会自动续期，不会再过期
- **现象**: 提示 portal/app-bound 加密 · **原因 / 解决**: 新版浏览器无法离线解密：改用 Cookie-Editor 导出 + `--cookie-file`
- **现象**: 错误码 1037 / 1060 · **原因 / 解决**: 用量超限稍后再试 / IP 被临时限制，换网络或等待
- **现象**: 生图提示额度用尽 · **原因 / 解决**: 账号 Imagen 额度用尽（设置页可查），等重置或换账号
- **现象**: 生视频超时 · **原因 / 解决**: 渲染需 5~15 分钟：调大 `timeout_seconds`，或稍后读对话查看
- **现象**: DSH 里看不到工具 · **原因 / 解决**: 新开一个会话；确认 patch 语法正确；分析/生视频超时则调大 `toolCallTimeoutMs`（默认仅 60 秒）
