...
### UI 自动化
### 操作 · 说明
- **操作**: `click / double / right` · **说明**: 鼠标左键/双击/右键点击（x,y 像素坐标）
- **操作**: `drag` · **说明**: 按住拖动（含 Shift 拖拽：画正圆等）
- **操作**: `type` · **说明**: 输入文字（中文安全：剪贴板 + Ctrl+V）
- **操作**: `key` · **说明**: 按键（SendKeys 语法，如 `^c`、`{ENTER}`、`{ESC}`）
- **操作**: `activate` · **说明**: 按窗口标题激活窗口
