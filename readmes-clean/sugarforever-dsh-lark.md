...
- **权限标识**: `im:message.p2p_msg:readonly` · **控制台中的权限名称**: 获取用户发给机器人的单聊消息 · **用途**: 接收用户与机器人的单聊消息 · **是否必需**: 是
- **权限标识**: `im:message.group_at_msg:readonly` · **控制台中的权限名称**: 获取群组中 @机器人的消息 · **用途**: 接收群聊中明确 @机器人的消息 · **是否必需**: 是
- **权限标识**: `im:message:send_as_bot` · **控制台中的权限名称**: 以应用的身份发消息 · **用途**: 让机器人回复单聊、群聊和话题消息 · **是否必需**: 是
...
### 权限标识 · 控制台中的权限名称 · 用途 · 是否必需
- **权限标识**: `im:message.group_msg` · **控制台中的权限名称**: 获取群组中所有消息 · **用途**: 配合 `requireMention: false` 接收群内全部消息 · **是否必需**: 仅关闭 @限制时需要
...
### 配置项 · 必填 · 默认值 · 说明
- **配置项**: `appId` · **必填**: 是 · **默认值**: 无 · **说明**: 飞书或 Lark 应用的 App ID
- **配置项**: `appSecret` · **必填**: 是 · **默认值**: 无 · **说明**: 应用的 App Secret，建议从环境变量读取
- **配置项**: `domain` · **必填**: 否 · **默认值**: `feishu` · **说明**: 中国版使用 `feishu`，国际版使用 `lark`
- **配置项**: `requireMention` · **必填**: 否 · **默认值**: `true` · **说明**: 群聊是否必须 @机器人
- **配置项**: `dmMode` · **必填**: 否 · **默认值**: `open` · **说明**: 单聊策略：`open`、`allowlist` 或 `disabled`
- **配置项**: `groupAllowlist` · **必填**: 否 · **默认值**: `[]` · **说明**: 允许使用机器人的群 `chat_id` 列表；空数组表示不限制
- **配置项**: `dmAllowlist` · **必填**: 否 · **默认值**: `[]` · **说明**: `dmMode: allowlist` 时允许访问的用户 `open_id` 列表
- **配置项**: `provider` · **必填**: 否 · **默认值**: Harness 默认值 · **说明**: 为这个渠道指定模型 Provider
- **配置项**: `model` · **必填**: 否 · **默认值**: Harness 默认值 · **说明**: 为这个渠道指定模型
- **配置项**: `workspace` · **必填**: 否 · **默认值**: 第一个已注册 Workspace；没有时为 DSH 进程工作目录 · **说明**: Agent 使用的工作目录；显式路径优先
- **配置项**: `agentPreset` · **必填**: 否 · **默认值**: Harness 当前默认 Preset · **说明**: Agent 使用的 Preset，决定工具、系统提示等组合
- **配置项**: `errorMessage` · **必填**: 否 · **默认值**: 内置中文提示 · **说明**: Agent 执行失败时返回给用户的文本，最长 500 个字符
