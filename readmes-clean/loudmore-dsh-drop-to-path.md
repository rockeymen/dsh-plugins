...
flowchart LR
    ...
    B -->|"Imagespng / jpg / webp / gif"| C["Native attachment cardpreview / remove"]
    C --> D["Hit send"]
    D --> E["Upload → workspace/.drops/"]
    E --> F["Message becomes plain text paths"]
    F --> G["Text-only model reads themwith vision / document tools"]
    B -->|"Other filespdf / docx / xlsx / video…"| H["Square chip in theattachment railformat icon · ✕ remove"]
    H --> D
```
