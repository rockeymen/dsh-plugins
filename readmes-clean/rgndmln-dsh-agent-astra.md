...
### Action · Result
- **Action**: `↑` / `↓` · **Result**: Previous / next semantic event
- **Action**: `Shift + ↑` / `Shift + ↓` · **Result**: Previous / next visible user or agent message
- **Action**: Pause, drag the rail, or use `Home` / `End` · **Result**: Inspect history without resetting your orbit or zoom
- **Action**: Click a node · **Result**: Select it and open its floating detail card
- **Action**: Double-click a node · **Result**: Deliberately focus the camera on it
- **Action**: Drag an exact file snapshot · **Result**: Put its real content into the native composer draft
