...
- 已安装 `dsh` 及 profile（在 dsh `0.1.0-rc.6` 上验证）
...
- `dsh web` 报 `EADDRINUSE`（3080 被占用）说明有旧实例还在跑：用 `netstat -ano | findstr :3080` 找到 PID，`taskkill /PID  /F` 关掉再启动。
...
### 键 · 默认 · 含义
- **键**: `language` · **默认**: `""` · **含义**: Windows OCR 的 BCP-47 语言标签，如 `zh-Hans`、`en-US`；空 = 用户配置文件语言
- **键**: `passthrough` · **默认**: `true` · **含义**: `true`：真视觉模型图片原样透传；`false`：所有图片一律走 OCR
- **键**: `ocrScript` · **默认**: 自带 `lib/ocr.ps1` · **含义**: PowerShell OCR 脚本的绝对路径覆盖
- **键**: `timeoutMs` · **默认**: `60000` · **含义**: 单张图片 OCR 超时（毫秒）
- **键**: `maxCacheEntries` · **默认**: `200` · **含义**: 单次运行 OCR 缓存上限（按附件 id）
