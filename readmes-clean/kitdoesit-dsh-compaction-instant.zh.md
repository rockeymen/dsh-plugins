...
### 入口 · 模块 · 作用
- **入口**: `recall` **工具**（给模型用） · **模块**: `dsh-compaction-instant/tool` · **作用**: 按类型恢复原文：`type:"seq"` 配合 `(seq N)`/`(seqs A-B)` 标记，`type:"result"` 配合 `result N` 指针，`type:"checkpoint"` 配合 `[checkpoint N]` 序号——把原始内容一字不差地恢复到当前工具结果里
- **入口**: `search` **工具**（给模型用，grep） · **模块**: `dsh-compaction-instant/tool` · **作用**: 在整个持久日志里按关键词/正则搜索——包括被压缩掉的内容——返回带 `(seq N)` 指针的匹配事件，可直接交给 `recall` 取回
- **入口**: `/recall` **命令**（给人用，grep） · **模块**: `dsh-compaction-instant/command` · **作用**: `/recall <关键词 · 正则>` 追加一条持久的用户消息，内含匹配事件和 seq 指针，下一轮模型就能看到
- **入口**: 共享核心 · **模块**: `dsh-compaction-instant/recall` + `dsh-compaction-instant/search` · **作用**: seq 解析（`12`、`3-7`、`seq 12` / `seqs 3-7`）、日志展开、预算、字段筛选；正则编译与命中展示
...
### 键 · 默认 · 含义
- **键**: `thresholdRatio` · **默认**: `0.5` · **含义**: 上下文用到多大比例时自动触发压缩（0.5 = 用到一半）
- **键**: `retainRatio` · **默认**: `0.05` · **含义**: 对话最新的一段（按窗口比例）一字不差地保留，不压缩
- **键**: `retainTokens` · **默认**: — · **含义**: 直接指定保留多少 token；与 `retainRatio` 二选一
- **键**: `manualRetainRatio` · **默认**: `0.05` · **含义**: 手动 `/compact` 时保留当前对话的比例（保证正在聊的内容不会被收走）
- **键**: `manualRetainTokens` · **默认**: — · **含义**: 手动模式直接指定保留 token 数；与 `manualRetainRatio` 二选一
- **键**: `auto` · **默认**: `true` · **含义**: 开启自动压缩：监听 `agent/pre-step` 压力事件和 `agent/request-error` 溢出恢复
- **键**: `maxTokens` · **默认**: `8192` · **含义**: 单个检查点总预算的下限（按内容密度估算的 token）
- **键**: `checkpointScale` · **默认**: `0.1` · **含义**: 实际预算 = `max(maxTokens, 被压缩掉的 token 数 × checkpointScale)`，再封顶于 `checkpointCap`——内容很多时不会把每条都压成一句话
- **键**: `checkpointCap` · **默认**: `65536` · **含义**: 检查点预算的封顶值
- **键**: `textTokens` · **默认**: `512` · **含义**: 每条助手文本的预算
- **键**: `userTextTokens` · **默认**: `1024` · **含义**: 每条用户文本的预算
- **键**: `toolCallTokens` · **默认**: `128` · **含义**: 每个工具调用单行的预算（这个永远不缩放——见下面的省略规则）
- **键**: `toolResultExcerptTokens` · **默认**: `256` · **含义**: 仅为兼容官方配置而接受；**不起作用**——工具结果本来就不占位置
- **键**: `includeReasoning` · **默认**: `false` · **含义**: 是否在检查点里保留推理过程
- **键**: `stripNoiseXml` · **默认**: `true` · **含义**: 是否去掉用户文本里配置的噪音标签
- **键**: `noisePatterns` · **默认**: 见 compiler · **含义**: 噪音标签的正则来源，按 `s` 模式匹配
- **键**: `toolKeyFields` · **默认**: 内置 · **含义**: 额外的"工具名 → 参数里的关键字段"映射，用于单行展示
- **键**: `toolArgTools` · **默认**: 见 compiler · **含义**: 白名单：这些工具的关键参数会显示在单行里（`read`/`write`/`edit`/`glob`/`grep`/`bash`/`shell`/`web_search`/`skill`/`subagent`/…）；其余工具只显示名字
- **键**: `hideTools` · **默认**: — · **含义**: 完全从检查点里去掉的内部管理工具
- **键**: `modelPolicies` · **默认**: — · **含义**: 按 provider/model 单独覆盖 `thresholdRatio`/`retain*`（与官方配置格式一致）
- **键**: `compactionRetries` / `maxOverflowRetries` · **默认**: `1` / `1` · **含义**: 重试次数，含义和官方引擎一样
- **键**: `summarizationProvider` / `summarizationModel` · **默认**: — · **含义**: 仅为兼容官方配置而接受；**不起作用**——本引擎从不调用模型
...
### 字段 · 含义
- **字段**: `checkpointScale` · **含义**: 压缩预算 = 被压缩 token 数 × 此比例
- **字段**: `checkpointCap` · **含义**: 缩放后预算的绝对封顶
- **字段**: `maxTokens` · **含义**: 一次编译检查点的总 token 上限
- **字段**: `auto` · **含义**: 注册步骤间自动压缩
- **字段**: `debug` · **含义**: 向日志文件写入引擎调试行
- **字段**: `debugLogPath` · **含义**: 调试日志路径（留空 = `$DSH_HOME/compaction-debug.log`）
...
### 内容 · Tokens
- **内容**: 中文（`你好，世界！`） · **Tokens**: 每个字 1（共 6）
- **内容**: 西里尔 / 阿拉伯文 · **Tokens**: 每个字符 1
- **内容**: 带重音拉丁文（`café`） · **Tokens**: 英文部分成组（`caf` + `é`）
- **内容**: Emoji（`😀`） · **Tokens**: 2（一个 emoji 占两个 UTF-16 单元）
...
- **负载**: 工具密集会话全量（3,181 节点：1,438 个工具调用 + 1,540 个工具结果） · **原文 tokens**: 2,523,012 · **编译后**: 226,205 · **保留**: **9.0%** · **压缩**: 91.0%
- **负载**: 另一个会话全量（864 节点） · **原文 tokens**: 685,088 · **编译后**: 62,705 · **保留**: **9.2%** · **压缩**: 90.8%
- **负载**: 同一个工具密集会话，最近 800 条消息 · **原文 tokens**: 625,927 · **编译后**: 45,031 · **保留**: **7.2%** · **压缩**: 92.8%
- **负载**: 纯文本（同一会话去掉全部工具行） · **原文 tokens**: 160,963 · **编译后**: 109,945 · **保留**: **68.3%** · **压缩**: 31.7%
...
### Cap · 编译后 · 保留 · 条目数 · 被丢
- **Cap**: 8,192 · **编译后**: 8,243 · **保留**: 0.33% · **条目数**: 111 · **被丢**: 2,090
- **Cap**: 32,768 · **编译后**: 22,263 · **保留**: 0.88% · **条目数**: 232 · **被丢**: 1,969
- **Cap**: 65,536（部署默认） · **编译后**: 55,737 · **保留**: 2.2% · **条目数**: 325 · **被丢**: 1,876
- **Cap**: 65,536 · **编译后**: 55,737 · **保留**: 2.2% · **条目数**: 325 · **被丢**: 1,876
- **Cap**: 131,072 · **编译后**: 131,047 · **保留**: 5.2% · **条目数**: 1,142 · **被丢**: 1,058
- **Cap**: 226,205（一条不丢的临界值） · **编译后**: 226,205 · **保留**: 9.0% · **条目数**: 2,199 · **被丢**: 0
...
AI 会用 `agentPresets.copy('standard', '')` 创建一份本地预设，在副本里改掉压缩行的 `name`，用 `standingKeyFor('')` 校验能不能正常挂载，还可以通过改 `agent-presets` 行（`config.default: `）把它设为默认。新预设会出现在 UI 选择器里；内置预设不受影响。
...
- **方法**: **1. 别名替换** · **内置预设里的引擎**: ✅ 自动（standard/code/cordis） · **要改预设文件吗**: 否 · **选择器里多出预设**: 否 · **安装成本**: 一条命令 + 手动 patch
- **方法**: **2. AI 复制副本** · **内置预设里的引擎**: 只有新预设 · **要改预设文件吗**: 副本 · **选择器里多出预设**: 是 · **安装成本**: 一句提示
- **方法**: **3. 手动预设** · **内置预设里的引擎**: 只有新预设 · **要改预设文件吗**: 副本 · **选择器里多出预设**: 是 · **安装成本**: 手动编辑
