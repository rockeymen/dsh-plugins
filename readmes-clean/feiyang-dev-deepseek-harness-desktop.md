...
- **Mode**: Instant Start · **Description**: Local fixed-directory runtime, second-level launch, auto-checks official updates with one-click update (replaces Quick Start) · **Best For**: Most users, recommended
- **Mode**: Full Source Build · **Description**: `git clone` + `pnpm install` + `pnpm run build` · **Best For**: Developers who want to modify/debug the source
- **Mode**: Local Repair · **Description**: Uninstall global `@deepseek-ai/dsh`, clean residue, reinstall · **Best For**: Fix broken installs, version issues, or koffi load failures
...
### Process · Description
- **Process**: Service backend · **Description**: Starts `dsh web` from the source repo (`%APPDATA%/dsh-desktop/deepseek-harness`), serves the API and hosts the frontend at the same address
- **Process**: Browser-side hot-reload watcher · **Description**: `pnpm run dev:web`, watches all `dsh.client` plugin sources; rebuilds the bundle automatically on changes, hot-reloads in the browser without refresh
...
### Dependency · Notes
- **Dependency**: Windows 10 / 11 (x64) · **Notes**: Runtime platform
- **Dependency**: Node.js ≥ 18 · **Notes**: Required for Instant Start mode; the client guides installation if missing
- **Dependency**: git · **Notes**: Only needed for source mode (pnpm auto-installs if missing)
- **Dependency**: Network · **Notes**: First install downloads dependencies (~hundreds of MB)
...
A: The client reuses a running dsh web service first; alternatively use `npm start -- --port ` to specify another port.
...
- **Project**: [Usage & Cost Tracker (dsh-usage-plugin)](https://github.com/feiyang-dev/dsh-usage-plugin) · **Description**: Per-call token/cache-hit stats, peak/off-peak billing, balance query, CSV/JSON/PNG export · **Installation**: One-click from the desktop app's recommended plugins, or `dsh plugin add @feiyang666/dsh-usage-plugin`
- **Project**: [Data Vault (dsh-vault)](https://github.com/feiyang-dev/dsh-vault) · **Description**: Auto backup / wipe detection / one-click restore — protects chat history and workspace data · **Installation**: One-click from the desktop app's recommended plugins, or `dsh plugin add @feiyang666/dsh-vault`
- **Project**: [DeepSeek-Harness](https://github.com/deepseek-ai/DeepSeek-Harness) · **Description**: Official CLI / Web service · **Installation**: —
