[English](README_en.md) | **中文**
  ...
  ![AI 小说作家 Logo](docs/assets/readme/ai-novel-writer-logo-transparent.png)

# AI 小说作家 / AI Novel Writer

  面向长篇小说创作的本地优先桌面工作台。它把“前提 → 角色 → 世界观 → 章节蓝图 → 草稿 → 审稿 → 修稿 → 定稿”组织为一条可追溯的创作流程；模型由你自行配置，项目资料留在你的电脑上。

  ![AI 小说作家——本地优先的长篇小说创作桌面工作台](docs/assets/readme/hero-zh-v2.png)
...
### 能力 · 说明
- **能力**: 结构化创作流程 · **说明**: 从前提、角色、世界观到章节蓝图、草稿、审稿、修稿和定稿，按阶段组织创作资产。
- **能力**: 章节级生成 · **说明**: 生成时围绕当前章节的蓝图和相关资料组织上下文，减少跨章跑题。
- **能力**: 审稿与修稿 · **说明**: 为草稿生成结构化审稿信息，并以报告作为修稿输入。
- **能力**: 角色卡与项目资料 · **说明**: 在项目内维护角色、世界观、蓝图、草稿和定稿；项目会话机制避免旧窗口向重新打开的项目写入数据。
- **能力**: 参考文本与知识库 · **说明**: 可导入常见文本格式作为参考资料；未配置 embedding 时仍可使用 SQLite FTS 全文检索。
- **能力**: 批量创作任务 · **说明**: 单独的批量章节创作任务可设为 1–10 章，支持暂停、取消；后处理失败会停止后续章节。
- **能力**: 中英文界面 · **说明**: 首次启动可跟随系统语言，手动选择会持久保存。
...
### 数据或行为 · 默认位置 / 去向
- **数据或行为**: 小说项目、角色、蓝图、草稿和定稿 · **默认位置 / 去向**: 你的项目目录与本地 SQLite 数据库。
- **数据或行为**: 导入的参考资料 · **默认位置 / 去向**: 保留在本地项目范围内，除非你自行把内容发送给云端模型。
- **数据或行为**: 本地模型请求 · **默认位置 / 去向**: 发送给你配置的本机或局域网推理服务。
- **数据或行为**: 云端模型请求 · **默认位置 / 去向**: 当你选择 OpenAI、DeepSeek、Gemini 或其他云端端点时，提示词和上下文会发送给该服务商。
- **数据或行为**: 模型配置与 API Key · **默认位置 / 去向**: 当前保存在本机用户目录 `~/.vela/models.json`；请保护操作系统账户，不要分享该文件。
- **数据或行为**: 应用偏好与更新延后设置 · **默认位置 / 去向**: 保存在 `~/.vela/config.json`。
