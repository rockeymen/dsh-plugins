...
### 输入形态 · 支持情况 · 边界
- **输入形态**: JSON 字符串或对象参数中的字符串 `command`/`cmd` · **支持情况**: 支持 · **边界**: 只检查已识别的类 shell 工具名。
- **输入形态**: 大小写、引号，或可见的 `bash -lc`/`pwsh -Command` wrapper · **支持情况**: 词法支持 · **边界**: 分类关键词必须仍在字符串中可见。
- **输入形态**: 数组命令、`argv`、嵌套命令对象、自定义 shell 工具名 · **支持情况**: 不支持 · **边界**: 不产生信号。
- **输入形态**: 不含可见分类关键词的别名或 wrapper · **支持情况**: 不支持 · **边界**: 预期存在漏判。
- **输入形态**: `echo "do not run tests"` 一类引用文本 · **支持情况**: 会被词法匹配 · **边界**: 因为不解析意图与执行，预期存在误判。
...
- **方面**: Session 日志 · **影响**: 不变；插件只读已有事件，不添加事件。
- **方面**: Prompt 与上下文 · **影响**: 不变。
- **方面**: Turn 延迟 · **影响**: 监听器同步扫描已结束的 turn，并排队本地文件 I/O；turn 路径不等待磁盘。
