...
### 文件 · 作用
- **文件**: `index.js` · **作用**: Host 半：注册 webServer 路由 `/dsh-liya-workspace/summary`，实时读工作区档案统计
- **文件**: `client.js` · **作用**: Client 半（手写 bundle）：设置页 `settings.section` 注册 + fetch 展示 + 极简 markdown 渲染
- **文件**: `cordis.patch.yml` · **作用**: 插件挂载行（insert 进配置树）
...
- 路由：`Invoke-WebRequest http://127.0.0.1:/dsh-liya-workspace/summary` → 200 + JSON
