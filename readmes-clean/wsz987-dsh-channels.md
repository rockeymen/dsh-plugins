...
**Harness Web · 渠道设置面板**
  ...
  ![Harness Web 渠道设置面板](./docs/ScreenShot/dsh-channels-setting.png)

**各平台对话框**

  ![微信对话](./docs/ScreenShot/weixin.jpg)
  ![QQ 对话](./docs/ScreenShot/qq.jpg)
  ![钉钉对话](./docs/ScreenShot/dingding.jpg)
  ![飞书对话](./docs/ScreenShot/feishu.jpg)
...
### 检查项 · 要求
- **检查项**: Harness CLI · **要求**: 已能运行 `npx @deepseek-ai/dsh`
- **检查项**: LLM 模型 · **要求**: 先在 Harness Web 的“设置 → 模型”中配置模型提供方、凭据和默认模型，并确认普通 Web 会话可以正常对话。参见 [Harness 官方配置参考](https://deepseek-harness.github.io/deepseek-harness/reference/config-catalog)
- **检查项**: 会话权限 · **要求**: 渠道会话遵循 Harness 权限预设。新会话默认通常为 `Workspace Write`，可修改当前 Workspace 内的文件，访问更大范围时需要审批
...
### 渠道 · 文本 · 图片 · 文件 · 流式回复 · 状态
- **渠道**: 微信 · **文本**: 支持 · **图片**: 收发 · **文件**: 入站读取 · **流式回复**: - · **状态**: ⚠️
- **渠道**: QQ · **文本**: 支持 · **图片**: 收发 · **文件**: 收发 · **流式回复**: 支持 · **状态**: ✅
- **渠道**: 钉钉 · **文本**: 支持 · **图片**: 收发 · **文件**: 收发 · **流式回复**: 支持 · **状态**: ✅
- **渠道**: 飞书 · **文本**: 支持 · **图片**: 收发 · **文件**: 收发 · **流式回复**: 支持 · **状态**: ✅
- **渠道**: Telegram · **文本**: 支持 · **图片**: 收发 · **文件**: 收发 · **流式回复**: 支持 · **状态**: 🚧
...
### 渠道 · 必要信息 · 登录方式
- **渠道**: 微信 · **必要信息**: 无 · **登录方式**: 扫码登录，凭据自动持久化
- **渠道**: QQ · **必要信息**: AppID、AppSecret · **登录方式**: [QQ 开放平台](https://q.qq.com/qqbot/openclaw/)创建机器人
- **渠道**: 钉钉 · **必要信息**: clientId、clientSecret（可选） · **登录方式**: 扫码或在[钉钉开放平台](https://open-dev.dingtalk.com/)创建应用
- **渠道**: 飞书 · **必要信息**: AppId、AppSecret · **登录方式**: 在[飞书开放平台](https://open.feishu.cn/app)创建应用，或扫码创建智能体
- **渠道**: ~~Telegram~~（待支持） · **必要信息**: ~~Bot Token~~ · **登录方式**: ~~在 [@BotFather](https://t.me/BotFather) 创建机器人并填写 Token~~
...
### 指令 · 说明
- **指令**: `/new` · **说明**: 开启全新会话（遇到 bug 可以尝试使用）
...
### 当前限制 · 临时处理方式 · 后续方向
- **当前限制**: 无法从渠道终止正在运行的会话 · **临时处理方式**: 模型长时间推理或工具调用未结束时，暂时需要等待本轮执行完成 · **后续方向**: Harness 已提供 `Agent.cancel()`，后续增加 `/stop` 指令
- **当前限制**: 无法在当前渠道会话中直接切换模型 · **临时处理方式**: 先在 Harness Web 中选择模型，再发送 `/new` 创建使用新默认模型的会话 · **后续方向**: 增加模型选择指令
- **当前限制**: 渠道内没有权限切换指令 · **临时处理方式**: 在 Harness Web 中调整未来新会话的默认权限，或修改对应会话的 Access 设置 · **后续方向**: 完善渠道内的会话管理能力
