...
- 🔒 **Read-only & isolated** — the share server binds `0.0.0.0:` on its own; it exposes nothing but `GET /s/<token>`. No RPC, no writes, no session reads on view.
...
### Half · Runs in · Role
- **Half**: Host (`src/host`) · **Runs in**: Node (harness process) · **Role**: Folds the session event log into a transcript + stats, stores the frozen snapshot, and serves it over a separate `node:http` server at `/s/<token>`. Also exposes a `/dsh-share` RPC (`create` / `revoke` / `list`).
- **Half**: Client (`src/client`) · **Runs in**: Browser · **Role**: Registers the **Share** button in `conversation.session.header.utilities` and renders the dialog (expiry picker, copy / revoke / open).
...
- **Variable**: `DSH_SHARE_HOST` · **Default**: `0.0.0.0` · **Description**: Bind address of the read-only share server.
- **Variable**: `DSH_SHARE_PORT` · **Default**: `3081` · **Description**: Port of the share server.
