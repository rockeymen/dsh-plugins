...
把本仓库路径或链接发给任意聊天，说一句：
> "按本仓库 README 的「自己装上用」一节，把 coral 注册为 MCP 工具（`$DSH_HOME/profiles//cordis.patch.yml` 加 `mcp-coral` 行，保存即生效）。"
...
# 1. 编辑 DSH 配置：$DSH_HOME/profiles//cordis.patch.yml 加一条：
#    - id: mcp-coral
...
### 段 · 关键项 · 默认 · 说明
- **段**: `memory` · **关键项**: `capacity_threshold` · **默认**: 10000 · **说明**: 记忆总数上限，超限先蒸馏再淘汰
- **段**:  · **关键项**: `governance_headroom` · **默认**: 0（自动） · **说明**: 治理余量 `max(10, min(容量/10, 200))`
- **段**:  · **关键项**: `hot_ttl_hours` / `max_hot_entries` / `max_warm_entries` / `max_cold_entries` · **默认**: 24/50/200/5000 · **说明**: 三级存储参数
- **段**:  · **关键项**: `cold_scan_lines` · **默认**: 2000 · **说明**: 冷库检索扫描行数（尾部最新；越大检索范围越广）
- **段**:  · **关键项**: `distill_sim_threshold` / `distill_min_cluster` · **默认**: 0.6/3 · **说明**: 蒸馏聚类阈值/最小簇大小
- **段**: `retrieval` · **关键项**: `weights` · **默认**: 0.6/0.2/0.2 · **说明**: 向量/Jaccard/时间 融合权重
- **段**:  · **关键项**: `top_k` / `tau_days` / `include_cold` / `vectorized_jaccard` · **默认**: 5/7/True/True · **说明**: 检索参数
- **段**: `heat` · **关键项**: `weights` · **默认**: 0.4/0.3/0.3 · **说明**: 频率/最近访问/重要性
- **段**:  · **关键项**: `cold_fold_interval_seconds` · **默认**: 30 · **说明**: 冷库热度增量落盘节流
- **段**: `storage` · **关键项**: `vector_save_interval_seconds` · **默认**: 5.0 · **说明**: 向量落盘节流（防 O(n²) 写盘）
- **段**:  · **关键项**: `max_bytes` / `warn_ratio` / `hard_ratio` · **默认**: 0/0.8/0.85 · **说明**: 磁盘配额（0 = 不限制）
- **段**: `threads` · **关键项**: `path` · **默认**: `memory_data/coral_threads.json` · **说明**: 推理线索链路存储（永不遗忘，不参与淘汰/治理/配额）
- **段**: `llm` · **关键项**: `base_url` / `api_key` / `model` · **默认**: 空/空/deepseek-chat · **说明**: 蒸馏 LLM 端点（OpenAI 兼容）；`base_url`+`api_key` 齐备才启用蒸馏
- **段**: `parallelism` · **关键项**: `embed_batch_window_ms` · **默认**: 8 · **说明**: 嵌入合批窗口（真实模型建议 4-8ms）
- **段**: `reload` · **关键项**: `check_interval_seconds` · **默认**: 2.0 · **说明**: 配置 mtime 检测节流

## API 参考

### 方法 · 签名 · 说明
- **方法**: `insert` · **签名**: `(content, importance=0.0) → MemoryItem \ · **说明**: None` · 重复返回 None 并合并访问统计
- **方法**: `search` · **签名**: `(query, top_k=None) → List[SearchHit]` · **说明**: `SearchHit.item/.score/.scores{vector,jaccard,time}`
- **方法**: `mark_important` · **签名**: `(item_id, importance=1.0) → bool` · **说明**: 显式重要性（热度权重 0.3），冷库也可标记
- **方法**: `reload_config` · **签名**: `(force=False) → cfg` · **说明**: 热重载；不传 force 时由 mtime 检测触发
- **方法**: `flush` · **签名**: `()` · **说明**: 强制落盘：冷库热度 + 温存 + 向量
- **方法**: `disk_usage` · **签名**: `() → dict` · **说明**: 磁盘明细（含配额比例），配额的"账单"接口
- **方法**: `stats` · **签名**: `() → dict` · **说明**: hot/warm/cold/total/vectors（O(1)）
- **方法**: `fuse_check` · **签名**: `(items) → bool` · **说明**: token 熔断（沿用旧版语义）
- **方法**: `_distill` · **签名**: `(cluster) → MemoryItem \ · **说明**: None` · LLM 蒸馏：相似簇压缩为摘要（配置 `llm` 段即启用；失败/未配置返回 None）
- **方法**: `delete` · **签名**: `(item_id) → bool` · **说明**: 按 item_id 从热/温/冷 + 向量库彻底删除（清理错记/残留）
- **方法**: `@register_tool` · **签名**: 装饰器 · **说明**: 注册 `memory_search(query, top_k)` / `memory_insert(content, importance)` / `memory_flush()` / `memory_delete(item_id)`
- **方法**: `thread_create` · **签名**: `(title, summary="", parent_thread_id=None, by="") → ThreadItem` · **说明**: 创建推理线索链路（永不遗忘）
- **方法**: `thread_status` · **签名**: `(thread_id=None, include_archived=False, query=None) → List[ThreadItem]` · **说明**: 查看链路：无参=活跃总览；指定 ID=详情含步骤链
- **方法**: `thread_advance` · **签名**: `(thread_id, note, done=False, by="") → ThreadItem` · **说明**: 推进链路（追加步骤节点），聊天间协作
- **方法**: `thread_interrupt` · **签名**: `(thread_id, reason="") → ThreadItem` · **说明**: 中断链路（内容保留，可恢复）
- **方法**: `thread_archive` · **签名**: `(thread_id) → ThreadItem` · **说明**: 归档链路（不在活跃总览，永不遗忘）
- **方法**: `thread_resume` · **签名**: `(thread_id) → ThreadItem` · **说明**: 恢复中断/归档的链路
- **方法**: `thread_link` · **签名**: `(child_id, parent_id) → bool` · **说明**: 把两条链路串成父子（线索链）
- **方法**: `config_get` · **签名**: `(path=None) → Any` · **说明**: 查看配置（点分路径；缺省全量）
- **方法**: `config_set` · **签名**: `(key_path, value) → dict` · **说明**: 改配置：热加载生效 + 原子写回配置文件（管理上下文缓存入口）
- **方法**: `memory_flush` · **签名**: MCP 工具 · **说明**: **持久化关键**：热区记忆默认只存内存（重启丢失），写重要记忆后调它落盘（温存 + 向量）
- **方法**: `build_dsh_cordis_plugin_js` · **签名**: `(sidecar_url) → str` · **说明**: 生成 DSH `harness.registerTool` 插件 JS（含 @Ne 水印）
- **方法**: `MemoryToolSidecar` · **签名**: `(host, port)` · **说明**: 极简 HTTP 桥：JS `execute` → `POST /rpc` → Python 注册表
- **方法**: `get_coral` · **签名**: `(config_path) → ThreeDogCoral` · **说明**: 全局单例（与 Agent 工具共享同一份记忆）
...
### 项目 · 结果（基准条件下）
- **项目**: 200 轮对话压测（翻译助手 + 20 轮冷却期画像） · **结果（基准条件下）**: 9 次画像、冷却期严格生效、容量精确收敛
- **项目**: 跨项目共享 · **结果（基准条件下）**: 共享+亲缘度 recall/precision 双 1.0；冷启动 0%→100%
- **项目**: 2 万次暴力压测 · **结果（基准条件下）**: 写入 2 万条 **83.7s**、检索 12ms/次、超容治理批量淘汰、重启一致 ✅
- **项目**: 并行（8C/16T） · **结果（基准条件下）**: 嵌入合批 3.9×~6.8×、位图 Jaccard 稳态 12-13×、8 路并发检索 6.5×
