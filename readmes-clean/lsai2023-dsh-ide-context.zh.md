...
它读取 **Claude Code IDE integration** 桥——与 Claude Code CLI 相同的 `~/.claude/ide/.lock` 文件与 MCP-over-WebSocket 协议——因此一个 bundle 同时支持 **IntelliJ IDEA** 与 **Visual Studio Code**。
...
- id: ide-context
  config:
    ...
    lockDir: ~/.claude/ide    # 可选；IDE .lock 文件所在目录
```
...
- 需要一个正在运行的 Claude Code IDE 会话，且已写出有效的 `~/.claude/ide/.lock` 文件。
