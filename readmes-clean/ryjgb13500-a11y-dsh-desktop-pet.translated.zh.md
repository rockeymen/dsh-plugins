...
- **功能 Feature**: 🛏 睡眠状态 Sleep · **说明 Description**: 鼻涕泡呼吸、上半身起伏、Z 字漂浮 — snot bubble breathing, torso bobbing, floating Z's
- **功能 Feature**: ⏰ 任务唤醒 Wake-up · **说明 Description**: 泡泡破裂 → 惊讶表情 → 惊叹号 → 转认真 — bubble pop → surprise face → exclamation mark
- **功能 Feature**: ✍️ 书写状态 Writing · **说明 Description**: 手+羽毛笔同步快速平移，模拟写作业 — hand + quill moving in sync, simulating writing
- **功能 Feature**: 🔄 完整状态机 State Machine · **说明 Description**: 睡眠→唤醒→书写→回睡，与 dsh 任务状态实时联动 — full cycle linked to dsh task status
- **功能 Feature**: 🎬 动作演示 Demo Mode · **说明 Description**: 右键菜单一键循环演示全部动画 — one-click animation loop without real tasks
- **功能 Feature**: 🎨 可视化编辑器 Editor · **说明 Description**: 拖拽摆放素材、调整动画参数 — drag-and-drop layout + animation parameter tuning
- **功能 Feature**: 📏 等比缩放 Scaling · **说明 Description**: 60%~200% 六档窗口缩放，素材位置零错位 — proportional window scaling with zero offset
- **功能 Feature**: 🖥 置顶透明 Overlay · **说明 Description**: 无边框透明窗口，常驻桌面不打扰 — frameless transparent always-on-top window
