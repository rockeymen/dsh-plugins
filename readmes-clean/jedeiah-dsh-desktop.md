...
- **字段**: `registry` · **类型**: string(URL) · **说明**: npm registry 换源，如 `https://registry.npmmirror.com`；缺省官方 npmjs
- **字段**: `lan_enabled` · **类型**: bool · **说明**: 局域网访问开关（默认 false）
- **字段**: `lan_port` · **类型**: number · **说明**: 局域网转发器端口（默认 3190）
- **字段**: `lan_token` · **类型**: string · **说明**: 局域网访问令牌（首次开启生成，128-bit 随机）
