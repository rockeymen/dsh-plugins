...
- **Matrix CRT 风格**: ![Matrix CRT 风格](assets/screenshot-matrix.png) · **dsh 原生风格**: ![dsh 原生风格](assets/screenshot-native.png)
...
- **Capability**: 💬 In-session query · **Description**: `ds_balance` tool: the agent can fetch the official balance snapshot anytime (`force: true` bypasses the cache)
- **Capability**: 🔄 Optional per-turn injection · **Description**: Fresh balance can be injected into the model context before every turn (off by default — the badge + `ds_balance` tool already cover it; cache-only read, never blocks the conversation)
- **Capability**: 🖥️ Sidebar badge · **Description**: Matrix green-phosphor CRT style: `▸ 余额 CNY ¥32.81 · 连接正常`, `刷新` button for on-demand refresh, 30s auto polling, rail state collapses into a status lamp
- **Capability**: 🔐 Zero-config key · **Description**: Reuses `DEEPSEEK_API_KEY` from dsh's credential service (never written to disk, never logged)
- **Capability**: 💱 Multi-currency · **Description**: CNY/USD both listed (CNY first, `$`/`€`/`£` rendered per currency); amounts stay strings end-to-end, no float math
- **Capability**: ⛰️ Peak/off-peak pricing · **Description**: Shows `高峰 HH:MM:SS` / `空闲` in the status bar; during peak hours it counts down to the next off-peak period (Beijing time). Before 2026-08-17 the same windows are previewed, with the official billing start shown in the tooltip.
- **Capability**: 💰 Session cost · **Description**: The sidebar badge also shows the current conversation's estimated cumulative cost (`本会话 ¥xx.xx`), calculated from official DeepSeek V4 peak/off-peak pricing; it follows the currently selected session automatically
- **Capability**: 🎨 UI style switch · **Description**: `原生`/`矩阵` button toggles between the Matrix CRT badge and a native dsh look
- **Capability**: 🛡️ Rate-limit friendly · **Description**: 30s TTL cache + request serialization (at most one in-flight request) + 5s timeout
...
### Key · Default · Meaning
- **Key**: `apiKeyEnv` · **Default**: `DEEPSEEK_API_KEY` · **Meaning**: Credential reference (env var name); change for a different account
- **Key**: `cacheTtlMs` · **Default**: `30000` · **Meaning**: Cache lifetime in ms
- **Key**: `pollIntervalMs` · **Default**: `30000` · **Meaning**: Background polling interval in ms
- **Key**: `injectEveryTurn` · **Default**: `false` · **Meaning**: Whether to inject the balance into context every turn (opt-in; the badge and `ds_balance` tool are on by default)
- **Key**: `requestTimeoutMs` · **Default**: `5000` · **Meaning**: Official-API request timeout in ms
...
### State · Behavior
- **State**: No API key configured · **Behavior**: Tool returns a Chinese hint; badge shows amber `未配置密钥`
- **State**: API failure + stale cache · **Behavior**: Last snapshot is returned, marked "snapshot expired Ns (last refresh failed, retrying)"
- **State**: API failure + no cache · **Behavior**: The real failure (HTTP status / network / timeout / bad response) is surfaced; badge shows amber `无信号`
- **State**: Invalid or rejected API key · **Behavior**: Tool returns the real reason; badge shows amber `未配置密钥`
- **State**: Injection-time API failure · **Behavior**: Silent degradation: nothing injected, conversation unaffected
...
### Phase · Status text
- **Phase**: Peak · **Status text**: `高峰 02:14:23` — counts down to the off-peak period
- **Phase**: Off-peak · **Status text**: `空闲`
- **Phase**: Before 2026-08-17 · **Status text**: Same windows are previewed; the tooltip notes `2026-08-17 00:00` as the billing start
...
`/plugins//client.js`), then runs `scripts/verify-client.mjs` (bundle registration,
...
- **能力**: 🎨 UI 风格切换 · **说明**: `原生`/`矩阵` 按钮在 Matrix CRT 与 dsh 原生风格之间切换，选择保存在 `localStorage`
- **能力**: 🛡️ 限流友好 · **说明**: 30s TTL 缓存 + 请求串行化（同一时刻最多一个在途请求）+ 5s 超时
...
### 键 · 默认值 · 说明
- **键**: `apiKeyEnv` · **默认值**: `DEEPSEEK_API_KEY` · **说明**: 凭证引用名（环境变量名），多账号时改这里
- **键**: `cacheTtlMs` · **默认值**: `30000` · **说明**: 缓存有效期（毫秒）
- **键**: `pollIntervalMs` · **默认值**: `30000` · **说明**: 后台轮询间隔（毫秒）
- **键**: `injectEveryTurn` · **默认值**: `false` · **说明**: 是否每轮注入余额到模型上下文（默认关闭，按需开启）
- **键**: `requestTimeoutMs` · **默认值**: `5000` · **说明**: 官方接口请求超时（毫秒）
...
### 状态 · 表现
- **状态**: 未配置 key · **表现**: 工具返回中文提示；徽章显示琥珀 `未配置密钥`
- **状态**: 接口失败 + 有旧缓存 · **表现**: 返回最后一次快照并标注"快照已过期 Ns（最近一次刷新失败，自动重试中）"
- **状态**: 接口失败 + 无缓存 · **表现**: 返回真实失败原因（HTTP 状态码 / 网络 / 超时 / 响应格式）；徽章显示琥珀 `无信号`
- **状态**: key 无效或未授权 · **表现**: 返回真实原因；徽章显示琥珀 `未配置密钥`
- **状态**: 每轮注入时接口失败 · **表现**: 静默降级：不注入、不打断对话
...
### 阶段 · 状态栏
- **阶段**: 高峰 · **状态栏**: `高峰 02:14:23`——倒计时到进入空闲阶段
- **阶段**: 空闲 · **状态栏**: `空闲`
- **阶段**: 2026-08-17 前 · **状态栏**: 按同一窗口预览；tooltip 标注 `2026-08-17 00:00` 为正式计费起点
