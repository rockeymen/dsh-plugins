...
![节点串总览（浅色模式）](docs/node-nav-overview.png)

## 功能

### 功能 · 说明
- **功能**: 节点串 · **说明**: 右缘纵向节点串，每条用户消息一个圆点；最多同时显示 15 个，窗口随阅读位置滑动（无滚动条），两端渐隐提示更多
- **功能**: 全量历史节点 · **说明**: host 半部从会话日志提取全部用户消息（含分页未加载的历史），导航在点击前就覆盖全部历史，页面保持原生分页不展开
- **功能**: 底部节点 · **说明**: rail 最底端固定一个方形「跳到底部（最新消息）」节点，形状与提示区别于消息节点
- **功能**: hover/focus 预览 · **说明**: 显示服务端全文（300 字符截断）与时间，键盘焦点同样触发
- **功能**: 点击跳转 · **说明**: scrollIntoView 居中 + 品牌蓝高亮环 1.2 秒
- **功能**: active 跟随 · **说明**: 滚动时药丸实时标出视口内最顶部的用户消息
- **功能**: 输入历史 · **说明**: 输入框为空时按 ↑ 回填最近一条用户输入，连续 ↑ 翻更早、↓ 翻回；编辑/发送后指针重置
- **功能**: details 避让 · **说明**: 右侧详情面板打开时节点串自动左移
- **功能**: 深色模式 · **说明**: 跟随 `body[data-ds-dark-theme]`
- **功能**: reduced-motion · **说明**: 系统减少动效偏好下禁用动画
- **功能**: 自动隐藏 · **说明**: 少于 2 条用户消息或非对话页时不显示
...
![hover 消息预览](docs/node-nav-hover.png)

**active 药丸跟随**：滚动聊天区时，药丸实时移动到视口内最顶部的用户消息节点。

![active 药丸跟随滚动](docs/node-nav-active.png)

**深色模式**：跟随 `body[data-ds-dark-theme]` 自动切换配色。

![深色模式](docs/node-nav-dark.png)
...
- **DOM 行状态**：锚点 key 形如 `<seq>:input-message<uuid>`，uuid 即消息 id，用 `[data-chat-anchor-key$=""]` 判定该消息行是否已加载进页面；
...
### 项目 · 数据源 · 本插件的取舍
- **项目**: [dsh-navbar](https://github.com/vlln/dsh-navbar) · **数据源**: DOM 扫描（同款思路） · **本插件的取舍**: 本插件采用相同的 DOM 扫描契约，但保留 React + `shell.overlay` 槽位形态（可停用/启用），并补充 details 面板避让
- **项目**: [dsh-turn-index](https://github.com/Simon314620/dsh-turn-index) · **数据源**: 会话快照 `binding(id).session` · **本插件的取舍**: 快照方案在部分 dsh 版本上只见最近窗口节点，故本插件改走 DOM
