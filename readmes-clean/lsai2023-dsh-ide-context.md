...
It reads the **Claude Code IDE integration** bridge — the same `~/.claude/ide/.lock` files and MCP-over-WebSocket protocol the Claude Code CLI uses — so one bundle serves both **IntelliJ IDEA** and **Visual Studio Code**.
...
- id: ide-context
  config:
    ...
    lockDir: ~/.claude/ide    # optional; where the IDE .lock files live
```
...
- A running Claude Code IDE session that has written a valid `~/.claude/ide/.lock` file.
