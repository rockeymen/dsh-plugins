...
- **Access 方式**: Harness 原生审批 · **沙箱扩权请求**: 每次询问用户 · **适合场景**: 命令变化大，必须逐次判断
- **Access 方式**: `Approve for me` · **沙箱扩权请求**: 规则筛选、可选模型复核，不确定时询问用户 · **适合场景**: 重复且边界明确的工作流
- **Access 方式**: `Full access` · **沙箱扩权请求**: 关闭沙箱审批边界 · **适合场景**: 已明确接受完整权限风险的环境
...
![](https://github.com/user-attachments/assets/ba3df97b-3d35-47b7-a8a4-cd7772c48eb6)
...
![](https://github.com/user-attachments/assets/30041b07-ec0d-42c9-aabf-67f619ac50af)
（互动的测试集，等完善了规则之后更新出来）
...
`@latest` 是 dist-tag，不是版本锁定。当前包使用 `publishConfig.tag: beta` 发布 beta 版本。发布 `` 时，beta 标签会更新；如果文档要跟随最新 beta，还必须同步移动 `latest`，否则 `@latest` 仍会指向旧版本。例如：
...
### 字段 · 默认值 · 限制
- **字段**: `mode` · **默认值**: `rules-and-llm` · **限制**: `rules-only` 或 `rules-and-llm`
- **字段**: `rules.commandPrefixes` · **默认值**: `[]` · **限制**: 最多 200 条；工具只能是 `shell` 或 `pwsh`
- **字段**: 单条 `prefix` · **默认值**: 无 · **限制**: 非空字面命令分段，最多 1000 字符
- **字段**: `rules.reviewerInstructions` · **默认值**: `''` · **限制**: 最多 8000 字符
- **字段**: `reviewer.timeoutMs` · **默认值**: `30000` · **限制**: 1000 到 120000 毫秒
- **字段**: `reviewer.provider/model` · **默认值**: 当前会话 · **限制**: 同时填写或同时省略
...
### 结果 · 插件动作
- **结果**: 固定高风险、解析歧义或关联失败 · **插件动作**: 转原生人工审批
- **结果**: 任一命令分段未匹配 · **插件动作**: 转原生人工审批
- **结果**: `rules-only` 完整匹配 · **插件动作**: 返回一次 `allowed-once`
- **结果**: reviewer 明确允许 · **插件动作**: 返回一次 `allowed-once`
- **结果**: reviewer 拒绝、升级、超时、报错或输出无效 · **插件动作**: 转原生人工审批
...
### 项目 · 行为
- **项目**: 审批上下文 · **行为**: 读取当前 escalation、关联的工具调用和有长度上限的 transcript
- **项目**: reviewer 输入 · **行为**: 区分可信说明与不可信工具数据，并在发送前限制长度和清理常见凭据格式
- **项目**: reviewer 权限 · **行为**: 使用全新 agent；不提供任何工具
- **项目**: 模型路由 · **行为**: 继承当前会话或使用显式 provider/model；凭据由 Harness 管理
- **项目**: Web 写入 · **行为**: loopback-only RPC 委托官方 Settings service 持久化
- **项目**: 网络 · **行为**: 模型请求通过 Harness 已配置的 provider 路由；插件 Web RPC 不对非 loopback 客户端开放
- **项目**: 批准范围 · **行为**: 只返回当前请求的一次性 `allowed-once`

## 故障排查

### 现象 · 检查
- **现象**: Access 菜单没有 `Approve for me` · **检查**: 确认插件安装在当前 Profile，重启 Profile，并检查 `--dump-config`
- **现象**: Web 设置卡片不显示 · **检查**: 使用 `web` Profile，并通过 `127.0.0.1` 或其他 loopback 地址访问
- **现象**: 命令匹配前缀但仍询问 · **检查**: 检查复合命令的每个分段、高风险信号、工具类型和 reviewer 结果
- **现象**: reviewer 没有运行 · **检查**: 确认模式不是 `rules-only`，规则已匹配，且请求会话有完整模型路由
- **现象**: provider/model 校验失败 · **检查**: 两者必须同时填写；要继承当前会话时同时清空
- **现象**: 保存提示 revision 冲突 · **检查**: 刷新设置卡片，基于最新值重新编辑和保存
- **现象**: 安装出现 peer warning · **检查**: 核对当前 Harness 版本与插件兼容性；当前验证基线为 `0.1.0-rc.6`。再按提示运行 `pnpm peers check`，不要忽略实际版本冲突
- **现象**: `@latest` 仍显示旧版本 · **检查**: 停止 Host，执行具体版本 fallback，检查 `list`；仍旧锁定时 remove 后重新 add
- **现象**: Web 可以配置但另一个 Profile 不生效 · **检查**: 插件与设置按 Profile 隔离，需要分别安装和配置
...
### 项目 · 说明
- **项目**: DeepSeek Harness · **说明**: 开发与验证基线为 `0.1.0-rc.6`；新版本在验证后跟进
- **项目**: Node.js · **说明**: `^22.19.0 ·  · >=24.0.0`
- **项目**: Cordis · **说明**: `^4.0.1`
- **项目**: npm 通道 · **说明**: 默认使用 `@latest`；需要跟随 beta 发布时使用 `@beta`
