# dsh-task-panel
  ...
  任务面板（Task Panel）—— DeepSeek Harness (DSH) 插件：六列看板 + 双队列任务队列，在子 session 中由 agent 串行执行需求，并完成验收闭环。

  · [View on GitHub](https://github.com/chinazkk/dsh-task-panel)

> dsh-task-panel 是一个社区维护的 DSH 插件，非 DeepSeek 官方产品。

  ![任务面板六列看板（示例数据）](assets/screenshots/task-panel-board.webp)
...
### 阶段 · 你能做什么
- **阶段**: 需求队列 · **你能做什么**: 新建 / 编辑 / 删除 / 绑定工作目录 / 丢执行
- **阶段**: 执行队列 · **你能做什么**: 置顶 / 撤回 / 删除
- **阶段**: 执行中 · **你能做什么**: 实时进度预览 · 「查看进度」直达子代理会话 · 暂停 / 停止
- **阶段**: 已暂停 · **你能做什么**: 恢复（重入执行队列）
- **阶段**: 待验收 · **你能做什么**: 一句话产物 · 查看对话（跳转真实子会话 / 摘要回退）· 通过 / 返工（附反馈）
- **阶段**: 验收完成 · **你能做什么**: 查看对话 · 产物展开/收起
...
### 文件 · 平台 · 职责
- **文件**: `src/index.ts` → `lib/index.js` · **平台**: Host · **职责**: 数据模型 + 状态机 + 双队列调度 + 子 session 派发 + 8 个 Agent 工具 + Client RPC（webServer 路由桥）+ 持久化
- **文件**: `src/client/index.ts` → `lib/client.js` · **平台**: Client（浏览器 bundle） · **职责**: 六列看板 + 需求表单 + 验收面板；经 `/plugins/dsh-task-panel/rpc` 调 Host
- **文件**: `cordis.patch.yml` · **平台**: bundle 层 · **职责**: 向 profile 插入 `dsh-task-panel` 插件行
