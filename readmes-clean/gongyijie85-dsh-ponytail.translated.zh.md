...
### 技能 · 用途
- **技能**: `ponytail` · **用途**: 核心模式:强制"最懒但能用的方案",YAGNI → stdlib → 原生 → 一行 → 最小代码。支持 lite / full(默认)/ ultra 三档
- **技能**: `ponytail-review` · **用途**: 只针对过度工程的代码评审:每行一个发现(位置、删什么、用什么替代)
- **技能**: `ponytail-audit` · **用途**: 全仓库过度工程审计(ponytail-review 的仓库版):排序输出"最大削减优先"
- **技能**: `ponytail-debt` · **用途**: 把代码里所有 `ponytail:` 注释收进债务台账,延迟不烂尾
- **技能**: `ponytail-gain` · **用途**: 展示 ponytail 的实测收益记分牌(基准中位数)
- **技能**: `ponytail-help` · **用途**: 全部模式/技能/命令的速查卡
