...
### 补丁 · 文件数 · 作用
- **补丁**: ui-conversation user-actions 插槽 · **文件数**: 4 · **作用**: 用户消息下方的 ✏️ 按钮座位 + 原位编辑锚点(`position: relative`)
- **补丁**: ui-workspace 版本折叠 · **文件数**: 2 · **作用**: 边栏永远隐藏版本 fork、当前版本映射回原始行、版本内活动折算进对话排序、恢复最后查看的版本
...
- **Patch**: `ui-conversation` user-actions slot · **Files**: 4 files · **What it adds**: the ✏️ button seat under user messages + the in-place edit anchor (`position: relative`)
- **Patch**: `ui-workspace` version-fork folding · **Files**: 2 files · **What it adds**: hide version forks from the sidebar (always), alias the open fork to its original row, fold fork activity into the conversation's recency, restore the last-viewed version
