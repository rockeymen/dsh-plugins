...
## Screenshots
  ...
  ![Badge over light content, text switches to dark](assets/badge-light.png)
  ![Badge over dark content, text switches to light](assets/badge-dark.png)
  ![Frosted-glass acrylic texture](assets/badge-acrylic.png)
...
- **Provider**: DeepSeek · **Balance endpoint**: `GET https://api.deepseek.com/user/balance` · **Auth**: API Key (`sk-...`) · **Notes**: total / granted / topped-up (CNY)
- **Provider**: Moonshot (Kimi) · **Balance endpoint**: `GET https://api.moonshot.cn/v1/users/me/balance` · **Auth**: API Key (`sk-...`) · **Notes**: available / voucher / cash (CNY)
- **Provider**: OpenAI · **Balance endpoint**: `GET https://api.openai.com/dashboard/billing/credit_grants` · **Auth**: Browser session token (`sess-...`) · **Notes**: OpenAI offers no API-key balance endpoint; session tokens expire
- **Provider**: Custom · **Balance endpoint**: any GET endpoint (Bearer auth) · **Auth**: your own · **Notes**: JSON field paths like `data.available_balance` or `balance_infos[0].total_balance`, such as relay stations
