...
# 启用/禁用（DSH 无原生命令）：编辑 /cordis.patch.yml 添加
...
### 文件 · 说明
- **文件**: `docs/marketplaces.json` · **说明**: 完整市场列表（Schema v2.5.0，id 为 m-XXXX 编号）
- **文件**: `docs/summary.json` · **说明**: 统计摘要（总数/分类计数/活跃数，面板快速加载用）
- **文件**: `MARKETS.md` · **说明**: 人类可读市场目录（自动生成，分类链接 + 简介）
- **文件**: `schema/schema.json` · **说明**: JSON Schema 定义（v2.5.0）
- **文件**: `data/` · **说明**: 流水线中间产物（排除名单/待复核/失败队列/变更日志，不发布）
...
### 级别 · 字段 · 说明
- **级别**: L0 标识 · **字段**: `id` · **说明**: 不可修改
- **级别**: L1 名称 · **字段**: `name` · **说明**: 谨慎（仅明显错误时）
- **级别**: L2 内容 · **字段**: `description` / `tags` / `usage_tip` / `npm_package` · **说明**: AI 可修正
- **级别**: L3 元数据 · **字段**: `popularity` / `item_count` / `status` / `last_plugin_update` · **说明**: 脚本维护，AI 不碰
...
### 分类 · 含义
- **分类**: `plugin` · **含义**: 插件（会话内搜索/发现类）
- **分类**: `marketplace` · **含义**: 插件市场（DSH 内安装的市场）
- **分类**: `website` · **含义**: 插件市场网站（独立发现站）
- **分类**: `library` · **含义**: 精选库（awesome 类目录）
