...
### 能力 · 状态
- **能力**: 会话列表/新建/续聊/fork/compact、SSE 流式消息 · **状态**: ✅
- **能力**: 会话列表真实标题（标题 → 目录名 → id 回退） · **状态**: ✅
- **能力**: 模型目录、reasoning effort、agent preset 切换 · **状态**: ✅
- **能力**: 工具卡片（bash/read/write/edit）、diff 与 Modified Files · **状态**: ✅
- **能力**: 工具参数流式显示 · **状态**: ✅
- **能力**: 权限/提问流、子代理会话树与后台子代理 · **状态**: ✅
- **能力**: Goal 创建/查看（sidebar + `/goal`） · **状态**: ✅
- **能力**: Esc 打断/取消（全量双按、mini 单按） · **状态**: ✅
- **能力**: 文本/图片附件 · **状态**: ✅（PDF 等二进制暂不支持）
- **能力**: `Allow always` 权限 · **状态**: ✅（会话内内存记忆，重启清空）
- **能力**: MCP / LSP / formatter / skills / integration 等外围路由 · **状态**: ❌（schema-valid stub）

opencode 子进程直接使用官方二进制（锁定 `1.18.18`）。

## 演示

![dsh-oc 核心功能演示（真实 DeepSeek 模型）](docs/demo/dsh-oc-demo.gif)
