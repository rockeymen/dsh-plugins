...
`GET /api/whale-pet/progress?session=`). It only reads — it **never
...
- **Session state**: Assistant tokens or tool calls are running · **Pet reaction**: `working`/`thinking`: faster swimming, input-area gaze, periodic bubbles
- **Session state**: One turn runs longer than 20s · **Pet reaction**: `focused`: a slight dive posture
- **Session state**: Tool fails (non-zero exit code or error node) · **Pet reaction**: `error`: pouring sweat drops, a trembling body, a pulsing red "！" mark and wide eyes for 3s
- **Session state**: Long turn (≥15s), goal completion, or plan exit · **Pet reaction**: `celebrating`: a 360° elliptical lap with continuous yaw and screen-space depth, while hearts stream every 650ms
- **Session state**: You are composing a reply in the chat input · **Pet reaction**: `listening`: gazes at the input area, shows a floating "？", and recaps "在呢，我听着～" on click
- **Session state**: Session is blocked on you (`pendingInteraction`: approval / question / plan-review) · **Pet reaction**: `awaiting`: same gaze and "？" as listening; click recaps "有个审批等你拍板" / "有个问题等你回答" / "有个计划等你过目". Wins over `working` because the turn often stays marked running.
- **Session state**: Forceful or prolonged drag release · **Pet reaction**: `dizzy`: freezes pointer interaction, rolls belly-up, weakly wobbles, and shows a depth-aware elliptical star orbit for about 4s
- **Session state**: Hover or drag while sleeping · **Pet reaction**: Wakes immediately and resets the idle clock
- **Session state**: No activity for 60s · **Pet reaction**: `sleeping`: closed eyes, slow breathing, z-z-z
- **Session state**: First idle/sleep of a local calendar day · **Pet reaction**: one unsolicited greeting ("今天也在～" / "又见面啦，第 N 天"); at most once a day
- **Session state**: A compaction node lands after the bind settle window · **Pet reaction**: one "记忆被压扁了一点，我还在～" bubble; never an LLM daemon
