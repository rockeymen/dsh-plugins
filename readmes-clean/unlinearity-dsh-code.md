...
![DSH-Code terminal with slash-command completion](docs/pictures/1.png)
...
- **Reference**: **DeepSeek Harness** · **Used in DSH-Code**: Plugin composition, scoped services, Agent Presets, durable sessions, tools, skills, policies, sandboxing, and delegation
- **Reference**: **Codex CLI** · **Used in DSH-Code**: Session navigation, bounded overlays, history inspection, stable bottom layout, and resize handling
- **Reference**: **Claude Code** · **Used in DSH-Code**: Slash-command discovery, thinking folds, approvals, questions, and turn steering
...
![Per-session Agent Preset picker](docs/pictures/2.png)

Use `/resume` to search persisted sessions without replacing the TUI.

![Searchable session resume picker](docs/pictures/3.png)
...
Use `/mode` before the first turn or start directly with `--mode `. The selected preset is written to the session and restored on resume.
...
### Action · Purpose
- **Action**: `/new [preset]` · **Purpose**: Create and enter another session without restarting the terminal
- **Action**: `/resume [id\ · **Purpose**: prefix]` · Search roots or all conversations; filter by cwd, order, and density
- **Action**: `/mode [preset]` · **Purpose**: Inspect or select the blank session's Agent composition
- **Action**: `/model` · **Purpose**: Switch among models advertised by the live LLM registry
- **Action**: `/plugin [query]` · **Purpose**: Inspect loader entries, enabled state, module identity, and fiber phase
- **Action**: `/permission <name>` · **Purpose**: Change the permission preset; Shift+Tab cycles presets
- **Action**: `/help` · **Purpose**: Browse local commands, Harness commands, skills, and key bindings
- **Action**: `Ctrl+O` · **Purpose**: Open the exclusive history detail view; switch entries and scroll all content
- **Action**: `Ctrl+R` · **Purpose**: Fold or reveal model reasoning
- **Action**: `@` · **Purpose**: Mention workspace files or bounded snapshots of persisted sessions
- **Action**: `Esc` / `Ctrl+C` · **Purpose**: Close the topmost surface or interrupt the active turn
