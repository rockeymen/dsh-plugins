![DSH 深度拆解](https://raw.githubusercontent.com/Bin-hy/dsh/main/docs/public/banner.svg)
...
flowchart TB
  subgraph 组合平面["组合平面 · 静态"]
    Profile["profile(web / headless)"] --> Bundle["bundle 层(dsh-base → dsh-web-app)"]
    Bundle --> Patch["cordis.patch.yml+ --patch overlay"]
  end

  subgraph 运行平面["运行平面 · 动态"]
    Driver["Agent Loop 驱动器(turn/step 状态机)"]
    Driver -->|"agent/pre-step"| PreStep["拦截链(compaction / 技能 / 指令)"]
    PreStep -->|"agent/request"| LLM["ctx.llm 流式管道"]
    LLM -->|"assistant/chunk*"| Log["SessionEvent 日志(唯一真源)"]
