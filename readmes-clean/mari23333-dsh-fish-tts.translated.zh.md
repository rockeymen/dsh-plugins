# dsh-fish-tts
  ...
  ![dsh-fish-tts — DeepSeek Harness 语音合成插件（仅支持 Fish Audio API）](./assets/readme/hero.svg)
...
- **对比项**: 合成引擎 · **dsh-fish-tts（本插件）**: **Fish Audio 官方 API**（仅支持；需自备 Key） · **常见 Edge TTS 朗读插件**: 微软 Edge 内置语音
- **对比项**: 音色 · **dsh-fish-tts（本插件）**: 你自己的音色 `reference_id`（含自行克隆的音色，须有使用权限） · **常见 Edge TTS 朗读插件**: Edge 内置固定音色库
- **对比项**: API Key · **dsh-fish-tts（本插件）**: **必须自备**（设置页 AES-256-GCM 加密保存） · **常见 Edge TTS 朗读插件**: 无需
- **对比项**: 适合谁 · **dsh-fish-tts（本插件）**: 已有 Fish Audio 账号、想要自己或克隆音色的用户 · **常见 Edge TTS 朗读插件**: 想免费快速试用、固定音色即可的用户
...
## 界面截图
  ...
  ![消息操作条上的朗读按钮](./assets/readme/screenshot-read-aloud.png)
  消息操作条上的「朗读」按钮

  ![输入栏的自动朗读开关](./assets/readme/screenshot-auto-read.png)
  输入栏的自动朗读开关

  ![语音合成设置页（模型 / 音色 / API Key / 代理）](./assets/readme/screenshot-settings.png)
  设置页：模型 / 音色 / API Key / 代理 / 试听
...
### Key · 默认 · 说明
- **Key**: `model` · **默认**: `''` · **说明**: 默认模型（设置页保存值优先）
- **Key**: `voice` · **默认**: `''` · **说明**: 默认音色 reference_id（设置页保存值优先）
- **Key**: `format` · **默认**: `wav` · **说明**: `wav` / `mp3` / `opus` / `pcm`
- **Key**: `apiKey` · **默认**: `''` · **说明**: 一般不填；优先使用设置页加密保存的 Key，其次环境变量 `FISH_API_KEY`
- **Key**: `apiKeyFile` · **默认**: `''` · **说明**: 读取指定 dotenv 文件的 `FISH_API_KEY`
- **Key**: `proxy` · **默认**: `''` · **说明**: HTTP(S) 代理（设置页保存值优先）
- **Key**: `stateDir` · **默认**: `$DSH_HOME/fish-tts` · **说明**: 设置/密钥文件目录
