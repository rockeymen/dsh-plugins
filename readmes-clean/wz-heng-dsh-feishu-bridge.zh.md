...
### 命令 · 作用
- **命令**: `/new [名称]` · **作用**: 开始一个新会话
- **命令**: `/sessions` · **作用**: 列出会话（点击切换）
- **命令**: `/switch ` · **作用**: 切换到某个已有会话
- **命令**: `/current` · **作用**: 查看当前会话信息
- **命令**: `/quiet` · **作用**: 只显示回复（默认）
- **命令**: `/verbose` · **作用**: 同时显示状态/结果行
- **命令**: `/help` · **作用**: 列出命令
...
### 环境变量 · 默认值 · 含义
- **环境变量**: `DEEPSEEK_API_KEY` · **默认值**: — · **含义**: 必填。和 SDK 自身读取的变量同名。
- **环境变量**: `DEEPSEEK_BASE_URL` · **默认值**: — · **含义**: 可选，用于 OpenAI 兼容代理。
- **环境变量**: `DSH_PROVIDER` · **默认值**: `deepseek-official` · **含义**: Provider 路由（见 SDK 文档）。
- **环境变量**: `DSH_MODEL` · **默认值**: `deepseek-v4-flash` · **含义**: 模型 id。
- **环境变量**: `DSH_MAX_TOKENS` · **默认值**: 未设置 · **含义**: 可选的单请求输出上限。
- **环境变量**: `DSH_CORDIS` · **默认值**: 未设置 · **含义**: 自定义 Cordis composition 路径；不填则用内置默认。
- **环境变量**: `DSH_SESSION_ROOT` · **默认值**: 未设置 · **含义**: runtime 写 JSONL session 日志的目录。
- **环境变量**: `DSH_WORKSPACE` · **默认值**: 当前目录 · **含义**: agent 工具操作的工作区。
- **环境变量**: `FEISHU_APP_ID` / `FEISHU_APP_SECRET` · **默认值**: — · **含义**: 必须同时配置，或都不配置。
- **环境变量**: `FEISHU_TRANSPORT` · **默认值**: `ws` · **含义**: `ws`（无需公网 URL）或 `webhook`。
- **环境变量**: `FEISHU_VERIFICATION_TOKEN` · **默认值**: — · **含义**: `FEISHU_TRANSPORT=webhook` 时必填。
- **环境变量**: `FEISHU_ENCRYPT_KEY` · **默认值**: 未设置 · **含义**: `FEISHU_TRANSPORT=webhook` 时必填——在飞书开发者后台为该事件订阅开启 "Encrypt Key" 并填入同样的值。用于校验每个请求的 `X-Lark-Signature`（见"安全姿态"）。
- **环境变量**: `FEISHU_DOMAIN` · **默认值**: `https://open.feishu.cn` · **含义**: Lark 国际版或走代理时修改。
- **环境变量**: `FEISHU_ALLOWED_OPEN_IDS` · **默认值**: *(空)* · **含义**: 逗号分隔。**必填**——为空则无人被授权。
- **环境变量**: `FEISHU_ALLOWED_CHAT_IDS` · **默认值**: *(空 = 不限制)* · **含义**: 逗号分隔的群聊白名单。
- **环境变量**: `DSH_FEISHU_BRIDGE_HOST` · **默认值**: `0.0.0.0` · **含义**: HTTP 服务绑定地址（健康检查 + webhook 路由）。
- **环境变量**: `DSH_FEISHU_BRIDGE_PORT` · **默认值**: `8788` · **含义**: HTTP 服务端口。
