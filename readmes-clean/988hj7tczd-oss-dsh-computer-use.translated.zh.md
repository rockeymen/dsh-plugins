...
### 工具 · 功能
- **工具**: `screen_observe` · **功能**: 看屏幕：AX 编号树 + 坐标（零视觉 token 成本）；AX 树为空自动降级视觉
- **工具**: `computer_click` / `computer_double_click` / `computer_right_click` · **功能**: **真人操作**：独立光标滑行到目标 + 像素级真实点击（看得见过程）
- **工具**: `computer_type` · **功能**: 文本输入（密码框自动拒绝）
- **工具**: `computer_key` · **功能**: 按键 / 快捷键（return、cmd+c…；聊天窗口回车发送）
- **工具**: `computer_scroll` · **功能**: 滚动
- **工具**: `computer_drag` · **功能**: 拖拽
- **工具**: `computer_wait` · **功能**: 等待
- **工具**: `app_list` / `app_launch` · **功能**: 列出 / 启动应用
