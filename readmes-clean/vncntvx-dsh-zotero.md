...
### 工具 · 用途
- **工具**: `zotero_search` · **用途**: 发现：按标题/作者/年份搜索库里的资料，`everything` 模式连全文索引一起搜；可限定某个分类或已保存搜索
- **工具**: `zotero_get` · **用途**: 检查：读取一条资料的结构化核心元数据，可选检查笔记、注释、附件的清单与预览。
- **工具**: `zotero_retrieve` · **用途**: 取证：按问题返回最相关的有界证据片段（注释、笔记、摘要、全文分块）
- **工具**: `zotero_attachment` · **用途**: 原文：解析条目或附件 ref，返回原始附件已验证的磁盘路径或链接 URL
- **工具**: `zotero_export` · **用途**: 引用：让 Zotero 按自己的 citation/export 能力生成结果（引用、CSL 参考文献表、`bibtex` / `biblatex` / `ris` / `csljson`）。
...
### 字段 · 默认值 · 含义
- **字段**: `baseUrl` · **默认值**: `http://127.0.0.1:23119/api` · **含义**: 本地 API 基础 URL。仅支持纯回环 HTTP。
- **字段**: `provider` · **默认值**: `local` · **含义**: 要选择的 provider id。
- **字段**: `timeoutMs` · **默认值**: `5000` · **含义**: 每个请求的 provider 超时时间。
- **字段**: `maxSearchResults` · **默认值**: `20` · **含义**: `zotero_search` `limit` 的上限。
- **字段**: `maxNoteScanRecords` · **默认值**: `200` · **含义**: `zotero_search` 补扫笔记正文的笔记数量上限。
- **字段**: `maxEvidenceChars` · **默认值**: `6000` · **含义**: 检索证据的总字符预算。
- **字段**: `maxEvidencePassages` · **默认值**: `4` · **含义**: 证据片段数量的上限。
- **字段**: `maxDetailChars` · **默认值**: `3000` · **含义**: `zotero_get` 摘要预览的字符预算。
- **字段**: `maxNoteBodyChars` · **默认值**: `30000` · **含义**: `zotero_get` 返回 note 条目自身正文的字符预算。
- **字段**: `maxNoteChars` · **默认值**: `2000` · **含义**: `zotero_get` 单条笔记预览的字符预算。
- **字段**: `maxNoteRecords` · **默认值**: `50` · **含义**: `zotero_get` 返回笔记数量的上限。
- **字段**: `maxAnnotationRecords` · **默认值**: `100` · **含义**: `zotero_get` 返回批注数量的上限。
- **字段**: `fulltextChunkWords` · **默认值**: `200` · **含义**: 进入证据排序的全文片段词数。
- **字段**: `maxFulltextChars` · **默认值**: `250000` · **含义**: 进入证据排序的全文大小上限。
- **字段**: `maxResponseBytes` · **默认值**: `16777216` · **含义**: 每个 API 响应的流式字节上限。
- **字段**: `maxExportChars` · **默认值**: `1000000` · **含义**: 导出输出的硬上限。不会中途截断。
- **字段**: `maxExportRefs` · **默认值**: `1000` · **含义**: 单次 `zotero_export` 的 refs 数量上限，保护请求行不超服务器 HTTP 头限制。
- **字段**: `defaultStyle` · **默认值**: `apa` · **含义**: 引用/参考文献使用的 CSL 样式。
- **字段**: `defaultLocale` · **默认值**: `en-US` · **含义**: 引用/参考文献使用的 CSL locale。
- **字段**: `webEnabled` · **默认值**: `true` · **含义**: 是否在会话顶部显示 Zotero 专属标签页；开关在每次页面加载时读取，切换后需刷新页面生效。
