...
### Key · Default · Meaning
- **Key**: `baseUrl` · **Default**: `https://opencode.ai/zen/go/v1/usage` · **Meaning**: The usage endpoint.
- **Key**: `timeoutMs` · **Default**: `15000` · **Meaning**: Fetch timeout in milliseconds.
...
### File · Role
- **File**: `index.js` · **Role**: Host half — `OpencodeUsageGateway` (`TypertRemoteService`, service key `opencodeUsage`)
- **File**: `typert.host.js` · **Role**: Hand-written Typert host manifest, registered via `exports["./typert"]`
- **File**: `client.js` · **Role**: Browser bundle in `window.__ModuleLoader__.load` format — mounts the Remote, registers the dock entry, renders the readout
