...
### Tool · What it does
- **Tool**: `xlsx_read` · **What it does**: List sheets of a `.xlsx`, or read one sheet as a markdown table (range-limited for large files; formula text preserved)
- **Tool**: `xlsx_write` · **What it does**: Write a 2D array to a new `.xlsx` (formula cells, header bold, column widths, number formats)
- **Tool**: `xlsx_edit` · **What it does**: Edit an existing `.xlsx`: add sheets, update cells (value or formula), append rows
- **Tool**: `pdf_create` · **What it does**: Generate a real PDF with headings, paragraphs, tables, lists, code blocks and footer page numbers; CJK text renders via an auto-detected system font
- **Tool**: `pdf_read` · **What it does**: Extract text from a PDF for reading into context
- **Tool**: `pptx_create` · **What it does**: Generate a `.pptx` deck from slide definitions (title / section / content / two-column / image / table / chart), with optional theme and speaker notes
- **Tool**: `pptx_read` · **What it does**: Extract slide text as markdown, optionally including speaker notes
