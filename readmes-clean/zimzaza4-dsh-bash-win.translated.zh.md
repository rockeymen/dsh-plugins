...
### 维度 · 机制 · 说明
- **维度**: **默认执行** · **机制**: 无沙箱，全权限 · **说明**: `git_bash` / `wsl_bash` 默认直接跑，可读写任意路径(绕开沙箱的代价)
- **维度**: **沙箱** · **机制**: WSL 内 bwrap(`sandbox: true`) · **说明**: 系统目录只读、workspace 可写、`/mnt/c` 其余只读;越界写入被拒并报 `[sandbox: file access denied under workspace-write mode]`
- **维度**: **审批** · **机制**: `ctx.approval` · **说明**: `require_approval: true` 执行前弹窗;升权请求(`sandbox_permissions` + `justification`)弹窗;`ask` 策略弹窗、`never` 自动拒绝
...
### 工具 · 后端 · UI 标题 · 能力
- **工具**: `git_bash` · **后端**: 本地 Git Bash(MSYS2) · **UI 标题**: Git Bash · **能力**: 批准模式(`require_approval`)、官方沙箱接入(`sandbox`，Windows 受限模式报 Cygwin 限制)、后台任务
- **工具**: `wsl_bash` · **后端**: WSL2(Linux) · **UI 标题**: WSL Bash · **能力**: **bwrap 真沙箱**(`sandbox: true`)、升权审批(`sandbox_permissions` + `justification`)、后台任务
