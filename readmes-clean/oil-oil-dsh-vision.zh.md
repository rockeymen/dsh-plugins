...
### 当前主模型 · 图片处理方式 · 最终回答者
- **当前主模型**: 支持图片 · **图片处理方式**: 原图直接发送，不压缩、不预先 OCR · **最终回答者**: 当前模型
- **当前主模型**: `deepseek-official` 等文本模型 · **图片处理方式**: 外部视觉模型读取原图，观察结果作为非可信附件上下文注入 · **最终回答者**: DeepSeek
- **当前主模型**: 云端视觉不可用 · **图片处理方式**: macOS Vision 或 Tesseract 本地降级 · **最终回答者**: DeepSeek
