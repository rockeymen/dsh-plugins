...
### Path · Role
- **Path**: `packages/governor-core` · **Role**: Framework-agnostic progress-fact engine (zero Cordis)
- **Path**: `packages/dsh-governor` · **Role**: DSH adapter plugin (function plugin, mounts DSH extension points)
- **Path**: `packages/dsh-bundle` · **Role**: Profile bundle (`dsh.bundle.patch` contract)
- **Path**: `packages/dsh-orcana-linux` · **Role**: Linux native hardening + the cross-platform `dsh-orcana` launcher
- **Path**: `packages/dsh-orcana-linux-bundle` · **Role**: Profile bundle for the Linux edition (`dsh.bundle.patch` contract)
- **Path**: `benchmark/` · **Role**: A/B harness: task manifests, patches, runner, reports
- **Path**: `scripts/` · **Role**: dev-install / smoke / bench-run
