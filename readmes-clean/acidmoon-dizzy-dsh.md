...
### 插件 · 能力 · 怎么用 · 状态
- **插件**: **余额查询** `dizzy-dsh-balance` · **能力**: DeepSeek 官方账户余额实时显示,每分钟自动刷新 · **怎么用**: 输入栏右侧常驻徽章;对话中直接问「余额」或调用 `balance_check` 工具;`/dizzy/balance` 命令 · **状态**: ✅ 稳定
- **插件**: **本月用量** `dizzy-dsh-usage-card` · **能力**: 本地会话日志聚合 token 用量:月度热力图 / 近 7 天趋势 / 今日分模型明细 / 峰谷时段 · **怎么用**: 对话区右侧「用量」Tab(对话、轨迹并列);悬浮弹窗看输入/输出/缓存分项;支持月份切换 + 60s 自动刷新 · **状态**: ✅ 稳定
- **插件**: **Agent 规则注入** `dizzy-dsh-agent-instructions` · **能力**: 向每个会话注入 Agent 规则:用户哨兵规则(第一性原理 / 对抗式审查 / 子代理优先 / 喵字开头)+ 开发规范(不重复造轮子 / 核心约定 / 防御性模式 / 类型安全) · **怎么用**: 装完即全局生效,所有会话、所有工作区;编辑规则文本**下一轮对话即生效**,无需重启 · **状态**: ✅ 稳定
- **插件**: **浏览器控制** `dizzy-dsh-kimi-webbridge` · **能力**: 通过 Kimi WebBridge(daemon + 浏览器扩展)控制你的**真实浏览器**:打开网页、读取页面、点击、填表、截图、抓包、存 PDF —— 带登录态的会话直接可用 · **怎么用**: 渐进式披露:模型先调用 `kimi_browser_activate` 引导工具,随后获得全套 `kimi_browser_*` 工具(导航/快照/点击/输入/截图/标签管理) · **状态**: ✅ 稳定

### 第三方插件(能力速览)

### 插件 · 能力 · 怎么用 · 状态
- **插件**: **视觉识别** `dsh-vision-toolkit` · **能力**: 看图问答 / 描述 / OCR / 元素定位 / 检测 / 像素对比 / 长截图 OCR / UI 还原 · **怎么用**: `vision_glance` / `vision_ground` / `vision_detect` / `vision_pixel_diff` 四个核心工具**随会话常驻**;其余工具加载 vision-tools skill 后可用 · **状态**: ✅ 稳定(v0.1.2)
- **插件**: **生成式 UI** `dsh-genui` · **能力**: 模型的回答中直接渲染可交互组件:数据卡片、图表、表格、表单、试卷判分、mermaid 流程图、3D 场景 · **怎么用**: 模型回答时自动输出 `dsh-ui` 围栏;`render_ui` 工具可把界面渲染到工具行 · **状态**: ✅ 稳定(v0.8.1)
- **插件**: **桌面通知** `dsh-notification` · **能力**: 会话跑完一轮任务时弹系统通知,切走也能知道进度 · **怎么用**: 设置 > 通知 可配:结束状态(完成/出错/中止/阻塞)、关键词包含/排除规则 · **状态**: ✅ 稳定(v0.1.1)
- **插件**: **IDE 侧边栏** `dsh-better-sidebar` · **能力**: VSCode 风格右侧侧边栏:资源管理器 / 编辑器 / 终端 / Git / 浏览器,按会话隔离 · **怎么用**: 界面右侧的侧边栏图标,即点即用 · **状态**: ✅ 稳定(v0.10.3)
- **插件**: **订阅登录** `dsh-subscription-auth` · **能力**: 用订阅会员账号 OAuth 登录模型提供商,而不是 API key:ChatGPT Plus/Pro、Claude Pro/Max、Grok、Kimi Code;登录后自动发现模型并出现在模型选择器 · **怎么用**: 设置 → 订阅服务 点「登录」;已登录渠道会出现在模型选择器,可选手动思考强度 · **状态**: ✅ 稳定(v0.2.1,有本地补丁)

### 收录的第三方预设(agent preset)

### 预设 · 能力 · 怎么用 · 状态
- **预设**: **Anchored Standard** `dsh-anchored-standard` · **能力**: 两阶段工具目录引导:首个模型请求只暴露 `pwsh/read`(Windows)或 `bash/read`(Linux),对齐 Minimal 的系统提示词条件;会话记录首次工具调用后,开放 Standard 的完整工具目录;阶段由持久会话事件推导,resume/刷新不丢状态 · **怎么用**: 运行 `scripts/install-anchored-standard.ps1` 装到 `~/.dsh/.agent-presets/`,重启后新会话预设选择「Anchored Standard (experimental)」 · **状态**: ✅ 稳定(v0.1.0,已对照 rc.6 核对)
...
### 插件 · 作者 · 项目 · 地址 · 版本 · 收录方式
- **插件**: dsh-vision-toolkit · **作者**: [Anionex](https://github.com/Anionex) · **项目**: dsh-vision-toolkit · **地址**: https://github.com/Anionex/dsh-vision-toolkit · **版本**: 0.1.2 · **收录方式**: 仓库快照
- **插件**: dsh-genui · **作者**: [omdsh-dev](https://github.com/omdsh-dev) · **项目**: dsh-genui · **地址**: https://github.com/omdsh-dev/dsh-genui · **版本**: 0.8.1 · **收录方式**: 仓库快照
- **插件**: dsh-notification · **作者**: [omdsh-dev](https://github.com/omdsh-dev) · **项目**: dsh-notification · **地址**: https://github.com/omdsh-dev/dsh-notification · **版本**: 0.1.1 · **收录方式**: 仓库快照
- **插件**: dsh-better-sidebar · **作者**: [omdsh-dev](https://github.com/omdsh-dev) · **项目**: DSH-better-sidebar · **地址**: https://github.com/omdsh-dev/DSH-better-sidebar · **版本**: 0.10.3 · **收录方式**: npm registry
- **插件**: dsh-anchored-standard · **作者**: [xiaobright](https://github.com/xiaobright) · **项目**: dsh-anchored-standard · **地址**: https://github.com/xiaobright/dsh-anchored-standard · **版本**: 0.1.0 · **收录方式**: 仓库快照(agent preset)
- **插件**: dsh-subscription-auth · **作者**: [Khellendros97](https://github.com/Khellendros97) · **项目**: dsh-subscription-auth · **地址**: https://github.com/Khellendros97/dsh-subscription-auth · **版本**: 0.2.1 · **收录方式**: 仓库快照 + 本地补丁
...
4. 可选:在 `settings.yaml` 的 `subscription-auth-` 段覆盖 `apiBaseURL` / `redirectPort` / `maxTokens`,或用 `models` 手动固定模型列表。密钥只进 credentials,settings 里不要写令牌。
...
### 文档 · 内容
- **文档**: [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) · **内容**: 架构与开发:双半区机制、平面规则、如何新增子包
- **文档**: [docs/THIRD-PARTY-UPDATE.md](docs/THIRD-PARTY-UPDATE.md) · **内容**: 第三方插件更新方案(git subtree 跟随上游 + 适配清单)
- **文档**: [docs/THIRD-PARTY-SNAPSHOTS.md](docs/THIRD-PARTY-SNAPSHOTS.md) · **内容**: 第三方插件上游登记表(仓库 / 版本 / commit / 补丁)
- **文档**: [docs/THIRD-PARTY-PATCHES.md](docs/THIRD-PARTY-PATCHES.md) · **内容**: 对快照的手工补丁登记(patches/ 目录 + 重放工具)