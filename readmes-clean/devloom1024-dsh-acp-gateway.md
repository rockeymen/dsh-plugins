...
### Capability · Detail
- **Capability**: ✅ Token-level streaming · **Detail**: `assistant/chunk` text deltas → `agent_message_chunk`
- **Capability**: ✅ Tool-call notifications · **Detail**: `tool_call` (pending) → `tool_call_update` (completed/failed)
- **Capability**: ✅ Full tool access · **Detail**: mounts the selected agent preset: bash, fs, web, skills, subagents, ...
- **Capability**: ✅ `session/list` / `session/load` / `session/delete` · **Detail**: resume persisted sessions with history replay
- **Capability**: ✅ `usage_update` · **Detail**: token usage from `assistant/message`
- **Capability**: ✅ Image / audio prompt content · **Detail**: image → DSH attachment; audio → textual reference
- **Capability**: ✅ Slash commands · **Detail**: `available_commands_update` + `/cmd` execution (incl. `/plan`, `/plan off` — the same channel the web GUI's Plan chip uses)
- **Capability**: ✅ Session modes · **Detail**: the **agent presets** (the web GUI's modes: Standard / Code(PTC) / Minimal / Creator / your custom presets), `session/set_mode` re-composes the agent, `current_mode_update`
- **Capability**: ✅ `user_message_chunk` · **Detail**: echo accepted prompts
- **Capability**: ✅ Embedded resource content · **Detail**: `resource` blocks expand into prompt text
- **Capability**: ✅ Session config options · **Detail**: ACP v1 `configOptions` (select) for `mode` (the agent presets), `model` (`provider/model`), `thought_level`, `permission` (read-only / workspace-write / danger-full-access)
- **Capability**: ✅ Permission approval flow · **Detail**: workspace-write asks the client through `session/request_permission` for mutating tools (edit/delete/move/execute)
- **Capability**: ✅ Elicitation · **Detail**: DSH `ask_user_question` surfaces as an ACP `elicitation/create` form; answers feed back as the tool result
- **Capability**: ✅ Thinking stream · **Detail**: `agent_thought_chunk` from DSH reasoning chunks
- **Capability**: ✅ Agent plan · **Detail**: `exit_plan_mode` markdown → ACP `plan` notification (entries)
- **Capability**: ✅ Session info · **Detail**: `session_info_update` on title changes — the deterministic fallback placeholder is suppressed, so a session's title notifies once (or on real change) and stays fixed
...
### Command · Purpose
- **Command**: `dsh-acp-gateway` · **Purpose**: the ACP agent itself (stdio direct mode)
- **Command**: `dsh-acp-server` · **Purpose**: alias of the above
- **Command**: `dsh-acp-agent` · **Purpose**: stdio bridge to a long-running server (bridge mode)
- **Command**: `dsh-acp-client` · **Purpose**: a scripted test client
...
Commands: `init`, `new [cwd]`, `prompt <text>`, `mode `,
...
### Symptom · Cause / fix
- **Symptom**: Editor stuck on "loading" · **Cause / fix**: The bridge could not reach any endpoint. Check `~/.dsh/acp/endpoint` points at a live server (`DSH_ACP_URL` overrides), or use direct mode (`npx -y dsh-acp-gateway`). The bridge now answers with a JSON-RPC error and re-reads the endpoint file instead of hanging
- **Symptom**: Typing `/` shows no slash commands · **Cause / fix**: Zed drops `available_commands_update` sent before the `session/new` response ([zed#60199](https://github.com/zed-industries/zed/issues/60199)); this gateway holds notifications until after the response — restart the agent and create a fresh session
- **Symptom**: `session/delete` leaves sessions behind · **Cause / fix**: Fixed: persisted session dirs are removed with direct fs
- **Symptom**: Multiple ACP instances · **Cause / fix**: Multiple clients can share one server (bridge mode); each direct-mode process is its own full DSH instance (~30-60 MB RSS each, 15-20s boot)
- **Symptom**: Plan mode does nothing · **Cause / fix**: Plan is toggled via `/plan` / `/plan off` slash commands, not a session mode
