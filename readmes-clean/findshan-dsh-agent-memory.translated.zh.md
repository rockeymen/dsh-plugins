...
### Key · 默认 · 含义
- **Key**: `memoryDir` · **默认**: `$DSH_HOME/memory` · **含义**: 记忆根目录
- **Key**: `dreamIntervalHours` / `dreamMinSessions` · **默认**: `24` / `5` · **含义**: 梦境整合门控
- **Key**: `model` / `apiKey` / `baseURL` · **默认**: `deepseek-chat` / env / `api.deepseek.com` · **含义**: 廉价模型（提取+整合）
- **Key**: `catalogBudgetTokens` / `catalogTopN` · **默认**: `1000` / `5` · **含义**: 目录注入预算与每文件行数
- **Key**: `searchTopK` · **默认**: `5` · **含义**: 默认检索条数
- **Key**: `autoExtract` · **默认**: `true` · **含义**: 消费 compact summary 自动提取
- **Key**: `dailyRetentionDays` · **默认**: `30` · **含义**: daily 保留天数，dream 后归档
