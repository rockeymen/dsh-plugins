...
- **Signal**: Package integrity · **How it is tested**: Local directories go through `npm pack`; links and unpublished files cannot hide packaging defects.
- **Signal**: Real registration · **How it is tested**: Rows come from DSH `--dump-config`; services and tool schemas come from an in-process Cordis probe.
- **Signal**: Deterministic exercise · **How it is tested**: The baseline runtime probe and declared tool calls run through the real tool runtime without model selection.
- **Signal**: Clean removal · **How it is tested**: The same profile is rebooted after uninstall and checked for bundles, capabilities, processes, ports, and owned-path residue.
- **Signal**: Repeatability · **How it is tested**: `--suite full` runs five isolated attempts and returns `flaky` when semantic outcomes disagree.
- **Signal**: Observer limits · **How it is tested**: Unavailable coverage is disclosed; a required unavailable observer returns `unsupported`, never a false pass.
...
### Need · Best fit
- **Need**: Fast static checks, manifest diagnostics, dependency guidance · **Best fit**: [dsh-plugin-doctor](https://github.com/zoahdev/dsh-plugin-doctor) or a plugin preflight check
- **Need**: Conflicts among several bundles before or during assembly · **Best fit**: `dsh-composition-check`
- **Need**: Plugin-owned unit logic · **Best fit**: Your test framework
- **Need**: Install, boot, exercise, uninstall, reboot, recovery, residue, and repeatability on a real host · **Best fit**: **DSH Testkit**
