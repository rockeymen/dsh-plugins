...
本项目积极参与并认可 [LINUX DO 社区](https://linux.do)。
  ...
  ![DSH Remote 手机端：按项目分组的会话列表](docs/phone/sessions.png)
  ![DSH Remote 手机端：对话、用量、模型和 Agent 预设](docs/phone/chat.png)
  ![在已有 Host 项目上新建会话，或在父级下建文件夹](docs/phone/create.png)

  ![手机端已配对 Host 状态](docs/phone/hosts.png)
  ![手机端查看 Host 产出](docs/phone/artifacts.png)

<sub>vivo 真机、局域网配对。主机名和局域网地址已打码。</sub>
...
### 手机上 · 实际发生的事
- **手机上**: 会话列表 · **实际发生的事**: Host 目录，按项目 / 工作区**标签**分组。Host 上的空白会话默认不出现；本机刚新建的会留下一行。
- **手机上**: 对话 / 轨迹 / 导出 · **实际发生的事**: 当前会话的实时投影：消息、工具、Host 提供的用量、模型和 Agent 预设。
- **手机上**: 新建会话 · **实际发生的事**: 绑到已有 Host 工作区，或让 Host 在允许的父级下建文件夹。手机不 `mkdir`，也看不到完整路径。
- **手机上**: 审批 · **实际发生的事**: 这台 Host 上待处理的工具审批。
- **手机上**: 产出 · **实际发生的事**: Host 投影出来的文件产出；「未验收」只是这台手机上的标记。
- **手机上**: 主机 · **实际发生的事**: 再配对一台电脑、看在线 / 空闲、解除配对。
...
### 部分 · 仓库
- **部分**: Host 插件（本仓库） · **仓库**: https://github.com/w2112515/dsh-remote-host
- **部分**: Android APK · **仓库**: https://github.com/w2112515/dsh-remote-android
- **部分**: 市场整合包 · **仓库**: https://github.com/w2112515/dsh-remote-pack
- **部分**: DeepSeek Harness · **仓库**: https://github.com/deepseek-ai/deepseek-harness
- **部分**: LINUX DO · **仓库**: https://linux.do
