...
### 模式 · 说明 · 适合场景
- **模式**: **极速启动** · **说明**: 本地固定目录秒级启动，后台自动检查官方新版并支持一键更新（取代原「快速启动」） · **适合场景**: 大多数用户，推荐
- **模式**: **源码完整安装** · **说明**: `git clone` + `pnpm install` + `pnpm run build` · **适合场景**: 开发者，想改源码/调试
- **模式**: **本地修复** · **说明**: 卸载全局 `@deepseek-ai/dsh`、清理残留并重装 · **适合场景**: 安装损坏、koffi 加载失败时修复
...
### 进程 · 说明
- **进程**: 服务端后端 · **说明**: 源码仓库方式启动 `dsh web`（`%APPDATA%/dsh-desktop/deepseek-harness`），提供 API 并托管前端，地址不变
- **进程**: 浏览器端热更 watcher · **说明**: `pnpm run dev:web`，监听全部 `dsh.client` 插件源码，改动后自动重建 bundle，浏览器免刷新热更
...
### 依赖 · 说明
- **依赖**: Windows 10 / 11（x64） · **说明**: 应用运行平台
- **依赖**: Node.js ≥ 18 · **说明**: 极速启动模式依赖，缺失时客户端引导下载
- **依赖**: git · **说明**: 仅源码模式需要（pnpm 缺失时自动安装）
- **依赖**: 网络 · **说明**: 首次安装需下载依赖（约数百 MB）
...
**Q: 端口 3080 被占用？**
A: 客户端会优先复用已运行的 dsh web 服务；也可用 `npm start -- --port ` 指定其他端口。
...
### 项目 · 说明 · 安装方式
- **项目**: [用量与消耗插件（dsh-usage-plugin）](https://github.com/feiyang-dev/dsh-usage-plugin) · **说明**: 每次调用的 token 用量/缓存命中统计、峰谷计费、余额查询、CSV/JSON/PNG 导出 · **安装方式**: 桌面端推荐插件一键安装，或 `dsh plugin add @feiyang666/dsh-usage-plugin`
- **项目**: [数据保险箱（dsh-vault）](https://github.com/feiyang-dev/dsh-vault) · **说明**: 自动备份 / 清空检测 / 一键恢复，保护聊天记录与工作区数据 · **安装方式**: 桌面端推荐插件一键安装，或 `dsh plugin add @feiyang666/dsh-vault`
- **项目**: [DeepSeek-Harness](https://github.com/deepseek-ai/DeepSeek-Harness) · **说明**: 官方 CLI / Web 服务 · **安装方式**: —
