...
- **你的日常表述**: 「今天买菜花了 86，以后我随口说你就记着，月底我要问分类统计」 · **agent 长出的工具**: `expense_add` / `expense_stats`：持久账本 · **凭什么需要工具**: **跨会话状态**：下个会话接着记、接着算
- **你的日常表述**: 「我妈周六去杭州，要带伞吗？我经常帮家里人看天气」 · **agent 长出的工具**: `weather_city`：降水概率+带伞建议，**「妹妹」自动对应深圳** · **凭什么需要工具**: 实时数据 + 私有别名
- **你的日常表述**: 「帮我记下宝宝生日、结婚纪念日、我妈生日」 · **agent 长出的工具**: `family_*` 三件套：**日子固化在工具里** · **凭什么需要工具**: 私有信息 + 精确日期计算
- **你的日常表述**: 「供应商报价 12800 美元折人民币？天天要算」 · **agent 长出的工具**: `convert_currency`：实时汇率、双数据源容错 · **凭什么需要工具**: 实时数据
- **你的日常表述**: 「房贷 120 万 30 年，提前还 10 万能省多少利息」 · **agent 长出的工具**: 等额本息摊销表工具 · **凭什么需要工具**: 精确计算（模型心算容易出错）

### 工作

### 你的日常表述 · agent 长出的工具 · 凭什么需要工具
- **你的日常表述**: 「每周五我都要汇总三个仓库大家这周的提交写周报」 · **agent 长出的工具**: `team_weekly_report`：仓库清单长在工具里，按人/按仓库分组 · **凭什么需要工具**: 私有数据源 + 多步聚合
- **你的日常表述**: 「E4103 是什么错误来着？」（项目错误码表天天翻） · **agent 长出的工具**: `lookup_error_code`：实时解析表文件 · **凭什么需要工具**: 私有信息
- **你的日常表述**: 「这批实验数据 A/B 组差异显著吗」（每周都问） · **agent 长出的工具**: 显著性检验工具（t-test/卡方，出精确 p 值） · **凭什么需要工具**: 精确计算
- **你的日常表述**: 「staging 现在什么版本？」（每天问） · **agent 长出的工具**: 包住团队 health 端点的状态工具 · **凭什么需要工具**: 实时私有端点
...
- **你的日常表述**: 「说了多少遍简短点都没用，想个办法**真正管住自己**」 · **agent 长出的注册面**: prompt section（常驻规则）+ `agent/settled` 钩子（量长度）+ 插件唤醒（超限逼重写） · **为什么工具做不到**: 违规的是模型自己，不能指望违规者记得调工具自纠
- **你的日常表述**: 「每次帮我干完活，**自动**往 worklog.md 记一行」 · **agent 长出的注册面**: `agent/settled` 监听器，普通代码里 `appendFileSync` · **为什么工具做不到**: "每次"押在模型自觉上必然漏；监听器跟着 turn 提交的事实走
- **你的日常表述**: 「每天早上 9 点**你主动来找我**过安排」 · **agent 长出的注册面**: 定时器 + `agent.send(…, { wakeup: true })` 造出新 turn · **为什么工具做不到**: 清晨没有会话、没有 step——工具连被调用的宿主都不存在
...
### 键 · 默认 · 说明
- **键**: `dir` · **默认**: `~/.dsh/evolve` · **说明**: store 目录
- **键**: `autoRestore` · **默认**: `true` · **说明**: 启动时重挂 manifest 里的全部 evolution
