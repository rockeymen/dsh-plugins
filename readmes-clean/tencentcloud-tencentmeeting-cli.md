...
### 参数 · 类型 · 必填 · 说明
- **参数**: `--meeting-id` · **类型**: string · **必填**: 二选一 · **说明**: 会议 ID（优先级高于会议码）
- **参数**: `--meeting-code` · **类型**: string · **必填**: 二选一 · **说明**: 会议码
...
### 参数 · 类型 · 必填 · 默认值 · 说明
- **参数**: `--meeting-id` · **类型**: string · **必填**: ✅ · **默认值**: — · **说明**: 会议 ID
- **参数**: `--subject` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 会议主题
- **参数**: `--start` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 会议开始时间，ISO 8601，如 `2026-03-12T14:00+08:00`
- **参数**: `--end` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 会议结束时间，ISO 8601，如 `2026-03-12T14:00+08:00`
- **参数**: `--password` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 会议密码（4~6 位数字）
- **参数**: `--timezone` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 时区，如 `Asia/Shanghai`
- **参数**: `--meeting-type` · **类型**: int · **必填**: — · **默认值**: `0` · **说明**: 会议类型：`0`-普通会议，`1`-周期性会议
- **参数**: `--join-type` · **类型**: int · **必填**: — · **默认值**: `0` · **说明**: 成员入会限制：`1`-所有成员可入会，`2`-仅受邀成员可入会，`3`-仅企业内部成员可入会
- **参数**: `--waiting-room` · **类型**: bool · **必填**: — · **默认值**: `false` · **说明**: 是否开启等候室
- **参数**: `--recurring-type` · **类型**: int · **必填**: — · **默认值**: `0` · **说明**: 周期类型（`--meeting-type=1` 时生效）：`0`-每天，`1`-每周一至周五，`2`-每周，`3`-每两周，`4`-每月
- **参数**: `--until-type` · **类型**: int · **必填**: — · **默认值**: `0` · **说明**: 周期结束类型（`--meeting-type=1` 时生效）：`0`-按日期结束重复，`1`-按次数结束重复
- **参数**: `--until-count` · **类型**: int · **必填**: — · **默认值**: `7` · **说明**: 限定会议次数（`--meeting-type=1` 时生效）：每天/每个工作日/每周最大 500，每两周/每月最大 50
- **参数**: `--until-date` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 周期结束日期（`--meeting-type=1` 时生效），ISO 8601，如 `2026-03-12T15:00+08:00`
- **参数**: `--sub-meeting-id` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 子会议 ID（`--meeting-type=1` 时生效）：仅修改该场子会议的时间；**不可与 `--recurring-type` / `--until-type` / `--until-count` / `--until-date` 同时使用**。不填则修改整个周期性会议
- **参数**: `--invitees` · **类型**: strings · **必填**: — · **默认值**: — · **说明**: 待变更的邀请成员 openid 列表，逗号分隔或重复传参；与 `--invitees-type` 配合使用
- **参数**: `--invitees-type` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 邀请变更策略：`replace`-全量替换邀请列表，`add`-新增邀请用户，`remove`-删除邀请用户；当指定 `--invitees` 时必填
...
### 参数 · 类型 · 必填 · 默认值 · 说明
- **参数**: `--meeting-id` · **类型**: string · **必填**: ✅ · **默认值**: — · **说明**: 会议 ID
- **参数**: `--sub-meeting-id` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 周期性会议子会议 ID，取消周期性会议的某个子会议时需要传入
- **参数**: `--meeting-type` · **类型**: int · **必填**: — · **默认值**: `0` · **说明**: 会议类型：`0`-普通会议，`1`-周期性会议（取消整场周期性会议时传 `1`）
...
### 参数 · 类型 · 必填 · 默认值 · 说明
- **参数**: `--start` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 分页查询起始时间值，ISO 8601，如 `2026-03-12T15:00+08:00`
- **参数**: `--end` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 分页查询结束时间值，ISO 8601，如 `2026-03-12T15:00+08:00`
- **参数**: `--show-all-sub` · **类型**: int · **必填**: — · **默认值**: `0` · **说明**: 是否展示全部子会议：`0`-不展示，`1`-展示
- **参数**: `--page-token` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 分页游标，从上一次响应中返回的 `next_page_token` 获取，首页不传
- **参数**: `--page-size` · **类型**: int · **必填**: — · **默认值**: `20` · **说明**: 每页大小，默认 20，最大 20
...
### 参数 · 类型 · 必填 · 默认值 · 说明
- **参数**: `--start` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 查询开始时间，ISO 8601，如 `2026-03-12T15:00+08:00`
- **参数**: `--end` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 查询结束时间，ISO 8601，如 `2026-03-12T15:00+08:00`
- **参数**: `--page-token` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 分页游标，从上一次响应中返回的 `next_page_token` 获取，首页不传
- **参数**: `--page-size` · **类型**: int · **必填**: — · **默认值**: `30` · **说明**: 每页大小，默认 30，最大 30
- **参数**: `--page` · **类型**: int · **必填**: — · **默认值**: — · **说明**: ⚠️ **已弃用**：页码（从 1 开始），请改用 `--page-token`
...
### 参数 · 类型 · 必填 · 默认值 · 说明
- **参数**: `--query` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 搜索关键词
- **参数**: `--query-field` · **类型**: string · **必填**: — · **默认值**: `all` · **说明**: `--query` 的搜索字段：`subject`-会议主题；`creator`-创建者昵称/备注名；`note`-用户对会议的备注；`all`-搜索所有字段
- **参数**: `--meeting-code` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 按会议号过滤（精确匹配，仅数字，无短横线）
- **参数**: `--start` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 搜索时间窗下限（ISO 8601，如 `2026-03-12T15:00+08:00`）。匹配条件：会议预约开始时间、实际开始时间或当前用户加入时间任一落在窗口内
- **参数**: `--end` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 搜索时间窗上限（ISO 8601，如 `2026-03-12T15:00+08:00`），语义同上
- **参数**: `--page-token` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 分页游标，从上一次响应中返回的 `next_page_token` 获取，首页不传
- **参数**: `--page-size` · **类型**: int · **必填**: — · **默认值**: `30` · **说明**: 每页大小，默认 30，最大 30
...
- **参数**: `--page-token` · **类型**: string · **必填**: — · **默认值**: — · **说明**: 分页游标，从上一次响应中返回的 `next_page_token` 获取，首页不传
- **参数**: `--page-size` · **类型**: int · **必填**: — · **默认值**: `30` · **说明**: 每页大小，默认 30，最大 30
- **参数**: `--pos` · **类型**: int · **必填**: — · **默认值**: — · **说明**: ⚠️ **已弃用**：分页起始位置值，请改用 `--page-token`
...
### 参数 · 类型 · 必填 · 默认值 · 说明
- **参数**: `--meeting-id` · **类型**: string · **必填**: ✅ · **默认值**: — · **说明**: 会议 ID
- **参数**: `--invitees` · **类型**: strings · **必填**: ✅ · **默认值**: — · **说明**: 待添加的受邀成员 `open_id` 列表，支持英文逗号分隔或重复传入该参数，最多 100 个
