...
- Substrate · **worker-thread (shipped)**: a `Worker` in the agent process · **this backend**: a fresh container per run
- Network · **worker-thread (shipped)**: full agent network access · **this backend**: `--network=none`
- Filesystem · **worker-thread (shipped)**: the whole host, as the agent user · **this backend**: read-only rootfs, no host mount
- Privileges · **worker-thread (shipped)**: the agent's · **this backend**: `--cap-drop=ALL`, `no-new-privileges`, `nobody`
- Runaway CPU · **worker-thread (shipped)**: measured busy-time budget · **this backend**: kernel CPU quota + wall clock
- Runaway memory · **worker-thread (shipped)**: V8 heap cap · **this backend**: cgroup memory limit → OOM kill
- Fork bomb · **worker-thread (shipped)**: — · **this backend**: `--pids-limit`
- Cold start · **worker-thread (shipped)**: ~milliseconds · **this backend**: ~200 ms
...
### Key · Default · Meaning
- **Key**: `dockerPath` · **Default**: `docker` · **Meaning**: Container CLI. Anything argv-compatible with `docker run`.
- **Key**: `image` · **Default**: `node:22-alpine` · **Meaning**: Needs a `node` on `PATH` and nothing else.
- **Key**: `network` · **Default**: `none` · **Meaning**: Docker network mode. **The main reason to use this backend.**
- **Key**: `memory` · **Default**: `512m` · **Meaning**: Memory ceiling; an overrun is an OOM kill reported as `worker-exit`.
- **Key**: `cpus` · **Default**: `1` · **Meaning**: Kernel-enforced CPU quota.
- **Key**: `pidsLimit` · **Default**: `128` · **Meaning**: Process ceiling: a fork bomb hits it, not the host.
- **Key**: `user` · **Default**: `65534:65534` · **Meaning**: `nobody:nogroup`. Empty uses the image default.
- **Key**: `readOnlyRootfs` · **Default**: `true` · **Meaning**: Read-only root, with a tmpfs at `/tmp`.
- **Key**: `tmpfsMb` · **Default**: `64` · **Meaning**: Size of that tmpfs.
- **Key**: `workspacePath` · **Default**: `''` · **Meaning**: Absolute host path to mount at `/workspace`. Empty mounts nothing.
- **Key**: `workspaceReadOnly` · **Default**: `true` · **Meaning**: Mount the workspace read-only.
- **Key**: `extraArgs` · **Default**: `[]` · **Meaning**: Extra `docker run` flags. **An escape hatch that can weaken every default above.**
- **Key**: `maxWallMs` · **Default**: `120000` · **Meaning**: Wall-clock ceiling for one run, including container start.
- **Key**: `maxOutputBytes` · **Default**: `4194304` · **Meaning**: Combined cap on logs plus completion value.
