...
- **`/spec` command family** — `init` · `new <goal>` · `status` · `list` · `show ` · `approve ` · `implement ` · `verify  [--deep]` · `archive ` · `validate [id]` · `edit `. Commands orchestrate and touch the filesystem only; proposal and implementation content is generated by the agent's main model through `agent.steer` with the full tool set.
...
- **OpenSpec-compatible layout** — `openspec/project.md`, `openspec/specs/<capability>/spec.md`, `openspec/changes/<change-id>/{proposal.md,tasks.md,design.md,verify.md,specs/<cap>/spec.md}`, archived to `changes/archive/YYYY-MM-DD-/`. Spec deltas use `## ADDED|MODIFIED|REMOVED Requirements` with `#### Scenario:` per requirement.
...
### Piece · Mechanism
- **Piece**: Commands · **Mechanism**: `commands.register` — one `/spec` command with a subcommand router. Handlers orchestrate and touch the filesystem (`ctx.fs`); generation is steered to the agent (`agent.steer` with a plugin-sourced `UserMessage`).
- **Piece**: State machine · **Mechanism**: A session projection (`specLoop`) folds standard events: `command/run`+`command/done` pairs transition on success only, and the agent's reply markers (`SPEC_CHANGE_ID: `, `SPEC_IMPLEMENTED`) complete the async stages. `implement` gates on the same projection.
- **Piece**: Task progress · **Mechanism**: The implement prompt mirrors `tasks.md` into the `todo_write` tool; the panel reads the shipped `todos` projection.
- **Piece**: Verification · **Mechanism**: Handler-side bounded judge call (`ctx.llm.stream`, `reasoningEffort: 'off'`, flash by default); ```` ```bash ```` blocks from `proposal.md` run first via `ctx.shell` and their output is included in the judge prompt.
- **Piece**: Archive merge · **Mechanism**: Deltas are merged requirement-by-requirement into `specs/<cap>/spec.md` (ADDED append, MODIFIED replace, REMOVED drop), then the change dir moves with one `mv` (`ctx.shell`).
- **Piece**: UI · **Mechanism**: `conversation.input.dock` slot entry; reads `useProjection('specLoop')` + `useProjection('todos')`; `locale` service for zh/en strings.
