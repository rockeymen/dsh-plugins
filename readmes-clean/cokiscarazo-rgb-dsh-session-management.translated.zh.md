...
### 能力 · DSH 官方 · 装本插件后
- **能力**: 归档会话 · **DSH 官方**: 支持 · **装本插件后**: 支持，另有一键全量归档
- **能力**: 取消归档 · **DSH 官方**: 无入口 · **装本插件后**: 单个 / 按工作区 / 按月份批量
- **能力**: 真正删除聊天记录 · **DSH 官方**: 仅隐藏，不删除 · **装本插件后**: 删除磁盘日志文件，运行中会话自动跳过
- **能力**: 导出会话数据 · **DSH 官方**: 单个手动导出 · **装本插件后**: 一键批量导出，格式与官方字节级兼容
...
一键批量导出全部会话：自动遍历所有根会话（含其子代理会话），逐个生成与官方格式一致的 ZIP 归档（`dsh-session-.zip`），无需再一个个手动点击。
