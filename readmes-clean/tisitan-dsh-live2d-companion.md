...
### 成员 · 说明
- **成员**: `enter(state)` / `showBubble(text, ms)` · **说明**: 切状态 / 冒气泡
- **成员**: `on(event, fn)` · **说明**: 订阅事件，返回退订函数
- **成员**: `registerState(name, def)` · **说明**: 注册/覆盖状态行为（expr/motion/pool/rotate/remotionMs/transientMs）
- **成员**: `registerLamp(name, spec)` · **说明**: 注册/覆盖状态灯（color/label/anim；自定义动画需自注入 @keyframes）
- **成员**: `quip(pool)` · **说明**: 从台词池抽一句
- **成员**: `setModel(path)` / `refreshModels()` · **说明**: 热切换模型 / 重新扫描模型列表
- **成员**: `openModelPanel()` / `closeModelPanel()` · **说明**: 打开 / 关闭模型面板
- **成员**: `openModelViewer(path)` / `importModels(files)` · **说明**: 预览模型 / 导入文件列表
- **成员**: `state` / `binding` / `model` / `modelPath` / `modelList` / `bounds` / `gaze` · **说明**: 实时只读状态
- **成员**: `ctx` · **说明**: 完整共享上下文（实验性，结构可能随版本调整）
...
### 状态 · 触发事件 · 表情/动作 · 台词池
- **状态**: thinking · **触发事件**: `turn/start` / `assistant/chunk` · **表情/动作**: 疑惑 + 思考姿势循环 · **台词池**: thinking
- **状态**: working · **触发事件**: `tool/call` 等 · **表情/动作**: 兴奋 + 定期重放动作 · **台词池**: working
- **状态**: waiting · **触发事件**: `approval/asked` · **表情/动作**: 疑惑 + 摇头求确认 · **台词池**: waiting
- **状态**: error · **触发事件**: `llm/retry-started` · **表情/动作**: 困扰 + 转圈/概率 Glitch · **台词池**: error
- **状态**: done · **触发事件**: `turn/end`（6 秒后回 idle） · **表情/动作**: 开心 + 点头 · **台词池**: done
- **状态**: sleeping · **触发事件**: 空闲超时（前端计时） · **表情/动作**: Sleep + 打瞌睡循环 · **台词池**: sleeping
- **状态**: —（加班中） · **触发事件**: working 超时升级 · **表情/动作**: 严肃脸 → 困扰脸 · **台词池**: overtime
