...
Both commands load the same browser bundle from the local `web` profile. Port 3080 is used by default; if it is occupied, run `pnpm start -- --port ` or `npx @deepseek-ai/dsh web --port `. When the DeepSeek whale icon appears in the toolbar, click it to open the side panel.
