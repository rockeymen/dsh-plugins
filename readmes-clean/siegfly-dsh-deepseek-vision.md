...
### 路径 · 默认值 · 说明
- **路径**: `provider` · **默认值**: `deepseek-vision` · **说明**: 注册的路由 id（避开 `deepseek-official`）
- **路径**: `displayName` · **默认值**: `DeepSeek + Vision` · **说明**: 模型选择器里的名字
- **路径**: `deepseek.*` · **默认值**: — · **说明**: 与官方 `llm-deepseek` 段完全同构（apiKeyEnv / baseURL / thinking / reasoningEffort / maxTokens / models / retryPolicy…）
- **路径**: `deepseek.apiKeyEnv` · **默认值**: `DEEPSEEK_API_KEY` · **说明**: DeepSeek key
- **路径**: `vl.apiKeyEnv` · **默认值**: `QWEN_VL_API_KEY` · **说明**: VL 模型 key
- **路径**: `vl.baseURL` · **默认值**: `https://dashscope.aliyuncs.com/compatible-mode/v1` · **说明**: 任意 OpenAI 兼容 `/chat/completions` 网关
- **路径**: `vl.model` · **默认值**: `qwen3-vl-flash` · **说明**: VL 模型 id（贴图 OCR 型描述性价比最高；复杂视觉推理可换 `qwen-vl-max`）
- **路径**: `vl.describePrompt` · **默认值**: 详述 + 逐字提取的英文提示词 · **说明**: 图片描述指令
- **路径**: `vl.timeoutMs` · **默认值**: `120000` · **说明**: 单次描述请求硬超时
- **路径**: `vl.maxCacheEntries` · **默认值**: `64` · **说明**: 进程内描述缓存容量（LRU）
- **路径**: `vl.onFailure` · **默认值**: `fail` · **说明**: `fail` = 描述失败整个请求失败；`placeholder` = 降级为文字占位继续
