...
### 功能 · 行为
- **功能**: 节点导航条 · **行为**: 对话区右缘纵向短横线串，每条**真实用户消息**一个节点（系统注入的 goal 自动延续等不计入），恒定 20px 间距
- **功能**: 悬停弹出面板 · **行为**: 鼠标进入节点串区域：节点条短横线淡出、左侧**单行预览面板弹出**（0.18s 放大动画，覆盖节点条原位置，行内短横线落在原节点条短横线的横坐标上，如节点条变形展开）；移出后节点条恢复
- **功能**: 面板排版 · **行为**: 每条消息一行（左文字 + 右短横线），24px 行距、上下 8px 对称留白；悬停行时文字与短横线**同步高亮**并出现 8px 圆角底色；当前阅读位置整行（文字+短横线）以品牌蓝/白色高亮
- **功能**: 列表滑动 · **行为**: 最多显示 10 条；超过时鼠标悬停在节点串区域内**滚轮上下滑动列表**（页面不滚动），面板按比例同步滚动
- **功能**: 移出回中 · **行为**: 鼠标移出悬停区域后，列表平滑居中回当前阅读位置，恢复跟随
- **功能**: 跟随阅读位置 · **行为**: 激活节点（品牌蓝 / 深色下白色）随滚动侦测实时更新
- **功能**: 点击跳转 · **行为**: 平滑滚动到对应消息 + 全宽品牌蓝高亮横线（1.5s 淡出），流式输出干扰下亦有看门狗兜底落位，列表自动居中到目标节点
- **功能**: 自动隐藏 · **行为**: <2 条用户消息、空白会话、非对话视图（如轨迹页）时不显示
- **功能**: 渲染细节 · **行为**: 节点位置按 devicePixelRatio 对齐设备像素（粗细一致）；窗口调整 rAF 合帧，不拖慢界面
...
dsh plugin --profile  add file:<abs-path>
dsh --profile  --port 3090
```