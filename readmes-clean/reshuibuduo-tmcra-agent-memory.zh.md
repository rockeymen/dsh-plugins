# TMCRA — 本地 Agent Memory OS
  ...
  ![TMCRA — 本地 Agent Memory OS](assets/tmcra-github-cover-v1.png)
...
### 能力 · 用户得到的结果
- **能力**: 自动记忆闭环 · **用户得到的结果**: 提问前自动召回并保存 USER 来源，回答完成后保存独立的 ASSISTANT 来源
- **能力**: 跨会话、跨软件连续工作 · **用户得到的结果**: 同一项目在不同会话和已接入工具中共享项目进度，无需重新介绍上下文
- **能力**: 项目隔离与用户全局记忆 · **用户得到的结果**: 项目内容分开管理；明确选择的用户信息可供多个项目使用
- **能力**: Source / Fast / Slow 分层 · **用户得到的结果**: 保留可核对的原始来源，并建立面向快速召回和深层关系的派生记忆
- **能力**: 有来源的召回注入 · **用户得到的结果**: 返回候选记忆、证据窗口、角色、来源和召回轨迹，供宿主加入下一轮提示词
- **能力**: Visual Atlas · **用户得到的结果**: 按项目、会话、事件和证据查看个人记忆图谱
- **能力**: Personal Knowledge · **用户得到的结果**: 将记忆整理为学习、项目和个人三类知识页面，每项结论保留证据引用
- **能力**: 本地模型与 BYOK · **用户得到的结果**: 结构化写入和知识整理可使用本地模型，也可使用用户自己的 OpenAI 兼容 API
- **能力**: 用量账本 · **用户得到的结果**: 查看供应商、模型、任务、token、缓存命中和延迟；本地版不收取 TMCRA 服务费
- **能力**: 数据控制 · **用户得到的结果**: 查看原始消息，删除单条消息或整个项目，并清理相关派生数据
...
项目身份依次取自 `.tmcra/project.json`、Git origin、Git 根目录或规范化工作目录。在同一个仓库中接入 Codex、DeepSeek Harness 或其他适配器时，各工具共享 `project:` 记忆，同时保留各自的 `source_app`、原生 thread、Session、角色和 Agent 身份。

- `global:owner`：用户明确允许跨项目使用的长期信息。
- `project:`：需求、决策、进度、问题和 Agent 工作结果。
...
### 宿主 · 自动能力 · 当前状态
- **宿主**: Codex · **自动能力**: 提问前召回；USER / ASSISTANT 分轨写回；失败队列重试 · **当前状态**: 一条命令安装，已通过本地 FastAPI 跨软件 E2E
- **宿主**: DeepSeek Harness · **自动能力**: 原生 `agent/pre-step` 召回；`turn/end` 写回；保留多 Agent 身份 · **当前状态**: 技术预览，已通过真实 AgentLoop 双会话、类型、构建和包审计
- **宿主**: Claude Code · **自动能力**: 共用本地 Hook 链路 · **当前状态**: 手动注册，已通过共用 Hook 与跨软件 E2E
- **宿主**: ZCode · **自动能力**: 共用本地 Hook 链路 · **当前状态**: 手动注册；宿主打包流程仍待干净环境验收
- **宿主**: 其他工具 · **自动能力**: 通过本地 REST API 实现同一生命周期 · **当前状态**: 接口可用；需由宿主提供已验证的生命周期入口
...
### 方法 · 路径 · 用途
- **方法**: `GET` · **路径**: `/v1/health` · **用途**: 不包含敏感信息的健康状态
- **方法**: `GET` · **路径**: `/v1/projects` · **用途**: 列出本地项目
- **方法**: `GET` · **路径**: `/v1/sessions` · **用途**: 按项目列出 Session 来源分组
- **方法**: `POST` · **路径**: `/v1/recall` · **用途**: 根据当前用户问题召回证据
- **方法**: `POST` · **路径**: `/v1/messages` · **用途**: 写入一条带角色的原始消息
- **方法**: `GET` · **路径**: `/v1/messages` · **用途**: 查看已保存的原始消息
- **方法**: `DELETE` · **路径**: `/v1/messages/{message_id}` · **用途**: 删除一条消息及其派生记忆
- **方法**: `DELETE` · **路径**: `/v1/projects/{project_id}` · **用途**: 删除项目、全局派生、知识库和用量元数据
- **方法**: `GET` · **路径**: `/v1/projects/{project_id}/graph` · **用途**: 生成 Visual Atlas 数据
- **方法**: `POST` · **路径**: `/v1/projects/{project_id}/knowledge/build` · **用途**: 生成 Personal Knowledge
- **方法**: `GET` · **路径**: `/v1/projects/{project_id}/knowledge` · **用途**: 读取最近一次 Personal Knowledge 快照
- **方法**: `GET` · **路径**: `/v1/usage` · **用途**: 查看本地模型调用 token 用量
...
### 任务 · 正确数 / 总数 · 准确率
- **任务**: 信息更新 · **正确数 / 总数**: 71 / 78 · **准确率**: 91.0%
- **任务**: 跨会话整合 · **正确数 / 总数**: 90 / 133 · **准确率**: 67.7%
- **任务**: 单会话助手信息 · **正确数 / 总数**: 55 / 56 · **准确率**: 98.2%
- **任务**: 单会话偏好 · **正确数 / 总数**: 27 / 30 · **准确率**: 90.0%
- **任务**: 单会话用户信息 · **正确数 / 总数**: 67 / 70 · **准确率**: 95.7%
- **任务**: 时间推理 · **正确数 / 总数**: 101 / 133 · **准确率**: 75.9%
- **任务**: **总成绩** · **正确数 / 总数**: **411 / 500** · **准确率**: **82.2%**
