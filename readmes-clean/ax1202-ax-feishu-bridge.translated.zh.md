...
### 命令 · 作用
- **命令**: `/new` · **作用**: 为当前会话新建一个 Pi 会话
- **命令**: `/resume` · **作用**: 打开当前工作区的历史会话列表；可在卡片中切到全部会话
- **命令**: `/model` · **作用**: 打开模型选择卡片，切换当前会话使用的模型
- **命令**: `/thinking` · **作用**: 打开思考强度选择卡片，切换当前模型实际支持的档位
- **命令**: `/stop` · **作用**: 停止当前这条回复的处理
- **命令**: `/workspace` · **作用**: 查看当前会话绑定的工作区
- **命令**: `/workspace /path/to/project` · **作用**: 把当前会话切换到指定工作区，下一条消息生效
- **命令**: `/status` · **作用**: 查看当前会话的工作状态、模型、思考强度和上下文占用
- **命令**: `/commands` · **作用**: 查看机器人支持的全部命令
- **命令**: `/config` · **作用**: 查看运行时配置（仅限与机器人的私聊）
- **命令**: `/config groupKeywords 关键词1,关键词2` · **作用**: 设置群聊关键词触发并立即生效
- **命令**: `/config streamingReply false` · **作用**: 关闭流式展示，改用普通回复卡片
- **命令**: `/config clear groupKeywords` · **作用**: 清除某项运行时配置覆盖

## Pi 里怎么管理

### 命令 · 作用
- **命令**: `/feishu setup` · **作用**: 打开初始化配置
- **命令**: `/feishu start` · **作用**: 启动飞书桥接
- **命令**: `/feishu stop` · **作用**: 停止飞书桥接
- **命令**: `/feishu restart` · **作用**: 重启桥接，并重新加载最新代码和配置
- **命令**: `/feishu status` · **作用**: 查看连接状态、当前 owner 和配置
- **命令**: `/feishu autostart` · **作用**: 开关自动启动
- **命令**: `/feishu debug` · **作用**: 查看最近 20 条调试日志
- **命令**: `/feishu reset` · **作用**: 清除配置和映射，但保留会话历史
...
### 命令 · 作用
- **命令**: `/feishu setup` · **作用**: 重新配置机器人；问答与二维码在 DSH 进程所在终端进行，已有配置时先确认覆盖
- **命令**: `/feishu status` · **作用**: 查看连接状态、当前 owner 和配置
- **命令**: `/feishu autostart` · **作用**: 开关自动启动
- **命令**: `/feishu debug` · **作用**: 查看最近 20 条调试日志
- **命令**: `/feishu reset confirm` · **作用**: 清除配置和映射，但保留会话历史
...
### 变量 · 说明
- **变量**: `FEISHU_APP_ID` · **说明**: 飞书/Lark 应用 ID
- **变量**: `FEISHU_APP_SECRET` · **说明**: 飞书/Lark 应用密钥
- **变量**: `FEISHU_DOMAIN` · **说明**: `feishu` 或 `lark`，默认 `feishu`
- **变量**: `FEISHU_GROUP_POLICY` · **说明**: `open` 或 `mention`，默认 `open`
- **变量**: `FEISHU_GROUP_KEYWORDS` · **说明**: 群聊关键词，逗号或分号分隔；命中后无需 @
- **变量**: `FEISHU_GROUP_ALSO_ON_REPLY` · **说明**: `1` 时回复机器人消息可继续追问，无需再次 @
- **变量**: `FEISHU_IGNORE_BOT_MESSAGES` · **说明**: 是否忽略其他机器人消息，默认 `true`
- **变量**: `FEISHU_LANGUAGE` · **说明**: `zh` 或 `en`
- **变量**: `FEISHU_REACT_EMOJI` · **说明**: 收到消息时的表情回应，默认 `Get`
- **变量**: `FEISHU_AUTO_START` · **说明**: `1` 或 `0`
- **变量**: `FEISHU_CARD_ACTION_MODE` · **说明**: `webhook` 或 `ws`，默认 `webhook`
- **变量**: `FEISHU_CARD_ACTION_WEBHOOK_HOST` · **说明**: 卡片回调监听地址，默认 `0.0.0.0`
- **变量**: `FEISHU_CARD_ACTION_WEBHOOK_PORT` · **说明**: 卡片回调端口，默认 `3001`（DSH 用 `HARNESS_CARD_ACTION_WEBHOOK_PORT`，默认 `3002`）
- **变量**: `FEISHU_CARD_ACTION_WEBHOOK_PATH` · **说明**: 卡片回调路径，默认 `/webhook/card`
- **变量**: `FEISHU_PROMPT_NOTIFY_SEC` · **说明**: 长任务超过多少秒后在飞书发一条“仍在处理中”提示，默认 `180`，`0` 关闭
- **变量**: `FEISHU_PROMPT_TIMEOUT_SEC` · **说明**: 任务硬超时秒数，超时后中止任务并报失败，默认 `0`（不设硬超时，长期运行也不会被报失败）
- **变量**: `FEISHU_PARSE_INTERACTIVE_CARDS` · **说明**: 是否把 interactive 卡片转为 Pi 可读文字，默认 `true`
- **变量**: `FEISHU_INCLUDE_QUOTED_MESSAGE` · **说明**: 回复/引用消息时是否带入原消息内容，默认 `true`
- **变量**: `FEISHU_QUOTED_MESSAGE_MAX_CHARS` · **说明**: 引用消息最多带入的字符数，默认 `8000`
- **变量**: `FEISHU_SEND_MAX_RETRIES` · **说明**: 飞书接口临时失败时的重试次数，默认 `2`
- **变量**: `FEISHU_STREAMING_REPLY` · **说明**: 是否启用 CardKit 单卡流式回复，默认 `true`
- **变量**: `FEISHU_STREAM_PRINT_FREQUENCY_MS` · **说明**: 流式逐字显示的刷新间隔，默认 `50`
- **变量**: `FEISHU_STREAM_PRINT_STEP` · **说明**: 每次显示的字符数，默认 `1`
- **变量**: `FEISHU_STREAM_PUSH_INTERVAL_MS` · **说明**: 向飞书推送最新正文的间隔，默认 `120` 毫秒
- **变量**: `FEISHU_EXT_DEV` · **说明**: `1` 时显示本地开发标识 `DEV`
...
### 字段 · 说明
- **字段**: `promptNotifySec` · **说明**: 长任务超过多少秒后在飞书发一条“仍在处理中”提示，默认 `180`，`0` 关闭
- **字段**: `promptTimeoutSec` · **说明**: 任务硬超时秒数，超时后中止任务并报失败，默认 `0`（不设硬超时，长期运行也不会被报失败）
...
### 路径 · 内容
- **路径**: `~/.pi/agent/feishu/config.json` · **内容**: 机器人凭证和基础配置
- **路径**: `~/.pi/agent/feishu/runtime-overrides.json` · **内容**: 通过私聊 `/config` 保存的运行时配置覆盖
- **路径**: `~/.pi/agent/feishu/state.json` · **内容**: 飞书会话和 Pi 会话的映射
- **路径**: `~/.pi/agent/feishu/bridge.json` · **内容**: 从飞书发起的 Pi 任务路由信息
- **路径**: `~/.pi/agent/feishu/debug.log` · **内容**: 调试日志
- **路径**: `~/.pi/agent/locks.json` · **内容**: 当前飞书连接的 owner 锁
- **路径**: `~/.pi/agent/sessions/` · **内容**: 每个飞书会话对应的 Pi 会话文件
- **路径**: `~/.dsh/feishu/` · **内容**: DSH 侧的配置、状态与日志（仅安装 DSH 时产生；未装 Pi 的机器不会创建 `~/.pi`）
