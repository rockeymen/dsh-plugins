...
```bash
npx -y github:1841220388zzzcccxxx-star/dsh-wechat install
```
...
```bash
git clone https://github.com/1841220388zzzcccxxx-star/dsh-wechat.git
cd dsh-weixin
npm install
npm run check
...
重启`dsh web`，打开**设置→插件→微信**，生成二维码，用微信扫描，在手机上确认，根据需要输入显示的配对数字。当页面报告健康的 iLink 长轮询连接时，帐户已准备就绪。

手机帐户必须已经可以使用微信机器人功能。腾讯独立推出该功能；该插件无法为不合格的帐户启用它。

当前版本支持直接短信和已包含微信转录的语音消息。它隔离每个帐户的凭据、同步游标、重复数据删除和 Harness 会话。有关完整的行为、安全模型和验证命令，请参阅中文部分。