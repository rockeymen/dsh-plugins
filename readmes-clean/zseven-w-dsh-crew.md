![DSH Crew](./docs/images/dsh-crew-logo.png)

# DSH Crew

  A [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) plugin: dispatch work to DSH agents from Claude Code / Codex, without giving up the host's native subagent UI.
  ...
  <sub>npm: `@zseven-w/dsh-crew` &middot; Current plugin release: `0.1.0-rc.1` &middot; Tested with DSH `0.1.0-rc.6`</sub>

  ![DSH Crew — settings page](./docs/images/dsh-crew-overview.png)

<sub>The DSH Crew settings page — host integrations, dispatch policy, execution and the multimodal bridge</sub>
...
<sub>Claude Code sees dsh-crew workers as native subagents, with a statusline segment tracking running tiers, elapsed time and tokens.</sub>

  ![DSH Crew](./docs/images/dsh-crew-jobs.png)

<sub>The DSH Crew panel sees the same run from the harness side: which host dispatched each job, its tier and effort, live progress and token usage.</sub>
...
- **Command**: `/dsh-crew:config` · **What it does**: Show or set this session's defaults: `tier=flash\ · pro`, `effort=off\ · high\ · max`, `mode=auto\ · hub\ · standalone`, `timeout=<seconds>`, `policy=auto\ · flash-only\ · pro-only`, `escalate=true\ · false`, `reset`
- **Command**: `/dsh-crew:on` · `/dsh-crew:off` · **What it does**: Turn dispatch for this session on or off (off is a hard switch: the tool refuses)
- **Command**: `/dsh-crew:status` · **What it does**: Live status of worker jobs: tier, progress, tokens, current tool
...
### Command · What it does
- **Command**: `/dsh-config` · **What it does**: Show or set this session's defaults: `tier=flash\ · pro`, `effort=off\ · high\ · max`, `mode=auto\ · hub\ · standalone`, `timeout=<seconds>`, `policy=auto\ · flash-only\ · pro-only`, `escalate=true\ · false`, `reset`
- **Command**: `/dsh-status` · **What it does**: Live status of worker jobs: tier, progress, tokens, current tool

## MCP tools

### Tool · Description
- **Tool**: `dsh_run_worker` · **Description**: Synchronous task dispatch (`tier`: flash/pro, `effort`: off/high/max, `cwd`), waits for result
- **Tool**: `dsh_spawn_worker` · **Description**: Asynchronous task dispatch, returns job id (for parallel fan-out)
- **Tool**: `dsh_worker_status` · **Description**: Query real-time progress of all jobs (turn/step/current tool/token)
- **Tool**: `dsh_worker_result` · **Description**: Fetch result, can specify `wait_seconds` to wait
- **Tool**: `dsh_worker_cancel` · **Description**: Cancel specified job, terminate its runtime process
...
### Tool · Description
- **Tool**: `describe_image` · **Description**: Answer questions by viewing images (screenshots, designs, charts, etc.), results cached by provider + model + image + question
- **Tool**: `generate_image` · **Description**: Generate image from text description, save to specified absolute path; output is flat bitmap (requires OpenPencil for layer editing)
