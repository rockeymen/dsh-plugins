...
- **Idempotent.** Stable ids (`pi-<uuid>` / `oc-` / `codex-` / `claude-`); re-imports skip what already exists.
...
- `export.mjs` writes `$DSH_HOME/exports//<session-id>.md` (`--source`, `--project`, `--limit`, `--since`, `--out`, `--no-reasoning`, `--no-tools`).
...
flowchart LR
    subgraph sources["Local data"]
        PI["pi sessions~/.pi/agent/sessions/*.jsonl"]
        OC["opencode sessions~/.local/share/opencode/opencode.db"]
        CX["codex sessions~/.codex/sessions/**/*.jsonl"]
        CC["claude-code sessions~/.claude/projects/**/*.jsonl"]
        AG["pi agents & promptsopencode agents"]
    end
    subgraph plugin["dsh-import-agents"]
        R["readerspi / opencode / codex / claude-reader"]
        C["convertturn structure + tool events"]
        W["writedsh JSONL persistenceor ctx.sessionPersistence"]
        S["skillsSKILL.md bundles"]
    end
...
**Agents & prompts → skills.** Written to `$DSH_AGENTS_HOME/skills/<name>/SKILL.md` (default `~/.agents/skills/`), discoverable via `ctx.skills.list()`. Name conflicts are renamed `<name>-` (e.g. `k3-reviewer-opencode`); existing bundles are only completed, never clobbered; identical content is skipped; frontmatter records `metadata.source` / `metadata.kind`.
