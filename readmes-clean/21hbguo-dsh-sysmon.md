...
### 功能 · 说明
- **功能**: 实时指标 · **说明**: CPU / MEM / 磁盘 / GPU 使用率 + 进度条；网络 ↓下载 / ↑上传 速率分行显示，各带独立进度条（按近期峰值自动缩放）
- **功能**: 进程网络 TOP10 · **说明**: 悬停 ↓下载 / ↑上传 行，浮层显示进程网络 TOP10（有字节计数：近 3s 速率均值 + EMA 平滑；无字节计数：自动降级为按活跃连接数排序）
- **功能**: 磁盘多挂载点 · **说明**: 物理挂载点全部列出（根优先、按已用降序，最多 6 个），过滤 proc/sysfs/tmpfs 等虚拟文件系统
- **功能**: GPU 支持 · **说明**: AMD 直接读 sysfs（零子进程）；NVIDIA 走 `nvidia-smi`（后台异步采样，不阻塞事件循环）
- **功能**: 拖动 / 折叠 / 隐藏 · **说明**: 按住标题栏拖动，位置持久化（localStorage）；点击标题折叠成单行；× 隐藏后右下角出现「性能」召唤按钮
- **功能**: 省资源 · **说明**: client 轮询 2s（仅页面可见时，标签页隐藏即停）；host 快照缓存 `cacheMs`，多次轮询不重复读 `/proc`
...
flowchart LR
    subgraph browser[浏览器 · DSH Web GUI]
        W[悬浮窗 client.js原生 DOM + fetch]
        C[设置卡片settings.plugin.item]
    end
    subgraph host[宿主进程]
        R[GET /api/sysmon/snapshot]
        S[SysmonCollectorcacheMs 缓存]
        P[/proc + node:os + statfs]
        SS[后台采样ss -tinp 每 3s]
        NV[后台采样nvidia-smi 每 5s]
    end
    W -- 2s 轮询（仅可见时） --> R
    R --> S
    S --> P
...
### 现象 · 原因 · 解决
- **现象**: 右下角没有悬浮窗 · **原因**: 插件未装配，或设置 `enabled` 为关 · **解决**: 确认插件行已装配（link / bundle / 注入）；设置页 → 系统性能看板 → 启用
- **现象**: 悬浮窗显示 `offline` · **原因**: 快照路由不可达（host 重启中、路由未注册） · **解决**: 等待下一个 2s 轮询自动恢复；确认插件已启用
- **现象**: 网络速率显示 `--` · **原因**: 非 Linux 平台 · **解决**: 网络总速率仅 Linux 支持（依赖 `/proc/net/dev`）
- **现象**: 进程 TOP10 显示「活跃连接」而非速率 · **原因**: 本机 `ss` 为精简构建，无字节计数 · **解决**: 正常降级行为，无需处理
- **现象**: GPU 行不出现 · **原因**: 非 AMD / NVIDIA，或驱动工具缺失 · **解决**: 静默降级为不显示，不影响其他指标
- **现象**: 构建报 `cannot locate the dsh checkout` · **原因**: 未设置 `DSH_CHECKOUT` 且常见路径无 DSH 检出 · **解决**: `export DSH_CHECKOUT=/path/to/dsh-source-checkout` 后重跑
- **现象**: 设置卡片提示「未向设置页暴露」 · **原因**: 部署的 DSH 版本未把 `sysmon` 命名空间加入设置白名单 · **解决**: 编辑 `~/.dsh/settings.yaml` 直接配置，或为 `dsh-host-apiproxy` 的 `WEB_SETTINGS_NAMESPACES` 白名单补充 `sysmon` 后重启
...
### key · 类型 · 默认 · 说明
- **key**: `enabled` · **类型**: boolean · **默认**: `true` · **说明**: 总开关；关闭后停用快照路由、隐藏悬浮窗并停止采集
- **key**: `cacheMs` · **类型**: number · **默认**: `2000` · **说明**: host 两次采集之间的最小间隔（100–60000，步长 100）
...
- **指标**: CPU · **来源（Linux）**: `/proc/stat` jiffies 两次差值 · **非 Linux 回退**: `os.cpus()` tick 差值
- **指标**: 内存 · **来源（Linux）**: `/proc/meminfo` MemAvailable · **非 Linux 回退**: `os.totalmem()/freemem()`
- **指标**: 磁盘 · **来源（Linux）**: `/proc/mounts` + `statfs`（物理挂载点，根优先） · **非 Linux 回退**: `statfs('/')`
- **指标**: 网络（总速率） · **来源（Linux）**: `/proc/net/dev` 非回环字节差值 · **非 Linux 回退**: 不可用（null）
- **指标**: 网络（进程 TOP10） · **来源（Linux）**: `ss -tinp`：有字节计数按速率差（TCP，同用户）；无字节计数自动降级为按活跃连接数排序 · **非 Linux 回退**: 不可用（空列表）
- **指标**: 进程数 · **来源（Linux）**: `/proc` 数字目录计数 · **非 Linux 回退**: 不可用（null）
- **指标**: GPU · **来源（Linux）**: AMD sysfs 直接读（零子进程）/ NVIDIA `nvidia-smi`（异步 5s） · **非 Linux 回退**: 不可用（null）
- **指标**: 负载 / 运行时长 · **来源（Linux）**: `os.loadavg()` / `os.uptime()` · **非 Linux 回退**: 同左
