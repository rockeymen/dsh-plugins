# dsh-mcp-manager
  ...
  MCP 可视化管理器：装没装、连没连，一目了然
  `查看列表` `新增删除` `启用停用` `连接状态` `连接测试` `中英双语`
  设置 → MCP 一站管理 DeepSeek Harness 里的所有 MCP 服务器，
  无需再手改 `cordis.patch.yml` —— 所有修改即改即生效（HMR 热应用）。
...
### 现象 · 原因与解决
- **现象**: 报 `minimum release age` / 版本不足 24h · **原因与解决**: 装的版本发布不足 24 小时。等 24h 或重跑一次（脚本会自动补 `minimumReleaseAgeExclude`）。
- **现象**: 报「找不到 profile 目录」 · **原因与解决**: 先跑一次 `dsh web`，让它初始化 `~/.dsh/profiles/web`。
- **现象**: 页面出现**两个 MCP 页签** · **原因与解决**: 双挂载：`~/.dsh/profiles/web/cordis.patch.yml` 还留着旧的手动挂载行，删掉那段 `- insert: ... mcp-manager ...`（脚本会自动清）。
- **现象**: 装完没看到 MCP 页签 · **原因与解决**: 硬刷新（Cmd/Ctrl+Shift+R）；仍没有就重启 DSH 一次（host 半首次挂载需要）。
- **现象**: Obsidian MCP 报 401 · **原因与解决**: 检查 headers 格式：应为 `Authorization: Bearer <api-key>`，不要带引号（表单已支持直接粘贴 `"Key": "value"` 自动去引号）。
- **现象**: 修改配置后未生效 · **原因与解决**: 本插件所有修改走 HMR 热应用，等 1–2 秒自动刷新；页面右上角可手动刷新。
...
### 字段 · 说明
- **字段**: `patchFile` · **说明**: 要编辑的用户补丁层绝对路径。默认 `$DSH_HOME/profiles/web/cordis.patch.yml`。
