...
![](https://github.com/user-attachments/assets/3b1f3a25-f962-4e02-a9ef-d23e0d01fc8e)
...
两种命令都会从本机 `web` profile 加载同一个浏览器 bundle。默认端口为 3080；被占用时执行 `pnpm start -- --port ` 或 `npx @deepseek-ai/dsh web --port `。工具栏出现 DeepSeek 鲸鱼图标后，点击即可打开侧边栏。
...
- 只操作活动标签页，绝不静默切页。
- 网页文字会标记为不可信输入。默认「自动共享」只按需读取活动标签页且不额外弹窗；对隐私敏感时可选择「每次询问」，或用「关闭」完全阻断读取。在「每次询问」模式下，读取弹窗可以仅允许一次，也可以持久切回自动读取；之后仍可在设置中关闭。读取的页面文字会发送给当前选择的模型。
