...
- **设置面板——侧边栏里的「**通知**」入口与栏目内容**: ![通知设置栏目](screenshots/01-notifications-section.png) · **每种类型的音效选择器（官方下拉菜单）**: ![音效选择器](screenshots/02-sound-menu-open.png)
...
### 类型 · 触发时机 · 默认音效
- **类型**: 会话完成 · **触发时机**: 一轮会话正常结束（`turn/end` completed） · **默认音效**: 叮咚
- **类型**: 会话失败 · **触发时机**: 一轮会话出错中断，或宿主上报 agent 错误 · **默认音效**: 低鸣
- **类型**: 问问题 · **触发时机**: Agent 正在等你回答（`question/requested`） · **默认音效**: 轻响
- **类型**: 权限请求 · **触发时机**: Agent 请求执行需要授权的操作（`approval/requested`） · **默认音效**: 警示
