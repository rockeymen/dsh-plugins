...
- **🛡️ **Survive model outages****: Backup rotation, cooldowns, provider circuits, and recovery probes keep a failed model from ending the run. · **⚡ **Scale work sideways****: Five orchestration modes cover parallel research, staged plans, routing, reduction, and supervisor review. · **✅ **Deliver with confidence****: Budgets, run history, resumability, structured output, and reviewer passes keep the final result inspectable.
...
Configure model recovery, watch parallel work progress, inspect the actual model used by each subagent, and audit the run after it ends.
  ...
  ![HA Orchestrator settings gallery with model recovery, orchestration, custom subagents, and subagent editor](docs/settings-gallery.png)

  ![HA Orchestrator completed run and budget failure states](docs/run-states-gallery.png)
...
### Command · Description
- **Command**: `/ha` · **Description**: Show the current HA status (same as `/ha status`).
- **Command**: `/ha status` · **Description**: Show quarantine, failure counts, rotation cursors, switch history, and probe log.
- **Command**: `/ha diag` · **Description**: Show plugin diagnostics: service availability, persistence, language, and injection status.
- **Command**: `/ha reset` · **Description**: Clear quarantine, failure counts, cursors, and history.
- **Command**: `/ha probe  <model>` · **Description**: Manually probe a model to verify recovery.
- **Command**: `/orchestrate` · **Description**: List recent orchestrate runs (same as `/orchestrate runs`).
- **Command**: `/orchestrate runs` · **Description**: List the 10 most recent orchestrate runs.
- **Command**: `/orchestrate show <runId>` · **Description**: Show details of a specific orchestrate run.
- **Command**: `/orchestrate presets` · **Description**: List configured orchestration presets.
...
- **Card**: Model High Availability · **What you can do**: On/off, backup list (structured rows with inline dropdown editing, "Recommended backups", and an empty-state guide), and an "Advanced" section for cooldown, failure threshold, burst window, provider circuit threshold, probe recovery, context-overflow degrade, error-code filter, persist selection, and stop steering
- **Card**: Subagent Orchestration · **What you can do**: On/off, provider, default concurrency (6), max subagents per run (16), global concurrency cap, pipeline retry, merge/render limits, and delegation-depth limit — grouped into Basics / Concurrency & budgets / Advanced
- **Card**: Custom Subagents · **What you can do**: Add, edit, reorder, delete (avatar + model/effort badges); built-in reviewer/researcher/research-merger definitions; "AI Generate" creates one from a description; tool allow/deny lists and per-role fallback chains that handle start failures and child model errors without reading the global HA backup list
- **Card**: Diagnostics · **What you can do**: HA runtime (current default, quarantine with level and cooldown countdown, failure counts, cursors, probes, failover history, reset) and **expandable recent runs** (mode badge, duration, per-subtask status table with lastKey, result summary — history survives restarts)
- **Card**: System · **What you can do**: Plugin language (follow system / Chinese / English), the orchestration hint toggle, the live injection status, one-click config export/import, and the debug card toggle
