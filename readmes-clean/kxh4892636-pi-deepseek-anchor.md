...
Manual fallback: copy `index.ts` to `~/.pi/agent/extensions/anchored-standard/index.ts` (global) or `/.pi/extensions/anchored-standard/index.ts` (project-local).

## Behavior

###  · Request #1 (bootstrap) · Request #2+ (promoted)
- Tools · **Request #1 (bootstrap)**: `bash` + `str_replace_editor`, byte-identical to the official Minimal preset · **Request #2+ (promoted)**: full pi tool catalog
- System prompt · **Request #1 (bootstrap)**: `You are a helpful software engineer assistant.` (46 chars) · **Request #2+ (promoted)**: persona remains; stripped pi context returns as a user message
- Output budget · **Request #1 (bootstrap)**: adapter default (no cap; `bootstrapMaxTokens` is opt-in) · **Request #2+ (promoted)**: adapter default
...
- **Upstream (dsh preset)**: Minimal tool rows · **pi port**: `pi.registerTool` overrides: `bash` uses the byte-identical Minimal description and delegates to pi's built-in `createBashTool`; `str_replace_editor` implements the official schema and upstream filesystem semantics (`view` / `create` / `str_replace` / `insert`, 16000-char truncation, exact error messages)
- **Upstream (dsh preset)**: `tool-bootstrap` pre-step filter · **pi port**: `before_agent_start` + `before_provider_request`: request #1 is Minimal-exact; late appends (e.g. pi-memory) are dropped on request #1 and moved to a user message after promotion
- **Upstream (dsh preset)**: Persona row (`complete: true`) · **pi port**: `minimalPersona` + `personaScope: "always"` (default): persona remains the system prompt for the whole session; stripped context is re-delivered as a user message from request #2
- **Upstream (dsh preset)**: Durable event scan · **pi port**: phase derived from pi's durable session branch plus a `dsh-anchored-state` custom entry
- **Upstream (dsh preset)**: Zero-Anchored Standard mode · **pi port**: `zeroAnchor: true` in `PI_DSH_ANCHOR_CONFIG`
