[![dshfind](https://dshfind.com/api/badge/huanlinoto/dsh-plugin-anti-ads?lang=zh)](https://dshfind.com/zh/plugins/huanlinoto/dsh-plugin-anti-ads?ref=badge)
...
### 层 · 机制 · 关什么
- **层**: L1 设置写穿 · **机制**: 通过 dsh-ads 自身的存储契约（`localStorage['dsh-ads:settings']` + `dsh-ads:persist-changed` 事件）把所有广告位开关写成关闭，并监听 dsh-ads 的广播持续压制 · **关什么**: 两侧广告栏、对话内信息流、右下角弹窗、跑分、安全告警、贪玩蓝鲸海报
- **层**: L2 兜底清理 · **机制**: `MutationObserver` 监听页面变化，按形状（`position:fixed` + dsh-ads 的 z-index 区间，或全屏穿透遮罩层）识别并移除漏网弹层 · **关什么**: dsh-ads 未来版本绕过设置新增的广告位、改版后的根浮层
- **层**: L3 屏蔽动态广告源 · **机制**: 对 `/dsh-ads/registry.json` 的请求（含带 query 的变体）直接返回空列表 · **关什么**: 社区插件的自动横幅
- **层**: L4 深层拦截 · **机制**: 广播 dsh-ads 自身的 `dsh-ads:retired` 事件——它的「关闭所有广告」是模块级一次性开关，广告层在渲染前就检查这个标志，广播后整层隐藏到刷新 · **关什么**: 无视自身设置开关、任何新加广告位的 dsh-ads 版本
