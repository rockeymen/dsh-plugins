...
flowchart LR
  ...
  T --> H
  L --> H
  H --> R[/remfs RPC channeltrusted-host fence/]
  R --> A[Device authenticationpairing + per-device credential]
  A --> F[Filesystem capability layerallowlist + protected paths + realpath]
...
1. Open the phone URL (`https://.<tailnet>.ts.net`, or the LAN URL when
...
### Symptom · Fix
- **Symptom**: Phone shows the pairing screen forever · **Fix**: Read the code from `%USERPROFILE%\.dsh\remfs-pairing.txt`; codes expire after 10 min — restart the harness to generate a new one
- **Symptom**: Device revoked / re-pairing fails · **Fix**: Pairing codes are single-use; restart the harness for a fresh code
- **Symptom**: Phone gets 403 · **Fix**: Use the printed HTTPS/Tailscale/LAN URL; the GUI must run with those hosts trusted (one-click deploy does it)
- **Symptom**: LAN URL unreachable · **Fix**: Phone must be on the same Wi-Fi; re-run the launcher so the current LAN IP is detected
- **Symptom**: `npm.ps1` blocked by execution policy · **Fix**: Use `npm.cmd`, or `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned`
- **Symptom**: `npm view` 404s right after a publish · **Fix**: CDN edge cache — wait a minute or query with `Cache-Control: no-cache`
- **Symptom**: Plugin never appears after `dsh plugin add` · **Fix**: You must also append the loader row and restart `dsh web`
- **Symptom**: PC sleeps · **Fix**: keep_awake + power plan are handled by the deploy; see `keep_awake.ps1`
