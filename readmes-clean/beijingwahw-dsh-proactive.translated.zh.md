...
### 质变内核（core/，3.0 → 11.0）
### 内核 · 版本 · 一句话
- **内核**: evidence.ts · **版本**: 3.0 · **一句话**: 统一证据语言：Wilson 界 / 时间衰减 / 证据排序，铺满所有记忆层
- **内核**: resilience.ts · **版本**: 4.0 · **一句话**: 弹性执行：熔断器状态机 / 指数退避全抖动 / 错误分型
- **内核**: causal-kernel.ts · **版本**: 5.0 · **一句话**: 因果推断：Pearl do-干预 ATE / 混杂检测 / 反事实查询
- **内核**: free-energy.ts · **版本**: 6.0 · **一句话**: 主动推断：Friston 自由能，一公式统一利用/探索/好奇心/健康度
- **内核**: deliberation.ts · **版本**: 7.0 · **一句话**: 规划即推断：想象推演 + beam search × 技能宏 + 梦实现对账
- **内核**: metareasoning.ts · **版本**: 8.0 · **一句话**: 理性元推理：双过程仲裁 / 任意时搜索稳定停机 / 思考按 nat 计价
- **内核**: abstraction.ts · **版本**: 9.0 · **一句话**: 抽象泛化：状态骨架分解 + 结构类比，经验跨域「举一反三」
- **内核**: scientist.ts · **版本**: 10.0 · **一句话**: 科学家心智：贝叶斯最优实验设计（见上）
- **内核**: theorist.ts · **版本**: 11.0 · **一句话**: 理论心智：层级贝叶斯 + MDL 定律归纳（见上）
...
### 分组 · 工具
- **分组**: 执行与调度 · **工具**: `autonomous_execute` · `model_dashboard` · `run_benchmark`
- **分组**: 记忆与知识 · **工具**: `query_memory` · `query_experience` · `distill_knowledge` · `maintain_memory` · `memory_migration`
- **分组**: 元认知 · **工具**: `mental_report` · `self_knowledge` · `meta_cognition`
- **分组**: 自主治理 · **工具**: `manage_autonomy` · `manage_keys`
- **分组**: 基础设施 · **工具**: `manage_tenants` · `manage_encryption` · `manage_sync` · `manage_consensus` · `manage_hot_reload`
...
### 分组 · 脚本
- **分组**: 双心智 · **脚本**: verify-scientist · verify-theorist
- **分组**: 质变内核 · **脚本**: verify-unified-evidence · verify-resilience-governance · verify-causal-kernel · verify-active-inference · verify-deliberation · verify-metareasoning · verify-abstraction
- **分组**: 共生经济 · **脚本**: verify-symbiosis · verify-symbiosis-bridge · verify-belief-market · verify-futarchy · verify-energy-feedback · verify-full-agents · verify-observability
- **分组**: 学习与进化 · **脚本**: verify-self-evolution · verify-self-evolution-v2 · verify-knowledge-distillation · verify-policy-evolution · verify-meta-cognition · verify-meta-cognition-v2 · verify-meta-edge · verify-consensus-sync
...
- **官方 Tool 注册链路**：宿主加载 `@deepseek-ai/dsh-tools`（`ctx.tools` 服务）时，18 个 Tool 经 duck-typing 桥接注册进官方 ToolRegistry，纳入 pre/around/post 执行管线与模型可见面（参数转为官方 JSON Schema 子集）；宿主未提供时静默降级为内部 ToolRegistry + `ctx.provide('s