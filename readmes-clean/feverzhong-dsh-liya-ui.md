...
> 2. **层叠顺序**：host 默认样式（`:root{--liya-radius:16px}`）注入在 `` 开头，晚于 head 里的
...
### 表面 · 圆角
- **表面**: 输入框 / 搜索框（textarea、text/search/email/url） · **圆角**: 基础值 + 2px
- **表面**: 对话框 `[role="dialog"]` · **圆角**: 基础值 + 6px
- **表面**: 菜单 / 下拉 / 提示 `[role="menu"/"listbox"/"tooltip"/"combobox"]` · **圆角**: 基础值

基础值 = settings 配置 `radius`（默认 16），走 `--liya-radius` 变量。

## 原理

- host 半 `webServer.tapIndex()` 往 index.html 的 `` 后注入 `<style>`（官方 ui-theme bootTheme 同款 seam），
