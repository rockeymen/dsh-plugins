...
**Token Bank 是新一代个人AI资源管理中枢**，通过本地网关架构实现对 Claude、Cursor、Codex、WorkBuddy 等主流 AI 工具的一键纳管与智能调度，核心价值在于**用量透明化**、**成本优化**与**共享交换网络**——全链路 Trace 让每一笔 Token 消耗有据可查，智能路由自动在本地模型、免费额度、付费订阅与社区共享算力间无损切换，并通过 **P2P 算力共享网络**形成模型与智能体的去中心化共享交换网络，闲置资源变现为积分。技术层面通过**声明式应用 Handler**（CLI 环境变量注入 + 配置文件热补丁）实现零侵入接入，多协议适配层（Anthropic Messages、OpenAI Chat、Codex Responses）让 Agent 无感使用第三方模型；**统一资产层架构**使社区智能体可直接运行在客户既有的 agent 应用（Codex、Claude、Cursor 等）上，无需重复构建 harness，在用户沉淀的 MCP/Skill/Prompt 资产中执行，实现运行时与工具生态的双重复用；**场景路由引擎**支持转发策略学习、上下文无损压缩，并可为非多模态模型自动注入图片识别能力（视觉增强层）；通过 **MCP 内置中转**与**资源投射门控**构建个人知识与工具生态。产品采用**AI 原生架构**，摒弃传统硬编码规则，让智能体根据实际场景需求动态构建资产发现、个性化推荐、路由决策优化等核心能力并持续进化（以智能体构建智能体管理平台），实现高度灵活性与鲁棒性；系统通过**基于使用习惯的进化机制**，从真实调用记录与会话模式中自动提炼工作画像，驱动 MCP/Skill/Prompt/Agent 的个性化推荐与路由策略持续优化，实现多设备用量云端聚合、Agent 聚合编排等能力，成为真正意义上的**越用越懂你的 AI 管家**。
...
### 主线 · 你得到什么
- **主线**: **用的明白** · **你得到什么**: 一键纳管；全链路 Trace；多维盘点与多设备汇总；订阅 / 按量对照
- **主线**: **用的节省** · **你得到什么**: 无损换模；智能路由（本地源优先 + 任务分型）；场景策略；可选无损压缩
- **主线**: **用的简单** · **你得到什么**: 纳管 / 还原一键完成；CLI 多账号按目录分发；托盘常驻；改一个本地地址即可接入
- **主线**: **越用越懂你** · **你得到什么**: 工作画像；MCP / Skill / Prompt / Agent 个性化发现、沉淀与迭代
- **主线**: **闲置赚钱** · **你得到什么**: 贡献闲置算力赚积分；**雇佣智能体**；圈子共享与网络地图
...
### 层次 · 说明
- **层次**: **应用 Handler** · **说明**: `app-handlers.yaml` 声明式描述 CLI shim / 配置文件 patch / 会话扫描；WorkBuddy、Trae、Hermes、Kimi 等按强信号探测安装态
- **层次**: **路由** · **说明**: 统一「路由 = Selector 链」：个人/社区/免费/付费来源 + 预定义任务分型（design / repo-qa / chore / debug）
- **层次**: **资源投射** · **说明**: Skill / Prompt / MCP 只写入**已纳管且本机已装**的目标；无 stdio 的应用可走内置 MCP 中转
- **层次**: **数据面** · **说明**: 网关实时日志 + 本地会话补录（Claude / Codex / Cursor / WorkBuddy Trace 等）自动去重
...
### 应用 · 接入方式
- **应用**: Claude Code / Codex CLI / OpenCode / Hermes / Kimi Code · **接入方式**: CLI 透明托管：自动注入 `BASE_URL` 等环境变量，无需改命令
- **应用**: Claude Desktop / Codex Desktop / OpenClaw / WorkBuddy · **接入方式**: 配置文件写入：一键 patch 指向本地网关（缺省配置可在强信号确认后首次创建）
- **应用**: Trae Work · **接入方式**: 会话补录 + 手工填网关参数（IDE 内自定义模型）
- **应用**: Cursor / Copilot / Qwen / Grok 等 · **接入方式**: 会话统计，或 OpenAI 兼容入口改 `OPENAI_BASE_URL` / 网关专用 Key
...
### 模式 · 说明
- **模式**: **网关实时代理** · **说明**: 请求经 `localhost:11430` 转发，记录路由链、真实模型、Token、延迟、费用
- **模式**: **会话补录** · **说明**: 纳管但未走网关时，扫描本地会话日志（`~/.claude`、`~/.codex`、WorkBuddy Trace 等）补录用量
- **模式**: **自动去重** · **说明**: 同一次调用若既走网关又落进会话文件，只记一次
...
### 供给类型 · 包含 · 说明
- **供给类型**: **本地源** · **包含**: Ollama、免费 API、APP/API 订阅、按量付费 · **说明**: 本机网关直连转发，Key 不上云
- **供给类型**: **社区分享源** · **包含**: 社区共享算力网络 · **说明**: 消耗积分调用远程节点，模型列表动态下发
...
### 能力 · 说明
- **能力**: **设备注册** · **说明**: 每台机器自动分配 device_id，60s 心跳保持在线状态
- **能力**: **盘点快照** · **说明**: 按 1 / 7 / 30 天上报调用、Token、费用、本地源 / 社区分享 分布、Top 模型与应用
- **能力**: **云端合并** · **说明**: **个人** 页与 **盘点** 页展示各设备占比、在线状态、独立明细与汇总视图
- **能力**: **跨端一致** · **说明**: 订阅账户、按量配置、工具清单登录后同步，换设备无需重复配置
...
### 类型 · 管理方式 · 典型场景
- **类型**: **APP 订阅** · **管理方式**: 登记 ChatGPT / Claude / Gemini / Cursor 等套餐与月费 · **典型场景**: 仅统计官方订阅用量，或通过 OAuth 转为 API 供给
- **类型**: **API 订阅** · **管理方式**: 独立目录管理厂商 API 套餐（如火山引擎 Coding Plan） · **典型场景**: 走 API Key 网关，与 APP 订阅分开核算
- **类型**: **按量付费** · **管理方式**: 登记供给源、模型列表与 USD/百万 Token 刊例价 · **典型场景**: 供给源页仅可选用此处已配置的模型；费用估算以此为准
...
### 能力 · 说明
- **能力**: **自动扫描** · **说明**: 启动或手动重扫，发现本机已有 CLI 账号实例并入库
- **能力**: **手工新建** · **说明**: 网关页「CLI 实例」类型，补录扫描未覆盖的账号
- **能力**: **生效目录** · **说明**: 多实例时为每个账号指定工作目录；shim 按 `$PWD` 注入对应环境
- **能力**: **额度可视** · **说明**: 抓取 Claude / Codex 订阅额度，托盘与应用列表展示当日用量
...
### 类型 · 能力
- **类型**: **社区目录** · **能力**: 登录后拉取 MCP / Skill / Prompt / Agent 推荐清单（缓存优先，离线走内置兜底）
- **类型**: **投射** · **能力**: 只投射到**已纳管且本机已装**的目标；可取消；纳管时级联依赖
- **类型**: **内置 MCP 中转** · **能力**: 无 stdio 通道的应用：选应用 → 绑定 prompts/models/resources → 复制中转配置即可
- **类型**: **Prompt MCP** · **能力**: Prompt 经 `tokenbank-prompts`（`tb_get_prompt` / `tb_list_prompts`），按投射集过滤
- **类型**: **工作画像海报** · **能力**: 盘点页可生成四套气质的分享海报（专业 / 可爱 / 幽默 / 简约）
...
### 端口 · 用途
- **端口**: `11430` · **用途**: LLM 请求入口（`OPENAI_BASE_URL=http://host:11430/v1`）
- **端口**: `11431` · **用途**: Web 管理界面
...
### 页面 · 功能
- **页面**: **盘点** · **功能**: 多维统计：应用占比、**本地源 / 社区分享** 分布、模型排行、压缩节省、费用估算；**工作画像分享海报**
- **页面**: **网关** · **功能**: **一键纳管**（含 WorkBuddy / Trae 等）+ **CLI 多账号**；会话 Trace；场景 / 任务分型路由
- **页面**: **调试 / 游乐场** · **功能**: **Agent 聚合编排**（含社区智能体、图片输入）；工具流、停止续接
- **页面**: **资源** · **功能**: 社区推荐 **MCP / Skill / Prompt / Agent**；投射门控；**内置 MCP 中转**；画像推荐
- **页面**: **供给源** · **功能**: **本地源**与 **社区分享源**；模态（文本/图文/生图/嵌入）；测速与动态目录
- **页面**: **圈子 / 贡献 / 网络** · **功能**: 圈子共享 · 贡献节点 / **雇佣智能体** · 全球节点地图（网页亦可试用）
- **页面**: **配置** · **功能**: 网关端口、超时、并发 · 无损压缩 · 云端账号与转发 Key
...
### 变量 · 说明
- **变量**: `ADMIN_KEY` · **说明**: 管理后台密钥
- **变量**: `REQUEST_TIMEOUT` · **说明**: 单次转发超时秒数（默认 120）
