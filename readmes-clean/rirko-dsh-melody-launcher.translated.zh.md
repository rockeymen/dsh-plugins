![dsh-旋律启动器](public/launcher-icon.png)
...
### 原本要做的事 · 用启动器之后
- **原本要做的事**: 装 Node.js → 装 npm → `npx @deepseek-ai/dsh` · **用启动器之后**: 下载一个 exe，点「下载安装 DSH」
- **原本要做的事**: 手改 `.credentials.yaml` 填 API Key · **用启动器之后**: 界面里输入，自动写入并设为 0600 权限
- **原本要做的事**: 翻 GitHub 找插件、手敲 `dsh plugin add` · **用启动器之后**: 内置搜索 `dsh-plugin` Topic，一键安装
- **原本要做的事**: 编辑 profile 的 `package.json` 调加载顺序 · **用启动器之后**: 拖动列表，直接改官方 Profile
- **原本要做的事**: 开终端、记命令、盯输出 · **用启动器之后**: 一个按钮启动，实时日志面板
...
### 情况 · 行为
- **情况**: 检测到系统安装 · **行为**: 直接使用，不重复安装
- **情况**: 未检测到 · **行为**: 首页主按钮变为「下载安装 DSH」，引导首次部署
- **情况**: 执行安装 · **行为**: 通过 npm 将 `@deepseek-ai/dsh@latest` 装入启动器本地运行目录，并自动切换启动命令为本地可执行文件
...
### 环节 · 说明
- **环节**: **来源** · **说明**: Node.js 官网 `https://nodejs.org/dist/`，当前锁定 `v24.19.0`
- **环节**: **架构** · **说明**: 自动匹配 `win-x64` 或 `win-arm64`
- **环节**: **校验** · **说明**: 下载官方 `SHASUMS256.txt`，逐字节 SHA-256 比对；校验失败自动重下一次，仍失败则中止并报错
- **环节**: **续传** · **说明**: 使用 HTTP `Range` 请求，网络中断后可从断点继续
- **环节**: **解压** · **说明**: 调用 Windows 自带 `tar.exe` 解压到临时目录，校验完整性后原子重命名到最终位置
- **环节**: **位置** · **说明**: `%APPDATA%\dsh-launcher\node-runtime\`
...
### 内容 · 路径
- **内容**: 启动器设置 · **路径**: `%APPDATA%\dsh-launcher\settings.json`
- **内容**: 本地 DSH 运行目录 · **路径**: `%APPDATA%\dsh-launcher\dsh-runtime\`
- **内容**: Node.js 便携运行时 · **路径**: `%APPDATA%\dsh-launcher\node-runtime\`
- **内容**: DSH 凭据 · **路径**: `$DSH_HOME\.credentials.yaml`
- **内容**: DSH Profile 清单 · **路径**: `$DSH_HOME\profiles\\package.json`
...
### 项 · 默认值
- **项**: `DSH_HOME` · **默认值**: 环境变量 `DSH_HOME`，否则 `%USERPROFILE%\.dsh`
- **项**: Profile 名称 · **默认值**: `web`
- **项**: 工作目录 · **默认值**: 系统「文档」目录
- **项**: 启动命令 · **默认值**: `npx --yes @deepseek-ai/dsh web`（检测到本地安装后自动切换为 `dsh web`）
- **项**: 启动后自动打开网页 · **默认值**: 开启

## 安全设计

### 措施 · 实现
- **措施**: **渲染进程隔离** · **实现**: `contextIsolation: true`、`nodeIntegration: false`、`sandbox: true`
- **措施**: **受控 IPC 面** · **实现**: 渲染层只能通过 `contextBridge` 暴露的固定接口与主进程通信，无法直接访问 Node API
- **措施**: **凭据文件权限** · **实现**: `.credentials.yaml` 以 `0600` 写入，目录 `0700`；先写临时文件再原子重命名，避免写坏原文件
- **措施**: **外链白名单** · **实现**: `shell.openExternal` 只允许 `http:` / `https:` 协议
- **措施**: **输入校验** · **实现**: 包名、Profile 名、GitHub 仓库名在进入主进程逻辑前统一校验；目录参数必须是绝对路径
- **措施**: **下载完整性** · **实现**: Node.js 运行时下载后强制 SHA-256 校验，不匹配即丢弃
...
### 命令 · 作用
- **命令**: `npm run dev` · **作用**: 启动 Vite 开发服务器 + Electron 主进程（热更新）
- **命令**: `npm test` · **作用**: 运行 Vitest 测试套件
- **命令**: `npm run build` · **作用**: TypeScript 类型检查（`tsc --noEmit`）+ Vite 生产构建
- **命令**: `npm run preview` · **作用**: 预览构建产物
- **命令**: `npm run package:win` · **作用**: 构建并用 electron-builder 打包 Windows 便携版
