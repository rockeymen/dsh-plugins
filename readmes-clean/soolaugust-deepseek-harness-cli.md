...
### Action · Effect
- **Action**: Type text + `Enter` · **Effect**: Send to the agent
- **Action**: `↑` / `↓` · **Effect**: History navigation (cross-session persistent)
- **Action**: `Shift+Tab` · **Effect**: Cycle permission preset
- **Action**: `Ctrl+O` · **Effect**: Expand/collapse tool group
- **Action**: `Ctrl+C` · **Effect**: Cancel current turn while running; quit at the prompt
- **Action**: `/exit` `/help` `/clear` `/session` `/model` `/permission` · **Effect**: Slash commands
