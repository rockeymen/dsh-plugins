...
### 配置项 · 环境变量 · 默认值
- **配置项**: 域名（server_name / 证书 CN） · **环境变量**: `DOMAIN` · **默认值**: `localhost`
- **配置项**: 证书链路径（容器内） · **环境变量**: `SSL_CERT` · **默认值**: `/etc/nginx/certs/fullchain.pem`
- **配置项**: 私钥路径（容器内） · **环境变量**: `SSL_KEY` · **默认值**: `/etc/nginx/certs/privkey.pem`
...
### 文件 · 说明
- **文件**: `data/plugins.json` · **说明**: 插件目录（1080 个仓库的整理结果）
- **文件**: `data/stats_history.json` · **说明**: 走势图数据：每次爬取的真实快照（时间/插件数/Stars），历史由 git 提交记录重建
- **文件**: `scripts/build_data.py` · **说明**: 数据构建脚本：抓取、分类、安装命令、README 摘要、精选标记、走势数据
- **文件**: `.github/workflows/update-plugins.yml` · **说明**: 每小时自动抓取并提交新数据
- **文件**: `assets/css/` `assets/js/` · **说明**: 页面样式与应用脚本（含 SVG 走势图渲染）
- **文件**: `assets/fonts/` · **说明**: 自托管字体（Space Grotesk + JetBrains Mono）
- **文件**: `assets/img/` · **说明**: 品牌素材：Token Bank logo（SVG）、Wink 品牌图（PNG）、DeepSeek logo
