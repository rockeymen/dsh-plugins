...
### 文件 · 角色
- **文件**: `lib/index.js` · **角色**: 服务端：`GET /api/session-cost/summary?session=`（增量折叠会话事件并按模型计价）、`GET /api/session-cost/balance`（DeepSeek 余额，loopback-only 精确路由，`?refresh=1` 强制绕过缓存）
- **文件**: `lib/cost.js` · **角色**: 纯函数：按模型 token 折叠（replace-last-sample 语义）+ CNY 单价表 + 费用计算
- **文件**: `lib/balance.js` · **角色**: 纯函数：DeepSeek 余额接口查询与状态归一化
- **文件**: `lib/client.js` · **角色**: 浏览器端：`conversation.composer.dock` 槽位（id `session-cost`, order 100）+ `settings.plugin.item` 设置卡片；显示方式存 localStorage（`dsh-session-cost:config`），"并入统计栏"模式把费用/余额段追加进自带统计行 DOM（MutationObserver 在 React 重渲染后重新挂载）
...
- **模型**: deepseek-v4-flash · **输入（缓存未命中）**: ¥1 · **输入（缓存命中）**: ¥0.02 · **输出**: ¥2
- **模型**: deepseek-v4-pro · **输入（缓存未命中）**: ¥3 · **输入（缓存命中）**: ¥0.025 · **输出**: ¥6
- **模型**: deepseek-chat（V3 遗留，默认） · **输入（缓存未命中）**: ¥2 · **输入（缓存命中）**: ¥0.5 · **输出**: ¥3
- **模型**: deepseek-reasoner（V3 遗留，默认） · **输入（缓存未命中）**: ¥4 · **输入（缓存命中）**: ¥1 · **输出**: ¥16
