...
### dsh 偏好 · 标签页效果
- **dsh 偏好**: `system`（默认） · **标签页效果**: 跟随 Hana 当前主题（明暗 + 配色）
- **dsh 偏好**: `light` / `dark` · **标签页效果**: dsh 内置原生配色
...
2. **配色**：壳桥 `getComputedStyle` 读宿主 `theme.css`（宿主动态端点 `/api/plugins/theme.css?theme=`，返回压平为 `:root` 的扁平版，插件页面 link 即生效）的 16 个主题变量（bg 层次 / 文字三阶 / accent 三态 / border / green / danger / userBg / overlay 三档 / dropOverlay），回传给注入的 `dsh-hana-theme` cordis 插件，写 **body 层 `!important` 覆盖**（dsh 的 presenter 把 token 以 inline style 写到 body，覆盖必须同层才压得过）
...
dsh 的 `/api/events.mux` **要求 WebSocket 升级**：GET 返回 `426 Upgrade Required`，用 fetch + SSE 解析是错的。事件流必须 `ws://127.0.0.1:/api/events.mux`（Node 22+ 内置全局 WebSocket）：
...
### 键 · 默认 · 说明
- **键**: `apiKey` · **默认**: 空 · **说明**: DeepSeek API Key（插件设置界面填写，Agent 不代填）。安装后必填（无 key web host 起不来），改后重启 Hana 生效
- **键**: `model` · **默认**: `deepseek-v4-flash` · **说明**: dsh 任务模型 id（provider 固定 deepseek-official，模型名 pass-through 直传）：内置 `deepseek-v4-flash` / `deepseek-v4-pro`，可填其他 DeepSeek 模型 id；改后对新任务立即生效（直读 config.json）
- **键**: `agentPreset` · **默认**: `standard` · **说明**: dsh_run 提交任务的默认 agent 预设；工具调用可显式覆盖；改后对新任务立即生效
- **键**: `reasoningEffort` · **默认**: `high` · **说明**: 默认推理强度：off / high / max；工具调用可显式覆盖
- **键**: `approvalTimeoutMs` · **默认**: `30000` · **说明**: 审批挂起超过该时长无人应答自动 rejected（应答方失联检测）；0=禁用；改后对新审批立即生效
- **键**: `nodePath` · **默认**: 空 · **说明**: 启动 web host 的 node.exe（需 Node 24+）。**安装后必填本机 node 路径**（不预设默认值），改后重启 Hana 生效
- **键**: `defaultCwd` · **默认**: 空 · **说明**: 默认沙箱工作目录。**安装后建议设为实际项目目录**（为空且未传 cwd 时报 `cwd 不能为空`）
- **键**: `defaultTimeoutMs` · **默认**: 600000 · **说明**: 默认超时（毫秒）
- **键**: `webPort` · **默认**: 3080 · **说明**: dsh Web UI 端口：>0 插件加载即拉起 web host（卸载一并回收），0 关闭
- **键**: `callbackMode` · **默认**: `summary` · **说明**: 异步完成回调输出体量：summary=只带最终结论摘要（默认，省上下文）/ full=全量
...
- **v0.7.9**（2026-08-15）：agnes 审查处置——health 返回结构、轮询 hidden 降频、beforeunload 清理