...
### 操作 · 命令 / 位置
- **操作**: 安装 · **命令 / 位置**: `dsh plugin --profile web add github:ymh0000123/dsh-client-masquerade`
- **操作**: 卸载 · **命令 / 位置**: `dsh plugin --profile web remove dsh-client-masquerade`
- **操作**: 补丁应用 · **命令 / 位置**: 设置页 **User-Agent 补丁 → 应用**，或 `node node_modules/dsh-client-masquerade/patches/apply-pi-ai-useragent-patch.mjs`（重启 `dsh web` 生效）
- **操作**: 补丁还原 · **命令 / 位置**: 设置页 **User-Agent 补丁 → 还原**，或 `node .../apply-pi-ai-useragent-patch.mjs --revert`（重启 `dsh web` 生效）
- **操作**: 清除伪装头 · **命令 / 位置**: 设置页点 **Off**，或 `mask_client action=off provider=`（可留空 `headers` 字段）
...
- 伪装头写入 provider 配置的 `headers` 字段（`settings.yaml` 中 `llm-pi-ai.providers..headers`），由 pi-ai 适配器在**每次请求**（Anthropic Messages 与 OpenAI 兼容协议均覆盖）原样发送。
...
### 预设 · 写入的请求头
- **预设**: `claude-code` · **写入的请求头**: `user-agent: claude-cli/2.0.0 (external, cli)`、`anthropic-client: claude-code/2.0.0`、`x-app: cli`、`x-stainless-*` 系列
- **预设**: `codex` · **写入的请求头**: `user-agent: codex-tui/0.145.0 (...)`、`openai-client: codex/0.48.0`、`x-stainless-*` 系列
- **预设**: `custom` · **写入的请求头**: 任意（通过 `headersJson` 传入）
...
### 文件 · 用途
- **文件**: `index.js` · **用途**: 安装模式 Host 插件（cordis 主入口：`name` + `apply`）
- **文件**: `client.js` · **用途**: 安装模式 Web 客户端 bundle（`__ModuleLoader__` 格式，设置页）
- **文件**: `cordis.patch.yml` · **用途**: bundle patch：`dsh plugin add` 后自动挂载的插件行
- **文件**: `host.body.js` / `client.body.js` · **用途**: 动态插件模式的 paste-ready 代码体
- **文件**: `plugin.js` · **用途**: 动态代码体编程加载器（剥离注释头）
