...
### 现象 · 处理
- **现象**: 手机一直停在配对界面 · **处理**: 读取 `%USERPROFILE%\.dsh\remfs-pairing.txt`;配对码 10 分钟过期——重启 harness 生成新的
- **现象**: 设备被吊销/重新配对失败 · **处理**: 配对码一次性;重启 harness 获取新码
- **现象**: 手机 403 · **处理**: 用打印的 HTTPS/Tailscale/LAN 地址;GUI 需把这些主机加入信任(一键部署自动完成)
- **现象**: LAN 地址不通 · **处理**: 手机需在同一 Wi-Fi;重跑启动脚本让 LAN IP 重新检测
- **现象**: `npm.ps1` 被执行策略拦截 · **处理**: 用 `npm.cmd`,或 `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned`
- **现象**: 发布后 `npm view` 404 · **处理**: CDN 缓存——等一分钟或用 `Cache-Control: no-cache`
- **现象**: `dsh plugin add` 后插件没出现 · **处理**: 必须补 loader 行并重启 `dsh web`
- **现象**: 电脑睡眠 · **处理**: 部署已处理 keep_awake + 电源计划,见 `keep_awake.ps1`
