...
### 平台 · npm 包 · 源码位置 · 安装
- **平台**: pi CLI · **npm 包**: `pi-working-activity` · **源码位置**: [`extensions/`](extensions/index.ts) · **安装**: `pi install npm:pi-working-activity`
- **平台**: DeepSeek Harness · **npm 包**: `dsh-working-activity` · **源码位置**: [`packages/activity/working-activity/`](packages/activity/working-activity/) · **安装**: `dsh plugin --profile  add dsh-working-activity`
...
### 字段 · 来源
- **字段**: 💰 成本 · **来源**: Pi 官方 `usage.cost.total`（含缓存折扣、阶梯价）
- **字段**: 🔥 token · **来源**: input + output + cacheRead + cacheWrite
- **字段**: 缓存命中 · **来源**: cacheRead / (input + cacheRead)
- **字段**: 🧠 思考 · **来源**: reasoning token（output 的子集，不重复计费）
...
### 键 · 类型 · 默认值 · 说明
- **键**: `frames` · **类型**: `string` · **默认值**: `"moon"` · **说明**: 动画预设名，`"random"` 每轮随机
- **键**: `narrate` · **类型**: `boolean` · **默认值**: `true` · **说明**: ⏵ 模型自述开关（默认开，显式 `false` 关闭）
- **键**: `contextWarnAt` · **类型**: `number` · **默认值**: `80` · **说明**: 上下文预警阈值（百分比），`0` 关闭
- **键**: `contextDangerAt` · **类型**: `number` · **默认值**: `95` · **说明**: 上下文危险阈值，超过后变红
- **键**: `showTokPerSec` · **类型**: `boolean` · **默认值**: `false` · **说明**: 流式输出时显示 `~tok/s` 估算速率
- **键**: `workRemindAt` · **类型**: `number` · **默认值**: `3` · **说明**: 累计活跃 N 小时提醒喝水，`0` 关闭
- **键**: `customPhrases` · **类型**: `string[]` · **默认值**: `[]` · **说明**: 追加到思考文案池的自定义短句
- **键**: `customActions` · **类型**: `object` · **默认值**: — · **说明**: 自定义工具→文案映射，如 `{"my_tool": ["搞一下","整一个"]}`
- **键**: `debugLog` · **类型**: `boolean` · **默认值**: `false` · **说明**: 调试日志（`~/.pi/agent/working-activity-debug.log`，>512KB 自动截断）
- **键**: `mode` · **类型**: `string` · **默认值**: `"lively"` · **说明**: `"minimal"` 只保留功能性信息
- **键**: `features` · **类型**: `object` · **默认值**: — · **说明**: 单特性开关，如 `{"rareEggs": false}`，覆盖 mode 默认

## 命令

### 命令 · 说明
- **命令**: `/activity settings` · **说明**: 打开可搜索设置面板，实时预览动画和文案
- **命令**: `/activity doctor` · **说明**: 检查配置、主题、预设和持久化权限
- **命令**: `/activity` · **说明**: 打开动画预设交互选择器
- **命令**: `/activity frames <name>` · **说明**: 直接切换预设，如 `/activity frames claude`
- **命令**: `/activity frames random` · **说明**: 每轮随机一个预设
- **命令**: `/activity narrate on\ · **说明**: off` · 开关模型自述
- **命令**: `/activity mode lively\ · **说明**: minimal` · 总开关：花哨 / 极简
- **命令**: `/activity feature` · **说明**: 列出所有特性开关状态
- **命令**: `/activity feature <名> on\ · **说明**: off\ · auto` · 单特性开关，auto 恢复跟随模式
- **命令**: `/activity status` · **说明**: 显示当前所有配置
- **命令**: `/activity warn <0-100>` · **说明**: 修改上下文预警阈值，0=关闭
- **命令**: `/activity danger <n>` · **说明**: 修改红色危险阈值，必须不低于 warn 阈值
- **命令**: `/activity tps on\ · **说明**: off` · 开关流式 `~tok/s` 估算显示
- **命令**: `/activity remind <0-24>` · **说明**: 设置累计活跃提醒间隔，0=关闭
- **命令**: `/activity phrase add <文案>` · **说明**: 追加自定义思考短语
- **命令**: `/activity phrase list` · **说明**: 列出所有自定义短语
- **命令**: `/activity stats` · **说明**: 本轮+会话统计：工具数、想/干比、💰成本、🔥token、缓存命中、思考 token
- **命令**: `/activity config export` · **说明**: 导出当前配置 JSON 到当前目录 `working-activity.export.json`
- **命令**: `/activity config import <路径>` · **说明**: 从文件导入配置并立即生效（自动校验）
