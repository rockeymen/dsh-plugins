...
不重做聊天界面：Electron 只负责窗口、托盘、工作区、API Key 和启动编排，对话、工具调用、审批还是官方 `dsh web`。第三方思考强度、识图模型兜底、主题和背景图这类地方补了一点。深度 GUI 爱好者，欢迎各种需求、建议和 PR。
  ...
  ![Deepseek-Harness-Desktop](assets/screenshot-home.png)
...
设置 → 模型 → 编辑自定义提供方，给模型勾上思考强度。保存后，输入栏的模型菜单会出现「推理等级」。
  ...
  ![设置里为第三方模型勾选思考强度](assets/screenshot-thinking-settings.png)

  ![输入栏选择推理等级](assets/screenshot-thinking-chat.png)

### 识图模型

设置 → 模型 → 识图模型，选一个支持图片输入的模型。主模型不能识图时，会先调用它识别图片内容，再把描述交给主模型。

  ![设置里配置识图模型](assets/screenshot-vision-settings.png)

  ![主模型不能识图时由识图模型先看图](assets/screenshot-vision-chat.png)
...
背景图铺在整个界面后面，设好之后可以调毛玻璃、像素化和玻璃透明度：数值越低，侧栏、对话框和输入框越通透。
  ...
  ![外观页的主题库：内置主题和自定义主题](assets/screenshot-theme-library.png)

  ![背景图、毛玻璃、像素化和玻璃透明度](assets/screenshot-theme-wallpaper-settings.png)

  ![铺了背景图之后的对话界面](assets/screenshot-theme-wallpaper-chat.png)
...
### 操作 · 方式
- **操作**: 设置 · **方式**: `Ctrl+,` 或托盘菜单
- **操作**: 插件市场 · **方式**: 设置 → 插件 →「插件市场」；`Ctrl+Shift+M`、托盘或标题栏也会打开这一页
- **操作**: 重启 Harness · **方式**: `Ctrl+Shift+R`
- **操作**: 重新加载界面 · **方式**: `Ctrl+R`
- **操作**: 开发者工具 · **方式**: `Ctrl+Shift+I`
- **操作**: 关闭窗口 · **方式**: 默认最小化到托盘（可在设置里改）
