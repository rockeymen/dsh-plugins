...
### 标签 · 形态 · 说明
- **标签**: **bundle** · **形态**: 目录：`skills\<name>\SKILL.md` · **说明**: 一个目录，正文在 `SKILL.md`，可附带脚本/模板/资源等**多文件**（如 docx 的 scripts/、guizang-ppt-skill 的 assets/）。适合功能复杂的 skill。
- **标签**: **flat** · **形态**: 单文件：`skills\<name>.md` · **说明**: 整个 skill 就是**一个 Markdown 文件**。适合纯指令型 skill。
...
### 方法 · 路径 · 说明
- **方法**: GET · **路径**: `/api/skill-manager-ytxue/state` · **说明**: 双目录状态快照（含 description 与 checked 红绿点状态）
- **方法**: POST · **路径**: `/api/skill-manager-ytxue/check` · **说明**: 仅检查未检查项并修复，返回 `{checked, fixed, skipped}`
- **方法**: POST · **路径**: `/api/skill-manager-ytxue/enable` · **说明**: body `{name}`
- **方法**: POST · **路径**: `/api/skill-manager-ytxue/disable` · **说明**: body `{name}`
- **方法**: GET · **路径**: `/api/skill-manager-ytxue/list-dir` · **说明**: 目录浏览（选择器用），`?path=` 缺省为用户主目录
- **方法**: POST · **路径**: `/api/skill-manager-ytxue/import` · **说明**: body `{source, target?, conflict?, dryRun?}`；dryRun 预检返回 `{kind, pending, conflicts, failed}`；执行返回 `{kind, imported, skipped, failed}`
