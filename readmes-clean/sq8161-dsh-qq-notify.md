...
### 变量 · 说明
- **变量**: `{workspace}` · **说明**: 会话工作区完整路径
- **变量**: `{project}` · **说明**: 工作区目录名（项目名）
- **变量**: `{time}` · **说明**: 对话结束时间（本地时间 YYYY-MM-DD HH:mm:ss）
- **变量**: `{request}` · **说明**: 本次对话最近一条用户输入（截断至 200 字）
- **变量**: `{result}` · **说明**: 最近一条助手回复（截断至 300 字）
- **变量**: `{model}` · **说明**: 当前使用的模型
- **变量**: `{provider}` · **说明**: 当前使用的模型提供商
- **变量**: `{sessionTitle}` · **说明**: 当前会话标题
- **变量**: `{sessionId}` · **说明**: 会话 ID
- **变量**: `{turn}` · **说明**: 结束的回合序号
...
- 非敏感配置（预设、AppID、UserOpenID、开关）保存在插件安装目录：`/node_modules/dsh-qq-notify/.dsh-qq-notify/dsh_qq_notify_config.json`（与工作区无关，重装插件后请手动迁移）
