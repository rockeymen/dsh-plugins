...
### Key · Default · Meaning
- **Key**: `baseURL` · **Default**: `$SEARXNG_BASE_URL` · **Meaning**: Base URL of the instance, e.g. `http://127.0.0.1:8080`. Must be http(s) and have `json` in `search.formats`.
- **Key**: `language` · **Default**: none · **Meaning**: Locale passed as SearXNG's `language` parameter, e.g. `zh-CN`, `en-US`.
- **Key**: `engines` · **Default**: none · **Meaning**: Comma-separated engine allowlist, e.g. `bing,duckduckgo`.
- **Key**: `categories` · **Default**: none · **Meaning**: Comma-separated category filter, e.g. `general,it`.
- **Key**: `authHeader` · **Default**: none · **Meaning**: Authorization header value, for instances fronted by an API-key gate. Sent verbatim.
...
- **Plugin version**: 0.1.0 · **`@deepseek-ai/dsh-web`**: `>=0.1.0-rc.1 <0.2.0` (tested against `0.1.0-rc.6`) · **Notes**: Initial release
