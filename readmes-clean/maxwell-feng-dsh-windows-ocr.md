...
### Key · Default · Meaning
- **Key**: `language` · **Default**: `""` · **Meaning**: BCP-47 tag for Windows OCR, e.g. `zh-Hans`, `en-US`. Empty = user profile languages.
- **Key**: `passthrough` · **Default**: `true` · **Meaning**: `true`: genuine vision models receive images untouched; `false`: OCR everything.
- **Key**: `ocrScript` · **Default**: bundled `lib/ocr.ps1` · **Meaning**: Absolute path override for the PowerShell OCR script.
- **Key**: `timeoutMs` · **Default**: `60000` · **Meaning**: Per-image OCR timeout.
- **Key**: `maxCacheEntries` · **Default**: `200` · **Meaning**: Bound on the per-run OCR cache (keyed by attachment id).
