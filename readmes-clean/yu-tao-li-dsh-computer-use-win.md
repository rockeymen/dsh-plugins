...
- **Window-cropped capture + UIA tree (`snapshot`)**: ![screenshot 1](assets/screenshot-1.png) · **OCR fallback (CJK UI, word boxes in screen coordinates)**: ![screenshot 2](assets/screenshot-2.png)
...
### Group · Tools
- **Group**: Observe · **Tools**: `health` · `snapshot` (tree + screenshot) · `accessibility_tree` · `list_windows` · `find` · `element_info` · `ocr`
- **Group**: Act · **Tools**: `click` · `double_click` · `move` · `drag` · `scroll` · `type_text` (clipboard / sendinput / background) · `keypress` · `focus` · `invoke` · `set_value`
- **Group**: Window · **Tools**: `activate_window` · `move_window` · `close_window` (WM_CLOSE; the app may veto with a save dialog) · `wait_for` · `wait`
