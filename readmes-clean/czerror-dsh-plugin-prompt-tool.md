...
### 项目 · 关系 · 复用内容
- **项目**: [dsh-anchored-standard](https://github.com/xiaobright/dsh-anchored-standard) · **关系**: **集成（上游，跟踪 main）** · **复用内容**: 加载时直引子模块 `vendor/dsh-anchored-standard/preset/` 的 `agent.cordis.yml` 与全部 `*.mjs` 模块（更新即生效），Minimal 真实 schema 首轮锚定机制
- **项目**: [dsh-router-standard](https://github.com/yjh051108/dsh-router-standard) · **关系**: 参考 · **复用内容**: 复杂度启发式正则、近距离注入原则、持久事件推导状态（resume 安全）
- **项目**: [dsh-super-injector](https://github.com/yjh051108/dsh-super-injector) · **关系**: 参考 · **复用内容**: 缓存铁律（静态进 system 头、动态走消息尾）、首轮锚定铁律、开发工具链（dev_* 注入/热重载）
- **项目**: [dsh 破限者（1449690477/dsh）](https://github.com/1449690477/dsh) · **关系**: 姊妹项目 · **复用内容**: `skills/` 技能目录（sandboxmod/SKILL.md）与之同源，常驻层 AGENTS.md 机制一致
...
### 断言 · 结果
- **断言**: turn1 reasoning 首词 we · **结果**: **5/5**
- **断言**: 首请求工具 · **结果**: [bash, str_replace_editor]（5/5）
- **断言**: 首请求 maxTokens · **结果**: 256000（无 cap，5/5）
- **断言**: 首请求前注入消息 · **结果**: 纯净（仅 user，5/5）
- **断言**: preset.md 注入 · **结果**: 恰好一次，we 确认后同 turn 注入（5/5）
- **断言**: 晋升后目录 · **结果**: resident 集：bash + str_replace_editor + dev_tool_search / skill_search / skill_load + 已解锁工具（上游 v2.2 起）
