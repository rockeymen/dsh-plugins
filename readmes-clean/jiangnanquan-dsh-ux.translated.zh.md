...
### 部分 · 内容
- **部分**: **dsh-enhance**(仓库根目录) · **内容**: DSH web 插件:主题、布局、折叠胶囊、账户用量
- **部分**: **dsh-desktop**(`desktop/` 子目录) · **内容**: 无边框 Electron 桌面壳,双击即用
...
### 类别 · 内容
- **类别**: 主题 · **内容**: Solarized 浅色配色(深色保留官方默认)+ Maple Mono 字体栈(检测到才启用)
- **类别**: 布局 · **内容**: 加宽消息列、紧凑间距、收窄输入队列、禁用橡皮筋滚动
- **类别**: 折叠胶囊 · **内容**: 思考块聚合为 `Think ×N` 胶囊、工具调用聚合为 `A → B → C` 链路胶囊,点击展开,最大限度压缩对话行数
- **类别**: 账户用量 · **内容**: 底部显示官方账户余额、本轮 token 成本、近 30 天用量图表
