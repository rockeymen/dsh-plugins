...
### Approach · Result
- **Approach**: Generic tunnel (SSH port forward, Caddy, binding `0.0.0.0`) · **Result**: Page loads; `settings.*` / `credentials.*` / `host.listDirectory` return 403
- **Approach**: LAN-only plugin without authentication · **Result**: Usable on the local network; not suitable for public exposure
- **Approach**: Password prompt without header rewriting · **Result**: Requests are authenticated, but the privileged APIs remain blocked
...
```mermaid
flowchart LR
    A[Phone or remote browser] --> B[Public tunnelcloudflared / ngrok / frp / SSH]
    B --> C[dsh-full-remote127.0.0.1:3081authentication + header rewrite]
...
### Bind · Purpose
- **Bind**: `127.0.0.1` (default) · **Purpose**: The tunnel runs on the same machine
- **Bind**: `192.168.x.x` · **Purpose**: A device on the same network, without a tunnel
- **Bind**: `0.0.0.0` / `::` · **Purpose**: Bind every interface. This is not an address to open; the panel reports a separate reachable address.
