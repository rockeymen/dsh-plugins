...
- 选择 `.mpkg` 文件 → 浏览器内解析容器 → 自动提取 `preview.gif`（壁纸的动态预览）作为**动态背景**（用 `` 元素承载，GIF 动画可靠循环播放）；视频类壁纸自动播放内嵌 mp4
...
### 控件 · 说明
- **控件**: 选择 .mpkg 文件 · **说明**: 自动取 preview.gif（或按时间取素材）作动态背景；也可直接选 mp4/webm
- **控件**: 可调参数 · **说明**: 壁纸自带的参数与当前值（只读展示，供对照壁纸引擎 App）
- **控件**: 图片链接 / 本地图片 · **说明**: 普通图片或 GIF
- **控件**: 面板不透明度 · **说明**: 50–100%
- **控件**: 磨砂模糊 · **说明**: 整张壁纸的模糊程度 0–40px（0=清晰）
- **控件**: 统一虚化 · **说明**: 整屏朦胧感一个条控制；聊天区/新会话按钮可独立选择是否跟随
- **控件**: 对话框/弹层/遮罩虚化 · **说明**: 三个独立开关+程度条
- **控件**: 镜头缩放/位置 · **说明**: 背景画面放大（10–2000%）与平移；缩小可看到画面边缘的组件
- **控件**: 侧边栏/标题栏透出壁纸 · **说明**: 开关；关闭后对应区域纯色不透明
- **控件**: 轻度锐化 · **说明**: 提升低清观感；GIF 卡顿就关
...
### 工具 · 用途
- **工具**: `unmpkg.py` · **用途**: mpkg 容器解析/提取（PKGM0014/0018）
- **工具**: `tex2png.py` · **用途**: TEXV0005 纹理解码（DXT5/R8 等）
- **工具**: `mdl_explorer.py` · **用途**: .mdl 结构探索（块标签/网格/浮点区段）
- **工具**: `xref.py` · **用途**: wallpaper64.exe 字符串 xref + 反汇编（capstone）
- **工具**: `MDL-格式分析笔记.md` · **用途**: .mdl 格式逆向进展（容器/网格已破解，骨骼=JSON，动画待续）
