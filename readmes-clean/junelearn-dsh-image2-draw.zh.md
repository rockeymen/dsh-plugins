...
> 和隐私政策自行选择。
>
> 新用户可使用兑换码 `99F509ABC6C38F77` 兑换赠送额度。
...
- 生成图片会直接显示在聊天界面；专用工具卡片支持缩略图、点击放大和另存为。
- 图片附件由客户端卡片解析，不会注入不支持图片输入的 DeepSeek 模型上下文。
- 图片同时保存到当前会话工作目录的 `outputs/image2/`，遇到重名时自动编号，不会
  覆盖已有文件。
...
![Image2 聊天内生成结果](./assets/image2-result-card.png)

### 插件设置页面

![Image2 插件设置页面](./assets/image2-settings.png)
...
聊天内图片附件和专用工具卡片的部分实现参考并改编自采用 MIT 许可证的
[dsh-multimodal](https://github.com/MC5lan/dsh-multimodal)。完整的第三方版权和
