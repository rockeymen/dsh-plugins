...
### 半 · 文件 · 职责
- **半**: Host · **文件**: `lib/index.js` · **职责**: 挂载 `/api/skin-switcher/state` 与 `/api/skin-switcher/use` 路由；扫描 `~/.dsh/profiles/web/node_modules` 发现皮肤；管理 boot patch 的 managed section
- **半**: Browser · **文件**: `lib/client.js` · **职责**: 注册设置面板 `settings.section`「皮肤」页；切换后轮询 state 直至配置监视器确认，再刷新页面加载新的客户端插件图
...
3. 通过 `dsh plugin --profile web add ` 安装进 profile
