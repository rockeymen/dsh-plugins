...
- Implementation · **Link (recommended)**: `skills/<name>` is a **symlink** to the source · **Copy**: full tree copy
- Sync · **Link (recommended)**: **No sync problem**: one file on both sides, editing edits the source · **Copy**: evolves independently (state records the source for reference)
- Source deleted · **Link (recommended)**: panel marks it "broken link", one-click removal (source untouched) · **Copy**: unaffected
- Fits · **Link (recommended)**: long-lived sources you maintain · **Copy**: throwaway sources (.skill packages, repos you'll delete), or a global version you can change freely
