...
aflare 目前处于 **v0.8.1 阶段**。核心 Runtime 能力（DAG 调度、WAL 崩溃恢复、Saga 事务补偿、幂等、重试/熔断）已实现并通过 CI 验证。v0.8 重点交付离线/内网首选项体验、隐私安全硬化、本地 LLM 丝滑接入与 CLI 体验优化；v0.8.1 为发布审计修复（国内安装 404、`aflare mcp` 入口、安全漏洞清零）。国产芯片（昇腾/寒武纪/海光）适配为基础支持，持续完善中。硬件设备控制（机器人等）不在内置范围——用户可通过自定义节点或 MCP Server 自行接入，数据不出内网。
...
### 功能 · 状态 · 验证状态
- **功能**: **ReAct Agent 对话** (`aflare chat`) · **状态**: ✅ · **验证状态**: 有测试
- **功能**: **守护进程式 Agent** (`aflare agent`) · **状态**: ✅ · **验证状态**: 有测试
- **功能**: **300+ 技能模板**（16 个领域） · **状态**: ✅ · **验证状态**: 有测试
- **功能**: **7 类可插拔能力**（反思/HITL/效用驱动等） · **状态**: ✅ · **验证状态**: 有测试
- **功能**: **多源输入融合**（stdin + 定时任务 + 文件监听） · **状态**: ✅ · **验证状态**: 有测试
- **功能**: DAG 并行调度 · **状态**: ✅ · **验证状态**: 有测试 + TLA+ 形式化验证
- **功能**: WAL 崩溃恢复 + Session 持久化 · **状态**: ✅ · **验证状态**: 有测试
- **功能**: Saga 事务补偿 · **状态**: ✅ · **验证状态**: 有测试
- **功能**: Idempotency（幂等） · **状态**: ✅ · **验证状态**: 有测试
- **功能**: Retry / Rate Limit / Circuit Breaker · **状态**: ✅ · **验证状态**: 有测试
- **功能**: HMAC 审计链 · **状态**: ✅ · **验证状态**: 有测试
- **功能**: Secret 脱敏 · **状态**: ✅ · **验证状态**: 有测试
- **功能**: 表达式引擎（字节码 IR + 向量化） · **状态**: ✅ · **验证状态**: 有测试
- **功能**: 关键词匹配生成工作流 · **状态**: ✅ · **验证状态**: 有测试
- **功能**: MCP 协议支持（Server/Client） · **状态**: ✅ · **验证状态**: 有测试
- **功能**: LLM 节点（22+ 模型） · **状态**: ✅ · **验证状态**: 有测试
- **功能**: 安全等级（L0-L3） · **状态**: ✅ · **验证状态**: 有测试
...
### 能力 · 类型 · 说明
- **能力**: `reflection` · **类型**: 反思/自我批评 · **说明**: 每轮执行后自动评估输出质量，触发自我修正
- **能力**: `human-in-loop` · **类型**: 人机协同 · **说明**: 关键操作暂停，请求人类确认后继续
- **能力**: `utility` · **类型**: 效用驱动 · **说明**: 6 维度评分（正确性/完整性/效率/安全/清晰/可操作），优化决策
- **能力**: `adaptive` · **类型**: 学习型/自适应 · **说明**: 从反馈中学习，跨轮次改进表现
- **能力**: `memory` · **类型**: 有状态 · **说明**: 跨会话长期记忆，记住用户偏好
- **能力**: `planning` · **类型**: 规划式 · **说明**: 行动前生成计划，逐步执行
- **能力**: `workflow` · **类型**: 工作流/管道式 · **说明**: 优先使用已有模板，稳定可预测
...
### 版本 · 状态 · 重点
- **版本**: v0.6 · **状态**: 已完成 · **重点**: Agent 记忆基础设施、语音 AI 工具链、WAL 持久化、TLA+ 验证
- **版本**: v0.7 · **状态**: 已完成 · **重点**: 金融场景增强（Saga / 幂等 / 审计链）、ReAct Agent 对话、300+ 技能模板、7 类可插拔能力、Agent 统一事件循环
- **版本**: v0.8 · **状态**: 已完成 · **重点**: 离线/内网首选项体验、隐私安全硬化、本地 LLM 丝滑接入、CLI 体验优化（template run / 智能命令提示）、CI 提速
- **版本**: **v0.8.1** · **状态**: **已完成** · **重点**: 发布审计修复：国内安装 404、`aflare mcp` 子命令、execute 白名单错误定位、govulncheck 漏洞清零
- **版本**: v0.9 · **状态**: 计划中 · **重点**: 国产芯片适配完善、Agent 能力深化
- **版本**: v1.0 · **状态**: 计划中 · **重点**: 稳定 API、LTS
...
### 等级 · 说明
- **等级**: **L0** · **说明**: 宽松：允许所有节点，沙箱降级时仅警告
- **等级**: **L1** · **说明**: 标准：沙箱降级时警告，启发式拦截
- **等级**: **L2** · **说明**: 严格：无 bwrap 沙箱时拒绝执行 code_interpreter，命令白名单校验
- **等级**: **L3** · **说明**: 极严：禁用 code_interpreter 节点，最大安全策略
