...
### 依赖 · 版本 · 用途
- **依赖**: [Node.js](https://nodejs.org/) · **版本**: ≥ 18 · **用途**: 运行数据同步脚本（可选）
- **依赖**: Python 3 · **版本**: 任意 · **用途**: 本地起一个静态服务器（可选，也可用 `npx serve` 等）
...
### 通道 · 平台 · 访问地址 · 更新方式
- **通道**: 🌍 国际 · **平台**: GitHub Pages · **访问地址**: `https://aust24lzy.github.io/dsh-plugin-hub/` · **更新方式**: 每小时自动
- **通道**: 🇨🇳 国内 · **平台**: CloudStudio · **访问地址**: `https://1d6da375127842fe97d4a7e3e98c670b.app.workbuddy.link/` · **更新方式**: 手动部署快照
...
### 密钥名 · 说明
- **密钥名**: `TENCENT_SECRET_ID` · **说明**: 腾讯云 API 密钥 SecretId
- **密钥名**: `TENCENT_SECRET_KEY` · **说明**: 腾讯云 API 密钥 SecretKey
- **密钥名**: `COS_BUCKET` · **说明**: COS 存储桶名称（`mfnx-1469339292`）
- **密钥名**: `COS_REGION` · **说明**: 存储桶地域（`ap-guangzhou`）
...
### 方向 · 说明 · 涉及文件
- **方向**: 分类规则 · **说明**: 优化 `dsh-plugin` 生态的分类关键词与命中精度 · **涉及文件**: `scripts/sync.mjs`
- **方向**: 前端交互 · **说明**: 新功能、性能优化、无障碍、主题适配 · **涉及文件**: `web/app.js`、`web/styles.css`
- **方向**: 智能助手 · **说明**: 意图识别、约束抽取、推荐评分逻辑 · **涉及文件**: `web/chat-engine.js`
- **方向**: 文档 · **说明**: 完善说明、示例、FAQ · **涉及文件**: `README.md`
- **方向**: Bug 修复 · **说明**: 同步脚本、前端逻辑 · **涉及文件**: 任意
