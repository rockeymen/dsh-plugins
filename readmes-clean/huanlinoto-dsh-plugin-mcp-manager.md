...
### 文件 · 职责
- **文件**: `src/registry.ts` · **职责**: 服务器注册表：eemeli `yaml` Document API 读写 insert 行 + `loadOverlayPatches` 校验 + serverName 唯一
- **文件**: `src/tools.ts` · **职责**: agent 面 `mcp_*` ×4 工具（defineTool，规范 JSON 输出）
- **文件**: `src/index.ts` · **职责**: Node half：`/api/mcp-manager` 路由 + `ctx.tools.schemas()` 过滤 + mcp 工具注册
- **文件**: `src/client/index.ts` · **职责**: client half：`settings.section` 注册「MCP」面板
- **文件**: `src/client/Panel.tsx` · **职责**: 面板 UI：服务器列表 + 增删改表单 + 工具浏览
...
### 路由 · 方法 · 说明
- **路由**: `/api/mcp-manager/servers` · **方法**: GET · **说明**: 列出服务器 + 工具数 + 状态
- **路由**: `/api/mcp-manager/servers` · **方法**: POST · **说明**: 新增（body: `{config}`）→ HMR 挂载
- **路由**: `/api/mcp-manager/servers/` · **方法**: PUT · **说明**: 编辑（整块替换 config）
- **路由**: `/api/mcp-manager/servers/` · **方法**: DELETE · **说明**: 删除
- **路由**: `/api/mcp-manager/tools` · **方法**: GET · **说明**: 工具浏览（`mcp__` 前缀按 serverName 分组）
- **路由**: `/api/mcp-manager/status` · **方法**: GET · **说明**: 运行态状态
