...
### 痛点 · 装上之后
- **痛点**: 模型只会说「我无法生成视频」 · **装上之后**: DSH 会先查知识库，再直接调用视频工具，产出文件路径
- **痛点**: Base URL / API Key 藏在 YAML 深处 · **装上之后**: 打开 DSH WebUI 设置页，像填外卖地址一样填完即用
- **痛点**: 分镜写得像散文 · **装上之后**: 内置技能会把提示词收紧成可生成、可复用的导演指令
- **痛点**: 生图、生视频、配音要装三四个插件 · **装上之后**: 一个插件，四类模型，按需开关
- **痛点**: 想做专业项目却没有流程约束 · **装上之后**: 原创制作手册提供闸门、检查点和降级顺序
...
### 能力 · 工具 · 配置方式 · 模型示例
- **能力**: 👁️ 视觉 · **工具**: `directorx_view_image` · **配置方式**: `openai-chat` / `mock` · **模型示例**: `gpt-4o-mini`、任意兼容 VLM
- **能力**: 🎨 图像 · **工具**: `directorx_generate_image` · **配置方式**: `openai-images` / `modelverse-tasks` / `mock` · **模型示例**: `gpt-image-1`、`doubao-seedream-*`
- **能力**: 🎥 视频 · **工具**: `directorx_generate_video` · **配置方式**: `openai-videos` / `modelverse-tasks` / `kling` / `runway` / `mock` · **模型示例**: `sora-2`、`MiniMax-H3`、`kling-v2`、`gen4.5`/`gen4_turbo`/`hailuo3`
- **能力**: 🔊 音频 · **工具**: `directorx_generate_audio` · **配置方式**: `openai-tts` / `mock` · **模型示例**: `gpt-4o-mini-tts`、任意兼容 TTS
...
### 工具 · 一句话说明 · 典型用法
- **工具**: `directorx_view_image` · **一句话说明**: 给 DSH 装上眼睛 · **典型用法**: 读截图、读剧照、检查生成图有没有崩脸
- **工具**: `directorx_generate_image` · **一句话说明**: 生成关键帧/参考图 · **典型用法**: 角色定妆照、场景概念图、首尾帧
- **工具**: `directorx_generate_video` · **一句话说明**: 生成视频片段 · **典型用法**: 图生视频、首尾帧转场、动作镜头
- **工具**: `directorx_generate_audio` · **一句话说明**: 生成旁白/音频 · **典型用法**: 广告口播、短剧对白、音效底子
- **工具**: `directorx_knowledge_search` · **一句话说明**: 搜片场百科 · **典型用法**: 查运镜术语、模型参数、平台规格
- **工具**: `directorx_video_process` · **一句话说明**: 确定性剪辑：裁剪/变速/规格/音量 · **典型用法**: 统一素材规格、抠节奏片段
- **工具**: `directorx_video_concat` · **一句话说明**: 多片段拼接（硬切/交叉淡化） · **典型用法**: 多镜头成片、蒙太奇
- **工具**: `directorx_audio_mix` · **一句话说明**: 多轨混音 + 旁白闪避 · **典型用法**: BGM + 旁白 + 音效合成
- **工具**: `directorx_video_subtitle` · **一句话说明**: 软字幕轨 / 硬烧字幕 · **典型用法**: 成片加字幕（转写 srt 直用）
- **工具**: `directorx_video_zoom` / `_pip` · **一句话说明**: Ken Burns 运镜 / 画中画 · **典型用法**: 推拉镜头、贴纸角标
- **工具**: `directorx_audio_beat` · **一句话说明**: 节拍检测（ebur128） · **典型用法**: 卡点混剪的切点依据
- **工具**: `directorx_style` · **一句话说明**: 风格/镜头语言注入（知识库实文） · **典型用法**: 锁定黑色电影/赛博朋克等质感
- **工具**: `directorx_preflight` · **一句话说明**: 生成前四道闸门审计 · **典型用法**: 批量生成前把关
- **工具**: `directorx_propose` / `_proposals` / `_proposal_update` · **一句话说明**: 占位提案状态机 · **典型用法**: 先方案后生成，批准才花钱
- **工具**: `directorx_canvas_*`（14 个） · **一句话说明**: 画布完全掌控 · **典型用法**: 分镜板上画布、连线、分组、布局
