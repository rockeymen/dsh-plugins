...
### 字段 · 默认值 · 说明
- **字段**: `enabled` · **默认值**: `true` · **说明**: 总开关
- **字段**: `providerId` · **默认值**: `deepseek-official` · **说明**: 被统计为 DeepSeek 的 provider 路由
- **字段**: `balanceRefreshMinutes` · **默认值**: `10` · **说明**: 余额刷新间隔（分钟）
- **字段**: `pricingSchedules` · **默认值**: 内置两套（见下） · **说明**: 分时段价格计划（time-aware pricing，优先于 `prices`；未配置时使用内置 legacy + 2026-08-17 官方价）
- **字段**: `prices` · **默认值**: — · **说明**: 旧版分模型价格表（legacy）。**仅真正自定义的 `prices` 会覆盖默认分时定价引擎**：旧版 0.1.0 设置系统持久化的内置默认表（结构比对、与行序无关）会被识别为隐式默认，自动切换到 `DEFAULT_SCHEDULES`，升级用户无需手动删除 `prices`
...
- **DeepSeek wire 字段**: `prompt_cache_hit_tokens` 或 `prompt_tokens_details.cached_tokens`（适配器优先取后者） · **harness `TokenUsage`（适配器转换）**: `cacheReadTokens` · **仪表盘桶**: `cacheHitInputTokens`
- **DeepSeek wire 字段**: `prompt_tokens - cacheRead`（不相交；适配器丢弃原生 `prompt_cache_miss_tokens`） · **harness `TokenUsage`（适配器转换）**: `inputTokens` · **仪表盘桶**: `cacheMissInputTokens`
- **DeepSeek wire 字段**: `completion_tokens` · **harness `TokenUsage`（适配器转换）**: `outputTokens` · **仪表盘桶**: `outputTokens`
- **DeepSeek wire 字段**: `completion_tokens_details.reasoning_tokens` · **harness `TokenUsage`（适配器转换）**: `reasoningTokens` · **仪表盘桶**: `reasoningTokens`
- **DeepSeek wire 字段**: （DeepSeek 不上报） · **harness `TokenUsage`（适配器转换）**: `cacheWriteTokens`（缺省） · **仪表盘桶**: 计 0
