...
DSH Web 选中批注插件：选文字 → 批注 → 回车随消息发给模型，回复按批注编号逐条对照
...
### 功能 · 说明
- **功能**: 选中即批注 · **说明**: 选中助手文字 → 工具条「批注」→ 直接写批注（可留空），点选外处或 Esc 收起
- **功能**: 编号脚标 + 高亮 · **说明**: 原文位置亮蓝编号 + 高亮，视口内锚定、碰撞避让，滚出屏幕不丢失
- **功能**: 跨回合收集 · **说明**: 任意多条批注跨消息/回合累积，编号从 1 开始
- **功能**: 「批注 ×N」标签 · **说明**: 输入框旁小标签，悬浮显示全部批注内容，可逐条删除
- **功能**: 回车随消息发送 · **说明**: 批注块 + 输入框问题一起发给模型（模型收到完整内容）
- **功能**: 气泡隐藏批注块 · **说明**: 发送瞬间（浏览器绘制前）批注块从气泡 DOM 隐藏，只留问题 + 「批注 ×N」标签，hover 可见内容；刷新后历史消息自动修复
- **功能**: 回复逐条对照 · **说明**: 消息内注入格式指令，模型按「Annotation 1：…」…「Annotation N：…」逐条回应
- **功能**: 回复批注芯片 · **说明**: 回复里的「Annotation N：」渲染为可悬浮芯片，hover 显示该批注的原文 + 批注内容
...
### 做 · 不做
- **做**: 只 `dsh plugin add` / 只写 `bundles` · **不做**: **不要**再在 profile/home `cordis.patch.yml` insert 同 id
...
### 版本 · 内容
- **版本**: v1.3.x · **内容**: 回复逐条对照：格式指令注入 + 「Annotation N：」可悬浮芯片（TreeWalker 快照修复）
- **版本**: v1.2.x · **内容**: 气泡隐藏批注块：MutationObserver 微任务零闪烁 + 轮询兜底 + 历史消息修复
- **版本**: v1.x · **内容**: 自包含批注流（取代 v0.9 chip 设计）：capture Enter 拼稿随消息发送
- **版本**: v0.9.x · **内容**: 早期 chip 设计（insertReference + slash codec），已被 v1.x 取代
