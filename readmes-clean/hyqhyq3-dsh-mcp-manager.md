...
> The MCP server's OAuth provider must allow a loopback redirect (`http://127.0.0.1:/mcp-manager/callback/`), which is where the DSH GUI webserver receives the code. The origin is derived from your browser's own address, so any host/port the GUI is served on works.
