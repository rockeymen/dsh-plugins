...
- **会话头部按钮(撤销/恢复/快照)**: ![1](docs/1.png) · **快照管理面板(差异/回退/删除/清理/导出/导入)**: ![2](docs/2.png)

### 设置:快捷键与快照参数 · 局外程序窗口 v2(保存/撤销/恢复/回退/删除/清理/导出/导入/设置)
- **设置:快捷键与快照参数**: ![3](docs/3.png) · **局外程序窗口 v2(保存/撤销/恢复/回退/删除/清理/导出/导入/设置)**: ![4](docs/4.png)

### 一键导出全部快照(可备份/迁移) · 局外窗口:双击快照行查看差异 · 局外窗口:最小化到系统托盘
- **一键导出全部快照(可备份/迁移)**: ![5](docs/5.png) · **局外窗口:双击快照行查看差异**: ![6](docs/6.png) · **局外窗口:最小化到系统托盘**: ![7](docs/7.png)

## 功能一览

### 入口 · 能力
- **入口**: **WebUI 头部按钮** · **能力**: 「撤销」(红)「恢复」(绿)「快照」三个按钮,所有会话可见
- **入口**: **快照管理面板** · **能力**: 可视化列表(时间/类型/原因/文件数/所在库)、**每行差异预览**(增删行高亮)、**回退到任意快照版本**(回退前确认框显示将修改的文件摘要)、**删除快照**、手动保存、清理过期、刷新
- **入口**: **键盘快捷键** · **能力**: 默认 **Ctrl+Alt+Z**(撤销)/ **Ctrl+Alt+Y**(恢复),设置 → 通用 可自定义
- **入口**: **双模式保存** · **能力**: **手动保存**(主动存档,永不被自动清理)+ **自动保存**(配置变化 1.5 秒防抖自动存档,保留最近 20 份);**两种模式存储位置不同**
- **入口**: **可配置参数** · **能力**: 设置 → 通用 →「快照设置」:自动保存开关、防抖毫秒、自动档保留数量、**后悔档保留数量、自动清理开关**、手动快照目录、自动快照目录,保存即生效
- **入口**: **自动清理** · **能力**: 超量自动删除:自动档保留最近 N 份、后悔档保留最近 M 份(已消费的优先清),手动存档永不清除;可设置关闭(关闭后全部保留),面板可手动「清理过期」
- **入口**: **对话指令** · **能力**: 对 AI 说"撤销上一步 / 回退 / 恢复 / 保存快照 / 查看快照",自动调用工具完成;配置变更时 AI 会主动提示"已自动存档,随时可撤销"
- **入口**: **撤销/重做栈** · **能力**: 连续多步撤销;撤销后可重做;有新变更则阻止重做;每次撤销前先把当前状态存为"后悔档"
- **入口**: **启动异常自检** · **能力**: 上次 DSH 崩溃/被杀未正常启动时,启动后自动检测并在快照列表与面板顶部警告,引导你回退到上次正常状态
- **入口**: **导出/导入** · **能力**: 一键把全部快照打包为 ZIP(备份/换机迁移);导入自动按类型分流、重复跳过不覆盖;面板按钮 / 对话工具 / 离线 CLI 均可
- **入口**: **局外工具** · **能力**: CLI(`snapshot/undo/redo/restore/remove/list/diff/prune/export/import/status`)+ **程序窗口 v2**(启动异常自检横幅与一键回退、导出/导入、双击行差异预览、清理过期、设置面板、系统托盘常驻)+ 安全装插件包装器
...
### 库 · 默认路径(可在设置中修改) · 内容
- **库**: 手动库 · **默认路径(可在设置中修改)**: `<快照根>\manual\` · **内容**: 手动保存的快照(永不自动清理)
- **库**: 自动库 · **默认路径(可在设置中修改)**: `<快照根>\auto\` · **内容**: 自动快照、启动基线、撤销后悔档(自动档保留最近 20 份)
- **库**: 旧库(兼容) · **默认路径(可在设置中修改)**: `<快照根>\` 根 · **内容**: 旧版扁平布局,读取兼容,启动时自动迁移到新库
...
- **回退到指定版本**:面板里点快照行「回退到此版本」;或对 AI 说"回退到 ";或 CLI `restore -Id `。
- **删除快照**:面板里点「删除」;或 CLI `remove -Id `。
...
powershell -NoProfile -ExecutionPolicy Bypass -File "tools\dsh-undo-savepoint.ps1" restore -Id  -Force
powershell -NoProfile -ExecutionPolicy Bypass -File "tools\dsh-undo-savepoint.ps1" remove -Id 
...
### 端点 · 说明
- **端点**: `GET /api/undo/status` · **说明**: `{canUndo, canRedo, total}`
- **端点**: `GET /api/undo/list` · **说明**: 快照列表(含 location: manual/auto/legacy)
- **端点**: `GET/POST /api/undo/settings` · **说明**: 读/写保存参数(自动保存、防抖、保留数、目录),POST 即时生效
- **端点**: `POST /api/undo/undo` · **说明**: 撤销上一步
- **端点**: `POST /api/undo/redo` · **说明**: 恢复
- **端点**: `POST /api/undo/restore` · **说明**: body `{id}` 回退到指定版本
- **端点**: `POST /api/undo/remove` · **说明**: body `{id}` 删除快照
- **端点**: `POST /api/undo/snapshot` · **说明**: body `{reason}` 手动保存
- **端点**: `POST /api/undo/pick-dir` · **说明**: 弹出系统目录选择器,返回选中路径
