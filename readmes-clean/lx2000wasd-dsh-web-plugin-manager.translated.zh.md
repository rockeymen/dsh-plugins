...
>
> 若已存在未经验证的插件导致 profile 无法启动：先用 `dsh --profile <name> --patch ` 等方式排查，或手动清理对应依赖/`cordis.patch.yml` 行后，再安装管理器接管。
...
```sh
dshpm install  --profile <name>   # npm 名 / github:user/repo / git URL / tarball / 本地路径
...
### 能力 · 说明
- **能力**: 查看 · **说明**: 合并展示层栈/依赖/挂载行/运行条目；手动安装未挂载的依赖显示「未挂载」并可一键挂载
- **能力**: 实时启停 · **说明**: managed 块编辑 patch，经 loader 直接应用，实时生效、重启后持久
- **能力**: 安装 · **说明**: 官方 dsh plugin CLI + 质量门 + 自动回滚；非 bundle 自动写挂载行并实时加载；git 源自动 clone、已发布 npm 优先
- **能力**: 更新 · **说明**: 检查更新（npm dist-tag / git HEAD / 安装 commit），更新带质量门与回滚
- **能力**: 健康检查 · **说明**: 依赖图/缺失/循环/重复行 id/同名注册冲突（服务/工具/section/路由）/peer 版本/官方包重复；运行中追加 pending 与失败诊断
- **能力**: 环境管理 · **说明**: 启停、复制/转移插件、创建/重命名/删除 profile（官方 profile 只读）
- **能力**: 市场 · **说明**: awesome-dsh-plugins 双源合并、24h 缓存、已安装徽标、15s 超时、代理支持、失败负缓存
- **能力**: agent 工具 · **说明**: plugin_status/install/uninstall/toggle + 安装守卫（拦截裸命令并引导）+ 提示词注入
