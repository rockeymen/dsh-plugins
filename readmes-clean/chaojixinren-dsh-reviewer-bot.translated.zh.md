...
- 与 DSH 的关系 · **现有方案**: 外部进程调用 · **DSH Reviewer Bot**: 原生插件，共享 `ctx`
- 平台支持 · **现有方案**: 仅 GitHub · **DSH Reviewer Bot**: GitHub / GitLab / Gitea / 本地
- 评审规则 · **现有方案**: prompt 内写死 · **DSH Reviewer Bot**: 规则包可独立发布安装
- 本地迭代 · **现有方案**: 推 PR 等 CI · **DSH Reviewer Bot**: `dshrb review --local` / `dshrb replay`
- 运行形态 · **现有方案**: 一次性 Action · **DSH Reviewer Bot**: Action / Daemon / DSH profile / CLI
- Docker · **现有方案**: 写模式硬依赖 · **DSH Reviewer Bot**: 可选隔离后端
...
### 命令 · 作用 · 最低信任
- **命令**: `@dsr review` · **作用**: 重新评审 · **最低信任**: untrusted
- **命令**: `@dsr explain ` · **作用**: 解释某文件改动 · **最低信任**: untrusted
- **命令**: `@dsr diagnose` · **作用**: 读失败 CI 定位原因 · **最低信任**: trusted-read
- **命令**: `@dsr fix` · **作用**: 改代码并跑校验 · **最低信任**: trusted-write + `allow-write`
- **命令**: `@dsr rules` · **作用**: 打印生效规则 · **最低信任**: untrusted
...
## Star 历史
    ...
    ![](https://api.star-history.com/chart?repos=chaojixinren/dsh-reviewer-bot&type=date&legend=top-left&sealed_token=I6mGiUsP3J8qGlfTB1jOBHZ79XbZO6ffLQwc3rCVJhK9MgZDTIMb8VMm5adRF67Btc1-hBIZ-iu4MJ7DcQopmf80YjoCEEpMLybU_7FTCVIUnqSGGOzNFqhTAy2ZeCes0DAQRGneC0FO_hf2YcWls7-6nqGvWT4RR-dXBQX-Jukoc7DP1Ps4NG24oeXI)
