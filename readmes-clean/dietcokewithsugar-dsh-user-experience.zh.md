...
### 支持 · 解析引擎
- **支持**: React + TypeScript（.ts / .tsx） · **解析引擎**: TypeScript 编译器 API（TSX）
- **支持**: React + JavaScript（.js / .jsx） · **解析引擎**: 同一引擎，.js 也可能含 JSX，统一按 TSX 解析
- **支持**: Vue 3（.vue SFC） · **解析引擎**: `@vue/compiler-sfc` 拆分 + `@vue/compiler-dom` 模板 AST；`<script>` / `<script setup>` 块复用 TypeScript 引擎，行号平移到整个 .vue 文件
- **支持**: CSS / SCSS / Sass / Less / PostCSS · **解析引擎**: 保守提取间距、紧凑布局与装饰内容候选；视觉结论仍需真实页面证据
- **支持**: 真实页面（可选） · **解析引擎**: 当前会话有浏览器/截图工具且项目可运行时，检查相关路由和视口
- **支持**: Persona 任务模拟（可选） · **解析引擎**: 可以在浏览器中执行关键任务时，记录操作步骤并评估流程冗余
...
### 能力 · 入口 · 说明
- **能力**: Persona 初始化 · **入口**: `/ux init` · **说明**: 模型从 README / package.json / 路由结构生成 1-3 个画像草稿，**经用户确认后**写入 `.ux/personas.yml`；文件已存在时直接加载，不重复询问
- **能力**: Persona 上下文注入 · **入口**: 自动 · **说明**: 每次请求按当前项目注入生效画像与走查协议（对齐 AGENTS.md section provider 模式）
- **能力**: 源码与 CSS 走查 · **入口**: `/ux scan` · **说明**: 先确定范围，再逐 persona 独立走查、合并成一份报告；以 Nielsen 原则为基础的 27 条规则，模型判断为主、AST/CSS 求证为辅
- **能力**: 按产品类型调整重点 · **入口**: 自动 · **说明**: 从项目文档和本次业务流程判断 `consumer`、`enterprise`、`ecommerce`、`content`、`finance`、`healthcare`、`developer-tool`、`internal-tool` 或 `other`，使用对应的体验要求
- **能力**: 三级证据 · **入口**: 自动 · **说明**: 源码/CSS 为 `static`，真实截图/DOM/尺寸为 `rendered`，记录 Persona 任务步骤后为 `interactive`；缺少浏览器能力时自动降级
- **能力**: 输出语言 · **入口**: 自动 / 配置 · **说明**: 显式配置 `outputLanguage` 时优先使用；`auto` 模式下先跟随当前用户语言，再回退到项目主 README。报告卡片和 AI 任务 Prompt 支持中英文
- **能力**: **改动触发的自动走查** · **入口**: 自动 · **说明**: 改完前端文件，回合收尾时自动对**所属的完整组件 / 页面**跑一次走查（不是 diff 那几行——缺失型问题在 diff 里根本不存在）；安静出报告，只在一级 / 二级问题时提示一句
- **能力**: 报告卡片 · **入口**: 自动 · **说明**: 首屏只展示关键信息：`[一级问题] 管理员页面` + 一句话说清出了什么事 + 用户会遇到什么；文件路径、规则 ID、内部编号折叠在「技术细节」里，展开后一键复制成结构化 YAML 直接粘给 AI
- **能力**: 问题确认闭环 · **入口**: 卡片按钮 / 直接说话 · **说明**: 点「确认存在 / 不是问题」，或直接说「第 2 条不成立」「这几条都对」「三级以下全部忽略」——**全程不需要记任何编号**；判定写入会话日志，重放完整恢复
- **能力**: 确认后生成 AI 任务 Prompt · **入口**: 卡片按钮 · **说明**: 用户确认问题属实后，一键复制包含观察现象、发生场景、用户影响与验收目标的任务 Prompt；**不预设具体代码改法**，明确说明插件只读到部分代码、要求 AI 补齐完整上下文，文案问题允许直接修改文案
- **能力**: 隐式确认 · **入口**: 自动 · **说明**: 下次走查时某条问题消失、且那个位置确实被重新扫描 = 用户把它改掉了 = 这条成立。用户什么都不用点，而这个信号比人工点确认更硬
- **能力**: 报告输出 · **入口**: 自动 · **说明**: Markdown 按严重度排序（**上界面用一级~四级问题，P0~P3 退为内部标识**），共性问题（≥2 画像命中）在前
- **能力**: 术语表 · **入口**: 自动 · **说明**: R-02 判定增量持久化到 `.ux/glossary.yml`，后续只做增量比对

### 三档运行模式（按场景自动选择）

### 模式 · 行为 · 什么时候用上
- **模式**: `auto` · **行为**: 跑完直接出报告，不打断、不索要确认 · **什么时候用上**: CI / headless；**改动自动触发的走查**（agent 自己发起的，就该由 agent 自己消化）
- **模式**: `review` · **行为**: 出报告后一次性批量确认（勾选多条一并提交） · **什么时候用上**: 用户主动发起 `/ux scan`
- **模式**: `interactive` · **行为**: 逐条确认 · **什么时候用上**: 需要精细调优规则时手动指定
...
### 状态 · 含义
- **状态**: `pending` · **含义**: 尚未判定
- **状态**: `confirmed_explicit` · **含义**: 用户点了「确认存在」
- **状态**: `confirmed_implicit` · **含义**: 下次走查中消失，且该位置确实被重新扫描
- **状态**: `rejected` · **含义**: 用户点了「不是问题」
- **状态**: `stale` · **含义**: 该位置本次未被扫描（或代码已整块删除），无法判定
...
### ID · 规则 · 验证路径
- **ID**: R-01 · **规则**: 错误提示无行动指引 · **验证路径**: 模型（AST 仅提取错误分支文案）
- **ID**: R-02 · **规则**: 术语不一致（条件触发：仅当本轮无一级 / 二级问题） · **验证路径**: 模型（AST 仅提取候选位置）
- **ID**: R-03 · **规则**: 不可逆操作文案泛化 · **验证路径**: 模型
- **ID**: R-04 · **规则**: 不可逆操作缺二次确认 · **验证路径**: model+ast
- **ID**: R-05 · **规则**: 有 loading 无 empty · **验证路径**: model+ast
- **ID**: R-06 · **规则**: 有 success 无 error · **验证路径**: model+ast
- **ID**: R-07 · **规则**: 提交中按钮未禁用 · **验证路径**: model+ast
- **ID**: R-08 · **规则**: 无超长内容兜底 · **验证路径**: model+ast
- **ID**: R-09 · **规则**: 深色/浅色模式适配缺失 · **验证路径**: **ast**（快车道，零 token）
- **ID**: R-10 · **规则**: 布局拥挤或分组层级不清 · **验证路径**: 源码/CSS 候选 + **必须有 rendered 证据**
- **ID**: R-11 · **规则**: 长列表缺少分页、虚拟滚动、折叠或数量限制 · **验证路径**: model+ast；可以 static 风险结论输出
- **ID**: R-12 · **规则**: Emoji/装饰元素与视觉语言不一致 · **验证路径**: 源码/CSS 候选 + **必须有 rendered 证据**
- **ID**: R-13 · **规则**: 页面用途或主要操作不清 · **验证路径**: 源码候选 + **必须有 rendered 证据**
- **ID**: R-14 · **规则**: 关键任务存在冗余交互 · **验证路径**: **必须有 interactive Persona 任务记录**
- **ID**: R-15 · **规则**: 功能分类不符合用户任务 · **验证路径**: **必须有 interactive 功能寻找记录**
- **ID**: R-16 · **规则**: 导航层级过深或缺少位置感 · **验证路径**: **必须有 interactive 导航记录**
- **ID**: R-17 · **规则**: 长时间操作缺少进度反馈 · **验证路径**: model+ast；static 证据
- **ID**: R-18 · **规则**: 表单字段或必填项过多 · **验证路径**: 源码候选 + **必须有 rendered 证据**
- **ID**: R-19 · **规则**: 表单校验反馈过晚 · **验证路径**: **必须有 interactive 表单任务记录**
- **ID**: R-20 · **规则**: 中途退出会丢失表单进度 · **验证路径**: **必须有 interactive 离开/恢复记录**
- **ID**: R-21 · **规则**: 缺少退出、取消或撤销路径 · **验证路径**: **必须有 interactive 任务记录**
- **ID**: R-22 · **规则**: 选项过多且缺少默认值或推荐 · **验证路径**: 源码候选 + **必须有 rendered 证据**
- **ID**: R-23 · **规则**: 同一操作跨页面不一致 · **验证路径**: **必须有 rendered 跨页面证据**
- **ID**: R-24 · **规则**: 相似组件的行为不一致 · **验证路径**: **必须有 interactive 对比记录**
- **ID**: R-25 · **规则**: 首次使用、离线或无权限状态缺失 · **验证路径**: model+ast；static 范围证据
- **ID**: R-26 · **规则**: 对比度、字号或触控热区影响使用 · **验证路径**: **必须有 rendered 测量证据**
- **ID**: R-27 · **规则**: 响应速度影响关键任务 · **验证路径**: **必须有 interactive 计时证据**
...
### 文件 · 是否提交 git · 说明
- **文件**: `.ux/personas.yml` · **是否提交 git**: ✅ 提交 · **说明**: 项目级共识，团队共享；CI 模式依赖它
- **文件**: `.ux/glossary.yml` · **是否提交 git**: ✅ 提交 · **说明**: 术语表与判定，复用价值高
- **文件**: `.ux/rules.local.yml` · **是否提交 git**: ❌ gitignore · **说明**: 个人走查偏好，不强加给团队。支持 `mode` 与 `autoScan`，其余键宽容忽略
- **文件**: `.ux/history.jsonl` · **是否提交 git**: ❌ gitignore · **说明**: 指纹历史账本：指纹、首次/末次出现、终态、每次走查的 scope。这是**长期指标数据**，不是判定结果
