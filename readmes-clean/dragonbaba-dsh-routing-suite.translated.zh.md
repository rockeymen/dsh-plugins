...
预设构图源自DSH在MIT下的官方Standard预设。它提供与标准模式相同的正常编码功能；路由插件本身并不行使这些功能。
...
请参阅[`SOURCE_PROVENANCE.md`](./SOURCE_PROVENANCE.md)、[`THIRD_PARTY_NOTICES.md`](./THIRD_PARTY_NOTICES.md)]和[`ACKNOWLEDGEMENTS.md`](./ACKNOWLEDGEMENTS.md)。没有分布式的上游 Injector 或 Router 实现；官方标准组成包含在 MIT 下。