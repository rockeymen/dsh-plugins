...
### Thing · Requirement
- **Thing**: OS · **Requirement**: Windows (uses PowerShell `Get-Clipboard`)
- **Thing**: Browser · **Requirement**: Chromium-based (Edge / Chrome)
- **Thing**: DSH · **Requirement**: Web GUI running on the same machine & Windows session as the files
- **Thing**: Note · **Requirement**: DSH backend must be on the same PC as the browser (localhost setup works)
