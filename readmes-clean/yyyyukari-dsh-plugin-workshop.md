...
### 需求 · 操作
- **需求**: 找 DSH 插件 · **操作**: 默认即「插件话题」列表（`topic:dsh-plugin` 收录的仓库）
- **需求**: 看飙升榜 · **操作**: 时间下拉选 近7天/近30天/近90天飙升 + ★最热
- **需求**: 中文搜索 · **操作**: 直接输入「天气」「翻译」等中文词，自动映射英文关键词
- **需求**: 看中文描述 · **操作**: 工具栏「描述中文」；详情页「翻译 README」
- **需求**: 安装/更新/卸载插件 · **操作**: 详情页「一键安装（订阅）/ 更新到最新 / 卸载」，按仓库类型自动选择安装路径（需要本机 git，缺失时会提示）
- **需求**: 提高搜索额度 · **操作**: 工具栏 ⚙ → 填入 GitHub Personal Access Token
...
- [x] 社区索引收录：✅ 三大列表全部收录——[awesome-dsh-plugin](https://awesome-dsh-plugin.com/)（UI Enhancements 分类）/ [awesome-deepseek-harness](https://github.com/0xsline/awesome-deepseek-harness) / [awesome-dsh-plugins](https://github.com/AdamPlatin123/awesome-dsh-plugins)
