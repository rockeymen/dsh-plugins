...
- **Command**: `/new [name]` · **What it does**: Start a fresh session
- **Command**: `/sessions` · **What it does**: List sessions (tap one to switch)
- **Command**: `/switch ` · **What it does**: Point at an existing session
- **Command**: `/current` · **What it does**: Show current session info
- **Command**: `/quiet` · **What it does**: Only show replies (default)
- **Command**: `/verbose` · **What it does**: Also show status/result lines
...
- **Env var**: `DEEPSEEK_API_KEY` · **Default**: — · **Meaning**: Required. Same var the SDK itself reads.
- **Env var**: `DEEPSEEK_BASE_URL` · **Default**: — · **Meaning**: Optional, for an OpenAI-compatible proxy.
- **Env var**: `DSH_PROVIDER` · **Default**: `deepseek-official` · **Meaning**: Provider route (see SDK docs).
- **Env var**: `DSH_MODEL` · **Default**: `deepseek-v4-flash` · **Meaning**: Model id.
- **Env var**: `DSH_MAX_TOKENS` · **Default**: unset · **Meaning**: Optional per-request output cap.
- **Env var**: `DSH_CORDIS` · **Default**: unset · **Meaning**: Path to a custom Cordis composition; omit to use the bundled default.
- **Env var**: `DSH_SESSION_ROOT` · **Default**: unset · **Meaning**: Where the runtime writes its JSONL session logs.
- **Env var**: `DSH_WORKSPACE` · **Default**: current dir · **Meaning**: The workspace the agent's tools operate in.
- **Env var**: `FEISHU_APP_ID` / `FEISHU_APP_SECRET` · **Default**: — · **Meaning**: Both required together, or leave both unset.
- **Env var**: `FEISHU_TRANSPORT` · **Default**: `ws` · **Meaning**: `ws` (no public URL needed) or `webhook`.
- **Env var**: `FEISHU_VERIFICATION_TOKEN` · **Default**: — · **Meaning**: Required when `FEISHU_TRANSPORT=webhook`.
- **Env var**: `FEISHU_ENCRYPT_KEY` · **Default**: unset · **Meaning**: Required when `FEISHU_TRANSPORT=webhook` — enable "Encrypt Key" for this event subscription in the Feishu console and paste the same value here. Used to verify each request's `X-Lark-Signature` (see "Security posture").
- **Env var**: `FEISHU_DOMAIN` · **Default**: `https://open.feishu.cn` · **Meaning**: Change for Lark international / a proxy.
- **Env var**: `FEISHU_ALLOWED_OPEN_IDS` · **Default**: *(empty)* · **Meaning**: Comma-separated. **Required** — empty means nobody is authorized.
- **Env var**: `FEISHU_ALLOWED_CHAT_IDS` · **Default**: *(empty = no restriction)* · **Meaning**: Comma-separated group allowlist.
- **Env var**: `DSH_FEISHU_BRIDGE_HOST` · **Default**: `0.0.0.0` · **Meaning**: HTTP server bind address (health check + webhook route).
- **Env var**: `DSH_FEISHU_BRIDGE_PORT` · **Default**: `8788` · **Meaning**: HTTP server port.
