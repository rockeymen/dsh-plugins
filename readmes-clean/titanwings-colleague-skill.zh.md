![COLLEAGUE.SKILL — 蒸馏他们的思维方式](../social-preview.png)

# 🧬 dot-skill（同事.skill）

### *"你们搞大模型的都是码圣！血肉苦弱！赛博飞升！"*

🧑‍💼  你的同事跳槽、导师毕业、搭档转岗，带走了整套工作方法和上下文？
💞  你的家人、老友、伴侣渐行渐远，你想留住和 TA 相处的方式？
...
>
> ![dot-skill 微信群二维码](../assets/wechat-group-qr-12.png)
>
...
<sub>同事 · 导师 · 搭档 · 上下游协作者</sub>
<sub>前任 · 伴侣 · 父母 · 朋友 · 家人</sub>
<sub>名人 · 创作者 · 公众表达者 · 小说角色</sub>

<sub>Work Skill + Persona 双层架构 —— 既学他的技术规范和工作流程，也学他的说话方式和职场姿态。支持飞书 / 钉钉 / Slack 自动采集。</sub>
<sub>🆕 生活照片分享功能即将上线 —— 让你蒸馏出的关系不只是回消息，还能像真人一样发照片、分享生活片段，告诉你 TA 今天过得怎么样。</sub>
<sub>内置完整的 六维度研究工具链（字幕下载 → 文稿清洗 → 研究合并 → 质量检查）。不是模仿语气，而是复现他的心智模型和判断框架。</sub>
...
### 宿主 · 说明
- **宿主**: 🟣 **Claude Code** · **说明**: slash command 原生支持
- **宿主**: 🟠 **Hermes Agent** · **说明**: 一键安装，`/dot-skill` 直接调用
- **宿主**: 🔵 **OpenClaw** · **说明**: 完整兼容
- **宿主**: ⚫ **Codex** · **说明**: skill name 调用
- **宿主**: 🔷 **DeepSeek Harness** · **说明**: 原生 filesystem skill 发现，`/dot-skill` 直接调用
...
- **来源**: 🟢 飞书（自动采集） · **消息记录**: ✅ API · **文档 / Wiki**: ✅ · **多维表格**: ✅ · **备注**: 输入姓名即可，全自动
- **来源**: 🟡 钉钉（自动采集） · **消息记录**: ⚠️ 浏览器 · **文档 / Wiki**: ✅ · **多维表格**: ✅ · **备注**: 钉钉 API 不支持历史消息
- **来源**: 🟣 Slack（自动采集） · **消息记录**: ✅ API · **文档 / Wiki**: — · **多维表格**: — · **备注**: 需管理员安装 Bot；免费版限 90 天
- **来源**: 💬 微信聊天记录 · **消息记录**: ✅ SQLite · **文档 / Wiki**: — · **多维表格**: — · **备注**: 需先用 WeChatMsg / PyWxDump / 留痕等工具导出
- **来源**: 📄 PDF / 图片 / 截图 · **消息记录**: — · **文档 / Wiki**: ✅ · **多维表格**: — · **备注**: 手动上传
- **来源**: 📦 飞书 JSON 导出 · **消息记录**: ✅ · **文档 / Wiki**: ✅ · **多维表格**: — · **备注**: 手动上传
- **来源**: ✉️ 邮件 `.eml` / `.mbox` · **消息记录**: ✅ · **文档 / Wiki**: — · **多维表格**: — · **备注**: 手动上传
- **来源**: 📝 Markdown / 直接粘贴 · **消息记录**: ✅ · **文档 / Wiki**: ✅ · **多维表格**: — · **备注**: 手动输入
...
### 宿主 · `<TARGET>` 路径
- **宿主**: Claude Code · **`<TARGET>` 路径**: `~/.claude/skills/dot-skill`
- **宿主**: OpenClaw · **`<TARGET>` 路径**: `~/.openclaw/workspace/skills/dot-skill`
- **宿主**: Codex · **`<TARGET>` 路径**: `~/.codex/skills/dot-skill`
- **宿主**: DeepSeek Harness · **`<TARGET>` 路径**: `~/.dsh/skills/dot-skill`（全局）或 `.dsh/skills/dot-skill`（项目）
- **宿主**: Hermes · **`<TARGET>` 路径**: clone 后跑 `python3 tools/install_hermes_skill.py --force`
...
### 命令 · 说明
- **命令**: `/dot-skill` · **说明**: 统一主入口
- **命令**: `/{character}-{slug}` · **说明**: 调用完整 Skill（Persona + Work）
- **命令**: `/{character}-{slug}-work` · **说明**: 仅工作能力
- **命令**: `/{character}-{slug}-persona` · **说明**: 仅人物性格
- **命令**: `python3 tools/skill_writer.py --action list ...` · **说明**: 列出三类 Skill
- **命令**: `python3 tools/version_manager.py --action rollback ...` · **说明**: 回滚历史版本
...
> 六维度 research：著作 · 访谈 · 决策 · 表达 DNA · 外部评价 · 时间线
...
### 家族 · Persona 内容 · 附加模块
- **家族**: 🧑‍💼 **colleague** · **Persona 内容**: 6 层性格结构：硬规则 → 身份 → 表达风格 → 决策模式 → 人际行为 → Correction · **附加模块**: ➕ **Work Skill**：负责范围、工作流程、输出偏好、经验知识库
- **家族**: 💞 **relationship** · **Persona 内容**: 表达 DNA · 情绪触发点 · 冲突模式 · 修复模式 · **附加模块**: —
- **家族**: 🌟 **celebrity** · **Persona 内容**: 心智模型 · 决策启发式 · 表达 DNA · 外部评价对照 · **附加模块**: ➕ 六维度 research 档案（著作 / 访谈 / 决策 / 时间线...）
...
### 家族 · 信源优先级（高 → 低）
- **家族**: 🧑‍💼 **colleague** · **信源优先级（高 → 低）**: 他**主动写的**长文（设计文档 / 评审意见） **›** **决策类回复** **›** 日常群聊消息
- **家族**: 💞 **relationship** · **信源优先级（高 → 低）**: 完整的聊天记录 **›** 往来信件 / 朋友圈 / 日记 **›** 旁人描述
- **家族**: 🌟 **celebrity** · **信源优先级（高 → 低）**: 第一人称著作 / 博客 / 长访谈 **›** 决策记录（发布会、commit、采访）**›** 他人评价
