...
### 开局条件 · Ability
- **开局条件**: DSH `standard` · **Ability**: **91**
- **开局条件**: DSH `PTC`（code 模式） · **Ability**: **92**
- **开局条件**: DSH `minimal`（官方 RL 条件） · **Ability**: **99 / 96**
- **开局条件**: `anchored-standard`（首轮 minimal、随后全量工具） · **Ability**: **98 / 99**
...
### 检查点 · 预期值
- **检查点**: 首份 `request/header` 的 `system` · **预期值**: 恰好是 `You are a helpful software engineer assistant.`
- **检查点**: 首份 `request/header` 的 `tools` · **预期值**: 恰好是 `bash, str_replace_editor`
- **检查点**: 首块 reasoning · **预期值**: `We` / `We need` 开头，`let me = 0`
...
### 任务类型 · 建议 preset · 依据
- **任务类型**: 维护 / 修 bug / 读代码再动手 / 长工程 · **建议 preset**: **anchored-standard**（本仓库） · **依据**: Project2 维护题：minimal 99/96、anchored 98/99
- **任务类型**: 从零写新项目 / 绿场构建 / 交付导向 · **建议 preset**: `code`（PTC）或 standard 系 · **依据**: Mario 绿场题：code 10/10 vs anchored 6/10
- **任务类型**: 简单问答、短任务 · **建议 preset**: 随便 · **依据**: 简单任务各条件都饱和，测不出差别
...
- **能力**: 首请求 RL 锚定 · **上游 [dsh-anchored-standard](https://github.com/xiaobright/dsh-anchored-standard)**: ✅ · **本仓库**: ✅ 完全保留
- **能力**: 晋升后 resident 目录 · **上游 [dsh-anchored-standard](https://github.com/xiaobright/dsh-anchored-standard)**: ✅ · **本仓库**: ✅ 保留
- **能力**: 部署插件晋升后常驻 · **上游 [dsh-anchored-standard](https://github.com/xiaobright/dsh-anchored-standard)**: ❌ 全部按需解锁 · **本仓库**: ✅ `antigravity_agent` / `antigravity_agent_status` / `vision` / `web_search`
- **能力**: 每轮格式守卫 · **上游 [dsh-anchored-standard](https://github.com/xiaobright/dsh-anchored-standard)**: ❌ · **本仓库**: ✅ `format-guard`（参照 router-standard 的 near-field 模式）
- **能力**: npm 一键安装 · **上游 [dsh-anchored-standard](https://github.com/xiaobright/dsh-anchored-standard)**: ❌ · **本仓库**: ✅ `npx dsh-anchored-plus`
