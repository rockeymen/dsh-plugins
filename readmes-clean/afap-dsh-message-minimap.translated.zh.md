...
### 关注点 · 做法
- **关注点**: 锚点来源 · **做法**: 会话包给每个聊天节点渲染 `data-chat-flow-kind` / `data-chat-flow-key` 的稳定包裹层；用户消息 kind 为 `"user"`。
- **关注点**: 滚动容器 · **做法**: 从第一个可见 flow item 向上找最近的 `overflow-y: auto/scroll` 祖先；导出布局（`data-conversation-scroll`，不滚动）下自动隐藏。
- **关注点**: 几何映射 · **做法**: `刻度 y = 消息内容偏移 / scrollHeight × 轨道高`；滑块同理（最小高度 16px）。
- **关注点**: 数据同步 · **做法**: `MutationObserver`（childList/subtree/characterData，覆盖流式输出）+ 容器 `scroll` + `ResizeObserver` + 1s 轮询兜底（应对迟挂载/会话切换），rAF 节流 + 浅比较避免渲染抖动。
- **关注点**: 挂载点 · **做法**: `conversation.session.header.utilities` 槽位（活动会话常驻），组件本身只渲染 `position: fixed` 的轨道，无内联占位。
- **关注点**: 样式 · **做法**: 与官方包一致地注入 `<style data-plugin-css>`，全部使用 DSW 主题变量，自动适配明暗主题。

## 7. 日志与排错

### 现象 · 排查方向
- **现象**: 看不到导航条 · **排查方向**: 确认已重启 `dsh web`；会话需已有用户消息且内容可滚动；F12 Console 搜 `dsh-message-minimap`。
- **现象**: 刻度位置偏移 · **排查方向**: 偶发的图片/附件异步加载会改变高度——MutationObserver 会自动校正；若持续异常，滚动一下或缩放窗口触发重算。
- **现象**: 点击不跳转 · **排查方向**: 检查是否在导出/打印式布局（`data-conversation-scroll`）下——该布局无内部滚动容器，插件自动隐藏。
- **现象**: 样式异常 · **排查方向**: 确认主题变量（`--dsw-*`）存在；本插件不自带配色，全部跟随 DSW 主题。
