...
### 特性 · 说明
- **特性**: 🪙 常驻显示 · **说明**: 会话输入框下方（`conversation.composer.dock` 插槽）常驻余额读数条，随时可见
- **特性**: 🔄 点击刷新 · **说明**: 点击读数条即时重新查询，无需重启插件
- **特性**: 💰 余额明细 · **说明**: 悬停显示总余额、赠送余额、充值余额与可用状态
- **特性**: 🌗 主题适配 · **说明**: 使用 DSH 主题 CSS 变量（`--dsw-alias-*`），自动适配明暗模式
- **特性**: 🔐 密钥安全 · **说明**: API Key 只经 Host 环境变量传给本地子进程，不回显、不进浏览器、不落库
- **特性**: 🧩 即插即用 · **说明**: 读取 DSH 凭据库中已配置的 `DEEPSEEK_API_KEY`，无需额外设置界面
...
### 问题 · 排查
- **问题**: 显示「余额获取失败」且悬停提示 `no-credential` · **排查**: 未在 DSH 凭据库配置 `DEEPSEEK_API_KEY`，见[凭据配置](#凭据配置)
- **问题**: 提示 `no-node` · **排查**: 当前环境未解析到 node 可执行文件，确认 DSH 宿主具备 Node 运行时
- **问题**: 提示 `api-error` / `bad-response` · **排查**: 余额 API 返回异常（如 Key 失效、网络受限），悬停提示会给出具体信息
- **问题**: 提示网络类错误 · **排查**: DSH 宿主进程需可访问 `api.deepseek.com`（模型调用本身就走该域名，通常无碍）
