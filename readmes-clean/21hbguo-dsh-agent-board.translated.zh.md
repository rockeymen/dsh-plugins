...
###  · 特性 · 说明
- 🎯 · **特性**: **树形层级图** · **说明**: 以主 agent（实心圆）为根、子代理（空心圆）为分支的层级森林；多主会话并排显示；**四色状态语义**：🟢绿=working（在跑）/ 🔵蓝=完成（未查看）/ ⚪灰=空闲（完成且已打开过，已读弱化）/ 🔴红=停滞
- ⚡ · **特性**: **SSE 实时推送** · **说明**: 数据变化事件级推送，看板即时刷新；断线自动回退轮询兜底
- 🚦 · **特性**: **三态生命周期** · **说明**: working（在跑）→ **完成**（settle 后弱化保留 30 分钟，每根最多 12 条防堆积）→ 超时消失；有在跑子代理的根自动展开，子代理立即可见
- 🔔 · **特性**: **停滞自动告警** · **说明**: 子代理静默超过阈值（默认 10 分钟）→ 自动向父会话注入 notice 提醒（GUI 可见、不唤醒模型、不耗 API 额度）——不再需要手动轮询 `list_agents` 催进度
- 📝 · **特性**: **进展一目了然** · **说明**: 每个节点同行显示：当前动作（正在执行的命令 `⚙` / 输出中 `✍`）+ 最新答复节选 + 状态；工具与文本都是实时信号
- 🖱️ · **特性**: **单击直达，行首折叠** · **说明**: 单击任意节点打开会话；展开/折叠只走行首 ▸/▾（有子节点的根）；**打开 = 已读**：完成节点由蓝变灰（空闲），跨刷新持久（localStorage）；当前会话带「当前」标记
- 💾 · **特性**: **跨重启持久** · **说明**: 完成态存档落盘（`~/.dsh/agent-board-archive.json`），重启后不丢；会话标题/名字自动恢复
- 🪟 · **特性**: **常驻悬浮窗** · **说明**: 可拖拽、折叠、隐藏（右下角召唤）；侧边栏「Agent 看板」按钮开关；位置状态持久化
...
### 参数 · 默认 · 说明
- **参数**: `scanIntervalMs` · **默认**: 60000 · **说明**: 停滞扫描周期
- **参数**: `stallThresholdMs` · **默认**: 600000 · **说明**: 静默多久算停滞（10 分钟）
- **参数**: `remindIntervalMs` · **默认**: 600000 · **说明**: 同一子代理两次提醒最小间隔
...
### 端点 · 说明
- **端点**: `GET /api/agent-board/agents` · **说明**: JSON 快照（roots + rows + label/action/reply/status）
- **端点**: `GET /api/agent-board/stream` · **说明**: SSE 数据变化信号（`data: changed`）
