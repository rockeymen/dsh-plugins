...
### 能力 · 落点
- **能力**: 声明式拓扑构造（Fluent API：addNode / addEdge / addConditionalEdge） · **落点**: `StateGraph`
- **能力**: 纯函数增量补丁：节点只返回 `Partial<State>`，引擎不可变合并 · **落点**: `StateGraph.run`
- **能力**: 防死循环熔断（默认 25 次迭代，超限抛错） · **落点**: `StateGraph` 内置
- **能力**: 双向事件流观测（`graph/*` 事件，追加式轨迹） · **落点**: `ctx.emit` 事件总线
- **能力**: 隔离的状态图实例工厂 · **落点**: `ctx.graph.create()`

## 核心接口

### 接口 / 类型 · 签名 · 说明
- **接口 / 类型**: `NodeHandler<T>` · **签名**: `(state: T, ctx: Context, signal?: AbortSignal) => Promise> \ · **说明**: Partial<T>` · 节点业务执行体，返回需合并的状态增量；第二参数仍为 `ctx`，第三参数接收本次运行的取消信号
- **接口 / 类型**: `ConditionHandler<T>` · **签名**: `(state: T, ctx: Context, signal?: AbortSignal) => string \ · **说明**: Promise<string>` · 动态路由，返回下一个 NodeName 或 `"__END__"`；第二参数仍为 `ctx`，第三参数接收本次运行的取消信号
- **接口 / 类型**: `GraphExecutionResult<T>` · **签名**: `{ graphId: string; finalState: T; trajectory: string[]; iterations: number }` · **说明**: 每次执行的 graphId（区分重复/并发轨迹）、最终状态、全量跳转轨迹、迭代次数
- **接口 / 类型**: `ctx.graph.create<T>(maxIterations?)` · **签名**: → `StateGraph<T>` · **说明**: 创建隔离的图实例
...
### 事件 · 载荷 · 时机
- **事件**: `graph/start` · **载荷**: `{ graphId, initialState, entryPoint }` · **时机**: 图开始执行
- **事件**: `graph/node-start` · **载荷**: `{ graphId, node, state, iteration }` · **时机**: 每个节点执行前
- **事件**: `graph/node-end` · **载荷**: `{ graphId, node, state }` · **时机**: 节点补丁合并后
- **事件**: `graph/node-error` · **载荷**: `{ graphId, node, error }` · **时机**: 节点抛错或被取消中断（随后上抛；取消还会补发 `graph/error`）
- **事件**: `graph/error` · **载荷**: `{ graphId, error, state, lastNode }` · **时机**: 迭代熔断 / 目标节点缺失 / 路由抛错 / 取消（`graph/start` 之后）
- **事件**: `graph/end` · **载荷**: `{ graphId, finalState, trajectory, iterations }` · **时机**: 仅正常终止（END 或无出边）
