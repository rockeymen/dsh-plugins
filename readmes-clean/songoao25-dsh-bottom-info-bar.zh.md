...
### 现象 · 处理
- **现象**: 安装后刷新页面没看到信息栏 · **处理**: 需**重启** `dsh web`（宿主进程加载插件）
- **现象**: 余额显示「未配置 DEEPSEEK_API_KEY」 · **处理**: 在 设置 → 模型 配置 DeepSeek Key
- **现象**: 余额显示「⚠ 刷新失败，显示上次快照」 · **处理**: 网络/Key 临时故障，60s 后自动重试。**将鼠标悬停在警告上**可查看详细解释和重试时间
- **现象**: 显示「未配置 OpenCode Go」 · **处理**: 在 设置 → 模型 配置 `OPENCODE_GO_API_KEY`，或用 opencode CLI 登录 OpenCode Go
- **现象**: 如何绑定 ChatGPT 订阅？ · **处理**: 安装配套插件 **dsh-chatgpt-subscription**，在官方页面用 ChatGPT 账号授权——它维护的令牌即本信息栏额度显示所读取的令牌
- **现象**: ChatGPT 额度显示异常/为空 · **处理**: wham 接口未公开、可能变更；失败自动保留上次快照并 60s 重试。**悬停 "⚠ 刷新失败"** 可查看重试说明
- **现象**: 简洁模式为什么显示不同的窗口？ · **处理**: 简洁模式优先显示时间最短的窗口（5小时 > 周 > 月），因为刷新最快。若 5 小时窗口不可用，则降级到周或月窗口。**额度与倒计时严格来自同一窗口**，确保信息匹配
- **现象**: 为什么看不到模型的思考过程？ · **处理**: DSH 界面层不渲染模型的内部思考过程，属 DSH 自身界面限制，非插件问题
- **现象**: 想改回原生统计栏 · **处理**: 卸载本插件并重启
