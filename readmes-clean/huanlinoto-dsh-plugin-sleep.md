[![dshfind](https://dshfind.com/api/badge/huanlinoto/dsh-plugin-sleep?lang=zh)](https://dshfind.com/zh/plugins/huanlinoto/dsh-plugin-sleep?ref=badge)
...
### 字段 · 类型 · 默认值 · 说明
- **字段**: `maxDurationMs` · **类型**: number · **默认值**: `60000` · **说明**: 单次 sleep 上限毫秒数。超过会被 clamp 到此值，并在返回中标 `clamped: true`。
- **字段**: `defaultDurationMs` · **类型**: number · **默认值**: `0` · **说明**: 当模型省略 `duration_ms` 时的回退值。
...
### 参数 · 类型 · 必填 · 说明
- **参数**: `duration_ms` · **类型**: integer · **必填**: 是 · **说明**: 暂停毫秒数，必须 ≥ 0。超过 `maxDurationMs` 会被 clamp。
- **参数**: `reason` · **类型**: string · **必填**: 否 · **说明**: 人类可读的暂停理由，会原样回显。
