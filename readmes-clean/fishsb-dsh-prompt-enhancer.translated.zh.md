...
### 配置项 · 说明
- **配置项**: 优化模式 · **说明**: 基础（默认，直发）/ 轻量 / 标准 / 智能；切换即时生效并持久化
- **配置项**: 记忆功能 · **说明**: 开 / 关；开启后发送前多轮迭代累积为记忆链（最多最近 4 轮输入/输出，以多轮对话注入并感知修改方向；首次自动轻量兜底），关闭后不读取/写入
- **配置项**: 上下文预算 · **说明**: 0 / 2000 / 4000 / 8000 字符；0 = 不注入上下文（记忆链同样受预算约束，链上限 2400 字符）
- **配置项**: 超时时间 / Token 上限 / 输出上限 · **说明**: 优化请求参数
- **配置项**: 模板 · **说明**: 内置模板 / 自定义模板
