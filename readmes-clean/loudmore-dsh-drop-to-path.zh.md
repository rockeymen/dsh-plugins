...
flowchart LR
    A["粘贴 / 拖入"] --> B{"文件类型?"}
    B -->|"图片png / jpg / webp / gif"| C["原生附件卡片预览 / 移除"]
    C --> D["点击发送"]
    D --> E["上传 → workspace/.drops/"]
    E --> F["消息转为纯文本路径"]
    F --> G["纯文本模型用 vision / 文档工具处理"]
    B -->|"其他文件pdf / docx / xlsx / 视频…"| H["附件栏方块标签格式图标 · ✕ 移除"]
    H --> D
```
...
### 类型 · 扩展名 · 上限 · 体验
- **类型**: 图片 · **扩展名**: `png` `jpg` `jpeg` `webp` `gif` · **上限**: 30MB · **体验**: 原生附件卡片(预览/移除),发送时自动转路径
- **类型**: 文档 · **扩展名**: `pdf` `doc` `docx` `xls` `xlsx` `ppt` `pptx` `txt` `md` `csv` `json` · **上限**: 100MB · **体验**: 附件栏方块标签(格式图标、hover 显全名、✕ 移除),发送时附加路径
- **类型**: 压缩包 · **扩展名**: `zip` · **上限**: 100MB · **体验**: 同上
- **类型**: 视频 · **扩展名**: `mp4` `mov` `webm` `mkv` `avi` · **上限**: 100MB · **体验**: 同上
- **类型**: 音频 · **扩展名**: `mp3` `wav` `flac` `m4a` · **上限**: 100MB · **体验**: 同上
...
### 场景 · 之前 · 现在
- **场景**: 看图问答 · **之前**: 截图 → 存工作区 → 手写路径 · **现在**: 截图 → Ctrl+V → 回车
- **场景**: 多图对比(pixel diff) · **之前**: 同上,每个路径手写 · **现在**: 发两张图即可
- **场景**: 长截图 OCR / UI 还原 · **之前**: 同上 · **现在**: 发图即可
- **场景**: 分析 PDF / xlsx / 视频 · **之前**: 拷进工作区,手写路径 · **现在**: 直接拖入,发送即可
...
### DSH 版本 · 状态
- **DSH 版本**: 0.1.0-rc.6(本仓库验证环境) · **状态**: ✅ 可用
