...
### Hook · 触发事件 · 手机交互（外出模式）
- **Hook**: 任务完成 · **触发事件**: `turn/end` 且 `reason.kind === 'completed'` · **手机交互（外出模式）**: 系统通知 + 手机推送（含 AI 最后输出）
- **Hook**: 提问 · **触发事件**: `ask_user_question` 工具调用 · **手机交互（外出模式）**: 手机点选选项 / 文字自由作答
- **Hook**: 权限请求 · **触发事件**: `approval/request` · **手机交互（外出模式）**: 手机 Approve / Deny
- **Hook**: 远程续聊 · **触发事件**: 文件队列 `~/.a4phone/dsh-jobs/`（a4p 写请求，插件回复） · **手机交互（外出模式）**: 手机发文字 → `agent.followup` 注入当前会话 → 回复推回手机
...
2. **文件队列协议**：`a4p resume` / `a4p listen` 把手机消息原子写入 `~/.a4phone/dsh-jobs/req-.json`；插件每秒扫描，处理后写 `resp-.json`，a4p 轮询取回并推回手机；插件同时刷新 `~/.a4phone/dsh-heartbeat.json` 心跳，a4p 据此快速判断 `dsh web` 是否在运行（未运行会快速失败而非干等超时）
