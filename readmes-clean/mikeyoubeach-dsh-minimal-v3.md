...
- **Dimension**: persona · **Official minimal**: `complete` prompt · **dsh-minimal-v3**: same (`You are a helpful software engineer assistant.`, `complete: true`, `includeRuntimeContext: false`)
- **Dimension**: shell · **Official minimal**: persistent `bash` (unusable on Windows) · **dsh-minimal-v3**: platform-adaptive: Windows→`pwsh`, others→`bash` (non-persistent)
- **Dimension**: auto-injection · **Official minimal**: none · **dsh-minimal-v3**: none (does NOT load AGENTS.md/CLAUDE.md/local overlay)
- **Dimension**: context compaction · **Official minimal**: none · **dsh-minimal-v3**: none
- **Dimension**: tools · **Official minimal**: only `bash` + `str_replace_editor` · **dsh-minimal-v3**: `pwsh\ · bash`, `read`, `write`, `edit`, `read_image`, `glob`, `grep`, `ask_user_question`, `todo_write`
- **Dimension**: subagent/workflow/goals/skills/web · **Official minimal**: none · **dsh-minimal-v3**: none (kept minimal)
