...
「导出注释 JSON」生成 `.wireframe-annotations.json`，可从其他设备或浏览器的注释面板导入并合并。同一项目 id 才允许导入，避免把注释误写到其他原型。JSON 是跨设备交换和备份方式，不是日常必经步骤。
...
按 `SKILL.md` 执行：先识别交付格式 → 确认输出目录 → 复制 `starter/` → 只改业务 `src/` → 用 `index.html` 验证画板、演示、视口、导航、修改和注释。组件与 project schema 见 `reference.md`，交付物内组件 API