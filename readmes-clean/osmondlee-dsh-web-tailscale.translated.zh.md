...
- 子命令：`-InstallAuth`（生成叠加层并校验插件已装）、`-NewPassword`（生成并持久化密码）、
  `-Status`、`-Stop`（含 cmd/npx 包装下孤儿进程兜底）、`-Password ''`、
...
### 决策 · 理由
- **决策**: 不传 `--host 100.x` · **理由**: rc.6 webserver schema 只接受 127.0.0.1/0.0.0.0（实测启动报错）
- **决策**: `--patch` 叠加层注入 `host: '0.0.0.0'` · **理由**: schema 合法；patch 语义是"整体替换目标行 config"，因此连同 `port` 一起重写
- **决策**: 必须叠加 dsh-web-auth · **理由**: 0.0.0.0 绑定下非回环 `/api` 可达（实测），官方信任围栏明确"不是认证层"，密码墙才是访问控制
- **决策**: `--trusted-host` 填本机 TS 地址 · **理由**: 官方围栏校验请求 `Host` 头 = 地址栏里的地址；带端口只匹配该端口，不带匹配任意端口
- **决策**: 不改 profiles/web/cordis.patch.yml · **理由**: `--patch` 层只被远程实例加载，桌面版实例行为不变
- **决策**: `tokenFile` 显式指定 · **理由**: 插件默认是 Linux 路径，Windows 上必须重定向，登录才能跨重启保持
- **决策**: 防火墙 RemoteAddress 100.64.0.0/10 · **理由**: 0.0.0.0 监听下把可达面收窄到 Tailscale 网段

## 故障排查

### 现象 · 原因 · 修复
- **现象**: 手机超时 · **原因**: Tailscale 未登录/手机未连 · **修复**: `tailscale status` 两节点在线；`tailscale ip -4` 非空，否则 `tailscale up`
- **现象**: 手机拒连 · **原因**: Windows 防火墙 · **修复**: 管理员执行上文 `New-NetFirewallRule`（100.64.0.0/10）
- **现象**: `/api` 403 · **原因**: Host 不在 `--trusted-host` · **修复**: 地址栏地址必须精确列入；域名不带结尾点；不能用通配符
- **现象**: 无登录卡片，`/api/auth/status`→404 · **原因**: 插件没挂载 · **修复**: 确认本地路径安装、启动带 `--patch` 叠加层
- **现象**: 登录报 503 未配置密码 · **原因**: 子进程没拿到环境变量 · **修复**: 用脚本启动，或先 `$env:DSH_WEB_AUTH_PASSWORD='...'`
- **现象**: 设置页空白 · **原因**: 官方非回环 settings 走 memory 作用域 · **修复**: 插件已内置客户端补丁（lanHosts 自动从 trustedHosts 派生）；失效时在叠加层补 `lanHosts`
- **现象**: `$.host expected "127.0.0.1" \ · **原因**: "0.0.0.0"` · **修复**: 传了 `--host 100.x` · 去掉 `--host`，交给叠加层注入
- **现象**: `web takes none of parent --patch...` · **原因**: `--patch` 放错位置 · **修复**: 顺序：`dsh web --patch <f> <应用 flag>`
- **现象**: `dsh plugin add` 报 pnpm 找不到 · **原因**: pnpm 不在 PATH · **修复**: `npm i -g pnpm`
- **现象**: 手机报 `Failed to load plugins … slot "conversation.composer.dock" is not declared` · **原因**: 某些桌面配套插件（如 dsh-balance）用一次性 effect 直接注册槽位，慢设备上先于 conversation 模块声明执行（时序竞态） · **修复**: 在叠加层对该插件行加 `disabled: true`（见 example 文件注释）；桌面端若复现同样处理
- **现象**: 两个实例同时跑 · **原因**: 共享同一 $DSH_HOME 会话存储 · **修复**: 可共存（原子写入），建议不要同时重度使用
