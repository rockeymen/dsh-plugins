...
- **预设**: Standard · **首轮看到**: 25 工具 + 长 prompt · **全程可用**: 25 工具 · **Project2 V4.1b（max）**: **91 / 92**
- **预设**: Minimal · **首轮看到**: 一句话 prompt + 2 工具 · **全程可用**: 只有 2 工具 · **Project2 V4.1b（max）**: **99 / 96** —— 但干不了完整开发
- **预设**: **锚定标准** · **首轮看到**: 一句话 prompt + 2 工具 · **全程可用**: **全部 25 工具** · **Project2 V4.1b（max）**: **98 / 99**
...
sequenceDiagram
    ...
    U->>A: 第一条消息（新会话）
    A->>M: 请求 #1任务匹配的锚定 personatools: bash + read（± edit/write）
    Note over M: 推理链锚定在任务匹配的开局上
    M->>A: 首个 tool/call
    Note over A: 晋升信号（持久化会话事件）
    A->>M: 请求 #2同一 persona · 全部 25 个 Standard 工具
...
### 任务 · Persona · 首轮工具
- **任务**: **spec** — 修复 / 维护 / 调试 · **Persona**: Minimal 原句 · **首轮工具**: `bash` + `read` + `edit`
- **任务**: **react** — 从零构建 / 新项目 · **Persona**: 实干者句（少废话、直接产出） · **首轮工具**: `bash` + `read` + `write`
- **任务**: **weak** — 语义模糊 · **Persona**: 模型自分类（Pro：分类指令版） · **首轮工具**: `bash` + `read`
...
###  · 得到 · 代价
- 相对 Minimal · **得到**: 多 23 个工具：glob/grep/edit/write/web_search/子代理/工作流/技能/目标/todo… · **代价**: 开局多一轮推理
- 相对 Standard · **得到**: 任务匹配的轨迹锚定（参考评测 98/99 vs 91/92）+ 比 Standard 更干净的首轮 · **代价**: 首轮目录由关键词分类决定；模糊任务回落到 weak 锚交给模型自选
