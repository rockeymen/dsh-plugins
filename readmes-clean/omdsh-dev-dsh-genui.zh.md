...
https://github.com/user-attachments/assets/f5db33ec-7471-4d4a-a85b-79c9962ab4ef
  ...
  ![实际渲染效果：可交互监控面板](./assets/showcase-panel.png)
  实际效果：模型输出的一块可交互监控面板（点「刷新」它会重新生成数据）
...
- **DOM 通道**：宿主没有该扩展点（包括原版 DSH 与旧版构建）时，插件观察会话 DOM 自行挂载渲染树。自 0.7.2 起**支持流式渲染**：模型写到哪渲染到哪，首个完成的组件立即出现，不用等整段回复写完。自 0.8.3 起围栏发现**多表面兼容**：同时匹配标准 `md-code-block` 表面、部分宿主构建使用的 deepsuite 风格 `.code-block` / `.code-block-small` 表面，并以「label+``」结构兜底——任何 banner 标注 `dsh-ui` 且含 `` 正文的元素都能被识别。即使你的 dsh 构建用了别的类名，围栏照常渲染（控制台会有一条一次性提示说明宿主 DOM 发生漂移）。
...
### 普通回答 · 装了 dsh-genui
- **普通回答**: "本月收入 ¥128,430，环比 +12.4%，建议关注转化率。" · **装了 dsh-genui**: 一行分析 + 旁边直接渲染：收入/订单/转化率三张统计卡、趋势图、进度条
- **普通回答**: 想再看别的？再打一段字问一遍 · **装了 dsh-genui**: 面板上就有「刷新」「切换视图」按钮，点一下，模型更新数据
...
- **函数图**：`plot` 画曲线，参数滑块拖动实时重绘，支持自动动画
  ...
  ![函数绘图：拖动滑块实时重绘](./assets/showcase-plot.png)
...
- **图错误自愈**：mermaid 渲染失败自动修复重试（剥反引号、引号化中文/空格标签、去 ``），仍失败才降级源码；错误图永不直接上屏
...
### 方向 · 结论 · 理由
- **方向**: 增量 patch（模型只发 diff 不重发全量 spec） · **结论**: 不做 · **理由**: fence 一次 200–800 token，重发代价极小；patch 协议的教学成本与出错率不值得。若未来出现秒级自动刷新面板再议
- **方向**: action 防抖/去重 · **结论**: ✅ 已做（300ms 尾沿，按 action 名独立） · **理由**: 连点刷屏是真实摩擦，收口点一处改动
- **方向**: 跨会话状态持久化（回放恢复 tabs/开关） · **结论**: 不做 · **理由**: 回放重置是更正确的默认行为（模型已用新 fence 更新过界面）；流式期间状态天然保留
- **方向**: MCP 适配器 / 独立画廊页 / i18n · **结论**: 不做 · **理由**: 无跨工具需求信号；画廊素材已被 `gallery.ts` + demo-prompts + README 截图覆盖；内置文案仅 6 处
