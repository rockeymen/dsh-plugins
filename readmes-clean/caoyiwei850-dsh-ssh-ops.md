...
### 工具 · 用途
- **工具**: `ssh_connect` · **用途**: 建立 SSH 连接并设为当前服务器
- **工具**: `ssh_exec` · **用途**: 在当前服务器执行 Agent 命令并返回结构化输出
- **工具**: `ssh_read` · **用途**: 按需读取右侧终端缓冲输出
- **工具**: `ssh_write` · **用途**: 向当前终端写入交互输入
- **工具**: `ssh_disconnect` · **用途**: 断开当前连接
- **工具**: `ssh_list` · **用途**: 查看当前活动连接的安全元数据（不包含保存资源或秘密）
- **工具**: `sftp_list` · **用途**: 列出远程目录（SFTP）
- **工具**: `sftp_read` · **用途**: 读取远程文件内容
- **工具**: `sftp_write` · **用途**: 写入远程文件
- **工具**: `sftp_mkdir` · **用途**: 新建远程目录
- **工具**: `sftp_delete` · **用途**: 删除远程文件或空目录
- **工具**: `sftp_rename` · **用途**: 重命名/移动远程路径
- **工具**: `tunnel_start` · **用途**: 建立本地或远程端口转发
- **工具**: `tunnel_list` · **用途**: 列出活动隧道
- **工具**: `tunnel_stop` · **用途**: 停止隧道
