...
### 深色 / Dark · 浅色 / Light
- **深色 / Dark**: ![Dark theme](./assets/screenshot-dark.png) · **浅色 / Light**: ![Light theme](./assets/screenshot-light.png)
...
### 路由 · 方法 · 说明
- **路由**: `/archived/list` · **方法**: GET · **说明**: 返回 `{ archived: [{ id, title, createdAt }] }`
- **路由**: `/archived/unarchive` · **方法**: POST · **说明**: body `{ sessionId }`，取消归档
- **路由**: `/archived/delete` · **方法**: POST · **说明**: body `{ sessionId }`，永久删除单个会话
- **路由**: `/archived/delete-project` · **方法**: POST · **说明**: body `{ cwd }`，删除某项目全部归档会话
- **路由**: `/archived/delete-all` · **方法**: POST · **说明**: body `{ confirm: true }`，删除全部归档会话
