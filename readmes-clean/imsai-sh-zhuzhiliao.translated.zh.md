...
### 真实玩具 · 合成兜底实现
- **真实玩具**: 松香黏滑摩擦产生脉冲 · **合成兜底实现**: 锯齿波振荡器，频率随转速升高（55~195 Hz），tanh 软削波增毛糙谐波
- **真实玩具**: 蝉鸣颗粒感 · **合成兜底实现**: 24~45 Hz 正弦低频调幅 + 带通摩擦噪声底
- **真实玩具**: 竹膜 + 筒腔共鸣 · **合成兜底实现**: 三个并联带通共振峰（1050 / 2150 / 3350 Hz）
- **真实玩具**: 转圈带来的"哇——哇——" · **合成兜底实现**: 带通滤波器中心频率随转动相位扫频
...
- 有 Bug、有想法、有更像真玩具的调参，欢迎提 Issue / PR
    ...
    ![](https://api.star-history.com/svg?repos=imsai-sh/zhuzhiliao&type=Date)
