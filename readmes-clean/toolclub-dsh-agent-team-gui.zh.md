...
### 你会得到什么 · 为什么有用
- **你会得到什么**: 每个 Agent 独立的 provider/model 与工具策略 · **为什么有用**: 一个小队可组合强规划、快实现和严审核模型
- **你会得到什么**: 全局持久化的 Agent 与小队 · **为什么有用**: 建一次队伍，即可跨项目、跨对话复用
- **你会得到什么**: 固定顺序或主 Agent 动态编排 · **为什么有用**: 固化可重复流程，或让当前对话的主 Agent 为每次任务安排所有成员
- **你会得到什么**: 串行/并行执行与 spawn/fork/chain 上下文 · **为什么有用**: 按任务选择协作拓扑，不被单一模式限制
- **你会得到什么**: 实时运行中心与官方 token 用量 · **为什么有用**: 查看规划、输出、重试、失败，以及每个成员由 provider 上报的真实 token
...
### 字段 · 类型 · 默认值 · 含义
- **字段**: `defaultProvider` · **类型**: `string` · **默认值**: `spawn` · **含义**: 派单/上下文没有选择其他 provider 时使用的已注册 dsh subagent provider。
- **字段**: `defaultExecutionMode` · **类型**: `serial \ · **默认值**: parallel` · **含义**: `serial` · 默认成员调度方式。
- **字段**: `defaultContextMode` · **类型**: `spawn \ · **默认值**: fork \ · **含义**: chain` · `spawn` · `spawn` 启动无父上下文的新子 Agent；`fork` 包含父会话已完成 turn 的前缀；`chain` 把每个串行成员的文本传给下一位。
...
### 字段 · 必填 · 含义
- **字段**: `name` · **必填**: 是 · **含义**: 显示名称。
- **字段**: `systemPrompt` · **必填**: 是 · **含义**: 传给子 Agent 的角色/persona。
- **字段**: `provider` · **必填**: 是 · **含义**: 已存在的 dsh provider 路由名。
- **字段**: `model` · **必填**: 是 · **含义**: 该 provider 下已有的 model id。
- **字段**: `maxTokens` · **必填**: 否 · **含义**: 单 Agent token 上限。
- **字段**: `toolScope.allow` / `toolScope.deny` · **必填**: 否 · **含义**: 应用于该子 Agent 的 dsh 工具名限制。

小队记录字段：

### 字段 · 必填 · 含义
- **字段**: `name` · **必填**: 是 · **含义**: Settings 与对话选择器使用的全局显示名称。
- **字段**: `members` · **必填**: 是 · **含义**: 小队可用且不重复的 Agent ID。
- **字段**: `collabNote` · **必填**: 否 · **含义**: 加入成员 prompt 的协作说明。
- **字段**: `executionOrder` · **必填**: 否 · **含义**: 包含全部成员的固定完整顺序；省略时由主 Agent 动态编排。
- **字段**: `executionMode` · **必填**: 否 · **含义**: 小队默认值：`serial` 或 `parallel`；省略时回退到插件配置。
- **字段**: `contextMode` · **必填**: 否 · **含义**: 小队默认值：`spawn`、`fork` 或串行专用 `chain`；省略时回退到插件配置。
...
通用形式是 `dsh plugin --profile <name> remove `。本 Web 组合包执行：
