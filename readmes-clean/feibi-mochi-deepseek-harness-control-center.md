...
- **Verification level**: Real environment checked for this release · **Coverage**: Windows + current Edge + DSH Web
- **Verification level**: Automated compatibility checks · **Coverage**: Browser notification failure, in-page fallback, cross-tab fallback, storage fallback, CSS-scale fallback, and synchronous/asynchronous desktop adapters
- **Verification level**: Capability-compatible targets · **Coverage**: Current Chrome, Edge, and Firefox on Windows/macOS/Linux; Safari on macOS; Electron/Tauri-style DSH wrappers that provide the requirements below
...
### Item · Behavior
- **Item**: Token accounting · **Behavior**: Listens to the `llm/stream` event and buckets per provider (`deepseek-official` vs. everything else) and per session; each usage event also locks its contemporaneous official price, so multiple sessions and pricing windows never mix.
- **Item**: Balance · **Behavior**: The `DEEPSEEK_API_KEY` from the credentials seam never leaves this machine except as the `Authorization` header of the official `/user/balance` request.
- **Item**: Session log · **Behavior**: The plugin writes no events; its data lives in `$DSH_HOME/storages/wallet.json`.
- **Item**: Local settings · **Behavior**: Layout, scale, visibility, reminder, and panel settings stay in browser-compatible local storage.
- **Item**: Permanent deletion · **Behavior**: Opt-in and host-gated. The wallet never advertises the action unless the host implements the matching session deletion path.
- **Item**: Model surface · **Behavior**: No tools registered, no prompt injection, zero token cost.
- **Item**: Recharge · **Behavior**: The URL is hardcoded to the official `https://platform.deepseek.com/top_up` and is not user-configurable (anti-phishing).
