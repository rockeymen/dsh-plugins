...
### 功能 · 说明
- **功能**: 📱 抽屉式布局 · **说明**: 手机端侧栏隐藏，对话区占满全屏；☰ 按钮滑出侧栏抽屉，点遮罩关闭
- **功能**: 🖥 桌面端不变 · **说明**: 非移动设备（桌面浏览器 UA）时，界面与原版完全一致
- **功能**: 📊 统计栏 · **说明**: 底部对话统计折叠为"一行预览 + 展开箭头"，点击展开完整统计
- **功能**: ⏱ 消息元信息 · **说明**: 每条消息下方的"时间 · 用时 · TTFT · tok/s"完整显示，紧凑竖线分隔
- **功能**: ⌨️ 回车设置 · **说明**: 设置 → 通用 → **手机端回车键**：回车 = 发送消息，或回车 = 插入换行
- **功能**: 🖼 聊天背景 · **说明**: 设置 → 通用 → **聊天背景**：选择图片、手动裁剪（拖动 + 缩放）、调节**背景模糊**与**毛玻璃**透明度；切换会话自动保留
- **功能**: 💬 AI 气泡与头像 · **说明**: 设置 → 通用 → **AI 聊天气泡**（浅金色开关）；**用户头像 / AI 头像**独立开关、自定义上传（圆形裁剪）、默认头像
- **功能**: 💰 余额显示 · **说明**: 模型选择器中显示供应商余额（DeepSeek `¥` / OpenRouter `$` / OpenAI `$`）；无余额 API 的厂商自动隐藏
- **功能**: 📎 文件上传 · **说明**: 输入 `/` → "上传文件"，或手机端点输入栏左下角 **＋** 按钮 → **上传文件**：选择手机上的文件 → 上传到当前工作目录，路径追加到输入框（可补充说明后发送，不自动发送）
- **功能**: ➕ 加号动作栏 · **说明**: 手机端输入栏左下角 **＋** 按钮弹出选择栏：**命令**（原生命令菜单）/ **上传文件**；桌面端保持原样
- **功能**: 🔍 网页搜索 · **说明**: 设置 → 插件 → 插件配置 → **网页搜索**：配置多个搜索服务（**Exa / Brave / Bing / Tavily / Firecrawl / DeepSeek**），各填 API Key；模型选择器中点 **搜索模型** 入口切换当前搜索服务
- **功能**: 🌐 中英双语 · **说明**: 全部插件界面文字接入应用语言系统，切换语言即时生效
- **功能**: ⚙️ 设置页适配 · **说明**: 手机端设置面板全屏显示、导航横向排列，插件列表完整显示
- **功能**: 🛠 工具管理 · **说明**: 设置 → **工具**：搜索栏按名称/描述实时过滤；每个工具右侧开关可**启用/禁用**，被禁用的工具对模型不可见、不可调用；清单存于 `~/.dsh/dshm-tools-config.json`，重启后依然生效
