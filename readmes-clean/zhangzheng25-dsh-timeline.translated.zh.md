...
### 改了什么 · 生效方式
- **改了什么**: client 代码（`src/client/`，构建后 `lib/client.js`） · **生效方式**: `pnpm run build` 后**刷新页面**即可，无需重启（DSH 对 `/plugins//client.js` 实时读文件、`no-cache`）
- **改了什么**: host 半端（`src/index.ts`）或 `cordis.patch.yml` · **生效方式**: **必须重启 DSH**（启动时加载）
