...
flowchart LR
  subgraph Host["Host 半区（Node.js）"]
    H["src/index.tssettings 命名空间 mobile-uienabled / breakpoint"]
  end
  subgraph Client["浏览器半区（lib/client.js，纯原生 DOM）"]
    C1["MobileUiEnginematchMedia(breakpoint) 驱动body.dsh-mobile 开关类"]
    C2["设置卡片settings.plugin.item 席位"]
    C3["注入样式表 + 顶部操作条+ viewport/PWA meta"]
  end
  DS["DSH settings（设置页 / settings.yaml）"]
  H --> DS --> C1
  DS --> C2
  C1 --> C3
```
...
### 现象 · 原因 · 解决
- **现象**: 窄屏下仍是桌面三栏 · **原因**: `enabled: false` 或断点小于视口宽度 · **解决**: 设置页/`settings.yaml` 检查 `mobile-ui.enabled` 与 `breakpoint`（范围 400–1280，步长 20）
- **现象**: 顶部操作条不出现 · **原因**: 移动布局未激活，或 frame 尚未被观察器挂载 · **解决**: 确认视口 ≤ 断点；刷新页面让 frame 重新挂载；看控制台是否有 `[mobile-ui] mount failed`
- **现象**: 面板按钮不显示 · **原因**: 未检测到 aionui 资源列/预览列 · **解决**: 该按钮只在 frame 内存在 `.aionui-explorer-col` / `.aionui-preview-col` 时显示；未安装 aionui 时属正常
- **现象**: 点 ⛶ 提示「添加到主屏幕」 · **原因**: iOS Safari 无网页 Fullscreen API · **解决**: 浏览器菜单 →「添加到主屏幕」，以 standalone 模式全屏运行
- **现象**: 返回键没反应/直接离开页面 · **原因**: 只在「移动布局 +（全屏 或 standalone）」时拦截返回 · **解决**: 先点 ⛶ 进全屏，或添加到主屏幕后运行；桌面浏览器 Esc 只关层
- **现象**: Android 返回键退出全屏又自动回来 · **原因**: 设计如此：关当前层后立即恢复全屏 · **解决**: 想退出全屏请点 ⛶ 按钮
- **现象**: 设置卡片提示「未暴露」 · **原因**: 当前 DSH 版本未向设置页暴露该命名空间 · **解决**: 直接编辑 `~/.dsh/settings.yaml` 配置；或为 dsh-host-apiproxy 的 `WEB_SETTINGS_NAMESPACES` 白名单补充 `mobile-ui` 后重启
- **现象**: 构建失败 `cannot locate the dsh checkout` · **原因**: 自动探测未命中 · **解决**: 设置 `DSH_CHECKOUT` 环境变量指向 DSH 源码目录后重试
...
### 字段 · 类型 · 默认 · 说明
- **字段**: `enabled` · **类型**: boolean · **默认**: `true` · **说明**: 总开关；关闭即卸载全部适配（样式/操作条/观察器）
- **字段**: `breakpoint` · **类型**: number · **默认**: `860` · **说明**: 视口宽度 ≤ 该值（px）时启用移动布局；范围 400–1280，步长 20
...
### 命令 · 作用
- **命令**: `git clone https://github.com/21hbguo/dsh-mobile-ui.git` · **作用**: 获取源码
- **命令**: `bash scripts/build.sh` · **作用**: 构建 host + client 产物到 `lib/`（需 DSH checkout）
- **命令**: `npm run typecheck` · **作用**: TypeScript 类型检查（`tsc --noEmit`）
- **命令**: `dev_install_package <目录>` · **作用**: DSH 内热装配插件（免重启）
- **命令**: `gh release download` / Releases 页 tgz · **作用**: 获取已构建的 npm 包产物
