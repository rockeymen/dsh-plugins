...
### 图标 · 功能 · 一句话说明
- **图标**: 🚀 · **功能**: **一键导出** · **一句话说明**: 点一下，把推荐配置打包成一个 ZIP
- **图标**: 📦 · **功能**: **一键导入** · **一句话说明**: 在另一台电脑点一下，环境就回来了
- **图标**: 👀 · **功能**: **先预览再导入** · **一句话说明**: 导入前完整预览，**绝不偷偷改你的配置**
- **图标**: ⚔️ · **功能**: **冲突处理** · **一句话说明**: 遇到同名配置，让你自己选：保留现有的 / 用导入的
- **图标**: 🗺️ · **功能**: **路径自动映射** · **一句话说明**: 换了电脑路径变了？自动检测并让你重新指定
- **图标**: 🔒 · **功能**: **密钥安全** · **一句话说明**: API Key 默认不导出；导入后提醒你重新填写
- **图标**: ↩️ · **功能**: **自动回滚** · **一句话说明**: 导入失败自动恢复原样，不会弄坏现有配置
- **图标**: 📸 · **功能**: **快照恢复** · **一句话说明**: 撤销一次导入：整文件还原 + 卸载新增插件（CLI 与 GUI 均支持）
- **图标**: 🔄 · **功能**: **远程同步** · **一句话说明**: 通过 Git 私有仓库推送 / 拉取可移植配置（密钥永不参与同步）
- **图标**: 🗂️ · **功能**: **配置档案 Profiles** · **一句话说明**: 保存多套配置（工作 / 个人），随时切换
...
### 方式 · 说明
- **方式**: **快速导出**（推荐） · **说明**: 一键导出推荐配置：设置 / UI / 模型 / 插件 / MCP / 技能 / 工作区等
- **方式**: **自定义导出** · **说明**: 自己勾选要导出的分类
...
### 选项 · 含义
- **选项**: **保留现有的** · **含义**: 不动目标电脑的原配置
- **选项**: **用导入的** · **含义**: 用备份里的配置覆盖
...
### 场景 · 行为
- **场景**: 默认备份 · **行为**: **不含任何密钥值**，只记录"哪些密钥需要填写"
- **场景**: 加密备份（可选） · **行为**: 设置密码后用 AES-256-GCM 加密，密码**绝不写入备份文件**
- **场景**: 导入后 · **行为**: 提示「3 个密钥需要重新填写」，输入后仅保存在内存中写入
...
### 动作 · 说明
- **动作**: 整文件还原 · **说明**: settings.yaml / settings.json / cordis.patch.yml 的 blob 写回 `$DSH_HOME`；快照时不存在、导入后新增的文件会被移除
- **动作**: 插件卸载 · **说明**: 导入期间新增的插件经官方 `dsh plugin remove` 卸载（与基线对比；旧快照无基线时只给提示）
- **动作**: 文件补偿 · **说明**: skills / agentPresets / pluginFiles / sessions 的 blob 写回原路径
- **动作**: 凭据 · **说明**: DSH 不回读凭据值——只提示人工重新填写
...
### 状态 · 含义
- **状态**: ✅ Excellent · **含义**: 同平台、配置齐全、版本兼容
- **状态**: 👍 Good · **含义**: 备份来自更旧版本的 DSH
- **状态**: ⚠️ Partial · **含义**: 跨平台 / 部分配置缺失 / 备份比当前新
- **状态**: ❌ Unsupported · **含义**: 备份版本超出支持范围（无法导入）
