...
### Entry · Where
- **Entry**: Status text (single persistent entry) · **Where**: Session header top-right (next to the Session log) "Save · 🟢 Working", click to open the settings popover
- **Entry**: System settings page · **Where**: Sidebar → Settings → Save-money
- **Entry**: Floating banner · **Where**: Top capsule when a pause is upcoming / active + **Disable save mode** button (disables the whole feature)

### Dynamic tools (Host)

### Tool · Purpose
- **Tool**: `save_money_status` · **Purpose**: Query state / **gate state (gate: open\ · closed)** / window / pause record / UTC projection
- **Tool**: `save_money_configure` · **Purpose**: Configure (enabled / timezone / warnMinutes / windows)
- **Tool**: `save_money_ignore` · **Purpose**: Skip the current or next window's pause (also releases the gate)
- **Tool**: `save_money_debug_tick` · **Purpose**: Dev tool: manually advance the state machine
