...
### State · Behavior
- **State**: Card present · **Behavior**: The file's absolute path is injected into **every** user message sent while the card is visible
- **State**: Card closed (`×`) · **Behavior**: The plugin calls the host `remove` RPC — the file is dropped from the pending registry and **no longer injected**
- **State**: Ctrl+V paste · **Behavior**: Pasted images ride the native draft-image pipeline — no card is created and the plugin never touches them
...
- **Model type**: **Multimodal** (reported `inputModalities` includes `image`) · **Image files (png/jpeg/webp/gif)**: Native `ImageBlock` via the attachments service — the image is part of the request, like a normal attached image, with **no extra prompt text** · **Other files**: Path text block
- **Model type**: **Text-only** (e.g. DeepSeek V4 series) · **Image files (png/jpeg/webp/gif)**: Path text block — the model can call read/vision tools to inspect them · **Other files**: Path text block
...
### Scenario · Result
- **Scenario**: Same file uploaded N times · **Result**: One copy on disk; all uploads resolve to the same path
- **Scenario**: Same content, different filename · **Result**: Reuses the first stored copy
- **Scenario**: Same name, different content · **Result**: Normal `-1` collision handling (correct)
- **Scenario**: Process restart · **Result**: The index file persists, dedup keeps working
...
### Mode · Destination
- **Mode**: Dynamic plugin · **Destination**: `<session workspace>/uploads/` (sandboxed shell/fs cannot leave the workspace)
- **Mode**: Static bundle · **Destination**: `$DSH_HOME/uploads` (default `~/.dsh/uploads`) via `node:fs` — the dsh data directory
