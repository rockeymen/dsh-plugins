# dsh-complete-notify
  ...
  ![dsh-complete-notify cover](assets/cover.png)
...
### 状态 · 颜色 · 文案
- **状态**: `completed` · **颜色**: 🟢 绿 · **文案**: 任务完成
- **状态**: `blocked` · **颜色**: 🟡 黄 · **文案**: 等待你的反馈（模型提问 / 等待审批）
- **状态**: `aborted` · **颜色**: 🔴 红 · **文案**: 任务已中断
- **状态**: `error` · **颜色**: 🔴 红 · **文案**: 任务失败
- **状态**: `max-tokens` · **颜色**: 🟠 橙 · **文案**: 达到 token 上限

### 弹窗效果预览

  ![任务完成（绿）](assets/screenshot-toast-completed.png)
  ![等待你的反馈（黄）](assets/screenshot-toast-blocked.png)
  ![任务已中断（红）](assets/screenshot-toast-aborted.png)
...
### 场景 · 行为
- **场景**: 页面可见，任一会话完成 · **行为**: toast（小结 + 时长/token/steps）+ 对应状态音效
- **场景**: 页面在后台，会话完成 · **行为**: 系统通知（含小结与统计）+ 对应状态音效 + 标题闪烁
- **场景**: 系统通知权限被拒 · **行为**: 降级为长时 toast（30 秒，回来也能看到）+ 标题闪烁
- **场景**: 一次完成 · **行为**: 只提醒一次（按会话去重，重新运行后再完成会再次提醒）
- **场景**: 子代理（subagent）会话 · **行为**: 不提醒
- **场景**: 多会话同时完成 · **行为**: toast 栈最多 3 条（FIFO）
