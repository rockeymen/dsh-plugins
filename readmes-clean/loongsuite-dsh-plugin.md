...
> market.
  ...
  ![One DeepSeek Harness turn as an OpenTelemetry GenAI trace, viewed in self-hosted Langfuse](docs/assets/langfuse-trace.png)
...
- **Component**: DeepSeek Harness · **Supported range**: `>=0.1.0-rc.6 <0.2.0` · **Fully verified version(s)**: `0.1.0-rc.6` headless and Web profiles
- **Component**: Node.js · **Supported range**: `>=22.19.0` · **Fully verified version(s)**: `22.19`, `24.19`, and `25.9` on macOS
...
`$DSH_HOME/profiles//cordis.patch.yml` (by default under `~/.dsh`):
...
- **Setting**: `enabled` · **Default**: `true` · **Meaning**: Disable collection without uninstalling the bundle.
- **Setting**: `endpoint` · **Default**: unset · **Meaning**: Shared OTLP/HTTP base URL; the plugin appends each signal path.
- **Setting**: `traceEndpoint` / `metricEndpoint` · **Default**: unset · **Meaning**: Complete signal-specific URL; takes precedence over `endpoint`.
- **Setting**: `headers` · **Default**: `{}` · **Meaning**: Headers added to both exporters.
- **Setting**: `serviceName` · **Default**: `OTEL_SERVICE_NAME` or `deepseek-harness` · **Meaning**: OpenTelemetry `service.name`.
- **Setting**: `resourceAttributes` · **Default**: `{}` · **Meaning**: Additional string-valued resource attributes.
- **Setting**: `captureContent` · **Default**: environment setting or `false` · **Meaning**: Export prompts, responses, tool definitions, arguments, and results.
- **Setting**: `contentMaxChars` · **Default**: `128000` · **Meaning**: Maximum serialized characters per captured content attribute.
- **Setting**: `exportMetrics` · **Default**: environment setting or `true` · **Meaning**: Export LLM duration and token metrics.
- **Setting**: `maxExportBatchSize` · **Default**: `512` · **Meaning**: Maximum spans per export batch.
- **Setting**: `maxQueueSize` · **Default**: `2048` · **Meaning**: Maximum queued spans. Must not be smaller than the batch size.
- **Setting**: `traceExportIntervalMs` · **Default**: `5000` · **Meaning**: Trace batch delay.
- **Setting**: `metricExportIntervalMs` · **Default**: `60000` · **Meaning**: Metric export interval.
- **Setting**: `exportTimeoutMs` · **Default**: `30000` · **Meaning**: OTLP export timeout.
- **Setting**: `debug` · **Default**: `false` · **Meaning**: Emit additional plugin lifecycle diagnostics through the DSH logger.
