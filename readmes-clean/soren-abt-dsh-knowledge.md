...
### 插件 · 平台 · 职责
- **插件**: `knowledge`（`ctx.knowledge`） · **平台**: host · **职责**: 核心引擎：存储域、分块、embedding、检索、`/knowledge/*` HTTP 服务
- **插件**: `tool-knowledge` · **平台**: host · **职责**: 12 个模型工具，消费 `ctx.knowledge`
- **插件**: `ui-knowledge` · **平台**: client · **职责**: 侧边栏底部入口（`sidebar.footer.action`）+ 工作区整页浮层（`shell.overlay`），Cherry Studio 式布局
...
### 字段 · 默认 · 说明
- **字段**: `embeddingProvider` · **默认**: `none` · **说明**: `openai` / `ollama` / `local`（进程内 transformers.js）/ `none`
- **字段**: `embeddingBaseUrl` · **默认**: `''` · **说明**: 端点基址，如 `https://api.openai.com/v1` 或 `http://127.0.0.1:11434`（`local` 不需要）
- **字段**: `embeddingModel` · **默认**: `''` · **说明**: 如 `text-embedding-3-small`；`local` 时为 Hugging Face 仓库 id（默认 `onnx-community/Qwen3-Embedding-0.6B-ONNX`）
- **字段**: `embeddingApiKey` · **默认**: `''` · **说明**: 可选；也可用环境变量 `KNOWLEDGE_API_KEY`
- **字段**: `rerankModel` / `rerankBaseUrl` / `rerankApiKey` · **默认**: `''` · **说明**: 重排模型（留空=不启用），Jina / SiliconFlow / Cohere v2 风格接口
- **字段**: `smartChunk` · **默认**: `true` · **说明**: 智能分段（标题/段落感知）；关闭后仅按 `chunkSeparator` 切分
- **字段**: `chunkSeparator` · **默认**: `\n\n` · **说明**: 智能分段关闭时的段落边界（可写 `\n`）
- **字段**: `chunkSize` · **默认**: `800` · **说明**: 分块字符数
- **字段**: `chunkOverlap` · **默认**: `100` · **说明**: 相邻分块重叠字符数
- **字段**: `topK` · **默认**: `6` · **说明**: 检索返回条数（1–50）
- **字段**: `searchMode` · **默认**: `auto` · **说明**: `auto` / `hybrid` / `vector` / `lexical`
- **字段**: `similarityThreshold` · **默认**: `0` · **说明**: 相似度阈值（0–1），低于该分数的结果被过滤
- **字段**: `mmrDiversity` · **默认**: `0` · **说明**: MMR 结果多样性（0–1，0=关闭）
- **字段**: `embeddingBatchSize` · **默认**: `32` · **说明**: 每次 embedding 请求的文本条数
- **字段**: `localModelCacheDir` · **默认**: `''` · **说明**: 本地模型缓存根目录；留空 = `<DSH_HOME>/cache/dsh-knowledge/local-models`（`DSH_HOME` 未设则为 `~/.dsh`）
- **字段**: `chunkStorePath` · **默认**: `''` · **说明**: 分块 SQLite 文件；留空 = `<DSH_HOME>/storages/knowledge-chunks.sqlite`
...
### 题型 · 纯词法 · 混合 · 纯向量
- **题型**: 直答型（问题含主题词，14 题） · **纯词法**: **0.929** · **混合**: 0.857 · **纯向量**: —
- **题型**: 换说法型（问题不含主题词，10 题） · **纯词法**: 0.600 · **混合**: 0.900（MRR 0.575） · **纯向量**: 0.900（**MRR 0.628**）

直答型问题纯词法已足够；本地模型向量的真实价值体现在换说法型问题——向量检索把 Hit@5 从 0.600 提升到 0.900。可对任意知识库复跑：

```bash
node scripts/eval-retrieval.mjs --file scripts/eval-rephrase.json --base  --mode hybrid
```
