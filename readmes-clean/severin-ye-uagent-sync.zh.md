...
### 类别 · 内容 · 方式
- **类别**: **子模块** · **内容**: 所有仓库，精确提交号 · **方式**: `git clone` + `git reset --hard`
- **类别**: **OpenCode 配置** · **内容**: 插件、MCP 服务器、模型供应商 · **方式**: 深度合并，绝不覆盖
- **类别**: **技能** · **内容**: 从 git 源安装的技能包 · **方式**: `skills add <源> -g`
- **类别**: **API 密钥** · **内容**: 名称 + 说明（绝不包含值） · **方式**: 模板文件 `keys/API.md` —— `keys/` 目录在 `usync-dotfiles` 中已 gitignore，真实值只存在于本机
- **类别**: **依赖** · **内容**: gh CLI、Ralph、Skills CLI · **方式**: winget/brew/apt/npm 自动安装
- **类别**: **Windows 修复** · **内容**: NTFS 路径问题 · **方式**: 自动检测问题文件名，应用 `git config core.protectNTFS`
- **类别**: **安装日志** · **内容**: 每次安装的来源与踩坑 · **方式**: `state/install-log.json` —— 可追溯
...
### 命令 · 作用
- **命令**: `init` · **作用**: 检测工作区，引导首次设置。只问一次。
- **命令**: `push` · **作用**: 导出状态 → 提交 → 推送到 GitHub。一条命令。
- **命令**: `pull` · **作用**: 从 GitHub 拉取 → 恢复一切。一条命令。
- **命令**: `export` · **作用**: 导出完整工作区状态为 JSON
- **命令**: `import` · **作用**: 从 JSON/URL 恢复（支持 `--dry-run` 预览）
- **命令**: `diff` · **作用**: 对比当前状态与已保存状态
- **命令**: `status` · **作用**: 查看每个子模块：提交、分支、是否脏
- **命令**: `verify` · **作用**: 环境健康检查：gh、git、配置、ralph、技能、子模块
- **命令**: `setup` · **作用**: 安装一切：gh、子模块、配置、ralph、Skills CLI、技能包
- **命令**: `create-repo` · **作用**: 创建**私有** GitHub 仓库（公开会警告）
- **命令**: `api-keys` · **作用**: 检测、生成模板或添加 API 密钥
- **命令**: `guide` · **作用**: 生成 `guide/SYNC-GUIDE.md` —— 恢复手册
- **命令**: `log` · **作用**: 读写安装溯源日志
- **命令**: `crystallize` · **作用**: 记录安装 + 重生成文档 + 导出状态 + 一键提交
- **命令**: `update` · **作用**: 更新智能体生态：插件、技能、MCP 工具、同步仓库、配置依赖
- **命令**: `changelog` · **作用**: 从最新更新报告起草分类变更日志
- **命令**: `inventory` · **作用**: 只读扫描 Codex/OpenCode/DeepSeek Harness 配置（不含密钥值）
- **命令**: `dashboard` · **作用**: 启动本地只读配置看板（默认监听 `127.0.0.1`）
