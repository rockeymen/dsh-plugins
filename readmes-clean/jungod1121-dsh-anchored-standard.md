...
- **Preset**: Standard · **First request sees**: 25 tools + long prompt · **Full task sees**: 25 tools · **Project2 V4.1b (max)**: **91 / 92**
- **Preset**: Minimal · **First request sees**: 1-line prompt + 2 tools · **Full task sees**: 2 tools only · **Project2 V4.1b (max)**: **99 / 96** — but too narrow for real work
- **Preset**: **Anchored Standard** · **First request sees**: 1-line prompt + 2 tools · **Full task sees**: **all 25 tools** · **Project2 V4.1b (max)**: **98 / 99**
...
- **Task**: **spec** — fix / maintain / debug · **Persona**: Minimal's exact prompt · **First-request tools**: `bash` + `read` + `edit`
- **Task**: **react** — build / create from zero · **Persona**: hands-on doer prompt · **First-request tools**: `bash` + `read` + `write`
- **Task**: **weak** — ambiguous · **Persona**: model self-picks (Pro: classify instruction) · **First-request tools**: `bash` + `read`
...
###  · You get · Trade-off
- vs Minimal · **You get**: 23 more tools: glob/grep/edit/write/web_search/subagents/workflows/skills/goals/todo… · **Trade-off**: one extra reasoning round for the bootstrap
- vs Standard · **You get**: task-matched trajectory anchors (98/99 vs 91/92 in the reference eval) + a first turn that is cleaner than Standard's · **Trade-off**: the first-request catalog is decided by keyword classification — ambiguous tasks fall back to the weak anchor and let the model pick
