...
### 冰原 · 深水
- **冰原**: ![冰原壁纸下的新会话](docs/preview/ice-hero.jpg) · **深水**: ![深水壁纸下的对话](docs/preview/deepwater-chat.jpg)

### 设置 · 自定义壁纸
- **设置**: ![设置里选择壁纸](docs/preview/settings.jpg) · **自定义壁纸**: ![导入的自定义壁纸](docs/preview/custom-wallpaper.jpg)
...
### 控件 · 作用
- **控件**: Wallpaper · **作用**: 预设 / 导入。滑杆只改壁纸透明度。
- **控件**: Glass blur · **作用**: 每个玻璃岛背后的 backdrop blur。壁纸保持清晰。
- **控件**: Liquid Glass · **作用**: 开/关材质。关了恢复官方表面，壁纸可单独留着。
...
### 面 · 文件 · 说明
- **面**: Host · **文件**: `host.js` · **说明**: 托管冰原 / 深水 JPEG。哈希 URL 一年 `immutable`，legacy URL `no-cache`。
- **面**: Client · **文件**: `client.js` · **说明**: `__ModuleLoader__` bundle：设置行 + 壁纸层 + 玻璃岛 CSS。源码在 `src/client/`，`client.js` 由 `npm run build` 生成。
