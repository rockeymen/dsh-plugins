...
### 字段 · 类型 · 说明
- **字段**: `global` · **类型**: `Record<string,string>` · **说明**: 所有请求注入
- **字段**: `providers` · **类型**: `Record<string, Record<string,string>>` · **说明**: 按 provider 路由注入
- **字段**: `models` · **类型**: `Record<'provider/model', Record<string,string>>` · **说明**: 按模型注入（最细粒度）
- **字段**: `hosts` · **类型**: `string[]` · **说明**: 目标主机后缀；空 = 除 `.local` 外全部（回环放行）
- **字段**: `fill` · **类型**: `boolean` · **说明**: 默认 `false`（覆盖同名头）；`true` 保留适配器头
