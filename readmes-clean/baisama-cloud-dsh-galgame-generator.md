...
### 放什么 · 位置
- **放什么**: 剧本文档 · **位置**: 工作区根目录任意 `.md` / `.txt`（如 `夏日回忆.md`）
- **放什么**: 人物立绘 · **位置**: `img_human/`（如 `img_human/mei.png`）
- **放什么**: 背景图片 · **位置**: `img_bg/`（如 `img_bg/classroom.png`）
- **放什么**: 背景音乐 · **位置**: `audio/`（如 `audio/bgm.mp3`）
- **放什么**: 开局/结束动画、CG 插图 · **位置**: `img_cg/`（如 `img_cg/opening.gif`、`img_cg/cg1.gif`、`img_cg/ending.mp4`）
...
### 写法 · 含义
- **写法**: `名字: 台词` · **含义**: 角色台词（自动显示该角色立绘）
- **写法**: `（旁白）` 或纯文本行 · **含义**: 旁白（立绘全部淡出）
- **写法**: `[bg 背景名]` · **含义**: 切换背景
- **写法**: `[show 名字 @ left/center/right]` / `[hide 名字]` · **含义**: 显式显示/隐藏并锁定立绘
- **写法**: `[bgm 文件.mp3]` / `[bgm off]` / `[bgm 文件.mp3 0.5]` · **含义**: 音乐播放/停止/音量
- **写法**: `[变量 名字 = 值]` / `[变量 名字 + 1]` · **含义**: 变量赋值/增减
- **写法**: `[if 变量 >= 2]` … `[else]` … `[endif]` · **含义**: 条件分支（支持 = == != > >= < <= 及中文）
- **写法**: `[选项]` + `- 文本 → 标签` · **含义**: 分支选项
- **写法**: `### 标签名` / `[跳转 标签]` · **含义**: 跳转点与跳转
- **写法**: `[结束 结局名]` · **含义**: 结束游戏