...
### 你已有的东西 · DSH 对应机制 · 说明
- **你已有的东西**: 一段脚本 / 函数（bash、TS、Python）供 agent 调用 · **DSH 对应机制**: **工具**：`defineTool()` + `ctx.tools.register` · **说明**: 参数 schema 自动校验 + 推导 `args` 类型
- **你已有的东西**: Claude Code **skill**（`SKILL.md`） · **DSH 对应机制**: DSH **skill**（`dsh-skill` 注册表 + `skill-filesystem`）——同样是 SKILL.md 的思路 · **说明**: 若本质是单次调用，也可包成工具
- **你已有的东西**: Claude Code **斜杠命令** · **DSH 对应机制**: DSH **人类命令**：`ctx.commands` · **说明**: 不经过模型回合直接派发
- **你已有的东西**: Claude Code **hook**（`PreToolUse`、`PostToolUse`、`SessionStart`…） · **DSH 对应机制**: DSH **事件**：`ctx.on('tools/pre-execute')`、`ctx.on('tools/post-execute')`、`ctx.on('session/event')`、`ctx.on('agent/pre-step')`… · **说明**: hook → 类型化事件；waterfall 里用 `next()` 委托
- **你已有的东西**: OpenClaw **hook / 中间件** · **DSH 对应机制**: 同样的 DSH **事件**接缝 · **说明**: 按事件所属域挑选
- **你已有的东西**: OpenClaw **工具** · **DSH 对应机制**: **工具**：`defineTool()` · **说明**: 返回规范化 JSON 值，不返回散文
- **你已有的东西**: **MCP server** · **DSH 对应机制**: DSH **MCP 客户端**（`dsh-mcp-client`） · **说明**: 追求低延迟也可包成原生工具
- **你已有的东西**: **子代理** · **DSH 对应机制**: DSH **subagent**（`dsh-subagent` + `tool-subagent`） · **说明**: 提供方：spawn / fork / codex / claude-code
- **你已有的东西**: **记忆 / 持久化** · **DSH 对应机制**: `ctx.effect()` + DSH **storage** 服务，或**会话事件** · **说明**: 会话日志是持久事实源
- **你已有的东西**: **后台任务** · **DSH 对应机制**: `ctx.jobs` + `tool-jobs` · **说明**: `run_in_background` 语义
- **你已有的东西**: 配置 / 环境开关 · **DSH 对应机制**: Schemastery **`Config`** 导出 · **说明**: 加载期即校验
- **你已有的东西**: 分发 · **DSH 对应机制**: **bundle**（`dsh.bundle` + `cordis.patch.yml`）+ **profile** · **说明**: `dsh plugin add`
...
### 你的 hook · DSH 事件 · 模式
- **你的 hook**: `PreToolUse`（允许/拒绝/询问） · **DSH 事件**: `tools/pre-execute` · **模式**: waterfall
- **你的 hook**: `PostToolUse`（改写/拦截/标注结果） · **DSH 事件**: `tools/post-execute` · **模式**: waterfall
- **你的 hook**: `Notification` / 会话开始 · **DSH 事件**: `session/event`、`session/created` · **模式**: emit
- **你的 hook**: `UserPromptSubmit` · **DSH 事件**: `agent/pre-step` · **模式**: waterfall
- **你的 hook**: `Stop` / `SubagentStop` · **DSH 事件**: `agent/turn-stopping` · **模式**: serial
...
export function apply(ctx: Context) {
  ...
  ctx.on('tools/pre-execute', (exec: ToolExecution, next: () => Promise) => {
