...
### 工具 · 作用
- **工具**: `memoir_record(section, title?, content)` · **作用**: 记录一条记忆，`section` 取值 `work` / `lessons` / `actions` / `note`
- **工具**: `memoir_read(scope?, section?, query?)` · **作用**: 读取记忆，`scope` 取值 `project`（默认）/ `global` / `all`

面板（`/api/dsh-memoir/*`）：

### 界面 · 说明
- **界面**: 项目记忆 tab · **说明**: 当前项目会话的记忆，按 工作记录 / 经验教训 / 行动指南 / 备注 分组，显示时间、标题、正文与会话来源
- **界面**: 全局记忆 tab · **说明**: 所有项目的记忆桶（项目名、路径、更新时间、条数），支持跨项目检索
- **界面**: 搜索框 · **说明**: 标题与正文的实时模糊过滤
- **界面**: 添加记忆 · **说明**: 手动记录一条（分类 + 标题 + 正文），与 agent 的 `memoir_record` 写入同一份数据
- **界面**: 删除 · **说明**: 每条记忆可单独删除，删除后自动重新生成项目记忆文件
...
### 场景 · 怎么用
- **场景**: 反复出现的环境坑（乱码 / 转义 / 路径 / 权限） · **怎么用**: 解决后记一条 `lessons`，附可复制的修复命令
- **场景**: 项目红线与约定（禁 emoji、发布前跑测试、分支规范） · **怎么用**: 记入 `actions`，自动注入给接手者
- **场景**: 难查 bug 的根因与结论 · **怎么用**: 记入 `lessons` / `work`，避免重复排查
- **场景**: 部署 / 上线的固定步骤清单 · **怎么用**: 记入 `actions`，新会话照单执行
- **场景**: 跨项目复用经验 · **怎么用**: 面板全局 tab 或 `memoir_read(scope: 'global', query: ...)` 检索
