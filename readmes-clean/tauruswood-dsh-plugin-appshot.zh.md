...
### ✅ 截取 · ❌ 不截取
- **✅ 截取**: 触发瞬间正在操作的那**一个**前台窗口（Chrome / VS Code / Finder / Terminal…） · **❌ 不截取**: 整个屏幕、桌面壁纸、菜单栏/Dock、其他窗口、后台应用
...
- **触发前（正在操作的前台窗口）**: ![触发前](docs/assets/before-double-command.png) · **触发后（自动截取并挂入 Composer）**: ![触发后](docs/assets/after-double-command.png)
...
### 权限 · 用途
- **权限**: **屏幕录制 (Screen Recording)** · **用途**: ScreenCaptureKit 捕获前台窗口画面
- **权限**: **辅助功能 (Accessibility)** · **用途**: 全局按键状态机监听 + 窗口唤起置顶
