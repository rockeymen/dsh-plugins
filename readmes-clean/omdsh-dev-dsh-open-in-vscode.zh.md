...
- 主机侧用配置的编辑器 CLI 打开该目录（默认 `code `），进程分离，
  编辑器比服务器活得更久。
...
重启 Web 服务器（`kill -TERM ` 并等待退出——切勿 `kill -9`，会撕裂
...
### 键 · 默认值 · 含义
- **键**: `command` · **默认值**: `code` · **含义**: 打开目录的可执行文件。默认值还会查找 Windows 的标准 VS Code 安装目录；其他命令按 PATH 解析。
- **键**: `args` · **默认值**: `[]` · **含义**: 目录路径前附加的参数。
...
### 动作 · 在哪里执行 · 是否需要审批
- **动作**: 在编辑器中打开工作区目录 · **在哪里执行**: 主机（用户手势） · **是否需要审批**: 否——用户主动点击了该行
- **动作**: 其他 · **在哪里执行**: — · **是否需要审批**: 插件没有工具、没有设置命名空间、没有任何模型可见面
