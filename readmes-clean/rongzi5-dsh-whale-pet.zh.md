...
可查看、删除，或手动写一条；聊天框本身不展示历史。写入有三条路：模型仍可输出
`[记住] 事实`；协调器也会从用户原话抽出第一人称事实（「我叫… / 我喜欢… / 记住：…」）；
面板里可以直接添加。60 字只限制给用户看的那句闲聊，`[记住]` 行不受限。
气泡展示前会剥掉标记行。
...
"进度 45%"，见 `GET /api/whale-pet/progress?session=`）。这份快照只读，
...
### 会话状态 · 桌宠反应
- **会话状态**: 正在产生 token 或运行工具 · **桌宠反应**: `working`/`thinking`：游动加快、看向输入区、周期性吐泡泡
- **会话状态**: 单轮持续超过 20s · **桌宠反应**: `focused`：轻微下潜/专注姿态
- **会话状态**: 工具失败（非零退出码或错误节点） · **桌宠反应**: `error`：连续冒汗 + 身体颤抖 + 红色"！"脉冲标记 + 瞪眼，持续 3 秒
- **会话状态**: 长任务完成（≥15s）、goal 完成或退出 plan · **桌宠反应**: `celebrating`：沿大椭圆 360° 绕圈，yaw 连续旋转并带近大远小深度，同时每 650ms 冒爱心
- **会话状态**: 你在输入框聚焦/打字（准备回复） · **桌宠反应**: `listening`：看向输入区、头顶漂浮 "？"，点击会回顾"在呢，我听着～"
- **会话状态**: 会话卡在等你（`pendingInteraction`：审批 / 提问 / 计划过目） · **桌宠反应**: `awaiting`：同样看向输入区并冒 "？"；点击回顾「有个审批等你拍板」/「有个问题等你回答」/「有个计划等你过目」。压过 `working`，因为卡住时回合往往仍标成 running
- **会话状态**: 大力或长时间拖拽后松手 · **桌宠反应**: `dizzy`：锁定鼠标交互、翻出白肚并轻微摇晃，腹部周围显示带景深的椭圆星轨，持续约 4 秒
- **会话状态**: 睡觉时悬停或拖拽 · **桌宠反应**: 立即醒来并重置空闲计时
- **会话状态**: 60s 无活动 · **桌宠反应**: `sleeping`：闭眼、呼吸和游动变慢、显示 z-z-z
- **会话状态**: 当天第一次空闲/睡觉 · **桌宠反应**: 主动说一句「今天也在～」/「又见面啦，第 N 天」；每天最多一次
- **会话状态**: 绑定稳定窗口后出现 compaction 节点 · **桌宠反应**: 说一句「记忆被压扁了一点，我还在～」；不跑定时 LLM
