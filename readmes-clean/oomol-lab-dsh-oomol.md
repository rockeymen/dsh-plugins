...
### Capability · Status
- **Capability**: Public npm package · **Status**: Available
- **Capability**: Hosted OOMOL MCP connection · **Status**: Available
- **Capability**: Progressive Connector and Action discovery · **Status**: Available
- **Capability**: Connector Action execution · **Status**: Available
- **Capability**: Write-only key configuration in Settings · **Status**: Available
- **Capability**: Live connection status and connection test · **Status**: Available
- **Capability**: Native right-side connection manager · **Status**: Available
- **Capability**: Personal OOMOL identity · **Status**: Available
- **Capability**: Team identity through the launch environment · **Status**: Available
- **Capability**: Team selection in Settings · **Status**: Planned
- **Capability**: Passwordless MCP-key pairing · **Status**: Planned
- **Capability**: Action-aware approval presentation · **Status**: Planned
- **Capability**: Workflow authoring and execution · **Status**: Not included
...
- **Symptom**: npm reports that `dsh-oomol` cannot be found · **Likely cause**: npm is using a different registry, or its metadata cache is stale · **What to do**: Verify that `npm config get registry` returns `https://registry.npmjs.org/`, then retry
- **Symptom**: The plugin does not appear in Settings · **Likely cause**: It was installed into another profile, or Harness has not restarted · **What to do**: Install into the `web` profile and restart `dsh web`
- **Symptom**: The key is shown as not configured · **Likely cause**: No key was saved for this profile · **What to do**: Save it in **Settings > Plugins > OOMOL Connector**
- **Symptom**: Connection state is **Unauthorized** · **Likely cause**: The OOMOL MCP key is invalid, expired, or revoked · **What to do**: Create a dedicated replacement key and save it again
- **Symptom**: Connection succeeds but the expected app is missing · **Likely cause**: The Provider is not connected, or the selected team identity is wrong · **What to do**: Check [OOMOL connections](https://console.oomol.com/connections) and `OOMOL_TEAM_NAME`
- **Symptom**: A launch-environment key cannot be changed in Settings · **Likely cause**: Environment credentials are read-only in the browser · **What to do**: Change `OOMOL_MCP_API_KEY` where Harness is launched and restart it
- **Symptom**: The plugin fails after an upgrade · **Likely cause**: Installed Harness and plugin dependencies are out of sync, or a compatibility regression occurred · **What to do**: Update `dsh-oomol`, restart Harness, and open an issue with both installed versions if the problem remains
