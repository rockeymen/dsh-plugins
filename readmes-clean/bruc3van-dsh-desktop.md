...
因此本项目只做一件事：**把官方 Web UI 原样装进原生窗口，把全部工程投入放在连接、安全、执行环境与桌面体验上——托盘常驻、双击即用、智能连接、应用内更新。** 这样做的好处是天然成立的：

### 用户需要什么 · 我们的做法
- **用户需要什么**: Harness 的全部能力，一个都不能少 · **我们的做法**: 窗口里就是官方 Web UI 本体，不用重写界面
- **用户需要什么**: 官方新功能要当天可用 · **我们的做法**: 跟进速度与自己的发版解耦：智能模式复用你已有的最新 dsh；内置运行时只是回退
- **用户需要什么**: 装好就能用，不折腾环境 · **我们的做法**: 安装包自带官方运行时：不用装 Node.js、不用敲命令，双击即用
- **用户需要什么**: 安全与隐私是底线 · **我们的做法**: 小边界 + 逐层加固：只用公开接口；更新链路防劫持；权限最小化
- **用户需要什么**: Agent 要跑在你自己的环境里 · **我们的做法**: 优先复用你的实例 / PATH 上的 `dsh` / npx 缓存包；macOS 对齐登录 shell 的 PATH；`node` 始终可用
- **用户需要什么**: 工作要在终端、桌面、远程机器之间延续 · **我们的做法**: 智能模式与 CLI、浏览器共享 `~/.dsh`；固定地址模式连接远程 dsh
- **用户需要什么**: 长任务不被浏览器标签页绑架 · **我们的做法**: 关闭窗口不退出应用，托盘常驻，随点随开
...
### 模式 · 适合场景 · 实际行为
- **模式**: **智能模式**（默认） · **适合场景**: 大多数本机用户 · **实际行为**: 依次尝试：复用 `127.0.0.1:3080` 上的官方实例 → PATH 上的 `dsh` → npx 缓存中的官方包 → 内置运行时。
- **模式**: **固定地址模式** · **适合场景**: 另一台机器、容器或自行维护的运行时 · **实际行为**: 直接连接你填写的 Web UI 地址，不启动任何运行时。
...
### 状态 · 含义
- **状态**: 复用你已启动的 dsh · **含义**: 复用你自己在终端里跑起来的实例（默认 `127.0.0.1:3080`）
- **状态**: 客户端启动·内置运行时 · **含义**: 客户端用安装包自带的运行时起的后台子进程
- **状态**: 客户端启动·npx 缓存的 dsh · **含义**: 客户端用 npx 缓存里那份官方包起的后台子进程
- **状态**: 客户端启动·本机安装的 dsh · **含义**: 客户端用 PATH 上你自己装的 `dsh` 起的后台子进程
- **状态**: 固定地址 · **含义**: 直连你填写的地址，客户端不启动任何运行时
...
### 数据 · 默认位置 · 管理方
- **数据**: 会话、凭据、模型配置与其他官方 Harness 状态 · **默认位置**: `~/.dsh` · **管理方**: 官方 `dsh` 运行时
- **数据**: 桌面端连接偏好 · **默认位置**: `~/.dsh-desktop/settings.json` · **管理方**: 桌面客户端
