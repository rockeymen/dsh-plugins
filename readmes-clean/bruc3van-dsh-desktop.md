...
- **更少的启动步骤**：智能模式会优先复用电脑上已经运行的官方 Web UI；没有可用实例时，再用你自己安装的 `dsh`（如果有）或内置运行时替你在后台启动 `dsh web`。
- **工作上下文自然延续**：智能模式与 `dsh` CLI、浏览器版共用 `~/.dsh`，已有会话、标题、凭据和模型配置无需搬家。
...
- **版本固定**：内置运行时版本随安装包发布，不能单独升级。需要紧跟官方最新版本时，请使用固定地址模式连接自行维护的运行时。
