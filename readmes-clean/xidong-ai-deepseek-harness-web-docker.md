...
- **Variable**: `DSH_AUTH_USER` · **Required**: Yes · **Default**: `admin` · **Description**: Basic Auth username
- **Variable**: `DSH_AUTH_PASSWORD` · **Required**: Yes · **Default**: None · **Description**: Basic Auth plaintext password (bcrypt hash generated automatically at container startup)
- **Variable**: `DEEPSEEK_API_KEY` · **Required**: Yes · **Default**: None · **Description**: DeepSeek API Key (referenced by the provider via apiKeyEnv)
- **Variable**: `DSH_WEB_PORT` · **Required**: No · **Default**: `3080` · **Description**: Host port exposed to the outside (change it when it conflicts with an existing service)
- **Variable**: `DSH_TRUSTED_HOSTS` · **Required**: No · **Default**: Empty · **Description**: Comma-separated extra trusted hosts, injected into the profile's `cordis.patch.yml`; by default Caddy rewrites Host/Origin to loopback, which covers normal access
- **Variable**: `DSH_VERSION` · **Required**: No (build-time) · **Default**: `0.1.0-rc.6` · **Description**: dsh version; rebuild with `--build` after changing it
...
- Newly installed tools are invoked with the `x ` prefix in the current session; bare commands become available after the container restarts
