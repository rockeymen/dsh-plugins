...
###  · 能力 · 说明
- 🎯 · **能力**: **中转站归属** · **说明**: 按 provider 的 `baseURL` 归一化 origin 分组——同一站的多把 key 合成一行，站名就是域名，不是你自己起的路由别名
- 🔍 · **能力**: **零配置发现** · **说明**: 从宿主的 provider 配置里读出中转站，不用你再填一遍。只读 `baseURL`，绝不碰旁边的凭据
- 💳 · **能力**: **余额** · **说明**: DeepSeek 官方、New API、Sub2API，每个账户一把普通 key 即可；不限额度的 key 报"已用"而不是假余额
- 📊 · **能力**: **用量分析** · **说明**: 今日/本月/累计三窗口、按站点/模型下钻、缓存命中率、一年活跃度热力图（悬停看当天模型构成）
- 🧮 · **能力**: **费用估算** · **说明**: 生效日期分段的费率表、分桶计价、峰谷时段；未定价的模型显示破折号而不是 0
- 🗂 · **能力**: **导出与诊断** · **说明**: CSV / JSON 导出，索引健康度，归因不上的行数单独列出
- 🔒 · **能力**: **只读回环** · **说明**: 两个端点仅接受回环 GET，且在 peer socket 地址上设防；从不读取提示词、工具参数或响应内容
...
### Provider · 模式 · 凭据 · 上游接口
- **Provider**: DeepSeek 官方 · **模式**: 余额 · **凭据**: provider 的 `apiKeyEnv` · **上游接口**: `/user/balance`
- **Provider**: New API 系（含 One API、VoAPI 等分支） · **模式**: 额度 · **凭据**: provider 的 `apiKeyEnv` · **上游接口**: `/api/usage/token/` + `/api/status`
- **Provider**: Sub2API · **模式**: 余额 · **凭据**: provider 的 `apiKeyEnv` · **上游接口**: `/v1/usage`
- **Provider**: Moonshot / Kimi · **模式**: 余额 · **凭据**: provider 的 `apiKeyEnv` · **上游接口**: `/v1/users/me/balance`
- **Provider**: 智谱 GLM / Z.ai · **模式**: 余额 · **凭据**: provider 的 `apiKeyEnv` · **上游接口**: `/api/paas/v4/balance`
- **Provider**: OpenRouter · **模式**: 余额 · **凭据**: **Management Key** · **上游接口**: `/api/v1/credits`
...
### 端点 · 说明
- **端点**: `GET /api/tokenledger/usage?days=&site=` · **说明**: 整个面板的数据：三窗口合计、按天/模型/站点、一年活跃度与逐日模型构成、账户列表、索引诊断
- **端点**: `GET /api/tokenledger/balance?account=` · **说明**: 某个账户的余额
