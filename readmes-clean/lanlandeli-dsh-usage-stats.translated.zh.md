...
### 模块 · 说明
- **模块**: 📈 **使用概览** · **说明**: 展示 Token、会话、消息、活跃天数、连续使用天数及最常用模型的历史累计值
- **模块**: 📊 **每日趋势** · **说明**: 展示最近 7 天或 30 天的 Token 变化，悬停可查看指定日期的模型用量
- **模块**: 🔥 **活跃热力图** · **说明**: 以颜色深度表示一年内各日期的 Token 用量，悬停可查看 Token 数量及调用轮次
- **模块**: 🎯 **范围筛选** · **说明**: 支持按工作区、主任务或子任务限定统计范围
- **模块**: 💾 **数据导出** · **说明**: 支持导出 CSV 或 JSON，用于归档或进一步分析
- **模块**: 🌐 **中英文界面** · **说明**: 自动跟随 Harness 的语言设置切换中文或英文
- **模块**: 🎨 **主题适配** · **说明**: 自动跟随 Harness 的浅色或深色主题
- **模块**: ⚡ **轻量运行** · **说明**: 无第三方图表库、无后台轮询，减少额外的网络请求与运行开销
...
### 配置项 · 说明 · 默认值
- **配置项**: `indexConcurrency` · **说明**: 同时读取历史会话的数量（`1`–`8`） · **默认值**: `2`
- **配置项**: `cacheWriteDelayMs` · **说明**: 更新本地统计前的等待时间（毫秒） · **默认值**: `1000`
- **配置项**: `cachePath` · **说明**: 自定义统计缓存位置 · **默认值**: Harness 数据目录
- **配置项**: `apiPath` · **说明**: 统计接口路径 · **默认值**: `/usage-stats/v1`
