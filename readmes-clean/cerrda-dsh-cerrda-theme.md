...
### 字段 · 作用
- **字段**: `dsh.bundle.patch: ./cordis.patch.yml` · **作用**: `dsh plugin add` 后自动把本包追加进 `dsh.profile.bundles` 层栈，`cordis.patch.yml` 在启动时参与组合
- **字段**: `dsh.client: { platform: "web" }` · **作用**: 声明浏览器 half，`dsh-client-modules` 把它收进 `window.__DSH_BOOT__` 名录，服务 `/plugins/dsh-cerrda-theme/client.js`
- **字段**: `main: lib/index.js` · **作用**: 空宿主 half（carrier 行）——让 loader 行激活，名录只收录已激活的行
- **字段**: `exports["./client"]: lib/client.js` · **作用**: 浏览器 bundle：`window.__ModuleLoader__.load({id, factory})` 注册，`require("react")` 取 React，CSS 以 `data-plugin-css` 风格 style 标签注入
