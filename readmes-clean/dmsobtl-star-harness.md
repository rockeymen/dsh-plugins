...
public class GhClient : MonoBehaviour {
    ...
    async System.Threading.Tasks.Task ReceiveLoop() {
        ...
        while (_ws.State == WebSocketState.Open) {
            var r = await _ws.ReceiveAsync(new ArraySegment(buffer), CancellationToken.None);
    ...
    public void Send(string json) =>
        _ws.SendAsync(new ArraySegment(Encoding.UTF8.GetBytes(json)),
...
### 文件 · 覆盖什么
- **文件**: `tests/core.spec.ts` (16) · **覆盖什么**: 事务回滚、负余额防线、交易原子性、四类拒绝语义、schema 校验、审计流水
- **文件**: `tests/protocol.spec.ts` (8) · **覆盖什么**: 帧解析、流式文本增量（含转义跨 chunk 截断）
- **文件**: `tests/e2e.spec.ts` (6) · **覆盖什么**: **穿过真实 agent loop**（浏览器拓扑）：帧入 → 工具执行 → 帧出、多轮流式、HUD 推送、坏帧容错
- **文件**: `tests/headless.spec.ts` (2) · **覆盖什么**: **无客户端拓扑**（Unity/Unreal 形态）：`GET /` 返回 404，但帧协议完整工作——这是“不依赖 three.js”的真正证据
- **文件**: `tests/layering.spec.ts` (10) · **覆盖什么**: **层次守护**：扫描 `src/framework/`，任何游戏/客户端词汇（地点/角色/货币/工具名/`three.js`）都会让它红
