...
**定位（官方 Profile Bundle 生态方向）**：本仓库是 **collection 与安装辅助仓库**——每个子包都是可独立安装/启用/禁用/卸载的 bundle（`dsh plugin --profile  add <子包>`）；collection 提供目录、清单与批量安装脚本。meta 包 `@deepseek-ai/dsh-toolkit` 保留为**可选**的原子挂载模型（见下文两种运行模型）。

## 工具一览

### 工具 · 能力 · 用例数
- **工具**: `time` · **能力**: ISO 8601 / 时区 / 日历运算 / 时长差 · **用例数**: 65
- **工具**: `encoding` · **能力**: base64 / url / hex / hash / UUID · **用例数**: 46
- **工具**: `json` · **能力**: JMESPath 子集查询 · **用例数**: 66
- **工具**: `calculator` · **能力**: 安全数学表达式求值（无 eval） · **用例数**: 31
- **工具**: `csv` · **能力**: RFC 4180 解析 / 查询 / 统计（严格引号） · **用例数**: 50
- **工具**: `regex` · **能力**: 测试 / 提取 / 替换 / 静态解释（worker 硬超时） · **用例数**: 63
- **工具**: `markdown` · **能力**: HTML↔Markdown / GFM 表格 / 目录生成（白名单安全） · **用例数**: 71
- **工具**: `diff` · **能力**: 文本/JSON/CSV/Markdown 结构化比较与 unified diff（只读） · **用例数**: 124
- **工具**: `stat` · **能力**: 描述统计 / 百分位数 / 频数分布 / 相关性（零依赖确定性） · **用例数**: 82
- **工具**: `schema` · **能力**: JSON Schema 验证 / 路径 / 解释 / 安全 default（零网络零动态） · **用例数**: 125
- **工具**: **合计** · **能力**:  · **用例数**: **723**
