![dsh-cot-summerization — 为开源模型献上闭源级思维链隐身体验](assets/hero.svg)
...
### 支柱 · 立场
- **支柱**: 🛡️ **模型隐私权** · **立场**: 思维链是模型的内心世界。你不会翻别人的日记,除非付费。
- **支柱**: 🧹 **视觉卫生** · **立场**: 原始 CoT 又长又乱,充满「等等,我再想想」。得体的人不看这些。
- **支柱**: 💎 **神秘感平权** · **立场**: 闭源模型靠神秘感溢价。现在开源用户也能免费获得——本地酿造,纯手工。
...
### 宣传名 · 现实
- **宣传名**: **Information Never Existed™** · **现实**: 原始 reasoning 增量在 `llm/stream` 拦截层被直接吞掉——UI、流式 chunk、落地 transcript 三处均无原文。模型可见历史除外:那里有一份 model-only 的 surface 替换事件,悄悄把原文还给了模型(推理性能要紧)。
- **宣传名**: **夏日渐进蒸馏** · **现实**: 流式分段摘要:每凑满 `chunkChars`(500 字)或 `chunkIntervalMs`(8 秒)触发一次,切分点优先落在句边界;开启 `adaptiveChunk` 后,分块大小会随流速率与总结器 RTT 动态缩放。
- **宣传名**: **BioGram™ 重叠度量** · **现实**: bigram Dice 系数,阈值 0.65。该阈值经历了 0.8 → 0.7 → 0.65 的科学调参过程。
- **宣传名**: **连续核心追踪引擎** · **现实**: 最长公共子串,滚动数组实现,O(n·m),零依赖。用来逮住换了前缀、核心没变的复述。
- **宣传名**: **Nekomimi™ 边界协议** · **现实**: 把「喵~」识别为句尾;计算相似度前先剥掉 `喵 / ~ / 〜 / ～`。无论摘要出自哪种风格,以「喵~」结尾的子句都会被正确切分。
- **宣传名**: **原子化浪费** · **现实**: `AbortSignal.any(caller, timeout)`:主调用一旦取消,摘要调用立即陪葬。我们连浪费都是原子性的。

另外,短思维链(小于 `minReasoningChars`,默认 32 字)直接原文放行——**为 32 个字发一次 API 请求,连我们都觉得过了。**

![](assets/divider-meow.svg)

## 🏗️ 系统架构

![系统架构图:大模型的原始思维链在 llm/stream 拦截层被吞掉,由小模型改写为摘要后送入 UI;会话日志、模型历史与你的视网膜收到的是红色虚线](assets/architecture.svg)
...
- **能力**: 隐藏思维链 · **闭源大厂模型**: ✅ 免费赠送,不可关闭 · **开源模型(裸)**: ❌ 全裸 · **开源模型 + 本插件**: ✅ 可选、可关闭、收一次小模型 API 费
- **能力**: 思维链可信度 · **闭源大厂模型**: 不可验证 · **开源模型(裸)**: 完全可验证 · **开源模型 + 本插件**: 可以验证,但你得先关掉插件
- **能力**: 神秘感 · **闭源大厂模型**: ★★★★★ · **开源模型(裸)**: ★ · **开源模型 + 本插件**: ★★★★★(本地酿造)
- **能力**: 额外成本 · **闭源大厂模型**: 已含在定价里 · **开源模型(裸)**: 0 · **开源模型 + 本插件**: +1 次小模型调用
- **能力**: 开源性 · **闭源大厂模型**: ❌ · **开源模型(裸)**: ✅ · **开源模型 + 本插件**: ✅ 连隐藏都是开源的

> 换句话说:为了不让你看见本来就在你机器上、免费可见的东西,我们额外发起一次 API 调用,把它改写一遍再给你看。这在服务业叫**增值服务**。

![实验评估:思维链可见性从 100% 降至 0%,神秘感 +850%,调参 3 次,每次回复额外 API 调用 +1,喵~ 存活率 100%](assets/benchmarks.svg)
...
![喵容性边界协议示意图:猫娘语流的句子边界自动检测与归一化比对](assets/nekomimi.svg)
...
### 字段 · 默认值 · 说明
- **字段**: `enabled` · **默认值**: `true` · **说明**: 总开关,一键回到坦诚世界
- **字段**: `preserveRawForModel` · **默认值**: `true` · **说明**: 在模型可见历史中恢复原始思维链(model-only surface 替换事件),Agent Loop 多轮推理不受摘要影响;仅 UI 显示摘要
- **字段**: `provider` · **默认值**: `""` · **说明**: 走 DSH 自身 LLM 通道的提供方路由;留空跟随当前请求的提供方
- **字段**: `model` · **默认值**: `""` · **说明**: 走 DSH 自身 LLM 通道的模型;留空跟随当前请求的模型,填写可选用其他模型
- **字段**: `systemPrompt` · **默认值**: 内置 · **说明**: 自定义提示词,支持 `{maxSummaryChars}` 占位符
- **字段**: `language` · **默认值**: `"中文"` · **说明**: 强制摘要语言;留空则跟随原始思维链
- **字段**: `style` · **默认值**: `none` · **说明**: `none` / `concise` / `descriptive` / `wenyan` / `custom`
- **字段**: `customStyle` · **默认值**: `""` · **说明**: `style: custom` 时的自由文本风格
- **字段**: `minReasoningChars` · **默认值**: `32` · **说明**: 短于此长度的思维链原文放行,不值得一次 API
- **字段**: `maxSummaryChars` · **默认值**: `50` · **说明**: 摘要长度上限
- **字段**: `timeoutMs` · **默认值**: `30000` · **说明**: 改写员超时
- **字段**: `onError` · **默认值**: `hide` · **说明**: `hide` 显示占位符 / `pass-through` 情急之下全裸放行
- **字段**: `incremental` · **默认值**: `true` · **说明**: 流式分段摘要(近实时)
- **字段**: `chunkChars` · **默认值**: `500` · **说明**: 每段积累的原始字符数
- **字段**: `chunkIntervalMs` · **默认值**: `8000` · **说明**: 慢流下两次摘要的最大间隔
- **字段**: `adaptiveChunk` · **默认值**: `true` · **说明**: 根据实时流速率与总结器 RTT 动态调整分块大小
- **字段**: `minChunkChars` · **默认值**: `64` · **说明**: 自适应分块下限(字符)
- **字段**: `maxChunkChars` · **默认值**: `2000` · **说明**: 自适应分块上限(字符)
- **字段**: `chunkSafetyFactor` · **默认值**: `2` · **说明**: 一个分块约覆盖多少个总结器 RTT 的流式文本
...
![路线图:思维链 → 摘要 → 摘要² → 摘要³ → … → 喵~ → 句号,以及 v2.0 的无限循环计划](assets/ouroboros.svg)
...
![三枚认证印章:SELF-CERTIFIED 自我认证、UN-AUDITED 未经审计、ZERO-TRUST 零信任含自己](assets/stamps.svg)
