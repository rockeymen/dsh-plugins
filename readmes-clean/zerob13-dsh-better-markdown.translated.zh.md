![dsh-better-markdown — fast streaming Markdown for DeepSeek Harness](https://raw.githubusercontent.com/zerob13/dsh-better-markdown/master/assets/banner.png)

# dsh-better-markdown

  用 [`markstream-react`](https://www.npmjs.com/package/markstream-react)
...
![DeepSeek Harness code blocks rendered inside dsh-better-markdown](https://raw.githubusercontent.com/zerob13/dsh-better-markdown/master/screenshot/code.png)

### 图片、链接与 KaTeX 数学公式

![Images, links and KaTeX math rendered by dsh-better-markdown](https://raw.githubusercontent.com/zerob13/dsh-better-markdown/master/screenshot/math.png)

### Mermaid 图表

![Interactive Mermaid flowchart rendered in DeepSeek Harness](https://raw.githubusercontent.com/zerob13/dsh-better-markdown/master/screenshot/mermaid.png)

## 功能范围

### 能力 · 行为
- **能力**: Assistant streaming Markdown · **行为**: 全部交给 `markstream-react`
- **能力**: Settled assistant Markdown · **行为**: 继续使用同一个 Markstream renderer
- **能力**: Mermaid · **行为**: 插件内置 `mermaid@11.16.1`，无需额外安装
- **能力**: Math · **行为**: KaTeX inline / display math
- **能力**: Code fences · **行为**: 使用 Markstream `MarkdownCodeBlockNode` + `stream-markdown` + Shiki；未知语言回退为可见纯文本
- **能力**: Raw HTML · **行为**: 转义为文本，不注入 DOM
- **能力**: Links and images · **行为**: 仅允许安全的外部协议
- **能力**: Plan review / trajectory 等静态 surface · **行为**: 继续使用 Harness 内置 `MarkdownText`；这些 surface 没有统一替换 slot
...
- [美人鱼](https://github.com/mermaid-js/mermaid)