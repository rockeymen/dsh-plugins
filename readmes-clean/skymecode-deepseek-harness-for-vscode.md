...
- **Setting**: `deepseekHarness.apiKey` · **Default**: empty · **Description**: DeepSeek API Key stored as plain text in user `settings.json` with `machine` scope
- **Setting**: `deepseekHarness.model` · **Default**: `deepseek-v4-flash` · **Description**: Default model for new sessions
- **Setting**: `deepseekHarness.reasoningEffort` · **Default**: `high` · **Description**: `off` / `high` / `max`
- **Setting**: `deepseekHarness.agentPreset` · **Default**: `standard` · **Description**: Default Agent Preset for new sessions
- **Setting**: `deepseekHarness.provider` · **Default**: `deepseek-official` · **Description**: Harness model-provider route
- **Setting**: `deepseekHarness.baseUrl` · **Default**: empty · **Description**: Optional API base URL
- **Setting**: `deepseekHarness.permissionMode` · **Default**: `workspace-write` · **Description**: `read-only` / `workspace-write` / `danger-full-access`
- **Setting**: `deepseekHarness.autoAttachSelection` · **Default**: `true` · **Description**: Automatically attach the active editor selection when sending
...
### Command · Description
- **Command**: `DeepSeek Harness: Open Workbench` · **Description**: Open the sidebar workbench
- **Command**: `DeepSeek Harness: Reload Workbench` · **Description**: Restart the runtime and reconnect
- **Command**: `DeepSeek Harness: Set API Key` · **Description**: Save the API Key
- **Command**: `DeepSeek Harness: Clear API Key` · **Description**: Clear the API Key
- **Command**: `DeepSeek Harness: Show Logs` · **Description**: Open diagnostic logs
