...
- **登录页 · 浅色**: ![](docs/screenshots/white-login.png) · **登录页 · 深色**: ![](docs/screenshots/black-login.png) · **登录页 · English**: ![](docs/screenshots/white-login-en.png)

### dsh 主界面（登录后） · 聊天 / 留言 · 设置页卡片 · 账号管理
- **dsh 主界面（登录后）**: ![](docs/screenshots/main-ui.png) · **聊天 / 留言**: ![](docs/screenshots/chat.png) · **设置页卡片 · 账号管理**: ![](docs/screenshots/card-front.png)

###  · 设置页卡片 · 权限与配额 · 
-  · **设置页卡片 · 权限与配额**: ![](docs/screenshots/card-back.png) · 
## 快速开始
...
### 错误码 · 含义 · 怎么办
- **错误码**: **30** · **含义**: 证书签发失败 · **怎么办**: 检查 80/443 是否放行（防火墙 + 云安全组都要开）、80 是否被占用、能否连通 Let's Encrypt
- **错误码**: **31** · **含义**: 拿不到公网 IP/域名 · **怎么办**: 服务器没有公网 IP，或探测失败。有域名就设 `MCP_GATEWAY_DOMAIN`；纯内网用走 HTTP 模式
- **错误码**: **32** · **含义**: 端口被占用 · **怎么办**: 换端口（`.env` 的 `MCP_GATEWAY_PORT`）或释放被占端口
...
### 场景 · 做法 · 用户看到的 · 需要放行
- **场景**: ✅ 公网服务器，80/443 都能开 · **做法**: 什么都不用做（默认） · **用户看到的**: HTTPS（自动证书） · **需要放行**: 80 + 443
- **场景**: ✅ 有自己的域名证书 · **做法**: `.env` 填 `MCP_GATEWAY_TLS_CERT/KEY`，端口随便改 · **用户看到的**: HTTPS（你的证书） · **需要放行**: 只有你的网关端口，80 完全不用
- **场景**: ✅ 机器上已有 nginx/caddy 反代 · **做法**: 反代在 80/443 用真实证书终结 TLS 并转发到密码门；`.env` 设 `MCP_GATEWAY_AUTO_TLS=0` + 高位端口，密码门只监听回环 · **用户看到的**: HTTPS（反代的证书） · **需要放行**: 反代管 80/443，密码门零公网暴露
- **场景**: ✅ 域名挂在 Cloudflare · **做法**: CF 边缘终结 TLS 转发到源站任意端口（配置同反代思路） · **用户看到的**: HTTPS（CF 证书） · **需要放行**: 源站只对 CF 开放
- **场景**: ⚠ 无公网 IP / 纯内网 · **做法**: `scripts/start-http.mjs` 或 `.env` 设 `AUTO_TLS=0` · **用户看到的**: HTTP 明文 · **需要放行**: 任意端口
- **场景**: ⚠ 只有裸 IP 且 80 开不了 · **做法**: 只能 HTTP（协议限制：http-01 固定走 80，裸 IP 又没有 DNS 可验证） · **用户看到的**: HTTP 明文 · **需要放行**: 任意端口
...
### 功能 · 谁可用 · 说明
- **功能**: **远程设置 + 重载补丁** · **谁可用**: 所有登录用户 · **说明**: 远程设置已应用（强制启用）；dsh 升级后若设置页出现异常，点"重载补丁"一键修复（自动重启网页服务并刷新页面，不用 SSH）
- **功能**: **修改密码** · **谁可用**: 本人改自己；主用户可改任何人 · **说明**: 改密后旧会话全部立即失效，需重新登录
- **功能**: **修改用户名** · **谁可用**: 本人改自己；主用户可改任何人 · **说明**: 改名后需用新用户名重新登录
- **功能**: **子用户管理** · **谁可用**: 仅主用户 · **说明**: 创建/删除子用户（子用户可用登录页进入，但没有管理权限）
- **功能**: **子用户权限** · **谁可用**: 仅主用户 · **说明**: 工作区白名单、每小时 token 上限、每日时长上限、沙盒级别、上传/git 下载开关、封禁
- **功能**: **聊天 / 留言** · **谁可用**: 所有登录用户 · **说明**: 左下角聊天按钮，支持标签（议题/拉取请求/讨论/公告/问题）
...
### 变量 · 默认 · 说明
- **变量**: `SETUP_KEY` · **默认**: 安装脚本自动生成 · **说明**: 首次配置密钥；JWT 会话密钥也从它派生，**安装后别删**
- **变量**: `MCP_JWT_SECRET` · **默认**: 空（从 SETUP_KEY 派生） · **说明**: 会话签名密钥。生产环境建议独立设置（`openssl rand -hex 32`），SETUP_KEY 泄露时不连带会话伪造
- **变量**: `MCP_DB_PATH` · **默认**: `./data/platform.db` · **说明**: 数据库文件（SQLite 自动建库，不需要 MySQL）
- **变量**: `MCP_DB_ENC_KEY` · **默认**: 空 · **说明**: 数据加密密钥。`openssl rand -hex 32` 生成。**设了就不能换，换钥匙旧数据全废**
- **变量**: `MCP_GATEWAY_HOST` · **默认**: `0.0.0.0` · **说明**: 网关监听地址
- **变量**: `MCP_GATEWAY_PORT` · **默认**: `443` · **说明**: 网关端口
- **变量**: `MCP_GATEWAY_UPSTREAM` · **默认**: `http://127.0.0.1:3080` · **说明**: dsh 网页地址（插件自动指向 dsh 实际端口，一般不用改）
- **变量**: `MCP_GATEWAY_REDIRECT_PORT` · **默认**: `80` · **说明**: 80 端口：ACME 证书验证 + 301 跳转 443
- **变量**: `MCP_GATEWAY_DOMAIN` · **默认**: 空 · **说明**: 自己的域名；留空自动用 `<公网IP>.sslip.io`
- **变量**: `MCP_GATEWAY_AUTO_TLS` · **默认**: 开 · **说明**: 留空=自动；`0` 关闭（明文 HTTP，危险）
- **变量**: `MCP_GATEWAY_ACME_EMAIL` · **默认**: 空 · **说明**: 证书到期提醒邮箱（可选）
- **变量**: `MCP_GATEWAY_ACME_STAGING` · **默认**: 关 · **说明**: `1`=用 LE 测试环境签发（调试用，浏览器不信任）
- **变量**: `MCP_GATEWAY_TLS_CERT` / `MCP_GATEWAY_TLS_KEY` · **默认**: 空 · **说明**: 两个都填 = 用你自己的证书（优先于自动 HTTPS）
- **变量**: `MCP_GATEWAY_PUBLIC_HOST` · **默认**: 空 · **说明**: 跳转固定用的公网 IP/域名（防 Host 伪造反射）
- **变量**: `MCP_DSH_ROOT` · **默认**: 自动探测 · **说明**: dsh 安装目录（`@deepseek-ai/dsh` 所在处），探测不到时手动指定
- **变量**: `MCP_DSH_RESTART_SERVICE` · **默认**: `dsh-web` · **说明**: 重载补丁后自动重启的 dsh systemd 服务名；显式留空不自动重启
- **变量**: `DSH_PASSWORDS_ENV_FILE` · **默认**: 空 · **说明**: 手动指定 `.env` 路径（插件自动传，一般不用填）
