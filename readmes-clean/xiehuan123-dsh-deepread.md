...
### Capability · Details
- **Capability**: 🎛️ Five modes · **Details**: `quick` key takeaways · `deep` in-depth reading · `map` knowledge map · `feynman` Feynman technique (11-step loop + spaced repetition) · `book` whole-book reading (see the comparison below)
- **Capability**: 🗺️ Knowledge-map mode · **Details**: Core question / core conclusion / ten content categories (conclusion, sub-claim, mechanism, fact, data, case, hidden premise, objection, limitation, actionable advice) / every claim paired with evidence (unverifiable claims marked "no evidence provided in the original text") / key data table (value & unit, time range, sample, baseline, source, location) / eight relation labels (supports, refutes, causes, explains, depends on, exemplifies, contrasts, limits) / **four confidence levels** (author intent, original facts & data, reasonable inference, unverifiable) / Mermaid mindmap / XMind outline / 5 active-recall questions
- **Capability**: 📥 Three inputs · **Details**: WeChat article URLs (`mp.weixin.qq.com` stable links) · files (`.txt/.md/.html/.pdf`, PDF via a built-in pure-JS extractor with Chinese ToUnicode mapping, page markers, and object-stream/xref-stream support) · pasted text
- **Capability**: 📤 Optional export · **Details**: Displayed in-session by default; `export` accepts `md` / `mm` (FreeMind, importable by XMind) / `html` (editor-style web report with light/dark theme) / `all`, written to `deepread-output/` in the workspace
- **Capability**: 🎨 Browser UI · **Details**: `deepread` tool result card (four-color confidence legend, collapsible sections) + a 📖 shortcut button next to the input area that opens a card-style reading panel (link/path/text + mode/export selection + reading focus + one-click start)
- **Capability**: 🔀 Batch compare · **Details**: Pass 2-10 documents via `batch` (url/path/text each) to get per-document summaries plus a cross-document report: comparison matrix, conflicts, complementarity, and synthesis
- **Capability**: 📍 Citations · **Details**: Reports carry page/paragraph provenance: arguments, quotes, and a dedicated citation table locate claims back to `【第N页】` markers in the source
- **Capability**: 🧮 Cost preview · **Details**: `estimate: true` previews token spend, model-call count, and expected time per mode without calling the model (CJK≈0.6 tok/char heuristic; rate/latency defaults are picked per model family and can be overridden explicitly)
- **Capability**: 📚 Recently read · **Details**: The Web panel keeps a local history of recent reads with one-click re-read (localStorage, no server round-trip)
- **Capability**: ⏳ Progress transparency · **Details**: Long reads / big PDFs / batches become official background jobs: the label states segment count and budget; the progress stream pushes 「精读第 3/20 段…」 line by line; job_output polls progress and the final report, job_kill cancels
...
- **Mode**: `quick` · **Best for**: "What is this article about?" at a glance · **Key output**: One-line summary, core claim, argument structure, quotes, key concepts, critical questions · **Cost**: Single call, fastest
- **Mode**: `deep` (default) · **Best for**: Reading one article carefully · **Key output**: Overview, core claim, argument structure (claim + evidence + verbatim quotes), argument flow, section highlights, quotes, key concepts, critical thinking · **Cost**: Long articles are auto-split, section-by-section + summary
- **Mode**: `map` · **Best for**: Research, fact-checking before citing · **Key output**: Core question & conclusion, ten content categories, claim-evidence pairing, key data table (five elements), eight relations, four confidence levels, Mermaid mindmap, XMind outline, active-recall questions · **Cost**: Structured pipeline, multiple calls
- **Mode**: `feynman` · **Best for**: Truly learning it and teaching it to others · **Key output**: 11-step loop: TOC → questions → per-chapter → claims/data/evidence → chapter mindmap → explain with the book closed → self-check gaps → correct against the source → merged mindmap → explain again → spaced review on days 1/3/7/14/30 · **Cost**: Longest output, most calls
- **Mode**: `book` · **Best for**: Whole books / very long texts · **Key output**: Table of contents, chapter flow, a full-book summary assembled from per-part deep reads · **Cost**: Processed part by part
...
1. **Trigger**: say something containing "deep-read / analyze / knowledge map / Feynman", e.g.
   ...
   - `Analyze this article in knowledge-map mode: `
...
- **Parameter**: `url` · **Type**: string · **Description**: Stable WeChat article link (`mp.weixin.qq.com` only; for anti-bot sites, paste the text)
- **Parameter**: `path` · **Type**: string · **Description**: Workspace file path (`.txt/.md/.markdown/.html/.pdf`)
- **Parameter**: `text` · **Type**: string · **Description**: Pasted text
- **Parameter**: `depth` · **Type**: enum · **Description**: `quick` / `deep` (default) / `map` / `feynman` / `book`
- **Parameter**: `export` · **Type**: enum · **Description**: `none` (default, in-session only) / `md` / `mm` / `html` / `all`
- **Parameter**: `refresh` · **Type**: boolean · **Description**: `true` forces a re-fetch and cache refresh (default `false`: a cached URL reuses the stored full text without network access)
- **Parameter**: `focus` · **Type**: string · **Description**: Reader's angle of interest, e.g. "argumentation logic", "research methodology"
- **Parameter**: `language` · **Type**: enum · **Description**: `zh` / `en` / `auto` (default)
