...
config 字段：`text`（覆盖 `preset.md` 文本，默认读文件）、`agentsText`（覆盖 `AGENTS.md` 文本，默认读文件）、`writeAgents`（是否写 `~/.dsh/AGENTS.md`，默认 true）、`writePreset`（是否生成 `~/.dsh/.agent-presets/prompt-tool/`，默认 true）、`injectPrompt`（锚定层：we 锚定确认