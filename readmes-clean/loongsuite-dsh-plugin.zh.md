...
> 当前状态：稳定的 `0.1.x` 版本。可从 npm 或 DSH 插件市场安装 `@loongsuite/dsh-plugin`。
  ...
  ![DSH 的一轮对话作为 OpenTelemetry GenAI 调用链，在自建 Langfuse 中查看](docs/assets/langfuse-trace.png)
  
  一轮 DSH 对话通过 OTLP 上报到自建 Langfuse：4 个 react step、每次调用的耗时与 token、
  `web_search` 失败后回退到 `bash`，以及 ENTRY span 上的 GenAI 语义属性。
  该截图显式开启了正文采集，默认是关闭的。
...
### 组件 · 支持范围 · 已完整验证版本
- **组件**: DeepSeek Harness · **支持范围**: `>=0.1.0-rc.6 <0.2.0` · **已完整验证版本**: `0.1.0-rc.6` 的 headless 与 Web profile
- **组件**: Node.js · **支持范围**: `>=22.19.0` · **已完整验证版本**: macOS 上的 `22.19`、`24.19` 和 `25.9`
...
大部分部署只需环境变量。也可以修改 `$DSH_HOME/profiles//cordis.patch.yml`（默认位于
`~/.dsh`）中的插件配置：
...
### 配置项 · 默认值 · 含义
- **配置项**: `enabled` · **默认值**: `true` · **含义**: 不卸载 bundle，直接停止采集。
- **配置项**: `endpoint` · **默认值**: 未设置 · **含义**: OTLP/HTTP 公共基地址；插件自动追加 signal 路径。
- **配置项**: `traceEndpoint` / `metricEndpoint` · **默认值**: 未设置 · **含义**: 完整的单 signal 地址；优先于 `endpoint`。
- **配置项**: `headers` · **默认值**: `{}` · **含义**: 同时添加到两个导出器的请求头。
- **配置项**: `serviceName` · **默认值**: `OTEL_SERVICE_NAME` 或 `deepseek-harness` · **含义**: OpenTelemetry `service.name`。
- **配置项**: `resourceAttributes` · **默认值**: `{}` · **含义**: 额外的字符串类型 Resource 属性。
- **配置项**: `captureContent` · **默认值**: 环境变量配置或 `false` · **含义**: 上报提示词、回复、工具定义、参数和结果正文。
- **配置项**: `contentMaxChars` · **默认值**: `128000` · **含义**: 每个正文属性序列化后保留的最大字符数。
- **配置项**: `exportMetrics` · **默认值**: 环境变量配置或 `true` · **含义**: 上报 LLM 耗时和 token 指标。
- **配置项**: `maxExportBatchSize` · **默认值**: `512` · **含义**: 每批最多上报的 span 数。
- **配置项**: `maxQueueSize` · **默认值**: `2048` · **含义**: 最多排队的 span 数，不能小于 batch size。
- **配置项**: `traceExportIntervalMs` · **默认值**: `5000` · **含义**: Trace 批量导出间隔。
- **配置项**: `metricExportIntervalMs` · **默认值**: `60000` · **含义**: Metric 导出间隔。
- **配置项**: `exportTimeoutMs` · **默认值**: `30000` · **含义**: OTLP 导出超时。
- **配置项**: `debug` · **默认值**: `false` · **含义**: 通过 DSH logger 输出额外的插件生命周期诊断。
