...
### 命令 · 说明
- **命令**: `DSH: Open WebUI` · **说明**: 打开（或聚焦）WebUI 内嵌面板
- **命令**: `DSH: Reload WebUI` · **说明**: 关闭并重新打开面板（强制刷新页面）
- **命令**: `DSH: Open in Browser` · **说明**: 在系统浏览器中打开 DSH 页面
- **命令**: `DSH: Start Server` · **说明**: 手动启动 DSH 服务（后台子进程）
- **命令**: `DSH: Stop Server` · **说明**: 停止由本扩展启动的服务进程

### 侧边栏「DSH 控制台」

### 区域 · 内容
- **区域**: 状态区 · **内容**: 在线/离线圆点、服务地址
- **区域**: 操作区 · **内容**: 打开 WebUI / 刷新页面 / 浏览器 / 启动服务 / 停止服务
- **区域**: 设置区 · **内容**: 「离线时自动启动服务」开关、启动命令输入框（留空自动检测，回车或点「保存」生效）
- **区域**: 运行日志 · **内容**: 扩展运行事件滚动列表（最新在前，最多保留 150 条）
...
### 设置项 · 默认值 · 说明
- **设置项**: `dsh.webuiUrl` · **默认值**: `http://127.0.0.1:3080/` · **说明**: DSH WebUI 服务地址（Webview 面板与控制台状态均基于此）
- **设置项**: `dsh.autoStart` · **默认值**: `true` · **说明**: 服务离线时是否自动启动
- **设置项**: `dsh.startCommand` · **默认值**: 空（自动检测） · **说明**: 启动 DSH 服务的完整命令；留空自动检测（见下）
