...
### Path · Content
- **Path**: `src/argp-graph-engine.ts` · **Content**: Main engine (graph build, pruning, closure lifecycle, recall/list tools)
- **Path**: `src/argp-t1-engine.ts` · **Content**: Earlier single-transaction validation engine
- **Path**: `src/recall-engine.ts` / `src/probe-engine.ts` · **Content**: Recall / probe helpers
- **Path**: `test/` · **Content**: Node test suite (`argp-graph-engine.test.ts`, `chain-unlock.test.ts`)
- **Path**: `spike/` · **Content**: Repro/validation scripts (each `node spike/NN-*.ts` is self-contained)
- **Path**: `docs/` · **Content**: Design (v1.0), migration design, roadmap, experiment records, design↔impl trace
...
- **Run**: 50-turn t-long (high thinking) · **Command**: `ARGP_DEEPSEEK_THINKING=enabled node spike/06-tlong.ts` · **What it validates**: L1/L2/L3 invariants, 7/7 anchors, 7/7 needles via recall
- **Run**: Production-scale · **Command**: `ARGP_DEEPSEEK_THINKING=enabled ARGP_WINDOW_TOKENS=100000 ARGP_RETAIN_TOKENS=33000 ARGP_MAX_PASSES=256 node spike/06-tlong.ts` · **What it validates**: Large-transaction pruning (34–35 atoms per transaction)
- **Run**: Baseline (compaction-basic) · **Command**: `node spike/07-baseline.ts` · **What it validates**: Same task with the stock summarizer for contrast
- **Run**: Synthetic 0-LLM · **Command**: `npm run spike8a` · **What it validates**: 28-atom single-transaction pruning with zero LLM calls
