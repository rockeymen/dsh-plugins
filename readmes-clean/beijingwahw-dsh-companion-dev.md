...
### 模块 · 能力 · 独立开关
- **模块**: **A · 对话智能导出** · **能力**: Markdown / PDF / JSON / **PNG 长图**导出、时间戳开关、隐私脱敏（手机号/邮箱/身份证/银行卡自动打码）、多会话批量 ZIP 打包；含中文的 PDF 由客户端 canvas 光栅化为**免打印多页 PDF**（无 `window.print()` 对话框冻结） · **独立开关**: `enableExport`
- **模块**: **B · 上下文交接摘要** · **能力**: 一键生成 ≤500 字四段式交接摘要、可编辑、复制到剪贴板、保存为模板、导入摘要作为新对话的 system 首条消息实现上下文继承 · **独立开关**: `enableHandoff`
- **模块**: **C · API 成本优化（开发者模式）** · **能力**: API Key 加密托管、**官方动态计价引擎**（每小时抓取 DeepSeek 与国产厂商官方定价页，新模型/调价自动导入，抓取失败静默降级内置快照）、**峰谷分时计价**（高峰时段按高峰价计费）、峰谷自动调度（高峰窗口实时解析自官方定价页）、模型智能路由、**日/月双档预算** 80%/100% 预警并自动暂停非必要调用、缓存命中折扣计费、每日/每周 Token 消耗与费用报表 · **独立开关**: `enableCost`
- **模块**: **D · 全局对话检索 + 对话内搜索** · **能力**: 关键词模糊搜索、时间范围筛选、自定义标签增删与按标签过滤、结果点击直达对话；**对话内搜索**（Ctrl+F 浮动查找栏，CSS Custom Highlight API 无侵入高亮，大小写/全词开关、查询历史、流式输出期间自动重同步） · **独立开关**: `enableSearch`
- **模块**: **E · 执行轨迹分析器（开发者）** · **能力**: Harness 原生轨迹解析为可交互时间轴（耗时/Token 拆分/模型/缓存命中）、一键定位最慢与最贵的 3 个步骤、异常自动标注（重试循环/Token 爆炸/缓存未命中/死循环）、两次轨迹 Diff 对比（HTML 报告导出）、日聚合趋势与历史基准线告警 · **独立开关**: `enableTrace`
- **模块**: **F · Prompt 工程工作台（开发者）** · **能力**: Prompt 版本管理（自动版本号/备注/标签/回滚）、A/B 测试面板（批量测试集、指标对比、人工评分胜率统计）、模板库（分类/变量插值/一键生成 Python·Node.js·curl 调用代码）、结构化输出 JSON Schema 批量校验与合规率统计 · **独立开关**: `enablePrompt`
- **模块**: **G · 多模型竞技场（开发者）** · **能力**: 同 Prompt 多模型并行对比（最多 5 个，输出/耗时/Token/费用并排）、批量评测排行榜（准确率/P50·P95·P99/成本/合规率综合得分，MD/HTML 报告）、模型推荐引擎（任务类型+预算+延迟+峰谷定价感知，附推荐理由）、外部厂商 Key 加密托管 · **独立开关**: `enableArena`
- **模块**: **H · 断点续跑与任务编排（开发者）** · **能力**: 可视化流水线（模型/Prompt/输入来源/条件分支/超时/重试/依赖，自动生成 YAML）、断点续跑（进度百分比、中间结果持久化、从最后成功步骤恢复）、批量任务队列（优先级/截止时间/失败策略/批量暂停恢复取消）、定时调度（Cron + 中文自然语言，峰谷空闲时段执行，历史归档） · **独立开关**: `enableOrchestrator`
- **模块**: **J · 安全与审计（企业开发者）** · **能力**: 多 API Key 管理（权限范围/轮换提醒/泄露检测）、操作审计日志（脱敏 Prompt 摘要/筛选/CSV·JSON 导出）、数据防泄漏 DLP（内置+自定义正则规则、严格模式拦截、发送前预检）、合规报表（调用/费用/模型占比/拦截/告警，HTML 导出可打印为 PDF） · **独立开关**: `enableSecurity`
...
### 字段 · 类型 · 默认值 · 说明
- **字段**: `enableExport` · **类型**: boolean · **默认值**: `true` · **说明**: 启用模块 A
- **字段**: `enableHandoff` · **类型**: boolean · **默认值**: `true` · **说明**: 启用模块 B
- **字段**: `enableCost` · **类型**: boolean · **默认值**: `true` · **说明**: 启用模块 C
- **字段**: `enableSearch` · **类型**: boolean · **默认值**: `true` · **说明**: 启用模块 D
- **字段**: `enableTrace` · **类型**: boolean · **默认值**: `true` · **说明**: 启用模块 E
- **字段**: `enablePrompt` · **类型**: boolean · **默认值**: `true` · **说明**: 启用模块 F
- **字段**: `enableArena` · **类型**: boolean · **默认值**: `true` · **说明**: 启用模块 G
- **字段**: `enableOrchestrator` · **类型**: boolean · **默认值**: `true` · **说明**: 启用模块 H
- **字段**: `enableSecurity` · **类型**: boolean · **默认值**: `true` · **说明**: 启用模块 J
- **字段**: `apiBaseUrl` · **类型**: string · **默认值**: `https://api.deepseek.com` · **说明**: DeepSeek API 基址（manifest 已放行该域名）
- **字段**: `apiTimeoutMs` · **类型**: number · **默认值**: `60000` · **说明**: 单次 API 调用超时（毫秒）
...
### 要求 · 实现
- **要求**: 数据本地化 · **实现**: 对话内容、设置、API Key 仅写入 Harness 插件沙箱的 `companion` 存储域，不上传任何第三方服务器
- **要求**: API Key 加密 · **实现**: AES-256-GCM（12 字节随机 IV + 认证标签），自描述载荷 `v1..<tag>.<ciphertext>`；密钥与密文分离存储
- **要求**: 网络权限 · **实现**: `manifest.json` 全量放行 DeepSeek 官方 API、全部国产与海外主流模型厂商端点、各厂商定价页及常见中转/聚合网关，供多模型竞技场直连与动态计价引擎实时抓取；存储域仅 `companion`；服务清单逐项列明
- **要求**: 无追踪 · **实现**: 无任何遥测 / 行为分析代码；`manifest.json` 显式声明 `tracking: false, telemetry: false`
- **要求**: Key 不外泄 · **实现**: 任何响应、日志、事件中不出现 Key 明文；导出与摘要内容仅在浏览器本地生成
...
### 来源仓库 · 吸收的能力 · 落点
- **来源仓库**: `dsh-usage-ledger` · **吸收的能力**: 官方定价页动态计价引擎（实时抓取/解析/兜底快照）、峰谷分时计价、多厂商价格目录与专用解析器、缓存命中折扣、日/月双档预算 · **落点**: `src/core/price/`、`src/modules/cost/`
- **来源仓库**: `dsh-conv-export` · **吸收的能力**: PNG 长图光栅导出、免打印对话框多页 PDF（foreignObject → canvas → JPEG → 零依赖 PDF 组装）、图片 data-URL 内联 · **落点**: `src/client/raster.ts`、`src/modules/export/`（kind:'raster' 载荷）
- **来源仓库**: `dsh-conv-search` · **吸收的能力**: 对话内搜索引擎（CSS Custom Highlight API 无侵入高亮）、浮动查找栏（快捷键/查询历史/大小写与全词开关）、流式输出 MutationObserver 重同步与命中锚点保持 · **落点**: `src/client/convsearch/`
