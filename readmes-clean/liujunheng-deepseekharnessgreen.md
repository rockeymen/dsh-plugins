...
### 操作 · 位置 · 说明
- **操作**: 会话管理 · **位置**: 主窗口「数据维护」区 · **说明**: 点击后弹出会话列表（标题 / 工作区 / 状态 / 有无日志），**可勾选（全选/全不选/单选）**后选择**恢复选中**（仅对"已归档"会话生效）或**永久删除选中**
- **操作**: 命令行 · **位置**: `--restore-session ` · **说明**: 复原（取消归档）指定会话
- **操作**: 命令行 · **位置**: `--purge-archived` / `--purge-session ` · **说明**: 永久删除：前者清全部归档，后者删指定会话
...
### 通道 · 更新对象 · 入口 · 更新源
- **通道**: 官方核心 · **更新对象**: dsh 本体（`runtime/dsh/` 的 npm 包） · **入口**: 「检查更新」 · **更新源**: 官方 npm / GitHub
- **通道**: 绿色版外围 · **更新对象**: 启动器 `launcher.py` / `DSH_Launcher.exe` / `plugins/` / 文档等 · **入口**: 「检查绿色版更新」 · **更新源**: 本项目 GitHub Release
...
- **形态**: exe 版 · **入口**: 双击 `DSH_Launcher.exe` · **需要 Python 吗**: 不需要 · **体积/说明**: exe 单文件（约 8MB）内嵌解释器；程序根目录必须与 `runtime/` 同级
- **形态**: 脚本版 · **入口**: 双击 `start.bat` · **需要 Python 吗**: 不需要（用内置） · **体积/说明**: 依赖 `runtime/python`（约 200MB）；内置缺失才回退系统 Python
...
### 问题 · 处理
- **问题**: 提示找不到 Python · **处理**: 说明内置便携 Python 缺失且下载失败（多为网络问题），按 start.bat 里的提示手动安装 Python 3，勾选 Add to PATH 即可兜底
- **问题**: 下载 Node 慢 