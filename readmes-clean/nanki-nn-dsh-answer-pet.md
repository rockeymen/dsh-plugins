...
`dsh-answer-pet` 是一个 DeepSeek Harness Web bundle 插件和可扩展回答状态宠物框架。核心负责会话进度、模型轨迹和状态卡；声明式 `PetTheme v1` 负责宠物 SVG、动画、宽高比和阶段文案。默认使用蓝鲸，也内置橘猫示例主题。
  ...
  ![dsh-answer-pet 蓝鲸宠物、多会话进度卡和模型工具调用轨迹演示](./assets/dsh-answer-pet-demo.gif)
...
### 主题 id · 名称 · 定位
- **主题 id**: `blue-whale` · **名称**: 蓝鲸 · **定位**: 默认主题，保持原有喷水、摆尾、眨眼和完成表情
- **主题 id**: `orange-cat` · **名称**: 橘猫 · **定位**: PetTheme v1 示例，包含摆尾、抬爪、说话和完成表情
...
### 阶段 · 主题接口 · 状态卡
- **阶段**: 空闲 · **主题接口**: `idle` 动画与文案 · **状态卡**: 不显示运行会话卡与数量
- **阶段**: `turn/start` · **主题接口**: `turn` · **状态卡**: 2%
- **阶段**: 思考（`step/start`） · **主题接口**: `think` · **状态卡**: 5% → 10%
- **阶段**: 输出（`assistant/chunk`） · **主题接口**: `stream` · **状态卡**: 10% → 90%，按 token 填充
- **阶段**: 工具（`tool/call`） · **主题接口**: `tool` · **状态卡**: 冻结当前进度并显示工具名
- **阶段**: 完成（`turn/end`） · **主题接口**: `done` · **状态卡**: 100%
