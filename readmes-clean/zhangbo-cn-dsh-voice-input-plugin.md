...
- **Browser**: Chrome / Edge (Windows) · **Mic (input, SpeechRecognition)**: ✅ Web Speech · **Reply playback (speechSynthesis)**: ✅ natural (Google / Edge neural) voices
- **Browser**: Safari · **Mic (input, SpeechRecognition)**: ✅ webkitSpeechRecognition (re-trigger on each gesture) · **Reply playback (speechSynthesis)**: ✅ natural OS voices
- **Browser**: Firefox · **Mic (input, SpeechRecognition)**: ⚠️ **not supported — browser limitation** (Mozilla has not shipped `SpeechRecognition`; local on-device recognition is still early-stage) · **Reply playback (speechSynthesis)**: ⚠️ `speechSynthesis` **is supported** (reply can be read aloud), but voices are OS-default / less natural
