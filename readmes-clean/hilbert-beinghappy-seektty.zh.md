...
### 能力 · 当前可用操作
- **能力**: 对话与运行 · **当前可用操作**: 流式回复、Markdown/GFM、代码块、链接、表格、推理显示切换、工具卡片折叠/展开/隐藏、模型重试、上下文压缩、最大输出与错误状态、Ctrl+C 停止当前轮次
...
- **能力**: 人机交互 · **当前可用操作**: 处理单选、多选、自定义回答、跳过、取消和计划审查；待处理交互失败后可通过 `/pending` 重试
...
- **能力**: 工具与产出文件 · **当前可用操作**: 动态工具目录、工具参数与安全边界说明、结构化结果和通用降级卡片；查看本轮生成文件、复制绝对路径，并在确认后交给外部程序打开
...
- **能力**: 主题 · **当前可用操作**: DeepSeek 暗色与亮色主题、True Color/256 色/16 色降级、`NO_COLOR` 支持；`/theme` 切换后立即生效并由 Harness Settings 持久化
...
### 键位 · 操作
- **键位**: `/` · **操作**: 打开命令与 Skill 候选
- **键位**: Enter / Shift+Enter · **操作**: 发送或确认 / 输入换行
- **键位**: Tab / Escape · **操作**: 在输入区与 Transcript 间切换 / 返回输入区或关闭当前弹窗
- **键位**: Shift+Tab · **操作**: 循环当前权限，进入完全访问前确认
- **键位**: Shift+Left / Shift+Right · **操作**: 跳到上一个或下一个用户轮次
- **键位**: Ctrl+P · **操作**: 打开完整命令面板
- **键位**: Ctrl+M · **操作**: 支持扩展键盘协议时打开模型选择器
- **键位**: Ctrl+S · **操作**: 打开会话恢复选择器
- **键位**: Ctrl+O / Ctrl+T · **操作**: 切换工具卡片显示 / 显示或隐藏推理内容
- **键位**: F2 / Ctrl+, / Cmd+, · **操作**: 打开设置
- **键位**: Ctrl+C · **操作**: 停止当前轮次、清空草稿，或二次确认退出
