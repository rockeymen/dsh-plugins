...
### 字段 · 类型 · 默认值 · 说明
- **字段**: `path` · **类型**: string · **默认值**: `$DSH_HOME/.credentials.yaml` · **说明**: 凭证文件绝对路径（运行时注入 `dshHome` 计算默认值）
- **字段**: `dshHome` · **类型**: string · **默认值**: 运行时注入 · **说明**: Harness home
- **字段**: `allowEnvFallback` · **类型**: boolean · **默认值**: `true` · **说明**: 是否启用继承环境与 `.env` 回退层
- **字段**: `passwordEnv` · **类型**: string · **默认值**: `DSH_CREDENTIAL_PASSWORD` · **说明**: 启动自动解锁密码的环境变量名
- **字段**: `watch` · **类型**: boolean · **默认值**: `true` · **说明**: 是否监听文件热重载
- **字段**: `debounceMs` · **类型**: number · **默认值**: `100` · **说明**: 热重载防抖毫秒数（≥ 0）
- **字段**: `rememberDays` · **类型**: number · **默认值**: `0` · **说明**: 免密登录天数：`0` = 每次都输密码，`1..30` 天，`-1` = 永远（补丁配置的初始值；滑块运行时值写入 `$DSH_HOME/.dsh-encrypt.json` 并优先生效）
- **字段**: `leakGuard` · **类型**: boolean · **默认值**: `true` · **说明**: 泄露检测与输出脱敏总开关（关闭后 HTTP/WS 输出不再扫描）
- **字段**: `leakMinMaskLength` · **类型**: number · **默认值**: `8` · **说明**: 参与脱敏的最小值长度（4–64；更短的值视为普通文本）
- **字段**: `leakMaxMaskLength` · **类型**: number · **默认值**: `256` · **说明**: 参与脱敏的最大值长度（16–1024；同时是流式脱敏的跨块回看窗口）
- **字段**: `maxUnlockAttempts` · **类型**: number · **默认值**: `5` · **说明**: 连续解锁失败多少次后进入锁定（≥ 1）
- **字段**: `lockoutBaseMs` · **类型**: number · **默认值**: `30000` · **说明**: 首次锁定窗口毫秒数（≥ 1000；之后每次失败 ×2）
- **字段**: `lockoutMaxMs` · **类型**: number · **默认值**: `900000` · **说明**: 锁定窗口上限毫秒数（≥ 1000）
...
### code · 含义
- **code**: `PASSWORD_WRONG` · **含义**: 摘要与验证器不匹配（错误密码在触碰任何条目之前即被拒绝）
- **code**: `PASSWORD_INVALID` · **含义**: 摘要不是 64 位小写 hex 的 SHA3-256
- **code**: `LOCAL_ONLY` · **含义**: 该操作（改密/移除密码/免密设置）仅允许 localhost
- **code**: `REMEMBER_EXPIRED` · **含义**: 免密票据已超过其窗口，需重新输入密码
- **code**: `REMEMBER_INVALID` · **含义**: 免密票据与本凭证库不匹配（或库中无票据）
- **code**: `VAULT_LOCKED` · **含义**: 凭证库已锁定；在「设置 → 加密安全」解锁（或导出 `DSH_CREDENTIAL_PASSWORD`）
- **code**: `VAULT_NOT_ENCRYPTED` · **含义**: 尚未设置密码，无锁可解/无密可改/可除
- **code**: `VAULT_ALREADY_ENCRYPTED` · **含义**: 已设密码；修改密码请用 `change-password`
- **code**: `VAULT_CORRUPTED` · **含义**: SHA3-256 完整性校验失败，错误中指名出问题的引用
- **code**: `VAULT_INVALID` · **含义**: 文档结构非法（版本、算法、字段形状）
- **code**: `VAULT_KEY_MISMATCH` · **含义**: GCM 认证失败：主密钥不匹配，或密文被替换
- **code**: `PASSWORD_INVALID` / `MASTER_KEY_INVALID` · **含义**: 参数或密钥材料非法
- **code**: `TOO_MANY_ATTEMPTS` · **含义**: 解锁失败次数过多，已进入锁定窗口（HTTP 429 + `Retry-After`，错误携带 `retryAfterMs`）
- **code**: `INTEGRITY_FAILED` · **含义**: 发行代码完整性校验失败（插件拒绝加载；合法重建后运行 `npm run integrity`）
