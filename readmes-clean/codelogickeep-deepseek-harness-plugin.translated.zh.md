...
### 插件 · 说明 · 形态
- **插件**: **钉钉桥接器** (dsh-dingtalk-bridge) · **说明**: 在钉钉里直接和 DSH Agent 对话，含会话管理控制台、**主动推送**（定时提醒→钉钉） · **形态**: 独立进程 ↔ DSH `/api`
- **插件**: **MiniMax 网页搜索** (minimax-search) · **说明**: 把 MiniMax 搜索注册为 DSH 宿主 Web 搜索 provider，`web_search` 工具直接可用 · **形态**: DSH 宿主插件 (`cordis.patch.yml`)
- **插件**: **官方定时调度** (dsh-schedule) · **说明**: 启用 DSH 官方持久定时任务（`schedule_create`/`list`/`delete`，after/at/every），到期唤醒 Agent · **形态**: DSH 宿主插件 (`cordis.patch.yml`)
...
### # · 插件 · 快速入口
- **#**: 1 · **插件**: 钉钉桥接器 · **快速入口**: [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md) 部署 · [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) 架构
- **#**: 2 · **插件**: MiniMax 搜索 · **快速入口**: [docs/MINIMAX-SEARCH.md](docs/MINIMAX-SEARCH.md) 一键接入
- **#**: 3 · **插件**: 官方定时调度 · **快速入口**: [docs/DSH-NOTES.md](docs/DSH-NOTES.md) 宿主机制 · `cordis.patch.yml` 已启用
- **#**: — · **插件**: 脚手架/方法论 · **快速入口**: [docs/PLUGIN-ECOSYSTEM.md](docs/PLUGIN-ECOSYSTEM.md) · [docs/DSH-NOTES.md](docs/DSH-NOTES.md)
...
### 指令 · 说明
- **指令**: `/status` · **说明**: 查看当前投递目标（会话、项目、标题、状态）
- **指令**: `/list` · **说明**: 按 DSH 工作区分组列出会话（▶=当前目标；`/list all` 平铺含未挂载）
- **指令**: `/use <序号\ · **说明**: 关键词\ · 会话ID>` · 切换投递目标到某个会话（**历史保留，切回可续聊**）
- **指令**: `/new [路径]` · **说明**: 新建一个 DSH 会话并设为当前目标（可选指定项目路径）
- **指令**: `/help` · **说明**: 显示帮助
...
│       └── minimax-search.mjs
├── scripts/
│   └── install-plugins.mjs  # 脚手架：同步 plugins/ → ~/.dsh/profiles//plugins/
├── test/                    # 集成测试（需要 DSH 在线）
