![dsh-lark-meeting-notifier 效果图](docs/screenshot.jpg)
...
### 配置项 · 说明 · 默认
- **配置项**: 提醒提前时间 · **说明**: 多选（分钟），每个是一个独立闹钟 · **默认**: 20、10、5
- **配置项**: 30 秒后自动停止闪烁 · **说明**: 开关 · **默认**: 开
- **配置项**: 提醒时自动展开 · **说明**: 开关 · **默认**: 开
- **配置项**: 刷新间隔 · **说明**: 30 / 60 / 120 秒 · **默认**: 30 秒
- **配置项**: 会议室名显示 · **说明**: 完整 / 简短 · **默认**: 完整

## 故障排查

### 现象 · 原因 · 解决
- **现象**: 面板显示「lark-cli 未安装」 · **原因**: 缺少 `@larksuite/cli` · **解决**: `npm install -g @larksuite/cli`
- **现象**: 面板显示「飞书未授权或缺少日历权限」 · **原因**: 未授权 / scope 缺失 / 登录态过期 · **解决**: `lark-cli auth login --scope "calendar:calendar:readonly"`
- **现象**: 列表为空 · **原因**: 今天确实没有剩余会议 · **解决**: `lark-cli calendar +agenda --as user` 核对

## 技术要点（二次开发参考）

- 数据源：`lark-cli calendar +agenda --as user`（今日日程列表）
- 会议室名：`lark-cli calendar events get --calendar-id primary --event-id  --need-attendee`，取 `type=resource` 参会人的 `display_name`
