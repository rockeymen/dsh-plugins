...
### 键 · 默认 · 说明
- **键**: `enabled` · **默认**: `false` · **说明**: 总开关（设置页可改）
- **键**: `region.target` · **默认**: `cn` · **说明**: 风控谁：`cn`=中国用户 \ · `non-cn`=非中国用户（反向）
- **键**: `region.policy` · **默认**: `block` · **说明**: `block`=拒绝回复 \ · `observe`=只记录+隐写标记 \ · `off`=关闭
- **键**: `region.minSignals` · **默认**: `2` · **说明**: 信号分阈值（strong=2 / medium=1 / weak=0.5）
- **键**: `region.refusalEndsAfter` · **默认**: `3` · **说明**: 拒绝 N 次后结束会话（区域「封号」阶梯）
- **键**: `region.showAttempts` · **默认**: `true` · **说明**: 拒绝文案追加「第 N/M 次尝试」
- **键**: `region.promptEnforcement` · **默认**: `true` · **说明**: 目标命中时注入模型级区域指令
- **键**: `region.ipCheck` · **默认**: `false` · **说明**: 公网 IP 归属查询（联系 ipinfo.io / ip-api.com，**默认关闭**）
- **键**: `region.webRtcCheck` · **默认**: `false` · **说明**: WebRTC IP 探测（联系 Google STUN，**默认关闭**）
- **键**: `region.blockOnUnknown` · **默认**: `false` · **说明**: 完全无信号时是否视为目标命中（fail-closed）
- **键**: `region.cnTimezones` / `region.blacklist` · **默认**: 内置 · **说明**: 中国时区表 / 代理中转域名黑名单（可整体替换）
- **键**: `abuse.*` · **默认**: 内置 · **说明**: 辱骂/严重有害/自伤词表（正则，可整体替换）；`strongPatterns`/`weakPatterns` 分级
- **键**: `abuse.llm.mode` · **默认**: `all` · **说明**: 辱骂判定模式：`all`=全部消息 LLM 语境判定（最准）\ · `fuzzy`=仅弱词调 LLM \ · `off`=纯词表
- **键**: `abuse.llm.enabled` · **默认**: `true` · **说明**: LLM 分类总开关
- **键**: `abuse.llm.provider/model` · **默认**: 跟随会话 · **说明**: 分类请求的模型（缺省复用会话主模型 / agent-default-model）
- **键**: `steganography` · **默认**: `true` · **说明**: 隐写标记
