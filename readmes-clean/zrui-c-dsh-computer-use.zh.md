![DSH Computer Use 图标](docs/assets/app-icon.png)

# DSH Computer Use

  面向 DSH 的文本优先浏览器与 macOS 后台控制。
...
Homebrew Cask 与官方 DMG 安装的是同一份 Universal 2 App，均经过 Developer ID 签名和 Apple 公证。普通用户不需要安装 Xcode、Swift，也不需要下载源码；系统中需要已有 DSH 和 Google Chrome。
  ...
  ![DSH Computer Use 设置中心](docs/assets/setup-center.png)

## 能做什么

### 控制面 · 感知 · 输入
- **控制面**: Chromium · **感知**: Playwright、CDP Accessibility/DOM、frame、标签页、可选 OCR · **输入**: 基于 ref 的导航、鼠标、键盘、表单、滚动、拖拽、上传
- **控制面**: macOS · **感知**: Accessibility 树优先，Vision OCR 补语义，独立窗口捕获 · **输入**: AX 语义动作、定向 SkyLight/CoreGraphics，无目标时才用全局 HID
