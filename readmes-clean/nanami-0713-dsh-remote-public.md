...
### 组件 · macOS · Windows · 说明
- **组件**: dsh-remote 插件（index.js） · **macOS**: ✓ · **Windows**: ✓ · **说明**: Node.js，平台无关
- **组件**: bridge（bridge/server.js） · **macOS**: ✓ · **Windows**: ✓ · **说明**: Node.js，平台无关
- **组件**: Web 客户端（设置中的「手机遥控」页） · **macOS**: ✓ · **Windows**: ✓ · **说明**: 浏览器平台无关
- **组件**: Flutter App · **macOS**: ✓（构建 iOS/Android） · **Windows**: ✓（构建 Android） · **说明**: 开发机均可
- **组件**: 一键启动 · **macOS**: `start_dsh_remote.sh` · **Windows**: `start_dsh_remote.ps1` · **说明**: 插件本身也会自动托管 bridge
...
- **维度**: 控制对象 · **DSH-Remote**: 只控制 DeepSeek Harness (DSH) · **Agents Anywhere**: Codex、Claude Code 等多个 Agent
- **维度**: 定位 · **DSH-Remote**: DSH 专用轻量手机遥控器 · **Agents Anywhere**: 多 Agent、多设备通用控制平台
- **维度**: 架构 · **DSH-Remote**: DSH 插件 + 本地 Node bridge + Flutter App · **Agents Anywhere**: FastAPI 后端 + Connector + Web/Android
- **维度**: 文件/终端 · **DSH-Remote**: 暂不支持 · **Agents Anywhere**: 支持远程文件、shell、终端
- **维度**: 多设备 · **DSH-Remote**: 本机 + 多台已配对手机 · **Agents Anywhere**: 支持多台设备接入
- **维度**: 适合场景 · **DSH-Remote**: 个人远程控制自己的 DSH · **Agents Anywhere**: 多 Agent、团队/自托管场景

简单说：**DSH-Remote 是为 DSH 量身定做的专用遥控器；Agents Anywhere 是面向多种 Agent 的通用平台。**

### 路径 · 说明
- **路径**: `bridge/` · **说明**: Node.js 桥接服务（可独立运行，也是插件的托管对象）
- **路径**: `app/` · **说明**: Flutter 手机 App（原生 + Web 双端）
- **路径**: `index.js` / `client.js` / `cordis.patch.yml` / `package.json` · **说明**: dsh-remote 插件本体
- **路径**: `docs/` · **说明**: 公网访问方案
- **路径**: `start_dsh_remote.sh` · **说明**: 一键启动 DSH + bridge（独立运行模式）
