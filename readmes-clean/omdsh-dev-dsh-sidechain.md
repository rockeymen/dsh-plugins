...
### 命令 · 用途
- **命令**: `/btw <问题>` · **用途**: 发起一次性侧问，适合快速确认信息
- **命令**: `/side <问题>` · **用途**: 创建可持续追问的侧会话
- **命令**: `/side list` · **用途**: 列出当前会话的直接子代理
...
### 配置项 · 默认值 · 说明
- **配置项**: `providerName` · **默认值**: `fork` · **说明**: 用于创建子会话的 subagent provider
- **配置项**: `persona` · **默认值**: 内置侧会话 persona · **说明**: 侧会话的行为约束；空字符串表示沿用部署 persona
- **配置项**: `readOnlyTools` · **默认值**: 未设置 · **说明**: 可选工具 allow-list，例如 `['read', 'grep', 'glob']`
