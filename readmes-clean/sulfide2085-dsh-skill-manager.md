...
### 文件 · 作用
- **文件**: `lib/index.js` · **作用**: host 半：`skillManager` 远程服务（list / content / setEnabled / installZip / 仓库接口）
- **文件**: `lib/skill-files.js` · **作用**: 磁盘约定：扫描根、frontmatter 解析、`.disabled` 启停
- **文件**: `lib/skill-zip.js` · **作用**: ZIP 解析/解压（store+deflate）、CRC32 校验、条目名安全检查、包内技能发现与安装
- **文件**: `lib/skill-repo.js` · **作用**: GitHub 归档下载（大小上限 / 超时 / 重试 / 缓存 / 镜像）、仓库技能扫描、按目录安装
- **文件**: `lib/client.js` · **作用**: 浏览器半：手写 bundle，注册 `settings.section` 分区（id: `skill-manager`，order: 17）
