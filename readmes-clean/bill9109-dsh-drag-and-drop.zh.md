...
### 症状 · 解决
- **症状**: 拖入文件没有任何反应 · **解决**: 重新拖一次并确认全页面提示出现。用 `dsh --profile web --dump-config · grep drag-and-drop` 确认 bundle 在 profile 里，并确认安装后已重启 Web UI + 硬刷新
- **症状**: 多个同名副本时插入了错误路径 · **解决**: 候选先按完整文件名和大小过滤，只在幸存者之间采样内容；仍有完全相同副本时插件会弹出选择列表——在那里选正确的路径
- **症状**: 大磁盘上路径定位很慢 · **解决**: 安装平台索引（Linux：`plocate`；Windows：Everything CLI），并让文件位于 Workspace 或常用目录内。每次搜索都有边界：索引命令 3 秒超时、最多保留 100 个候选、每个递归根最多访问 20,000 个目录项
- **症状**: macOS/Linux：拖入文件夹定位失败 · **解决**: 文件夹按名称匹配 Workspace 和常用目录；浏览器隐藏路径时，位于所有可搜索根之外的文件夹无法定位——把它移进 Workspace，或安装操作系统索引
- **症状**: 安装后插件没有加载 · **解决**: 重启 Web UI 并硬刷新浏览器——client bundle 只在插件进入 `__DSH_BOOT__` 后的全新页面加载时生效
