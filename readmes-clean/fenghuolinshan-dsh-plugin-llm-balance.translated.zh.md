...
### 颜色 · 余额 · 含义
- **颜色**: 🟢 绿 · **余额**: >= 100 · **含义**: 余额充足
- **颜色**: 🟡 黄 · **余额**: 20 ~ 99 · **含义**: 余额一般
- **颜色**: 🔴 红 · **余额**: 1 ~ 19 · **含义**: 余额偏低
- **颜色**: ⚪ 灰 · **余额**: < 1 · **含义**: 余额不足；或查询失败 / 加载中
...
### provider id · 接口 · 口径
- **provider id**: deepseek / deepseek-official · **接口**: `GET https://api.deepseek.com/user/balance` · **口径**: 余额（CNY；官方 `total_balance` 为字符串，数字同样兼容）
- **provider id**: moonshotai / moonshotai-cn · **接口**: `GET https://api.moonshot.cn/v1/users/me/balance` · **口径**: 余额（CNY）
- **provider id**: kimi-coding · **接口**: `GET https://api.kimi.com/coding/v1/usages` · **口径**: 套餐配额（顶层 usage=周限额 + limits 窗口明细（5h 限流等，window 对象归一化为 5h/周），含套餐等级）
- **provider id**: openai-codex · **接口**: `dsh-codex-connect`（`GET https://chatgpt.com/backend-api/wham/usage`） · **口径**: Codex Connect 配额（rateLimits 主 bucket（id `codex`，回退首个）窗口 = 剩余百分比（limit=100，18000s → 5h、604800s → 周，其余稳定时长标签）；可选 individualLimit → 月配额、credits → USD 余额或 Credits 段（`credits.unlimited=true` 时仅因百分比 UI 显示为有限 100/100——绿色 100% 而非灰色 ∞/∞，账户本身仍无限））
- **provider id**: opencode-go · **接口**: `GET https://opencode.ai/zen/go/v1/usage` · **口径**: OpenCode Go 套餐配额（`usage.rolling` → 5h、`usage.weekly` → 周：`percent` 为已用百分比 → amount=100-percent、limit=100，`resetsAt` → 重置时间；monthly 忽略；单个窗口无效只跳过该窗口，至少一个窗口有效才成功。⚠️ 端点目前无官方公开文档，可能变化）
...
### 字段 · 默认值 · 说明
- **字段**: refreshMs · **默认值**: 60000 · **说明**: 前端轮询间隔（毫秒）
- **字段**: timeoutMs · **默认值**: 15000 · **说明**: 服务端查询超时（毫秒）
- **字段**: provider · **默认值**: deepseek · **说明**: （兼容层）单 provider 模式；多 provider 模式无需设置，自动发现
- **字段**: apiKeyEnv · **默认值**: DEEPSEEK_API_KEY · **说明**: （兼容层）单 provider 模式的凭证引用名
- **字段**: baseURL · **默认值**: 按 provider 默认 · **说明**: （兼容层）单 provider 模式的可选 base URL 覆盖
