...
### 功能 · Agent 会做什么
- **功能**: 修改前理解仓库 · **Agent 会做什么**: 阅读适用的仓库指令、当前文档、已有决策、相关源码、测试和生成文件；必要时查看 Git 历史
- **功能**: 维护决策记录 · **Agent 会做什么**: 创建或更新负责当前决定的记录，并在实现、否决、替换或归档时保持生命周期准确
- **功能**: 编写可验证的验收条件 · **Agent 会做什么**: 把每项要求与可观察行为、可能失败的位置，以及能够确认或否定完成状态的证据对应起来
- **功能**: 完成整条集成链路 · **Agent 会做什么**: 根据项目实际结构检查定义、provider、注册或加载、consumer、持久化，以及面向用户或模型的行为
- **功能**: 保持仓库内容一致 · **Agent 会做什么**: 同步更新受影响的代码、公共接口、测试、快照、生成产物、当前文档和决策理由
- **功能**: 支持简化与删除 · **Agent 会做什么**: 调查真实使用方，并清理已经无效的注册项、export、文件、测试、文档和兼容层，而不是只删除主要源码
- **功能**: 只读审查 · **Agent 会做什么**: 在不修改文件的情况下，比较当前决策、源码、测试、生成产物和文档是否一致
- **功能**: 适配已有规范 · **Agent 会做什么**: 复用仓库已有的 ADR、RFC、设计文档或 Spec 体系，不另建一套相互竞争的文档结构
...
### 宿主 · 调用方式
- **宿主**: Codex · **调用方式**: `$ds-spec-loop`
- **宿主**: Claude Code · **调用方式**: `/ds-spec-loop`
- **宿主**: GitHub Copilot CLI · **调用方式**: `/ds-spec-loop`
- **宿主**: 其他兼容 Agent · **调用方式**: 使用该宿主规定的 Skill 调用方式，或直接点名 `ds-spec-loop`
...
### 宿主 · 用户级目录 · 仓库级目录
- **宿主**: Codex · **用户级目录**: `~/.agents/skills/ds-spec-loop` · **仓库级目录**: `.agents/skills/ds-spec-loop`
- **宿主**: Claude Code · **用户级目录**: `~/.claude/skills/ds-spec-loop` · **仓库级目录**: `.claude/skills/ds-spec-loop`
- **宿主**: GitHub Copilot CLI · **用户级目录**: `~/.copilot/skills/ds-spec-loop` 或 `~/.agents/skills/ds-spec-loop` · **仓库级目录**: `.github/skills/ds-spec-loop` 或 `.agents/skills/ds-spec-loop`
