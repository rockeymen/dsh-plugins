...
```bash
node server.mjs              # 默认端口 5177，也可 node server.mjs  指定
...
### 路由 · 说明
- **路由**: `/gold-monitor/` · **说明**: 看板页面（自 `/gold-monitor` 302 重定向，保证相对路径正确）
- **路由**: `/gold-monitor/api/history?range=` · **说明**: 历史数据代理（NBP 每日 / goldprice.dev 1 月 / ECB 汇率折算人民币，含缓存）
...
### 档位 · 数据源
- **档位**: 1 月 · **数据源**: goldprice.dev 每日 OHLC（含最高 / 最低）
- **档位**: 6 月 / 1 年 / 5 年 / 全部 · **数据源**: NBP 波兰央行每日金价（PLN/克），按当日 USD/PLN 汇率折算为美元/盎司，可回溯至 2013 年
- **档位**: 人民币/克曲线 · **数据源**: 按当日 USD/CNY 参考汇率（ECB，frankfurter.dev）折算
- **档位**: 无服务器时 · **数据源**: 降级为 freegoldapi.com 静态数据（跨域直连，仅美元曲线）
