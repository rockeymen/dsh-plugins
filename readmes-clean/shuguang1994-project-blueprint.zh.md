...
### 能力 · 说明
- **能力**: **自主文件发现** · **说明**: 扫描项目，按 30+ 文件名模式自动分类，不预设"有哪些文件"
- **能力**: **项目结构识别** · **说明**: 自动识别单项目/monorepo/前后端分离（2-3层）等项目组织方式
- **能力**: **智能依赖分类** · **说明**: 三层递进：知识库精确匹配 → 29 种命名模式启发推断 → 联网搜索
- **能力**: **业务类型推断** · **说明**: 两层启发式（结构特征 + 配置特征），覆盖 13 种业务类型
- **能力**: **动态 AGENTS.md** · **说明**: 从 70+ 组件知识库实时拼装，非固定模板
- **能力**: **模块速查表生成** · **说明**: 读取实际源码目录，按文件模式推断模块职责，无法确定时联网搜索
- **能力**: **文档体系** · **说明**: A/B/C/D/E 五级分类，按业务类型按需生成
- **能力**: **测试制度** · **说明**: 按项目阶段的分层测试策略，非强行写示例文件
- **能力**: **多 IDE 适配** · **说明**: 自动生成 CLAUDE.md / .cursor/rules / copilot-instructions 等指向文件
- **能力**: **增量模式** · **说明**: 已有项目只补缺失，不覆盖已有配置
- **能力**: **MCP 工具推荐** · **说明**: 按探测技术栈推荐 MCP 工具清单与组合，生成 `docs/B/B-05-MCP工具清单.md`（含安装命令，仅 MD 最小侵入）
- **能力**: **持续自适应** · **说明**: 生成的 AGENTS.md 内含 Agent 主动维护指令，随项目进展自动更新
- **能力**: **基础代码规范实写** · **说明**: 初始化即写入基础代码规范（命名/目录结构/错误处理/日志/安全/性能 6 类），B-01 实写 8 章，非占位符
- **能力**: **AI 高频错误防犯** · **说明**: 内置 7 大类 27 条 AI 高频错误知识库，初始化优先注入防犯规则，BUG 反哺持续迭代

## 生成内容一览

### 产物 · 说明
- **产物**: `AGENTS.md` · **说明**: 项目规范（架构原则约束：高内聚低耦合 / 组合优于继承 / 避免全局状态 / 纯函数优先）
- **产物**: `docs/` · **说明**: A/B/C/D/E 五级分类文档骨架 + 分类 README 维护指令（含 B-01-开发规范，实写 8 章）
- **产物**: `.github/workflows/ci.yml` · **说明**: CI 流水线（语言自适应，支持 GitHub/Gitee/其他）
- **产物**: `.gitignore` · **说明**: 按语言选择的精选规则
- **产物**: `.husky/pre-commit` · **说明**: JS/TS 项目提交前 lint 检查（非 JS 项目跳过）
- **产物**: `CLAUDE.md` · **说明**: Claude Code 指向文件
- **产物**: `.cursor/rules/project.mdc` · **说明**: Cursor 指向文件
- **产物**: `docs/B/B-03-测试指南.md` · **说明**: 测试制度（分层策略、编写时机、框架特定模式）
- **产物**: `docs/B/B-05-MCP工具清单.md` · **说明**: MCP 工具清单 + 组合建议 + 安装命令（按需生成）
...
### 层级 · 组件
- **层级**: **语言** (7) · **组件**: TypeScript/JavaScript, Go, Python, Java, Rust, Ruby, PHP
- **层级**: **框架** (15) · **组件**: NestJS, Next.js, Vue 3, React, Express, FastAPI, Flask, Django, Gin, Spring Boot, SvelteKit, Nuxt 3, Laravel, Hono, uni-app
- **层级**: **ORM** (6) · **组件**: Prisma, TypeORM, Drizzle, GORM, SQLAlchemy, JPA/Hibernate
- **层级**: **CSS** (5) · **组件**: Tailwind CSS, CSS Modules, Scoped CSS, Styled Components, SCSS
- **层级**: **UI 库** (4) · **组件**: Ant Design Vue, Element Plus, Naive UI, Vant
- **层级**: **测试** (6) · **组件**: Vitest, Jest, Pytest, Go testing, JUnit 5, Playwright
- **层级**: **Lint** (5) · **组件**: ESLint, Prettier, Biome, Ruff, golangci-lint
- **层级**: **部署** (5) · **组件**: PM2, Docker, Vercel, Docker Compose, GitHub Pages
- **层级**: **数据库** (2) · **组件**: MySQL, PostgreSQL
- **层级**: + 状态管理(3) + 包管理(5) + 通用规范(4) + 业务类型文档模式(12) = **70+ 组件** · **组件**:  · 
...
### 独创特性 · 说明 · 竞品现状
- **独创特性**: **全生命周期一站式生成** · **说明**: 一句话产出 AGENTS.md + 文档体系 + CI/CD + 测试制度 + Git 规范 · **竞品现状**: 竞品仅生成 AGENTS.md 单个文件
- **独创特性**: **自主发现引擎** · **说明**: 三层递进分类（精确匹配→模式推断→联网搜索），非简单读取 package.json · **竞品现状**: 竞品使用固定模板或简单文件扫描
- **独创特性**: **持续进化机制** · **说明**: 生成的 AGENTS.md 内含 Agent 主动维护规则，随项目成长自动更新 · **竞品现状**: 竞品生成静态文件，无自我维护能力
- **独创特性**: **业务类型感知** · **说明**: 13 种业务类型推断，驱动不同的文档结构生成 · **竞品现状**: 无竞品推断项目类型
- **独创特性**: **增量质量检测** · **说明**: 自动评估已有 AGENTS.md 质量，分级处理（完善→跳过/缺失→补充/无→全量） · **竞品现状**: 竞品全部覆盖或从头生成
- **独创特性**: **多 IDE 生态适配** · **说明**: 自动生成 CLAUDE.md / .cursor/rules / copilot-instructions 等指向文件 · **竞品现状**: 无竞品提供
- **独创特性**: **模块速查表自动生成** · **说明**: 读取实际源码目录，按文件模式推断职责，无法确定时联网搜索 · **竞品现状**: 无竞品提供
- **独创特性**: **MCP 工具自动推荐** · **说明**: **按**探测技术栈三层递进自动匹配 MCP 工具，输出组合建议（必装/推荐/可选）+ 可执行安装文档；双层联网保障命令不过时 · **竞品现状**: 竞品仅预设固定 MCP 配置（如 Project Genesis Phase 9），无自主推荐
- **独创特性**: **7 语言 15 框架知识库** · **说明**: 70+ 组件含 Commands + Conventions + CI，中文为第一语言 · **竞品现状**: 竞品最多覆盖 JS/TS 生态
