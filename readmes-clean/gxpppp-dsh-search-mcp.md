...
### 字段 · 说明
- **字段**: `id` · **说明**: 服务器唯一标识，`defaultServer` 用它引用
- **字段**: `kind` · **说明**: `tavily` / `brave` / `exa` / `perplexity` / `duckduckgo` / `custom`，自动补全默认端点、鉴权方式和工具名
- **字段**: `transport` · **说明**: `http`（streamable-http，默认）或 `stdio`（本机命令，如 duckduckgo）
- **字段**: `url` · **说明**: http 端点，如 `https://mcp.tavily.com/mcp/`
- **字段**: `command` / `args` · **说明**: stdio 启动命令，如 `npx` + `["-y", "duckduckgo-mcp-server"]`
- **字段**: `apiKey` · **说明**: 密钥（secret 字段，界面按密码框显示、落盘脱敏）
- **字段**: `apiKeyEnv` · **说明**: 或填环境变量/凭证名（如 `TAVILY_API_KEY`），运行时经 dsh 凭证服务解析
- **字段**: `authStyle` / `authParam` · **说明**: key 的注入方式：http 用 `query`（如 `tavilyApiKey`）或 `header`（如 `x-api-key`）；stdio 时 `authParam` 即注入的环境变量名（如 `BRAVE_API_KEY`）
- **字段**: `toolName` · **说明**: 该 MCP 的搜索工具名，如 `tavily_search` / `brave_web_search` / `web_search_exa` / `pplx_search` / `ddg_web_search`
- **字段**: `maxResults` · **说明**: 单服务器结果数上限（可选，默认取全局 `maxResults`）
...
- **kind**: `tavily` · **默认端点**: `https://mcp.tavily.com/mcp/` · **key 位置**: query `tavilyApiKey` · **默认工具**: `tavily_search` · **结果数参数**: `max_results`
- **kind**: `brave` · **默认端点**: `https://mcp.brave.com/mcp/` · **key 位置**: query `braveApiKey` · **默认工具**: `brave_web_search` · **结果数参数**: `count`
- **kind**: `exa` · **默认端点**: `https://mcp.exa.ai/mcp` · **key 位置**: header `x-api-key` · **默认工具**: `web_search_exa` · **结果数参数**: `numResults`
- **kind**: `perplexity` · **默认端点**: `https://mcp.perplexity.ai/mcp/` · **key 位置**: query `pplx_api_key` · **默认工具**: `pplx_search` · **结果数参数**: `max_results`
- **kind**: `duckduckgo` · **默认端点**: `npx -y duckduckgo-mcp-server`（stdio，免 key） · **key 位置**: — · **默认工具**: `ddg_web_search` · **结果数参数**: —
- **kind**: `custom` · **默认端点**: 自己填 · **key 位置**: 自己选 · **默认工具**: 自己填 · **结果数参数**: —
