...
### Tool · Description
- **Tool**: `read` · **Description**: Read source files with line numbers. Supports UTF-8, BOM-detected UTF-16, and WHATWG encoding labels. Also reads PDFs.
- **Tool**: `grep` · **Description**: Search file contents with Rust regex or literal strings.
- **Tool**: `glob` · **Description**: Find files. Gitignored files are included by default; `.git` and common large directories stay excluded.
- **Tool**: `run_program` · **Description**: Run one program with a literal argument list, without a shell.
- **Tool**: `bash` · **Description**: Run a POSIX bash command line and return merged stdout and stderr.
- **Tool**: `bash_status` · **Description**: Inspect one detached Bash job and its bounded log tail.
...
### Layer · Value · Meaning
- **Layer**: Codex per-item truncation · **Value**: 10,000 tokens or bytes · **Meaning**: Client history cap after `Wall time:` / `Output:`
- **Layer**: Server content ceiling · **Value**: 9,872 · **Meaning**: 10,000 minus 128 wrapper tokens
- **Layer**: Per-call ceiling · **Value**: 8,192 · **Meaning**: Both profiles; a single page cannot exceed this currently
- **Layer**: Burst aggregate · **Value**: profile default · **Meaning**: Remaining budget split across in-flight calls

### Value · Per-call token ceiling · Default burst tokens
- **Value**: `codex` (default) · **Per-call token ceiling**: 8,192 · **Default burst tokens**: 16,384
- **Value**: `cursor` · **Per-call token ceiling**: 8,192 · **Default burst tokens**: 32,768
...
### Value · Behavior
- **Value**: `unrestricted` (default) · **Behavior**: Any absolute path readable by the server user.
- **Value**: `normal` · **Behavior**: Repository paths plus Codex skill/plugin directories. Credentials and history under `.codex` stay inaccessible.
...
- **Parameter**: `pdf_mode` · **Values**: `auto` (default), `text`, `image` · **Meaning**: `auto`/`text` return page Markdown; `image` renders PNG content blocks.
- **Parameter**: `pages` · **Values**: `"7"` or `"7-12"` · **Meaning**: One page or one continuous range.
- **Parameter**: `pdf_cursor` · **Values**: opaque token · **Meaning**: Replayed verbatim from a previous response. Carries the source version and, when the response stopped inside a page, where to resume in it.

Page counts bound the work of one call:

### Mode · Without `pages` · Explicit range cap
- **Mode**: `auto`, `text` · **Without `pages`**: first 10 pages · **Explicit range cap**: 20 pages
- **Mode**: `image` · **Without `pages`**: page 1 · **Explicit range cap**: 4 pages
...
- **Variable**: `CODEX_MCP_PROTOCOL_VERSION` · **Default**: — · **Description**: MCP protocol version advertised to Codex.
- **Variable**: `AGENTSHIM_PROCESS_CALLS` · **Default**: `16` · **Description**: Per-instance concurrent process-call limit; 1–32.
- **Variable**: `AGENTSHIM_DETACHED_CALLS` · **Default**: `16` · **Description**: Per-instance live detached `bash` trees; 1–16.
- **Variable**: `AGENTSHIM_OUTPUT_BYTES` · **Default**: `32000` · **Description**: Per-call output ceiling in bytes; 4096–262144.
- **Variable**: `AGENTSHIM_BURST_TOKENS` · **Default**: profile default · **Description**: Shared projected model-token budget; 2048–32768.
- **Variable**: `AGENTSHIM_TOOL_TIMEOUT_SHELF` · **Default**: `600` · **Description**: The shelf value the server stays below so the client's `tool_timeout_sec` fires after the server's own Timeout. Effective max execution time is shelf minus 10 seconds; 15–3600.
- **Variable**: `AGENTSHIM_IDLE_TIMEOUT` · **Default**: off · **Description**: Idle shutdown in seconds for the `codex` profile only; 1–86400. Inbound JSON-RPC messages reset the deadline, and active foreground calls or detached trees defer shutdown.
- **Variable**: `AGENTSHIM_GREP_MEMORY_BYTES` · **Default**: `268435456` · **Description**: Per-call hard limit for retained `grep` candidates.
- **Variable**: `AGENTSHIM_GLOB_MEMORY_BYTES` · **Default**: `33554432` · **Description**: Per-call hard limit for retained `glob` matches.
- **Variable**: `AGENTSHIM_PDF_TEXT_MEMORY_BYTES` · **Default**: `67108864` · **Description**: Per-call memory budget for `auto`/`text` PDF reads.
- **Variable**: `AGENTSHIM_PDF_IMAGE_MEMORY_BYTES` · **Default**: `100663296` · **Description**: Per-call memory budget for `image` PDF reads.
- **Variable**: `AGENTSHIM_BASH` · **Default**: probed · **Description**: Absolute path to a GNU bash. In the DSH adapter, the same key set in plugin config `env` also drives load-time bash discovery, so it need not be preset on the host process.
- **Variable**: `AGENTSHIM_LOG_MODE` · **Default**: `errors` · **Description**: One of `off`, `errors`, `all`.
- **Variable**: `AGENTSHIM_LOG_DIR` · **Default**: platform default · **Description**: Override the log directory with an absolute path.
- **Variable**: `AGENTSHIM_RESPECT_GITIGNORE` · **Default**: `false` · **Description**: When `true`, `grep` and `glob` apply `.gitignore` / `.ignore` filters. Omitted `include_ignored` follows this default. Because the caller cannot read this setting, an empty result under active filtering ends with a line recommending `include_ignored=true`. `.git` and `node_modules`, `target`, `.venv`, `venv`, `dist`, `build`, `__pycache__` stay excluded either way. Binary, output-budget, and memory limits still apply.
