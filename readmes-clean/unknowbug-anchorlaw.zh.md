...
- **组件**: **扫描器** · **Python**: ✅ [anchorlaw-scanner](python/anchorlaw-scanner/) · **TypeScript**: ✅ [anchorlaw-scanner](typescript/anchorlaw-scanner/) · **成熟度**: **已验证** — 在真实项目上测试通过
- **组件**: **锚点系统** · **Python**: ✅ [anchorlaw](python/anchorlaw/) · **TypeScript**: — · **成熟度**: **实验性** — API 稳定，缺乏实践效能数据
- **组件**: **Source Provenance (v0.3/v0.7)** · **Python**: ✅ `source` 参数 + probe 类型（v0.7） · **TypeScript**: — · **成熟度**: **SCOPED** — Python 已实现（source 参数、缺失/static 对 test 标 INVALID、`probe:` 类型 v0.7）；1 个项目（CoreSwap）产出过带 source 的 anchor
- **组件**: **噪声卡** · **Python**: ✅ [anchorlaw](python/anchorlaw/) · **TypeScript**: — · **成熟度**: **未验证** — schema 已定义，无项目积累超过 10 张卡
- **组件**: **AI 上下文注入** · **Python**: ✅ [anchorlaw](python/anchorlaw/) · **TypeScript**: — · **成熟度**: **猜想** — 格式已定义，未做 A/B 对照实验
- **组件**: **降级验证 (v0.3)** · **Python**: — · **TypeScript**: — · **成熟度**: **猜想** — 三种模式已定义，无参考宿主之外的项目走过 Partial/Degraded 路径
...
- **模式**: **全功能** · **条件**: 代码自包含 + Anchorlaw 已安装 · **anchorlaw test**: ✅ · **置信度自动提升**: ✅
- **模式**: **半功能** · **条件**: 代码有未解析的外部依赖 · **anchorlaw test**: ❌ · **置信度自动提升**: ❌（anchor 仍记录 source，验证推迟）
- **模式**: **降级** · **条件**: Anchorlaw 未安装 · **anchorlaw test**: ❌ · **置信度自动提升**: ❌（人工对照 trace 审查）

这不是防御性条款——是诚实地标注当前能做到什么。详见 [协议规范 §9](spec/protocol-v0.18.md#9-degraded-verification-v03-draft)。
...
宿主级安装把 4 个 `anchorlaw_*` 工具**全局挂载**：以 `insert` 形态写入 `<dshHome>/profiles/` 下**每个 profile** 的 `cordis.patch.yml`（DSH 唯一读取的用户补丁层，热重载；`-Profile <name>` 可只挂一个），插件文件落 `/plugins/anchorlaw/`。挂载前由 `dsh/tests/check_plugin_schema.mjs` 把关——每个工具的 `parameters` 必须是编译后的 JSON Schema 对象根；扁平 spec 会以无顶层 type 的形态到达模型，导致所有会话报错（2026-08-13 事故门禁）。
...
├── spec/
│   └── protocol-v0.18.md       # 语言无关的协议规范（当前版）
├── python/
