...
### 变量 · 作用
- **变量**: `AGENTRQ_BASE_URL` · **作用**: 当前 AgentRQ 对外访问地址，例如 `http://localhost:2026`
- **变量**: `AGENTRQ_DOMAIN` · **作用**: 域名，不带协议，例如 `localhost`
- **变量**: `AGENTRQ_AUTH_JWT_SECRET` · **作用**: 签发会话 JWT 的密钥，建议 32 字符以上随机值
- **变量**: `AGENTRQ_AUTH_WORKSPACE_TOKEN_KEY` · **作用**: MCP workspace token 加密密钥，必须正好 32 bytes
- **变量**: `AGENTRQ_AUTH_ROOT_LOGIN_ENABLED` · **作用**: 是否启用 root login，建议仅本地初始化使用
- **变量**: `AGENTRQ_AUTH_ROOT_ACCESS_TOKEN` · **作用**: root login 使用的访问令牌
- **变量**: `AGENTRQ_ACCOUNTS_OAUTH2_CLI_GOOGLE_CLIENT_ID` · **作用**: Google OAuth2 Client ID
- **变量**: `AGENTRQ_ACCOUNTS_OAUTH2_CLI_GOOGLE_CLIENT_SECRET` · **作用**: Google OAuth2 Client Secret
- **变量**: `AGENTRQ_SQLITE_ENABLED` · **作用**: 是否使用 SQLite
- **变量**: `AGENTRQ_POSTGRES_ENABLED` · **作用**: 是否使用 PostgreSQL
