...
> **状态:0.7.0 —— 实验性。** 硬性索引上限(`PreToolUse` hook)对匹配到的直接编辑工具(`Edit|Write|MultiEdit`)确定性拦截、但**不是全局写保护**(shell 类工具——Bash、PowerShell、后台 Monitor 命令——以及 MCP 文件工具/外部编辑器/同步程序绕得过);纪律以**常驻规则**形式加载、靠模型遵守,**尽力而为、不保证每个任务都生效**(见 [SKILL.md](SKILL.md) §8)。假设**单写者/串行写入**。暂时别把它当"强制、可靠、跨 Agent"的记忆层来用。
...
- **Engramory** · **存储**: md 文件 · **召回**: 读索引 → 开文件 · **人能读**: ✅ · **类型本体论**: ✅ 角色式(4类) · **策展纪律**: ✅ 契约(模型执行) · **有界索引**: ✅ 150/200 行+字节 + hook · **基础设施**: 无
- CC 原生记忆 · **存储**: md 文件 · **召回**: 读索引 → 开文件 · **人能读**: ✅ · **类型本体论**: ✅ 同 4 类 · **策展纪律**: 部分(自动) · **有界索引**: ~200 行窗口* · **基础设施**: 无(内置)
- basic-memory · **存储**: md + SQLite · **召回**: 语义/全文检索 · **人能读**: ✅ · **类型本体论**: ✅ 自由 type · **策展纪律**: schema + 覆写检查 · **有界索引**: ❌(无加载索引) · **基础设施**: SQLite + 向量
- obsidian-second-brain · **存储**: md 库 · **召回**: 索引优先 + 检索 · **人能读**: ✅ · **类型本体论**: 文件夹分类 · **策展纪律**: ✅ 对账/lint · **有界索引**: 部分 · **基础设施**: 无
- mem0 / Zep · **存储**: 向量/图数据库 · **召回**: 语义 · **人能读**: ❌(DB) · **类型本体论**: 有类型(偏好/情景/程序; Zep 自定义) · **策展纪律**: 自动抽取 · **有界索引**: 不适用 · **基础设施**: 数据库 + 向量
- [agentmemory](https://github.com/rohitg00/agentmemory) · **存储**: SQLite + 向量索引(+可选图) · **召回**: 混合 BM25+向量(+可选图),RRF · **人能读**: ❌(DB/引擎) · **类型本体论**: ✅ 4 层生命周期(工作/情景/语义/程序) · **策展纪律**: 自动(捕获+去重+衰减) · **有界索引**: 不适用 · **基础设施**: iii 引擎(本地)+可选向量
...
默认创建 `/.engramory-memory/`。如果你已有记忆目录,传
