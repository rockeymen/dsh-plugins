...
### 配置项 · 类型 · 默认值 · 说明
- **配置项**: `includeSections` · **类型**: boolean · **默认值**: `true` · **说明**: 翻译系统提示词 sections（身份、工具指导等）
- **配置项**: `includeContexts` · **类型**: boolean · **默认值**: `true` · **说明**: 翻译运行时上下文 contexts（沙箱策略、审批策略、委托声明等）
- **配置项**: `includeTools` · **类型**: boolean · **默认值**: `true` · **说明**: 翻译工具描述与参数描述
- **配置项**: `verbose` · **类型**: boolean · **默认值**: `false` · **说明**: 调试日志
...
### 字典 · 条目数 · 覆盖内容
- **字典**: `core.json` · **条目数**: 23 · **覆盖内容**: 身份行、沙箱/审批策略、委托声明、工作区指令
- **字典**: `tools-fs-bash.json` · **条目数**: 39 · **覆盖内容**: read/write/edit/bash 工具与参数
- **字典**: `tools-fs-core.json` · **条目数**: 221 · **覆盖内容**: pwsh/fs/cordis 工具、plan-mode、sandbox 等
- **字典**: `tools-web-jobs-goal.json` · **条目数**: 264 · **覆盖内容**: web/jobs/goal/ralph/workflow/subagent 等
- **字典**: `cordis-api.json` · **条目数**: 1241 · **覆盖内容**: cordis 完整 API 目录（55 服务 + 56 事件 + 方法/参数/返回值）
