...
The original OpenClaw adapter was measured in a seven-turn workflow that installed, authenticated, and queried `bilibili-mcp`:
  ...
  ![Seven-turn token comparison](docs/images/token-comparison.png)

### Turn · Without Graph Memory · With Graph Memory
- **Turn**: R1 · **Without Graph Memory**: 14,957 · **With Graph Memory**: 14,957
- **Turn**: R4 · **Without Graph Memory**: 81,632 · **With Graph Memory**: 29,175
- **Turn**: R7 · **Without Graph Memory**: **95,187** · **With Graph Memory**: **23,977**
...
- **Stage**: OpenClaw origin · **Deliverable**: Context Engine, cross-session graph memory, dual-path recall · **Status**: Maintained
- **Stage**: Community graph engine · **Deliverable**: SQLite, FTS5, vectors, graph ranking, provenance · **Status**: Available
- **Stage**: DeepSeek Harness · **Deliverable**: Cordis adapter, native tools, auto-recall, Credentials · **Status**: Implemented and tested
- **Stage**: Graph Memory Pro · **Deliverable**: Visual graph workbench, controlled drag-and-drop, optional Neo4j · **Status**: Architecture reviewed; DSH Host and Client Plugins not yet implemented
...
On March 15, 2026, the project owner presented Graph Memory's architecture at the CLAW program event held in Tsinghua Science Park. The following owner-supplied materials and the [Sina Finance event report](https://cj.sina.com.cn/articles/view/7984421895/1dbe89c0700101nnpq) document that development.
  ...
  ![Graph Memory technical sharing](docs/images/history/tsinghua-sharing.jpg)
  ![Sina Finance event coverage](docs/images/history/sina-report.jpg)
...
The image below is the existing OpenClaw / ClawX-era Pro graph prototype. It demonstrates a previously explored interaction direction; it is not a shipped DSH frontend.
  ...
  ![Existing Graph Memory Pro prototype](docs/images/graph-ui.png)
...
- **Capability**: Native Cordis loading · **Status**: **Done** · **Notes**: No DSH fork required
- **Capability**: Cross-session auto-recall · **Status**: **Done** · **Notes**: Injected during Prompt Assembly
- **Capability**: Explicit record and search · **Status**: **Done** · **Notes**: `gm_record`, `gm_search`
- **Capability**: Vector backfill and migration · **Status**: **Done** · **Notes**: Model, dimension, and fingerprint tracked
- **Capability**: Visible plugin state · **Status**: **Done** · **Notes**: Active in Plugin Inventory
- **Capability**: Pro visual workbench · **Status**: **Not shipped** · **Notes**: Requires a DSH Client Plugin
...
Current beta: `1.6.0-beta.1`. Local acceptance used DeepSeek Harness `0.1.0-rc.5`. DSH remains in Developer Preview and may introduce compatibility-breaking changes. Testing covered tarball installation, active plugin state, 1024-dimensional vector backfill, semantic recall across Sessions, persistence across restarts, and FTS5 fallback. All 107 automated tests passed.
  ...
  ![Graph Memory active in the DSH plugin list](docs/images/dsh/plugin-inventory-active.png)

  Cross-session semantic recall in a fresh Session
  ![Cross-session vector recall in DSH](docs/images/dsh/vector-cross-session-recall.png)
...
### Tool · Purpose
- **Tool**: `gm_status` · **Purpose**: Plugin, store, extraction, recall, and vector state
- **Tool**: `gm_search` · **Purpose**: Explicit long-term graph search
- **Tool**: `gm_record` · **Purpose**: Persist a TASK, SKILL, or EVENT
- **Tool**: `gm_stats` · **Purpose**: Node, edge, type, and community statistics
