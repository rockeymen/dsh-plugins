...
### Package · Contents
- **Package**: `@rbelem/dsh-tui` · **Contents**: entry bundle: cordis patch layer + runtime glue plugin that spawns the TUI with `DSH_PORT`
- **Package**: `@rbelem/dsh-tui-linux-x64` / `-linux-arm64` · **Contents**: prebuilt binary for linux x64 / arm64
- **Package**: `@rbelem/dsh-tui-darwin-x64` / `-darwin-arm64` · **Contents**: prebuilt binary for macOS x64 / arm64
...
### Keys · Action
- **Keys**: `j`/`k`, `↑`/`↓` · **Action**: scroll chat / move sidebar selection / move picker selection
- **Keys**: `g`/`Home`, `G`/`End` · **Action**: jump to top / bottom of the chat
- **Keys**: `Ctrl+d` / `Ctrl+u` · **Action**: scroll half a page (chat)
- **Keys**: `Ctrl+d` · **Action**: quit from the composer (EOF)
- **Keys**: `Enter` · **Action**: submit composer; switch to the selected sidebar session; apply picker selection
- **Keys**: `Tab` · **Action**: cycle focus: chat → composer → sidebar
- **Keys**: `Ctrl+w` then `h`/`j`/`k`/`l` · **Action**: move focus between panes (sidebar / chat / composer)
- **Keys**: `Esc` · **Action**: return to chat (closes popups, pickers, editors)
- **Keys**: `n` · **Action**: new-session picker (chat or sidebar focus; `j`/`k` move, `Enter` create)
- **Keys**: `r` · **Action**: rename the selected sidebar session (inline editor: type, `Enter` commit, `Esc` cancel)
- **Keys**: `f` · **Action**: fork the selected sidebar session
- **Keys**: `a` · **Action**: archive the selected sidebar session
- **Keys**: `v` · **Action**: arm mouse selection mode in the chat (`v select · esc cancel`; drag to select, release to copy)
- **Keys**: `i` · **Action**: open the image viewer on the session's images (chat focus)
- **Keys**: `s` · **Action**: toggle the narrow-terminal session drawer (below 80 columns)
- **Keys**: `q` · **Action**: quit (chat or sidebar focus)
- **Keys**: `Ctrl+p` · **Action**: launcher: fuzzy search over commands, cached skills, and settings actions
- **Keys**: `Ctrl+t` · **Action**: theme picker (`j`/`k` move, `Enter` apply, `Esc` close)
- **Keys**: `Ctrl+,` · **Action**: settings view (note: unreachable from a raw terminal byte stream — crossterm maps `0x0c` to `Ctrl+l`; use the launcher's "open settings" action)
- **Keys**: `Ctrl+l` · **Action**: cycle UI locale (en ↔ zh), persisted
- **Keys**: `Ctrl+c` · **Action**: cancel the running turn; quit when idle
- **Keys**: `Ctrl+q` · **Action**: quit
- **Keys**: `Alt+q` · **Action**: queue popup: `j`/`k` scroll, `x` remove, `s` steer, `e` edit, `Esc` close
- **Keys**: `Shift+Enter` · **Action**: insert a newline in the composer (see composer notes below)
...
1. `build.sh --target ` builds one target into
   `dist/dsh-tui-<target>/` (binary + `SHA256SUMS`, byte-pinned) — darwin
   targets build on macOS runners.
2. `prebuild-packages.sh` assembles the four `prebuilds//`
