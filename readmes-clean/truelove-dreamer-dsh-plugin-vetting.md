...
### 威胁 · 输出 · 含义
- **威胁**: **恶意**（外传凭据、eval、持久化） · **输出**: risk 分数 → SAFE/LOW/MEDIUM/HIGH · **含义**: 蓄意滥用，需人工核实
- **威胁**: **误伤**（非恶意，但高权限路径写得太糙） · **输出**: `suggest narrowing` 建议区 · **含义**: 不扣分、不标可疑，只建议收窄权限
- **威胁**: **运行时动态加载**（下载后 eval、远端模块） · **输出**: 文档声明 + `[REVIEW]` 标记 · **含义**: 静态扫描看不到，**不在扫描范围**
...
### 能力 · 说明
- **能力**: `plugin_vet` 工具 / `/plugin-vet` 命令 · **说明**: 扫 `$DSH_HOME/profiles/*/node_modules` 下所有 `dsh-plugin-*`（含 `@scope/dsh-plugin-*`）第三方包
- **能力**: 恶意规则（15 条） · **说明**: 网络外传、凭据访问、代码执行/混淆、持久化、读会话日志、生命周期脚本（**含 install/prepare/prepublishOnly**——git 依赖安装时 prepare 也会执行）
- **能力**: 误伤规则（3 条） · **说明**: home 目录宽松读取、字符串拼接路径、递归遍历 home——标为"建议收窄"而非可疑
- **能力**: 传递依赖 · **说明**: 统计声明依赖数 + 就地扫描嵌套 `node_modules/*` 的生命周期脚本，报告"N 个未检查"
- **能力**: `[REVIEW]` 标记 · **说明**: eval/new Function 命中即降级信任，不看总分
- **能力**: **覆盖范围报告** · **说明**: 每个包报告"扫了 N 个文件、M 行代码、X 个依赖/脚本"，让边界显式可见
- **能力**: **运行时表面** · **说明**: 每个包报告 child_process / fetch / eval / socket 的**出现次数**——静态能告诉你插件运行时能触及哪些危险原语
- **能力**: **官方包哈希基线**（默认开） · **说明**: `@deepseek-ai/*` 豁免的同时记录内容哈希基线（`$DSH_HOME/.dsh-plugin-vetting/baseline.json`）；安装包与基线不一致 → 豁免自动失效并报警（供应链投毒检测）——从"信任名字"变"信任内容"
- **能力**: **插件工具门禁**（默认关） · **说明**: `config.gate: "deny-unvetted"` 时，tools 管线拒绝调用**门禁安装后注册**的工具（即插件提供的工具），除非工具名在 `allowlistTools` 白名单。挂载在 `tools.guard`（即 `tools/pre-execute` 同一道闸）——**只拦"模型→插件工具"的调用面**，不碰插件进程内代码
- **能力**: 运行时绊线（可选，仅日志） · **说明**: `config.monitor: true` 时监控 **harness 的 `ctx.subprocess` 通道**，可疑命令记警告日志；**只记不改**
- **能力**: 官方豁免 · **说明**: `@deepseek-ai/*` 自动豁免（但见哈希基线）
- **能力**: 白名单 · **说明**: `config.allowlist` 放行可信插件
