...
Host-level install mounts the 4 `anchorlaw_*` tools globally: it appends an `insert` row to every profile's `cordis.patch.yml` under `<dshHome>/profiles/` (the ONLY user patch layer DSH reads; hot-reloaded) — `-Profile <name>` mounts one profile only — and copies the plugin to `/plugins/anchorlaw/`. The mount is gated by `dsh/tests/check_plugin_schema.mjs`, which verifies every tool's `parameters` is a compiled JSON-Schema object root — a flat spec would reach the LLM without a top-level type and break every session (2026-08-13 incident guard).

Project-level install places the 11 `anchor-*` skills under `/.dsh/skills/` (DSH's native project-scoped root) — a session opened inside that project loads them, sessions elsewhere do not. DSH has no project-level plugin mechanism yet (upstream suggestion: [deepseek-ai/deepseek-harness discussion #306](https://github.com/deepseek-ai/deepseek-harness/discussions/306)); the `anchorlaw_*` tools come from the host-level global mount.
