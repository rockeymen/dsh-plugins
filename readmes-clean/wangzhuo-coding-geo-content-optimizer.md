...
- **变量**: `PIPELINE_LLM_MODEL` · **必填**: 否 · **默认值**: 空（同 OPENAI_MODEL_NAME） · **说明**: Pipeline 专用模型
- **变量**: `PIPELINE_MAX_CONCURRENCY` · **必填**: 否 · **默认值**: `4` · **说明**: 分块处理 / 联网检索并行并发度
- **变量**: `PIPELINE_MAX_INPUT_CHARS` · **必填**: 否 · **默认值**: `200000` · **说明**: 流水线输入文本上限（字符），超过自动截断并告警；`0` 关闭（token 成本控制）
- **变量**: `PIPELINE_STAGE3_MAX_SLICES` · **必填**: 否 · **默认值**: `0` · **说明**: Stage 3 提示词最多嵌入的切片数（top-K 截断，降 token 成本）；`0` 不限制
- **变量**: `PIPELINE_LENGTH_TOLERANCE` · **必填**: 否 · **默认值**: `0.10` · **说明**: 字数校准容差（±10%）。在 `user_requirements` 中检测到字数要求（如「≥1800字」「约1500字」）时，流水线程序侧测字数并定向扩写/压缩直到落带
- **变量**: `PIPELINE_MAX_LENGTH_ATTEMPTS` · **必填**: 否 · **默认值**: `2` · **说明**: 字数校准最大修正轮数（模型不会数数，靠程序测量+定向修正收敛；超限后接受残余偏差并告警）
- **变量**: `PIPELINE_QUALITY_JUDGE` · **必填**: 否 · **默认值**: `true` · **说明**: 是否启用 Stage 5 LLM 评分（关闭则只跑确定性 lint，零 token 成本、运行更快）
- **变量**: `PIPELINE_QUALITY_THRESHOLD` · **必填**: 否 · **默认值**: `80` · **说明**: GEO 基线达标线（默认 80，对齐项目硬要求 GEO≥80 才可发布；接入 judge prompt，并作为双闸门的 GEO 门）
- **变量**: `PIPELINE_SEARCH_MAX_QUERIES` · **必填**: 否 · **默认值**: `3` · **说明**: 联网检索每个文本提取的查询数（调小可缩短联网阶段耗时）
- **变量**: `PIPELINE_GATE_HARD_DATA` · **必填**: 否 · **默认值**: `false` · **说明**: 可选闸门：素材显著数字（≥1000）未出现在正文时拦截"硬数据模糊化"（默认关，避免误报）
- **变量**: `PIPELINE_TEMPERATURE_GEN` / `PIPELINE_TEMPERATURE_REWRITE` · **必填**: 否 · **默认值**: `0.8` / `0.4` · **说明**: Stage3 生成 / Stage4 改写的温度（设 0 可近似复现，便于调试）
- **变量**: `PIPELINE_REQUIREMENT_SATISFACTION_THRESHOLD` · **必填**: 否 · **默认值**: `80` · **说明**: 用户要求满足度达标线（双闸门第二轴门，仅当传入 user_requirements 时启用）
- **变量**: `GEO_API_KEY` · **必填**: 否 · **默认值**: 空 · **说明**: HTTP 写接口鉴权（配置后需 `Authorization: Bearer`；默认服务仅绑定 127.0.0.1，未配置 key 时启动会打印安全警告）
- **变量**: `TAVILY_API_KEY` · **必填**: 否 · **默认值**: - · **说明**: Tavily 搜索 Key（不配则用免费引擎）
- **变量**: `PGDATABASE_URL` · **必填**: 否 · **默认值**: — · **说明**: PostgreSQL 连接（配置后持久化）
- **变量**: `S3_*` · **必填**: 否 · **默认值**: — · **说明**: S3 存储配置
- **变量**: `HTTP_PORT` · **必填**: 否 · **默认值**: `5000` · **说明**: HTTP 服务端口
- **变量**: `HTTP_HOST` · **必填**: 否 · **默认值**: `127.0.0.1` · **说明**: HTTP 服务绑定地址（默认仅本机；需对外暴露时改 `0.0.0.0` 并设置 `GEO_API_KEY`）
- **变量**: `MCP_SSE_HOST` · **必填**: 否 · **默认值**: `127.0.0.1` · **说明**: MCP SSE 服务绑定地址（默认仅本机）
- **变量**: `HTTP_RATE_LIMIT_PER_MINUTE` · **必填**: 否 · **默认值**: `60` · **说明**: 每客户端 IP 每分钟最大请求数（写接口限流；0 关闭）
- **变量**: `MCP_SSE_PORT` · **必填**: 否 · **默认值**: `5001` · **说明**: MCP SSE 模式端口
...
### 工具 · 命令 · 作用
- **工具**: pytest + pytest-cov · **命令**: `pytest` · **作用**: 全量测试 + 覆盖率报告（当前 `--cov-fail-under=65` 防回归门槛；ECC 目标 80%，缺口为 web_ui 回调 / main.py HTTP 边角，属后续回归计划）
- **工具**: ruff · **命令**: `ruff check src tests` · **作用**: 代码规范（lint + import 排序）
- **工具**: bandit · **命令**: `bandit -r src -ll` · **作用**: 安全静态扫描（仅中高危）
...
### 层级 · 技术
- **层级**: Agent 框架 · **技术**: LangChain + LangGraph (create_react_agent)
- **层级**: LLM 调用 · **技术**: ChatOpenAI（兼容所有 OpenAI API）
- **层级**: Web API · **技术**: FastAPI + Uvicorn
- **层级**: Web UI · **技术**: Streamlit（可选）
- **层级**: MCP 协议 · **技术**: FastMCP（接入 Claude 等 AI 工具）
- **层级**: CLI · **技术**: argparse + asyncio
- **层级**: 重试机制 · **技术**: tenacity（指数退避 + JSON 自修复）
- **层级**: 持久化 · **技术**: PostgreSQL（可选）+ S3（可选）