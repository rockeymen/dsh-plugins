...
### Tool · Purpose
- **Tool**: `kanban_get` · **Purpose**: Read the current board (optional `includeArchived` for the trash)
- **Tool**: `kanban_add_card` · **Purpose**: Add a card (title, note, optional label / priority / color / dueDate)
- **Tool**: `kanban_update_card` · **Purpose**: Update title / note / label / priority / color / dueDate
- **Tool**: `kanban_delete_card` · **Purpose**: Soft-delete into the trash
- **Tool**: `kanban_restore_card` · **Purpose**: Restore a card from the trash
- **Tool**: `kanban_purge_card` · **Purpose**: Permanently delete a card
- **Tool**: `kanban_bulk_delete_cards` · **Purpose**: Bulk soft-delete multiple cards
- **Tool**: `kanban_move_card` · **Purpose**: Move across columns, or reorder within a column via `toIndex`
- **Tool**: `kanban_duplicate_card` · **Purpose**: Duplicate a card (carries all fields)
- **Tool**: `kanban_move_card_to_workspace` · **Purpose**: Move a card to another workspace's board
- **Tool**: `kanban_undo` · **Purpose**: Undo the last write operation
- **Tool**: `kanban_add_column` / `kanban_rename_column` / `kanban_delete_column` · **Purpose**: Column management
