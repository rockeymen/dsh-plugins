...
- **开环故障**: 无反馈 · **agent 上的表现**: 工具失败了照样重试，错上加错 · **本插件的对策**: 反馈回路：估计失败率，超阈值自动收紧
- **开环故障**: 无保护 · **agent 上的表现**: `rm -rf`、`git push --force` 直接执行 · **本插件的对策**: 前馈拦截：不可逆操作先 block，要确认
- **开环故障**: 无稳定边界 · **agent 上的表现**: 一轮里调几十次工具，上下文爆炸 · **本插件的对策**: 稳定阀：每轮调用计数，超限警告收敛
- **开环故障**: 无自检 · **agent 上的表现**: 任务需要的工具根本不存在，空转半天 · **本插件的对策**: 能控性检查：开干前先验证工具齐备
- **开环故障**: 振荡 · **agent 上的表现**: 同一命令成功/失败反复横跳 · **本插件的对策**: 振荡检测：结果翻转率过高即告警
- **开环故障**: 空转 · **agent 上的表现**: 相同参数的工具反复调用 · **本插件的对策**: 冗余检测 + 工具分布熵度量
...
### 工具 · 作用
- **工具**: `cybernetics_snap` · **作用**: 状态快照写入日志；`mode` 参数可手动临时切档（60 秒后恢复自适应）
- **工具**: `cybernetics_status` · **作用**: 读最近 N 条日志，输出 pHat / 档位 / 发散 / 振荡 / 熵 / 观测完整率摘要
- **工具**: `cybernetics_check` · **作用**: 能控性自检：目标 + 所需工具 → missing / controllable / advice
