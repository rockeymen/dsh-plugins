...
# 🐋 DeepSeek Harness Desktop
  ...
  · [Harness 上游记录](HARNESS_UPSTREAM.md)
...
### 平台 · 发行格式 · 使用方式
- **平台**: Windows x64 · **发行格式**: Portable ZIP · **使用方式**: 解压后双击 `DeepSeek Harness.exe`
- **平台**: Linux x64 · **发行格式**: AppImage · **使用方式**: 添加执行权限后直接运行
- **平台**: Debian / Ubuntu x64 · **发行格式**: DEB · **使用方式**: 使用系统包管理器安装
- **平台**: macOS Apple Silicon · **发行格式**: DMG · **使用方式**: 适用于 M 系列芯片
- **平台**: macOS Intel · **发行格式**: DMG · **使用方式**: 适用于 Intel 芯片
...
### 特性 · 实现
- **特性**: 🪶 **轻量** · **实现**: 使用系统 WebView；Windows 是 WebView2，macOS 是 WKWebView，Linux 是 WebKitGTK，不随应用捆绑 Chromium。
- **特性**: 🧩 **能力完整** · **实现**: 继续使用 Harness 的 Agent、会话、工具、插件、Typert RPC 与 Cordis profile，没有第二套业务内核。
- **特性**: 📦 **真正便携** · **实现**: 各平台发行包内置匹配平台的 Node.js 与完整生产运行时，用户不需要手动配置开发环境。
- **特性**: 🐋 **启动有反馈** · **实现**: 启动页按“读取便携运行时 → 启动本地服务 → 载入工作区”展示真实阶段，不用再等待 AppData 解压。
- **特性**: 🎨 **原生体验** · **实现**: 32px 自绘标题栏、深浅主题同步、优雅的鲸鱼加载动画，并抑制后台控制台闪窗。
- **特性**: 🖼️ **拖入附件** · **实现**: 图片继续走 Harness 原生预览与历史画廊；其他文件由独立、可安装的 DSH 插件提供拖放、输入卡片、历史卡片与下载。
- **特性**: 🔍 **源码完整** · **实现**: `harness/` 由本仓库直接跟踪，不是 Git 子模块；普通 clone 就能获得完整源码。
...
### 命令 · 用途
- **命令**: `pnpm tauri dev` · **用途**: 启动 Harness 与 Tauri 开发环境
- **命令**: `pnpm run harness:prepare` · **用途**: 校验、安装并按需构建 Harness
- **命令**: `pnpm run plugin:sync` · **用途**: 按 `external-plugins.json` 准备外部插件 checkout 与依赖
- **命令**: `pnpm run plugin:test` · **用途**: 构建并测试桌面桥及锁定版本的外部插件
- **命令**: `pnpm run check` · **用途**: 校验 Harness 源码完整性、TypeScript 与 Rust
- **命令**: `pnpm run build:frontend` · **用途**: 只构建桌面启动外壳，不生成原生发行包
- **命令**: `pnpm run build` · **用途**: 构建当前平台的完整自包含发行包
- **命令**: `pnpm run test:release` · **用途**: 在 Windows 上真实启动并冒烟测试 portable ZIP
- **命令**: `pnpm run verify:release-artifacts` · **用途**: 校验平台产物及 SHA-256
...
### 平台 · Release 资产命名
- **平台**: Windows x64 · **Release 资产命名**: `DeepSeek-Harness-Desktop-<version>-windows-x64-portable.zip`
- **平台**: Linux x64 · **Release 资产命名**: `DeepSeek-Harness-Desktop-<version>-linux-x64.AppImage`、`.deb`
- **平台**: macOS Apple Silicon · **Release 资产命名**: `DeepSeek-Harness-Desktop-<version>-macos-arm64.dmg`
- **平台**: macOS Intel · **Release 资产命名**: `DeepSeek-Harness-Desktop-<version>-macos-x64.dmg`
