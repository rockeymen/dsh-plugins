...
## 效果长这样
  ...
  ![dsh-overdrive 动画演示](https://raw.githubusercontent.com/temotee2103/dsh-overdrive/main/docs/demo-animation.gif)
...
### 平台 · 接入方式 · 状态
- **平台**: **Telegram** · **接入方式**: Bot API（长轮询） · **状态**: ✅ 真机验证通过（2026-08-16）
- **平台**: **WhatsApp** · **接入方式**: Baileys + 扫码配对，原生交互按钮 · **状态**: ✅
- **平台**: **Discord** · **接入方式**: Bot token，原生按钮 · **状态**: ✅
- **平台**: **Slack** · **接入方式**: Socket Mode（免公网 URL） · **状态**: ✅
- **平台**: **飞书** · **接入方式**: 官方 SDK，长连接 · **状态**: ✅
- **平台**: **钉钉** · **接入方式**: Stream 模式（WebSocket，免公网 URL） · **状态**: ✅
- **平台**: **企业微信** · **接入方式**: 回调 API（AES 加解密） · **状态**: ✅
- **平台**: CLI · **接入方式**: stdin/stdout（开发与 E2E） · **状态**: ✅
...
### 命令 · 作用
- **命令**: `/help` · **作用**: 命令列表
- **命令**: `/trace` · **作用**: 回放最近一轮轨迹（思考 + 工具调用）
- **命令**: `/task <需求>` · **作用**: 派发子任务
- **命令**: `/cron <分 时 日 月 周> <需求>` · **作用**: 定时任务（自带 5 字段调度器）
- **命令**: `/agents` · **作用**: 子任务状态（简化）
- **命令**: `/new` · **作用**: 重置会话
