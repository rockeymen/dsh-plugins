...
### 后端 · 实际执行 · 语法 / 路径 · 环境变量
- **后端**: `powershell`（默认） · **实际执行**: `pwsh -NoLogo -NoProfile -NonInteractive -Command <cmd>` · **语法 / 路径**: PowerShell；`C:\...` · **环境变量**: `$env:NAME`
- **后端**: `gitbash` · **实际执行**: Git for Windows `bash -lc <cmd>` · **语法 / 路径**: POSIX；`/d/WorkSpace`；PATH 含 `/usr/bin`、`/mingw64/bin` · **环境变量**: `$NAME`
- **后端**: `wsl` · **实际执行**: `wsl [-d <distro>] -e bash -lc <cmd>` · **语法 / 路径**: Linux；`/mnt/d/...` · **环境变量**: `$NAME`（经 WSLENV）
...
### 键 · 默认 · 说明
- **键**: `defaultShell` · **默认**: `powershell` · **说明**: 设置未覆盖时的后端
- **键**: `timeoutMs` · **默认**: 120000 · **说明**: 默认超时
- **键**: `maxTimeoutMs` · **默认**: 600000 · **说明**: 调用方 timeoutMs 上限
- **键**: `pwshPath` · **默认**: 自动探测 · **说明**: 固定 pwsh.exe 路径
- **键**: `gitBashPath` · **默认**: 自动探测 · **说明**: 固定 git bash.exe 路径
- **键**: `wslPath` · **默认**: 自动探测 · **说明**: 固定 wsl.exe 路径
