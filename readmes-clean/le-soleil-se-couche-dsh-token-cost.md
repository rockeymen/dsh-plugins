# dsh-token-cost
  ...
  ![dsh-token-cost](docs/banner.png)
...
- **单对话视图**：对话页面底部官方状态行（「首 token 平均 … · … tok/s」之后）直接嵌入本会话消耗费用，点击即可打开按请求的明细弹窗（时间 / 模型 / 缓存未命中 / 缓存命中 / 输出 / 费用，最新在上）。
  ...
  ![对话底部状态行费用展示](docs/screenshots/conversation-bottom.png)

  ![费用明细弹窗](docs/screenshots/cost-detail.png)
...
### 键 · 类型 · 默认 · 说明
- **键**: `enabled` · **类型**: boolean · **默认**: `true` · **说明**: 总开关（状态行费用显示 + 汇总卡片）
- **键**: `currency` · **类型**: 'cny' · **默认**: 'usd' · **说明**: `'cny'` · 显示币种
- **键**: `priceMode` · **类型**: 'auto' · **默认**: 'scheme-a' · **说明**: 'scheme-b' · `'auto'` · 自动按记录时间切换
- **键**: `customPrices` · **类型**: string (JSON) · **默认**: `''` · **说明**: 按模型覆盖价格
