...
### 系统 · 写法 · 例子
- **系统**: Windows · **写法**: `file:///盘符:/完整路径` · **例子**: `file:///D:/deepseek-harness-master/.dsh/lan-gate/lan-gate.mjs`
- **系统**: macOS / Linux · **写法**: `file:///完整路径` · **例子**: `file:///home/me/.dsh/lan-gate/lan-gate.mjs`
...
### 现象 · 原因与解决
- **现象**: 启动时报 `ERR_UNSUPPORTED_ESM_URL_SCHEME` · **原因与解决**: 第 2 步的路径没写成 `file:///` 形式，改过来再重启
- **现象**: `/lan-gate/status` 打不开或 404 · **原因与解决**: 插件没加载成功，检查第 2 步配置，重启后看启动日志
- **现象**: 面板显示「出错：EADDRINUSE」 · **原因与解决**: 3088 端口被占用：打开 `lan-gate.mjs`，把顶部 `PROXY_PORT = 3088` 改成其它端口（如 `3089`），重启 DSH
- **现象**: 手机打不开 `http://<电脑IP>:3088` · **原因与解决**: ① 确认手机和电脑在同一网络；② 系统防火墙放行：Windows 管理员 PowerShell 执行 `netsh advfirewall firewall add rule name="DSH LAN" dir=in action=allow protocol=TCP localport=3088`
- **现象**: 设置里看不到「内网访问」 · **原因与解决**: 面板只在本机显示，内网设备看不到是正常的；本机看不到说明插件未加载（回第 3 步验证）
- **现象**: 重启后手机又要重新批准 · **原因与解决**: 批准记录在 `<DSH数据目录>\lan-gate-state.json`，文件被删除或 DSH_HOME 变了才会丢；从旧版（临时插件）升级的用户需重新批准一次
- **现象**: 手机显示「已绑定到其他浏览器」 · **原因与解决**: 该设备的令牌已发放给另一个浏览器（例如换浏览器、清了 Cookie、或同 IP 的别的设备抢先认领了）。在本机面板撤销该设备再重新批准，手机会自动重新认领
- **现象**: 页面出现 429（请求过于频繁） · **原因与解决**: 该 IP 超过每分钟 120 个请求上限，等一分钟自动恢复；正常使用不会触发（网页应用远比这个频率低）
- **现象**: 用反向代理/隧道转发到本插件时全部被拒 · **原因与解决**: 前置代理过来的请求源 IP 不是本机局域网地址，需要在**前置代理**上配置登录认证后放行；然后把 `LISTEN_HOST` 改为 `127.0.0.1` 只让本机代理接入
...
### 常量 · 默认值 · 环境变量 · 说明
- **常量**: `PROXY_PORT` · **默认值**: `3088` · **环境变量**: `LAN_GATE_PORT` · **说明**: 内网访问端口
- **常量**: `LISTEN_HOST` · **默认值**: `0.0.0.0` · **环境变量**: `LAN_GATE_HOST` · **说明**: 监听地址；放在前置代理/隧道后建议改为 `127.0.0.1`
- **常量**: `RATE_LIMIT_PER_MIN` · **默认值**: `120` · **环境变量**: — · **说明**: 每 IP 每分钟请求上限，超限返回 429
- **常量**: `TARGET_HOST` · **默认值**: `127.0.0.1` · **环境变量**: — · **说明**: DSH 服务地址
- **常量**: `TARGET_PORT` · **默认值**: `3080` · **环境变量**: — · **说明**: DSH 服务端口
