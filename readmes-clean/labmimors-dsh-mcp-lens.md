...
For an immutable production reference, pin the reviewed rc.7 commit: `f21169f921e7ed032a4db5062685afb6f948c2d1`.
  ...
  ![Live DeepSeek Harness comparison: MCP Lens reduced model-visible tools, request tool JSON, and estimated API cost while both arms completed three of three tasks](assets/mcp-lens-comparison.svg)
...
- **Your problem**: **API input grows with every MCP tool** · **What MCP Lens changes**: The MCP surface always starts with only `mcp_search` and `mcp_call`. In our live three-task pilot, estimated V4 Flash cost fell **88.702%**.
- **Your problem**: **Large tool lists consume standing context** · **What MCP Lens changes**: With the same 1,000-tool server, complete Harness request-tool JSON fell from **674,249 B to 27,401 B**.
- **Your problem**: **You worry routing will reduce task completion** · **What MCP Lens changes**: In the tested customer, Chinese-ticket, and GitHub tasks, Lens and the direct client both completed **3/3** with correct arguments and results.
- **Your problem**: **Many similar tools widen the choice set** · **What MCP Lens changes**: Search narrows what the model sees at once, returns exact `inputSchema` values, and calls an explicit `server/tool` identity.
- **Your problem**: **Every server connects even when unused** · **What MCP Lens changes**: Connections are lazy. Activation starts no MCP process and opens no MCP socket.
- **Your problem**: **One server outage should not block the rest** · **What MCP Lens changes**: Other servers keep working, and Lens keeps the previous usable catalog when a refresh fails.
- **Your problem**: **Risky tools should be hidden by default** · **What MCP Lens changes**: No remote tool appears until it matches `allowTools`; `denyTools` always wins in search and calls.
...
- **Choose**: Official `@deepseek-ai/dsh-mcp-client` · **When it fits best**: You have a few stable tools that are used on most turns and want the simplest direct path.
- **Choose**: MCP Lens · **When it fits best**: You have dozens to thousands of tools, several MCP servers, long-tail capabilities, or repeated context/cost pressure.
...
- **Metric**: Recall@5 · **Released rc.7 ranker**: 0.062610 · **rc.8 candidate v3**: 0.246656 · **Difference**: +0.184046
- **Metric**: MRR · **Released rc.7 ranker**: 0.119999 · **rc.8 candidate v3**: 0.258684 · **Difference**: +0.138685
- **Metric**: nDCG@5 · **Released rc.7 ranker**: 0.051830 · **rc.8 candidate v3**: 0.204307 · **Difference**: +0.152477
...
- **Metric across three tasks**: Completed tasks · **Official direct client**: 3 / 3 · **MCP Lens**: 3 / 3 · **Difference**: Tie
- **Metric across three tasks**: Model-visible tools per request · **Official direct client**: 1,025 · **MCP Lens**: 27 · **Difference**: 97.366% fewer
- **Metric across three tasks**: `request/header.tools` JSON · **Official direct client**: 674,249 B · **MCP Lens**: 27,401 B · **Difference**: 95.936% smaller
- **Metric across three tasks**: Uncached input tokens · **Official direct client**: 199,751 · **MCP Lens**: 21,713 · **Difference**: 89.130% fewer
- **Metric across three tasks**: Cache-read input tokens · **Official direct client**: 934,912 · **MCP Lens**: 74,496 · **Difference**: 92.032% fewer
- **Metric across three tasks**: Estimated API cost · **Official direct client**: $0.0307204 · **MCP Lens**: $0.0034707 · **Difference**: 88.702% lower
...
- **Remote MCP tools**: 12 · **Direct-client schema JSON**: 4,862 B · **Lens schema JSON**: 1,114 B · **Reduction**: 77.088%
- **Remote MCP tools**: 100 · **Direct-client schema JSON**: 62,062 B · **Lens schema JSON**: 1,114 B · **Reduction**: 98.205%
- **Remote MCP tools**: 1,000 · **Direct-client schema JSON**: 647,962 B · **Lens schema JSON**: 1,114 B · **Reduction**: 99.828%
...
- **Field**: `catalogTtlMs` · **Default**: `86400000` · **Purpose**: Refresh a catalog after 24 hours
- **Field**: `idleDisconnectMs` · **Default**: `300000` · **Purpose**: Close an idle server after 5 minutes
- **Field**: `connectTimeoutMs` · **Default**: `30000` · **Purpose**: Connection deadline
- **Field**: `callTimeoutMs` · **Default**: `60000` · **Purpose**: Tool-call deadline
- **Field**: `discoveryTimeoutMs` · **Default**: `30000` · **Purpose**: Whole paginated discovery deadline
- **Field**: `maxDiscoveryPages` · **Default**: `1000` · **Purpose**: Maximum pages per discovery
- **Field**: `maxToolsPerServer` · **Default**: `10000` · **Purpose**: Maximum tools accepted from one server
- **Field**: `maxBytesPerTool` · **Default**: `1048576` · **Purpose**: Maximum projected metadata bytes per tool
- **Field**: `maxTotalCatalogBytes` · **Default**: `67108864` · **Purpose**: Maximum total catalog/cache bytes
- **Field**: `maxHttpResponseBytes` · **Default**: `16777216` · **Purpose**: Ma