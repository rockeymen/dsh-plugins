...
- **Tool**: `godot_run_project` · **Replaces (godot-mcp)**: `run_project` · **Purpose**: Launch the project in debug mode (`godot -d --path …`), wait for port 9090
- **Tool**: `godot_stop_project` · **Replaces (godot-mcp)**: `stop_project` · **Purpose**: Terminate the game process (tree-scoped kill)
- **Tool**: `godot_get_debug_output` · **Replaces (godot-mcp)**: `get_debug_output` · **Purpose**: Incremental stdout/stderr of the launched process
- **Tool**: `godot_command` · **Replaces (godot-mcp)**: all `game_*` (~130) · **Purpose**: Send any interaction-server command: `get_scene_tree`, `get_ui_elements`, `eval`, `get/set_property`, `call_method`, `click`, `key_press`, `screenshot`, `raycast`, `serialize_state`, `ui_*`, …
- **Tool**: `godot_screenshot` · **Replaces (godot-mcp)**: `game_screenshot` · **Purpose**: Viewport capture as base64 PNG
- **Tool**: `godot_ping` · **Replaces (godot-mcp)**: — · **Purpose**: Probe whether the game answers on 9090 (also reports installed/latest plugin version)
- **Tool**: `godot_headless_op` · **Replaces (godot-mcp)**: `read_scene`, `modify_scene_node`, `remove_scene_node`, `attach_script`, `create_resource`, `save_scene`, `create_scene`, `add_node`, `get_uid`, `manage_scene_signals`, … · **Purpose**: Headless static operations (`godot --headless --script godot_operations.gd`): 16 ops, no running game needed
- **Tool**: `godot_validate_script` · **Replaces (godot-mcp)**: `validate_script` · **Purpose**: Headless GDScript compile-check via `validate_script.gd` → `{valid, errors}`
- **Tool**: `godot_set_project_setting` · **Replaces (godot-mcp)**: `modify_project_settings`, `set_main_scene`, `manage_layers`, `manage_plugins`, `manage_translations` · **Purpose**: Set a typed key in any project.godot section (`PackedStringArray(...)` / `Vector2i(...)` / bool / …)
- **Tool**: `godot_manage_autoloads` · **Replaces (godot-mcp)**: `manage_autoloads` · **Purpose**: List / add / remove autoload singletons (`Name="*res://…"`)
- **Tool**: `godot_manage_input_map` · **Replaces (godot-mcp)**: `manage_input_map` · **Purpose**: List / add / remove input actions — **correct Godot 4 keycodes** (fixes godot-mcp's Godot 3 baseline bug)
- **Tool**: `godot_manage_export_presets` · **Replaces (godot-mcp)**: `manage_export_presets` · **Purpose**: List / add / remove export presets (`export_presets.cfg`)
- **Tool**: `godot_create_script` · **Replaces (godot-mcp)**: `create_script` · **Purpose**: GDScript template (extends / class_name / method stubs / source)
- **Tool**: `godot_create_project` · **Replaces (godot-mcp)**: `create_project` / `create_csharp_script` · **Purpose**: Project scaffold, optional Godot .NET `.csproj`
- **Tool**: `godot_export_project` · **Replaces (godot-mcp)**: `export_project` · **Purpose**: Headless export (`--export-release` / `--export-debug  <output>`)
