...
### Item · Description
- **Item**: Package · **Description**: [`dsh-workbench-plugin`](https://www.npmjs.com/package/dsh-workbench-plugin)
- **Item**: Version · **Description**: **0.1.8** (npm tag `latest`)
- **Item**: Registry · **Description**: https://registry.npmjs.org
...
- **Name**: **bash** · **Selection criteria**: `$SHELL` is bash; otherwise the default when `$SHELL` is not one of the remaining rows · **Assist verification**: Verified (including `failglob` and interactive history expansion)
- **Name**: **zsh** · **Selection criteria**: `$SHELL` is zsh · **Assist verification**: Verified (including default `nomatch`). Interactive zsh does **not** treat `#` as a comment by default, so notes are not written as a bare `#` line
- **Name**: **sh** · **Selection criteria**: `$SHELL` is sh; otherwise the fallback when bash and zsh are unavailable · **Assist verification**: Verified. `/bin/sh` may be a symlink to bash or dash; the symlink target is used as-is
- **Name**: **dash** · **Selection criteria**: Only when `$SHELL` is explicitly dash (`/bin/dash`, `/usr/bin/dash`, or `/usr/local/bin/dash`) · **Assist verification**: Same POSIX `:` no-op as sh. Dash is **not** included in the default candidate list
