...
### 理论 · 插件里的落地
- **理论**: 控制论：参考信号校准（闭环控制错误目标 = 白费） · **插件里的落地**: 规划代理先定义**答案空间**（`scope`：研究支撑什么判断/决策）与每个子问题的**验收标准**（`acceptance`），再开始研究
- **理论**: Ashby 必要多样性定律（控制器多样性 < 系统多样性 ⇒ 必有盲区） · **插件里的落地**: 规划代理枚举主题的**信息维度**，每个子问题映射一个维度，并输出**覆盖度自检** `coverage_gaps`
- **理论**: 信息论：信息 = 不确定性的减少 · **插件里的落地**: 研究子代理维护三态证据 `confirmed / uncertain / gaps`——条件熵的工程表达；报告保留置信度与矛盾，不掩盖不确定性
- **理论**: 信息论：边际信息增益（EIG）递减 ⇒ 无限搜索是错的 · **插件里的落地**: 每个研究子代理：预测（针对哪个高熵点、预期新增什么）→ 行动（web_search/web_fetch）→ 更新证据 → **边际增益验证**；**连续一轮零增益即停** + 轮次硬上限
- **理论**: 控制论：自适应控制（流程是活的，不是固定脚本） · **插件里的落地**: 研究阶段是**闭环再规划**：第 1 轮并行研究全部子问题；每轮结束收集 high-priority 缺口 → **自动派发下一轮补充研究**；规划声明的"盲区"会被**定向侦察验证**（假设被实验检验而非静态接受）；简单主题一轮收敛，复杂主题自动扩展，直到边际增益 ≈ 0
- **理论**: 信息论：率失真（给定"率"最小化失真） · **插件里的落地**: 综合子代理把证据**有损压缩**为最终报告：只保留对结论有区分度的信息，决策有用性最大化
- **理论**: 信息论：信道冗余/纠错（对抗幻觉=噪声） · **插件里的落地**: 可选**对抗性审查**子代理 = 奇偶校验：引用抽查（URL 可达性/支撑性）、覆盖度审计、矛盾与过度自信标注
...
```bash
dsh plugin --profile  add git+https://github.com/dsh-external/dsh-deep-research.git
dsh --profile         # 重启生效：工具 deep_research 随 profile 注入
```
...
dsh plugin --profile  remove @dsh-external/dsh-deep-research
...
### 参数 · 必填 · 说明
- **参数**: `topic` · **必填**: 是 · **说明**: 研究主题
- **参数**: `purpose` · **必填**: 否 · **说明**: 研究用途（要支撑的判断/决策）——用于定义答案空间；缺省时规划代理声明假设用途
- **参数**: `questions` · **必填**: 否 · **说明**: 已有问题清单（每行一个）；提供则跳过自动拆解
- **参数**: `depth` · **必填**: 否 · **说明**: 精度/容差：1=初步 2=深入（默认）3=穷尽——决定研究闭环轮次上限（depth+1）
- **参数**: `synthesize` · **必填**: 否 · **说明**: 综合子代理出最终报告（默认 true）；false 只返回三态证据
- **参数**: `review` · **必填**: 否 · **说明**: 对抗性审查（默认 false）：引用纠错 + 覆盖度审计 + 矛盾/过度自信标注

## 配置（可选）

### Key · 默认 · 说明
- **Key**: `subagentProvider` · **默认**: 引擎默认 `spawn` · **说明**: 子代理 provider
- **Key**: `maxParallel` · **默认**: `4` · **说明**: 每轮研究并发上限
- **Key**: `maxTotalAgents` · **默认**: 引擎上限 · **说明**: 整次运行子代理总数上限
- **Key**: `plannerModel` / `researcherModel` / `synthesizerModel` / `reviewerModel` · **默认**: 继承父配置 · **说明**: 模型分级（OpenAI 指南：规划/综合用强模型，执行用便宜模型）
