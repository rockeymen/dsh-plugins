...
### 渠道 · 位置 · 说明
- **渠道**: 浏览器通知（页面内横幅）＋ 系统原生通知（可选） · **位置**: 客户端 · **说明**: 两个独立设置项：「浏览器通知」在页面可见时于右上角弹出文字横幅；「系统原生通知」通过浏览器 Notification API 弹出操作系统通知，标签页在后台/最小化时也能收到（需浏览器通知权限）；完全离开页面时请配合系统通知使用
- **渠道**: 系统通知 · **位置**: 宿主机 · **说明**: macOS `osascript` / Linux `notify-send` / Windows PowerShell 原生 toast（Windows 10/11 操作中心），浏览器关闭也能收到；可选系统提示音（macOS `afplay` / Windows 系统内置提示音）
- **渠道**: 飞书群机器人 · **位置**: 宿主机 · **说明**: 文本消息，可选签名密钥（timestamp + HMAC-SHA256），可选自定义消息模板
- **渠道**: 钉钉群机器人 · **位置**: 宿主机 · **说明**: 文本消息，可选加签（timestamp + sign），可选自定义消息模板
- **渠道**: 企业微信群机器人 · **位置**: 宿主机 · **说明**: 文本消息，可选自定义消息模板
- **渠道**: 通用 Webhook · **位置**: 宿主机 · **说明**: 自定义 URL + headers + JSON/文本模板，占位符 `{{title}} {{body}} {{kind}} {{sessionId}} {{turn}} {{toolName}} {{reason}} {{time}}`，可对接 Slack / Discord / ntfy / Bark / Server酱 / PushPlus 等
