...
### Industry flow · Whale
- **Industry flow**: Doubao writes novel / script / shots · **Whale**: = DSH session model doing story/script/storyboard (pick a Doubao model and it IS "Doubao writes")
- **Industry flow**: MJ hero asset · **Whale**: = master-asset stage (MJ official API adapter to come; wanx/Seedream can substitute)
- **Industry flow**: image variations · **Whale**: = shot-assets stage (wanx ✅ verified · **Doubao Seedream ✅ wired**)
- **Industry flow**: Seedance video · **Whale**: = video stage (**Doubao Seedance ✅** / jimeng / kling / ComfyUI)
- **Industry flow**: JianYing final cut · **Whale**: = final-cut stage (JianYing draft export ✅ / ffmpeg ✅)
...
- **Provider**: jimeng (即梦) · **Channel**: sessionid, free daily quota · **Status**: ✅ protocol verified end-to-end; **text-to-video queue stays `SystemBusy` even off-peak (0 credits consumed)** — free route is now: wanx images → image-to-video
- **Provider**: tongyi-wanx (通义万相) · **Channel**: cookie+xsrf, free credits · **Status**: ✅ **live-verified: real whale image generated & downloaded** (free tier = text-to-image; video needs membership)
- **Provider**: kling official (可灵) · **Channel**: accessKey:secretKey JWT, api-beijing.klingai.com · **Status**: ✅ adapter written — not yet tested against a real key
- **Provider**: kling via DashScope · **Channel**: `sk-` key · **Status**: ✅ adapter written — not yet tested against a real key
- **Provider**: **kling-lipsync (可灵对口型)** · **Channel**: official JWT, `/v1/videos/lip-sync` · **Status**: ✅ adapter written against the official 3-13 contract (audio2video + text2video voice modes), 8 tests — not yet tested against a real key
- **Provider**: **wan video via DashScope (通义万相视频)** · **Channel**: `sk-` key, official free quota · **Status**: ✅ adapter written (same async protocol as kling) — model id to confirm on first real key
- **Provider**: doubao (火山方舟) · **Channel**: ARK API key · **Status**: ✅ Seedance video + **Seedream image** (assets) — not yet tested against a real key
- **Provider**: **doubao-web (豆包网页版)** · **Channel**: cookie, free web quota · **Status**: ✅ **live-replayed & parsed**: SSE chat for the LLM stages (story/script/shots) + image bot for assets. Pro-tier free quota runs on a 7-day window (image bot pauses when spent; text keeps working)
- **Provider**: ComfyUI local · **Channel**: workflow JSON builder + /prompt protocol · **Status**: ✅ protocol-tested (mock server), real GPU pending
- **Provider**: kling web (sessionid) · **Channel**: anti-bot one-time falcon token · **Status**: 📄 anatomy documented; automation needs a capture bridge (deferred)
