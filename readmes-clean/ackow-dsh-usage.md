...
![dsh-usage](assets/social-preview.png)
  ...
  把 DeepSeek Harness 的 token 账单摊开在侧栏。余额、命中率、成本曲线、热力图，一个「用量」按钮全看完。
  成本是估算的，命中的是真实缓存。
...
### 项 · 说明 · 默认
- **项**: `DEEPSEEK_API_KEY` · **说明**: DeepSeek 余额凭据（credentials 服务或环境变量） · **默认**: —
- **项**: `OPENROUTER_API_KEY` · **说明**: OpenRouter 余额凭据 · **默认**: —
- **项**: `cnyPerUsd` · **说明**: USD 换算汇率 · **默认**: `6.76`
- **项**: `pricing` · **说明**: 单价覆盖（可带 `schedules` 峰谷段） · **默认**: 内置官方刊例价