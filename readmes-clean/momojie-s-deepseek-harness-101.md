...
### 插件 · 路径 · 作用
- **插件**: @momojie-s/dsh-workspace-mcp · **路径**: `plugins/dsh-workspace-mcp` · **作用**: 按 workspace（session cwd）自动加载/卸载 MCP server，工具注册到 agent scope
- **插件**: @momojie-s/dsh-workspace-env · **路径**: `plugins/dsh-workspace-env` · **作用**: pwsh 命令自动注入 workspace `.env` 环境变量，实现 workspace 级环境变量隔离
- **插件**: @momojie-s/dsh-subagent-model · **路径**: `plugins/dsh-subagent-model` · **作用**: `subagent_model` 工具：委派子代理时可按次指定模型路由（provider/model/max_tokens），fork 自官方 tool-subagent
...
2. 在本仓执行：
   ```shell
   git submodule add https://github.com/Momojie-S/.git plugins/
   ```
3. 更新本 README 的插件目录表。