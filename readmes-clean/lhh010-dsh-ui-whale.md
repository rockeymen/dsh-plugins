...
- **加载机制变化**：0809 重构了客户端插件机制——旧的 `dsh.plugin.json` 清单 + `resolveClientPath`（`packages/plugin/plugin`）已删除，改为 **package.json 的 `dshClient` 声明**（`platform: 'web'`，可选 `inject`/`immediately`）+ `exports["./client"]` 指向构建产物；宿主扫描 loader 条目组成 boot 图，Web 端从 `/plugins//client.js` 拉取。本插件 package.json 已满足该声明，无需改动。
...
![眨眼](docs/眨眼.gif) ![摆尾巴](docs/摆尾巴.gif) ![摆腹鳍](docs/摆腹鳍.gif)
