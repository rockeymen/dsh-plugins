![local-shell-mcp logo](docs/assets/logo.svg)
...
- **Capability**: Real terminal access · **What it enables**: Run tests, build projects, inspect logs, and debug with persistent shell sessions.
- **Capability**: Workspace-aware file tools · **What it enables**: Read, write, patch, search, and review files under a controlled root.
- **Capability**: Git workflow support · **What it enables**: Run the standard Git CLI through shell tools without a second, incomplete Git abstraction.
- **Capability**: Browser automation · **What it enables**: Extract page text, capture PNG/PDF evidence, or run a full Playwright script.
- **Capability**: Remote workers · **What it enables**: Control NAT, firewall, HPC, NPU, or lab machines that can only connect outward.
- **Capability**: Agent Skills · **What it enables**: Discover, load, and read reusable `SKILL.md` workflows through three fixed tools without changing the MCP tool list.
- **Capability**: ChatGPT connector support · **What it enables**: OAuth 2.1, `/mcp`, discovery controls, and ChatGPT-compatible tool schemas.
- **Capability**: DeepSeek Harness plugin · **What it enables**: Install this repository as a DSH bundle and expose the complete LSM tool surface, including remote workers.
- **Capability**: ChatGPT Live Workspace · **What it enables**: Render a native MCP App for real-time activity, terminal, files, diffs, jobs, remotes, audit, and direct human/agent collaboration inside ChatGPT.
- **Capability**: Safer operations · **What it enables**: Workspace scoping, shell timeouts, output limits, environment filtering, audit logs, and secret scanning.
...
### Setting · Purpose
- **Setting**: `LOCAL_SHELL_MCP_PUBLIC_BASE_URL` · **Purpose**: Public HTTPS origin used by OAuth and ChatGPT.
- **Setting**: `LOCAL_SHELL_MCP_AUTH_MODE` · **Purpose**: Use `oauth` for public deployments.
- **Setting**: `LOCAL_SHELL_MCP_ALLOW_FULL_CONTAINER` · **Purpose**: Disable workspace restrictions only in disposable containers/VMs.
- **Setting**: `LOCAL_SHELL_MCP_REMOTE_ENABLED` · **Purpose**: Enable or disable remote worker control tools.
- **Setting**: `LOCAL_SHELL_MCP_UI_ENABLED` · **Purpose**: Mount or disable the shared OpenTUI/WebUI human interface.
- **Setting**: `LOCAL_SHELL_MCP_UI_PATH` · **Purpose**: WebUI mount path on the same service; default `/ui`.
- **Setting**: `LOCAL_SHELL_MCP_UI_WALLPAPER` · **Purpose**: Select `bing`, `aurora`, or `none` for the OpenTUI browser console background.
- **Setting**: `LOCAL_SHELL_MCP_SHELL_ENV_BLOCKLIST` · **Purpose**: Environment variables removed from spawned shell processes.
- **Setting**: `LOCAL_SHELL_MCP_FILE_DOWNLOAD_ENABLED` · **Purpose**: Enable tokenized file download links.
