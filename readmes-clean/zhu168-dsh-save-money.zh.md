...
### 入口 · 位置
- **入口**: 状态文字（唯一常驻入口） · **位置**: 会话头部右上角（Session log 旁）"省钱 · 🟢 工作中"，点击展开设置浮层
- **入口**: 系统设置页 · **位置**: 侧栏 → 设置 → 省钱插件
- **入口**: 浮动横幅 · **位置**: 即将暂停 / 已暂停时顶部胶囊 + **【停用省钱模式】** 按钮（点击直接停用整个功能）

### 动态工具（Host）

### 工具 · 用途
- **工具**: `save_money_status` · **用途**: 查询状态 / **闸门状态（gate: open\ · closed）** / 窗口 / 暂停记录 / UTC 投影
- **工具**: `save_money_configure` · **用途**: 配置（enabled / timezone / warnMinutes / windows）
- **工具**: `save_money_ignore` · **用途**: 忽略当前窗口或下一个窗口的暂停（同时放行闸门）
- **工具**: `save_money_debug_tick` · **用途**: 开发工具：手动推进状态机
