...
###  · Skill · 本插件
- 抓取 RSS · **Skill**: ❌ 只能教你怎么抓 · **本插件**: ✅ 真代码 `journal_scan` 直接抓
- 定时推送 · **Skill**: ❌ 做不到 · **本插件**: ✅ 计划接 dsh-schedule（every/at）
- 去重持久化 · **Skill**: ❌ 做不到 · **本插件**: ✅ 状态文件 JSONL 去重
- 关键词过滤 · **Skill**: 文本建议 · **本插件**: ✅ `journal_filter` 真过滤
...
### 环境变量 · 说明
- **环境变量**: `DSH_BARK_URL` · **说明**: Bark 推送 URL（例 `https://api.day.app/TOKEN/`），设置即启用
- **环境变量**: `DSH_FEISHU_URL` · **说明**: 飞书机器人 webhook，设置即启用
- **环境变量**: `DSH_JOURNAL_STATE` · **说明**: 状态文件路径（默认 `<workspace>/.dsh-journal-monitor/state.jsonl`）
- **环境变量**: `DSH_WORKSPACE` · **说明**: 工作区根（dsh 自动注入）
