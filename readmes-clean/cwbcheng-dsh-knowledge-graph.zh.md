...
![](https://github.com/user-attachments/assets/824ab99b-d291-4d06-8eb7-b91e947b1af4)
...
![](https://github.com/user-attachments/assets/5dfef153-25a0-431e-970c-dc344eef53d5)
...
### 文件 · 作用
- **文件**: [`src/index.host.js`](src/index.host.js) · **作用**: Host 半：异步 AI 拆分任务引擎（段落编号、分批、schema 校验、typed 诊断、模型路由、会话轨迹序列化）+ 知识图验证/质疑引擎（本地体检、LLM 审校、二次复核）
- **文件**: [`src/index.client.js`](src/index.client.js) · **作用**: Client 半：浮动工作台 UI、图渲染、双向定位、验证与质疑面板、修复应用/审计、历史、宽高调节、轨迹知识图标签页
...
### 文件 · 作用（常驻包）
- **文件**: [`lib/index.js`](lib/index.js) · **作用（常驻包）**: Host 半：任务引擎 + `/api/dsh-knowledge-graph` 路由（POST extract / POST append-extract / POST trajectory-extract / GET task-status / GET trajectory-status）
- **文件**: [`lib/client.js`](lib/client.js) · **作用（常驻包）**: Client 半：`__ModuleLoader__` 浏览器模块（fetch RPC + 手动样式注入）
- **文件**: [`cordis.patch.yml`](cordis.patch.yml) · **作用（常驻包）**: bundle patch：向组合插入 `dsh-knowledge-graph` 行
