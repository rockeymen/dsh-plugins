...
### 5小时用量 · 行为
- **5小时用量**: <60%（warnAt） · **行为**: **不注入**（零 prompt 成本）
- **5小时用量**: 60%-79% · **行为**: 注入一次：注意档——建议在合适节点提醒用户、暂停待恢复
- **5小时用量**: 80%-89% · **行为**: 注入一次：告急档——主动提醒、任务边界暂停
- **5小时用量**: 90% 起 · **行为**: **每增长 2% 递进一档**（90/92/94/...），措辞紧急程度递增：严重 → 濒临耗尽 → 即将耗尽 → 几乎耗尽 → 近极限 → 已用尽（100%），每档只提醒一次
