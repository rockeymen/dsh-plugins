...
### action · 参数 · 返回
- **action**: `now` · **参数**: `timezone?`（默认 UTC） · **返回**: 当前时间结构化表示
- **action**: `convert` · **参数**: `value` + `timezone`（必填） · **返回**: 转换后的结构化时间
- **action**: `add` · **参数**: `value` + `amount` + `unit`（`timezone?` 仅展示） · **返回**: 加减后的结构化时间
- **action**: `diff` · **参数**: `from` + `to` · **返回**: 固定时长差（无月份/年份）
...
- **适配 DSH**: DSH 0.1.0-rc.6（npm）（profile/bundle 插件系统）
...
> 注意：patch 是 id-targeted 语义（DSH 0.1.0-rc.6（npm））——裸 `- id:` 条目会报 `entry "xxx" not found`，必须用 `- insert:` 列表包裹。
## 已知限制
