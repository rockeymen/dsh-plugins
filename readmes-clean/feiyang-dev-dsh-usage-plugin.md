...
### 声明 · 作用
- **声明**: `dsh.bundle.patch`（`cordis.patch.yml`） · **作用**: 让 DSH 把它识别为**标准 bundle 插件包**：`dsh plugin --profile <名> add <包名>` 一条命令即可安装并自动接线，无需手改任何配置文件
- **声明**: `dsh.client` + `exports["./client"]` · **作用**: 让 web 客户端在 `/plugins/<包名>/client.js` 自动加载浏览器面板
...
### 现象 · 原因 / 处理
- **现象**: 面板报 `Unexpected end of JSON input` · **原因 / 处理**: 插件行缺少 `inject` 列表，路由未注册。按方法 B3 补上 inject 后重启
- **现象**: 面板一直空白 / 顶部无 tab · **原因 / 处理**: 插件未激活。看会话工作区 `dsh-usage-boot.log`；确认 `cordis.patch.yml` 里的行存在且 `name` 正确
- **现象**: 余额查询失败「未配置 DEEPSEEK_API_KEY」 · **原因 / 处理**: 在 设置 → 模型 里配置 API Key
- **现象**: 余额查询失败网络错误 · **原因 / 处理**: 确认能访问 `api.deepseek.com`（国内网络请配置代理）
- **现象**: `dsh plugin` 报 pnpm not found · **原因 / 处理**: 安装 pnpm：`npm install -g pnpm`
- **现象**: 安装时连不上 npm 官方源 · **原因 / 处理**: 配置镜像：`npm config set registry https://registry.npmmirror.com`（或对 pnpm 设 `pnpm config set registry ...`）后再执行安装命令
- **现象**: 卸载后仍报 `Cannot find package '@feiyang666/...'` · **原因 / 处理**: profile 里残留了包引用。删掉 `cordis.patch.yml` 中对应行与 `dsh.profile.bundles` 里的包名，重启
...
### 项目 · 说明 · 安装方式
- **项目**: [DeepSeek Harness 桌面版](https://github.com/feiyang-dev/DeepSeek-Harness-Desktop) · **说明**: Windows 桌面控制台：一键安装/启动/停止/重启 dsh web 服务，内置插件管理，**推荐插件区一键安装本插件** · **安装方式**: 下载桌面版，点几下即可
- **项目**: [数据保险箱（dsh-vault）](https://github.com/feiyang-dev/deepseekharnessdesktop-vault) · **说明**: 自动备份 / 清空检测 / 一键恢复，保护聊天记录与工作区数据 · **安装方式**: 桌面端一键安装，或 `dsh plugin add @feiyang666/deepseekharnessdesktop-vault`
- **项目**: [DeepSeek-Harness](https://github.com/deepseek-ai/DeepSeek-Harness) · **说明**: 官方 CLI / Web 服务 · **安装方式**: 见下方「运行 DeepSeek Harness」
