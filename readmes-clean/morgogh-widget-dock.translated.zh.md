...
### 工作台（对话页两侧） · 添加面板（右侧侧栏）
- **工作台（对话页两侧）**: ![widget-dock 工作台](assets/screenshots/workbench.png) · **添加面板（右侧侧栏）**: ![添加面板](assets/screenshots/panel.png)
...
### 图标 · 组件 · 数据来源
- **图标**: 📊 · **组件**: 上下文压力 · **数据来源**: `contextPressure` 投影（窗口占用比例）
- **图标**: ☑️ · **组件**: 待办 · **数据来源**: `todos` 投影（任务清单）
- **图标**: 🔐 · **组件**: 权限模式 · **数据来源**: `permissions` 投影（访问范围）
- **图标**: 💰 · **组件**: API 余额 · **数据来源**: DeepSeek `user/balance` 接口（含充值跳转、KEY 编辑）
- **图标**: 📈 · **组件**: Token 用量 · **数据来源**: `tokenUsage` 投影（输入/输出/缓存，自动格式化）
- **图标**: 🕒 · **组件**: 会话统计 · **数据来源**: `sessionStats` 投影（回合/步骤/耗时）
- **图标**: 🎯 · **组件**: 目标进度 · **数据来源**: `goal` 投影（阶段 + 目标）
- **图标**: 🧾 · **组件**: 成本估算 · **数据来源**: token × 官方单价（flash/pro 切换，分项费用）
...
### 微信赞赏 · 支付宝赞赏
- **微信赞赏**: ![WeChat QR](assets/sponsor/wechat-qr.jpg) · **支付宝赞赏**: ![Alipay QR](assets/sponsor/alipay-qr.jpg)