...
- **Rule**: `config-clobber` · **Severity**: error · **What it catches**: Default config fields your patch dropped by not restating them
- **Rule**: `dead-patch` · **Severity**: error · **What it catches**: A patch targeting an entry id that is not in the tree (with a did-you-mean)
- **Rule**: `tool-collision` · **Severity**: error · **What it catches**: Two mounted plugins registering the same tool name — dsh refuses to start
- **Rule**: `plugin-not-mounted` · **Severity**: warn · **What it catches**: A plugin installed into the profile that nothing ever loads
- **Rule**: `plugin-stale` · **Severity**: warn · **What it catches**: A third-party plugin with no npm release in 180+ days
- **Rule**: `entry-removed` · **Severity**: warn · **What it catches**: A shipped entry your patch layer removed
- **Rule**: `entry-toggled` / `entry-added` · **Severity**: info · **What it catches**: Every other difference from the shipped profile, so the diff is visible
...
- `dsh --profile  --dump-config` — the tree that actually boots (bundles → profile patch → home patch → overlays)
- `dsh --profile  --dump-default-config` — the same tree without your user layer
