...
### 坑 · 说明
- **坑**: **语音收不到** · **说明**: ① 别给小号**自己**发语音（QQ 不支持自己给自己发语音）；② 语音必须 **silk 格式**（`pip install pilk`，桥接已内置转换），mp3 base64 发不出去
- **坑**: **QQ 事件进不来** · **说明**: 必须配 **WebSocket 客户端**（NapCat 连 `ws://127.0.0.1:8765/api/qq/onebot`），配成 WebSocket 服务器会抢 8765 端口失败
- **坑**: **重启 NapCat 后 OneBot 服务丢失** · **说明**: NapCat 的 OneBot HTTP（3000）重启后有时不自动加载 —— 都在 **WebUI 里管理**（启用/保存），不要 kill 进程重启
- **坑**: **WebUI 每次要 token** · **说明**: 改 `napcat\config\webui.json` 的 `token` 字段为固定值即可
- **坑**: **回复延迟** · **说明**: 延迟主要来自 LLM 生成 + 整段 TTS 合成；回复越长越慢（分段推送优化是后续可选项）
- **坑**: **端口冲突** · **说明**: 8765 桥接、3000 OneBot、6099 WebUI —— 被占时改配置并同步改桥接的 `napcat_base`
