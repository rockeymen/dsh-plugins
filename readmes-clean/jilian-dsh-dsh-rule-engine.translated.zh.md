...
### 命令 · 作用
- **命令**: `/guard status` · **作用**: 引擎状态（规则数/置信度/放行/解锁）
- **命令**: `/guard rules` · **作用**: 规则清单 + 理解产物
- **命令**: `/guard active` · **作用**: 最近激活了哪些规则、为什么
- **命令**: `/guard log [N]` · **作用**: 最近 N 条审计
- **命令**: `/guard unlock [N]` · **作用**: 解锁配置写保护 N 分钟（仅用户）
- **命令**: `/guard bypass [N]` · **作用**: 临时整体放行 N 分钟（仅用户）
- **命令**: `/guard lock` · **作用**: 立即恢复全部守卫（取消解锁/放行）
- **命令**: `/guard revoke` · **作用**: 撤销全部授权记录
- **命令**: `/guard reload` · **作用**: 强制重解析 AGENTS.md
