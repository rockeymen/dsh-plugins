...
### 入口 · 行为
- **入口**: **拖拽到窗口任意位置** · **行为**: 全屏高亮浮层 → 松手 → 获取文件
- **入口**: **回形针按钮**（输入框工具行左侧，28px 圆形原生风格） · **行为**: 点击弹系统文件选择器（多选）
- **入口**: **hover 气泡** · **行为**: 「点击选择文件 · 也可把文件拖到窗口任意位置」，1.5s 自动消失，每次 hover 循环
...
### 引擎 · 完整流程耗时 · 结论
- **引擎**: 智谱 **glm-4v-flash**（国内直连，免费） · **完整流程耗时**: **~3.7s** · **结论**: ✅ 当前配置，最优
- **引擎**: 智谱 glm-4.6v · **完整流程耗时**: ~17s · **结论**: ❌ 慢 4.6 倍
- **引擎**: 智谱 glm-4.5v · **完整流程耗时**: 失败 · **结论**: ❌ 不返回符合 schema 的 JSON
- **引擎**: Gemini 免费 key（海外） · **完整流程耗时**: 429 额度用尽 / 兜底 codex-cli ~14s · **结论**: ❌
