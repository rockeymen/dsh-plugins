...
下面都是真实生成的 Archify 成品，不是产品效果图。点击画面即可打开对应的可分享交互状态。
  ...
  <sub>三个真实生成、校验通过的成品。 Signal Flow · Blueprint · Classic · [打开可交互验证作品集 ↗](https://tt-a1i.github.io/archify/gallery.html)</sub>

### 引导故事 · 路径探查 · 语义角色对比
- **引导故事**: [![Agent 工作流正在播放一个作者章节](docs/assets/archify-demo-story.png)](https://tt-a1i.github.io/archify/gallery/artifacts/agent-tool-call.workflow.html?theme=dark&present=1&play=1#view=happy-path) · **路径探查**: [![缓存未命中时从 Web App 到 Postgres 的路径](docs/assets/archify-demo-route.png)](https://tt-a1i.github.io/archify/gallery/artifacts/cache-miss.sequence.html?theme=dark&present=1#route=web~db) · **语义角色对比**: [![生产架构中后端与数据库角色的真实关系](docs/assets/archify-demo-lens.png)](https://tt-a1i.github.io/archify/gallery/artifacts/production-deployment.architecture.html?theme=dark&present=1#lens=backend~database)
- **引导故事**: 播放一次有限的命名章节。 · **路径探查**: 检查最短的作者有向路径。 · **语义角色对比**: 对比语义角色之间的真实流量。
...
### 深色 · 浅色
- **深色**: ![深色主题](docs/assets/archify-dark.png) · **浅色**: ![浅色主题](docs/assets/archify-light.png)
...
### 类型 · 最适合 · Prompt 中应包含
- **类型**: **Architecture** · **最适合**: 组件、服务、存储和系统边界 · **Prompt 中应包含**: 范围、核心组件、主要路径
- **类型**: **Workflow** · **最适合**: CI/CD、审批、工具调用、Runbook · **Prompt 中应包含**: 参与者、顺序、分支、异常
- **类型**: **Sequence** · **最适合**: API 调用、缓存回源、鉴权、异步链路 · **Prompt 中应包含**: 调用方、被调用方、返回、时序
- **类型**: **Data Flow** · **最适合**: 数据管线、血缘、PII、下游消费者 · **Prompt 中应包含**: 来源、转换、存储、边界
- **类型**: **Lifecycle** · **最适合**: 状态、重试、等待、终态 · **Prompt 中应包含**: 状态、事件、重试与取消路径
...
### 步骤 · 发生什么
- **步骤**: **生成** · **发生什么**: Agent 根据描述创建 Typed JSON IR。
- **步骤**: **校验** · **发生什么**: 内置 Validator 和布局规则检查源文件；失败时用机器可读 JSON 指出准确的局部修复。
- **步骤**: **预览（可选）** · **发生什么**: 仅 loopback 的桌面会话监听一个源文件，只刷新验证版本；失败时保留最后好图。
- **步骤**: **交付** · **发生什么**: 在目标同目录生成并检查候选；只有通过门禁的结果才原子替换目标文件，随后可选用 `--open` 打开这个确切成品。
- **步骤**: **迭代** · **发生什么**: Agent 修改源文件，不干扰无关结构。
...
### 操作 · 控制方式
- **操作**: 打开事实型 Diagram Guide · **控制方式**: <kbd>?</kbd>
- **操作**: 查找并聚焦语义节点 · **控制方式**: <kbd>/</kbd>
- **操作**: 追踪作者定义的上游 / 下游可达范围 · **控制方式**: 聚焦节点 → `Upstream` / `Downstream`
- **操作**: 探查有向路径并逐站检查 · **控制方式**: <kbd>R</kbd> 或 `PATH`
- **操作**: 对比一种或两种语义角色 · **控制方式**: <kbd>L</kbd> 或 `LENS`
- **操作**: 打开实时全局雷达 · **控制方式**: <kbd>M</kbd> 或 `MAP`
- **操作**: 播放故事 / 切换章节 · **控制方式**: <kbd>P</kbd> / <kbd>[</kbd> <kbd>]</kbd>
- **操作**: 进入 Presentation Stage · **控制方式**: <kbd>F</kbd>
- **操作**: 选择视觉风格（<kbd>S</kbd> 循环）/ 切换主题 / 打开 Export · **控制方式**: <kbd>S</kbd> / <kbd>T</kbd> / <kbd>E</kbd>
- **操作**: 缩放或复位 · **控制方式**: <kbd>+</kbd> / <kbd>-</kbd> / <kbd>0</kbd>

稳定链接可以恢复 `#focus=`、`#focus=&reach=upstream|downstream`、`#relation=`、`#route=~<target>`、`#lens=<kind>~<kind>` 和 `#view=<view-id>`。读者触发的动态有限运行、遵守 `prefers-reduced-motion`，并且不会进入标准导出。
...
### 使用位置 · 安装位置或方法 · 能力
- **使用位置**: **Raven** · **安装位置或方法**: ZIP 手动安装：将 `archify.zip` 解压到 `~/.raven/workspace/skills`，解压后会得到 `~/.raven/workspace/skills/archify` · **能力**: 完整 Renderer + Validation 工作流
- **使用位置**: **Claude Code** · **安装位置或方法**: `~/.claude/skills/` 或 `.claude/skills/` · **能力**: 完整 Renderer + Validation 工作流
- **使用位置**: **Codex CLI** · **安装位置或方法**: `~/.agents/skills/` 或 `.agents/skills/` · **能力**: 完整 Renderer + Validation 工作流
- **使用位置**: **opencode** · **安装位置或方法**: `~/.config/opencode/skills/`、`.opencode/skills/` 或 `.agents/skills/` · **能力**: 完整 Renderer + Validation 工作流
- **使用位置**: **Claude.ai** · **安装位置或方法**: Settings → Capabilities → Skills 中上传 `archify.zip` · **能力**: 取决于沙箱是否提供 Node.js
- **使用位置**: **Project Knowledge** · **安装位置或方法**: 把 `archify.zip` 上传到项目 · **能力**: Prompt 驱动的 Architecture Fallback
Claude.ai 中的上传入口：
