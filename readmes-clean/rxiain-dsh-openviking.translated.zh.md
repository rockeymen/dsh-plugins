...
### 工具 · 功能
- **工具**: `memsearch` · **功能**: 语义搜索（`auto`/`fast`/`deep`；deep 使用会话上下文）
- **工具**: `memfind` · **功能**: 快速语义查找，不带会话上下文
- **工具**: `memread` · **功能**: 读取 `viking://` URI（`abstract`/`overview`/`read`/`auto`）
- **工具**: `membrowse` · **功能**: 浏览 `viking://` 文件系统（`list`/`tree`/`stat`）
- **工具**: `memgrep` · **功能**: 精确/正则内容搜索（默认 `viking://resources/`）
- **工具**: `memglob` · **功能**: 按 glob 模式枚举文件
- **工具**: `memadd` · **功能**: 在 `viking://resources/` 下添加远程 URL 或本地文本文件
- **工具**: `memremove` · **功能**: 删除资源——需字面量 `confirm: true`
- **工具**: `memqueue` · **功能**: 查看观察者队列状态
- **工具**: `memcommit` · **功能**: 提交当前会话并提取持久记忆
