...
- **Component**: Base image · **Description**: `debian:13-slim` (small, overridable via the `BASE_IMAGE` build arg)
- **Component**: Built-in toolchain · **Description**: Node.js 22 LTS, pnpm, uv, Rust/cargo, git, build-essential, Caddy, podman, gh
- **Component**: Added user-level tools · **Description**: Rust/cargo (`~/.rustup` + `~/.cargo`), uv (`~/.local/bin`), pnpm (`~/.local/share/pnpm`) — persisted with `/home/dsh`
- **Component**: Container dev tool · **Description**: podman (apt), with rootless subuid/subgid mapping configured; nested rootless operation depends on the host runtime
- **Component**: dsh · **Description**: Global npm install of `@deepseek-ai/dsh`, same source as the official README's `npx @deepseek-ai/dsh web`; pinnable via `DSH_VERSION`
- **Component**: Auto-update · **Description**: Off by default (`DSH_AUTO_UPDATE=0`); opt in with `DSH_AUTO_UPDATE=1` to update dsh to the latest npm release on container start (only upgrades, never downgrades a pinned version). The image itself supports `Pull=newer` / `AutoUpdate=registry`
- **Component**: dsh web supervisor · **Description**: `dsh web` runs under a small supervisor (`dsh-web`) that restarts it automatically if it exits; run `dsh-restart` inside the container to restart dsh web without restarting the container
- **Component**: Exposure · **Description**: Caddy reverse proxy (`0.0.0.0:3081` → dsh's `127.0.0.1:3080`) rewriting `Host`/`Origin` to loopback, gzip-compressing UI assets (≈1.3 MB → ≈360 KB), with optional basic auth (`DSH_PROXY_USER` / `DSH_PROXY_PASSWORD`)
- **Component**: Observability · **Description**: OCI labels (`org.opencontainers.image.*`, incl. git revision), `HEALTHCHECK` (curl 3080 + 3081)
- **Component**: Runtime user · **Description**: uid 1000 (`dsh`); `/home/dsh` is the persisted user layer and `dsh` has passwordless sudo
...
- **Variable**: `DSH_PROXY_USER` / `DSH_PROXY_PASSWORD` · **Default**: *(empty)* · **Description**: Enable basic auth on the exposed proxy (recommended): without it, anyone who can reach port `3081` can drive the agent **and** read/write all settings & credentials (see [security.md](docs/security.md) "Security boundary"). Set both or neither — the entrypoint refuses to start if only one is set
- **Variable**: `DSH_AUTO_UPDATE` · **Default**: `0` · **Description**: Set to `1` to update dsh to the latest npm release on container start; keeps the in-image version when offline or on failure
...
### Document · Contents
- **Document**: [docs/deployment.md](docs/deployment.md) · **Contents**: Deployment & maintenance: prerequisites, Compose, Quadlet, auto-update, remote access, offline use, FAQ
- **Document**: [docs/security.md](docs/security.md) · **Contents**: Security notes: network exposure tradeoff, credentials, trusted workloads
- **Document**: [docs/build.md](docs/build.md) · **Contents**: Build configuration: build args, version pinning, reproducible builds
- **Document**: [docs/releasing.md](docs/releasing.md) · **Contents**: Image tags, release workflow (GitHub Releases + version alignment), image cleanup
- **Document**: [docs/design.md](docs/design.md) · **Contents**: Design references and related projects
- **Document**: [docs/development.md](docs/development.md) · **Contents**: Directory structure and local development