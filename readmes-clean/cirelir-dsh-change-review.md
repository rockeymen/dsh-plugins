...
### Feature · Description
- **Feature**: Auto tracking · **Description**: Listens to `write` / `edit` tool calls, records before/after content and timestamps
- **Feature**: Diff view · **Description**: LCS line-level diff: added (green) / removed (red) / context (gray) lines with both-side line numbers
- **Feature**: Session isolation · **Description**: Each session reviews only its own changes; switching sessions switches the review
- **Feature**: Subagent aggregation · **Description**: Changes made by subagents are aggregated into the root parent session
- **Feature**: Live updates · **Description**: SSE server push — the badge and list refresh instantly when files change (zero polling)
- **Feature**: Revert changes · **Description**: Undo ONE specific change (later non-overlapping changes are kept) or the WHOLE file (restore the pre-session snapshot; a file created in-session is deleted) — writes directly to disk
- **Feature**: Per-turn review · **Description**: Each turn's tail shows a card with THIS turn's changed files and expandable diffs (with per-op revert), distinct from the session-wide 「审查」tab
- **Feature**: Count badge · **Description**: The 「审查」(Review) tab shows the pending file count; badge background/text colors are customizable
- **Feature**: Color customization · **Description**: 12 colors configurable under **Settings → 修改审查** (Review), including the turn-tail card's background, border, and added/removed counts (different defaults); every color has an opacity slider (rgba), persisted in localStorage
- **Feature**: Clean sidebar · **Description**: Hides the Cordis plugin run indicator (`cordis-panel`) from the left sidebar

## 📸 Screenshots

### Review tab — file list + diff preview · Per-turn review card
- **Review tab — file list + diff preview**: ![Review tab](assets/screenshots/review-tab.png) · **Per-turn review card**: ![Per-turn review card](assets/screenshots/per-turn-card.png)

### Expanded diff with per-change revert · Color customization (Settings → 修改审查)
- **Expanded diff with per-change revert**: ![Diff detail](assets/screenshots/diff-detail.png) · **Color customization (Settings → 修改审查)**: ![Color settings](assets/screenshots/color-settings.png)

### Installed from the plugin market
- **Installed from the plugin market**: ![Plugin market](assets/screenshots/plugin-market.png)
...
- **Item**: Added line background · **Key**: `addBg` · **Light default**: `#e6ffec` · **Dark preset**: `#10251c`
- **Item**: Added line text · **Key**: `addFg` · **Light default**: `#1a7f37` · **Dark preset**: `#7ee787`
- **Item**: Removed line background · **Key**: `delBg` · **Light default**: `#ffebe9` · **Dark preset**: `#2d1415`
- **Item**: Removed line text · **Key**: `delFg` · **Light default**: `#cf222e` · **Dark preset**: `#ffa198`
- **Item**: Context background · **Key**: `ctxBg` · **Light default**: `#f6f8fa` · **Dark preset**: `#161b22`
- **Item**: Line numbers / markers · **Key**: `gutter` · **Light default**: `#57606a` · **Dark preset**: `#8b949e`
- **Item**: Badge background · **Key**: `badgeBg` · **Light default**: `#0969da` · **Dark preset**: `#4493f8`
- **Item**: Badge text · **Key**: `badgeFg` · **Light default**: `#ffffff` · **Dark preset**: `#0d1117`
- **Item**: Added count (turn-tail list) · **Key**: `turnAdd` · **Light default**: `#1a7f37` · **Dark preset**: `#7ee787`
- **Item**: Removed count (turn-tail list) · **Key**: `turnDel` · **Light default**: `#cf222e` · **Dark preset**: `#ffa198`
- **Item**: Card background (turn-tail) · **Key**: `turnBg` · **Light default**: `rgba(255,183,77,.1)` (light orange 10%) · **Dark preset**: `rgba(255,183,77,.1)` (light orange 10%)
- **Item**: Card border (turn-tail) · **Key**: `turnBorder` · **Light default**: `#ffb74d` (light orange) · **Dark preset**: `#ffb74d` (light orange)
