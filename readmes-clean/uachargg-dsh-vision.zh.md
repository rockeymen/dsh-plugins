...
### 文件 · 职责
- **文件**: `src/index.ts` · **职责**: Host 侧：`VisionService extends TypertRemoteService` —— `@Remote` `queue`/`config`/`status` + `agent/pre-step` 与 `llm/stream` 监听 + GLM 调用
- **文件**: `src/client/index.ts` · **职责**: 浏览器侧：`conversation.input.dock` 插槽 —— 粘贴捕获、提交拦截、模型/API Key 设置菜单
- **文件**: `src/patch.ts` + `scripts/apply-dsh-patch.mjs` · **职责**: 自愈式 dsh 附件授权补丁
- **文件**: `src/types.ts` · **职责**: 双面跨进程线协议类型（无损 JSON）
- **文件**: `cordis.patch.yml` · **职责**: bundle 组合层
- **文件**: `lib/` · **职责**: 构建产物（随包发布）
