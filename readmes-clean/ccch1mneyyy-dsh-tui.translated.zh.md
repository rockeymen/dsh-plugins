...
本插件被 **DeepSeek Harness 官方公众号** 推文收录，作为"内测用户精选插件"展示：
  ...
  ![DeepSeek Harness 官方公众号推文收录 dsh-TUI](screenshots/wechat-official.png)
...
### 分组 · 命令
- **分组**: 会话 · **命令**: `/new` 新会话 · `/resume` 恢复 · `/rename` 重命名 · `/clear` 清屏 · `/compact` 压缩 · `/export` 导出 Markdown · `/trace` 轨迹时间线
