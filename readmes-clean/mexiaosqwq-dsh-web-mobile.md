...
### 会话主页(全宽) · 目录抽屉 · 设置界面
- **会话主页(全宽)**: ![移动端会话主页](assets/hero.png) · **目录抽屉**: ![目录抽屉](assets/drawer.png) · **设置界面**: ![移动端设置界面](assets/settings.png)
...
### 插件 · 移动端适配
- **插件**: 文件树 (Explorer) · **移动端适配**: 圆角底部浮层,底边与输入框对齐;会话头部文件夹按钮一步开/关
- **插件**: 预览 (Preview) · **移动端适配**: 底部浮层;刷新/重进不会自动弹出
- **插件**: 任务看板 · **移动端适配**: 五列保持最小宽度,横向滚动;浮动按钮让位
- **插件**: SSH · **移动端适配**: 面板适配;侧栏入口间距
- **插件**: 宠物 (鲸鱼娘) · **移动端适配**: 手机端缩小,默认停在输入框上方,可拖动,不遮挡输入与统计
- **插件**: 会话统计行 · **移动端适配**: turns/steps/LLM/TTFT/cache/TPS 整合为一条固定高度横向滚动条
- **插件**: 远程配对 (remote-web-ui) · **移动端适配**: footer 图标与 Files/Session log 分行排列
- **插件**: web-ui 设置 · **移动端适配**: 插件项卡片、设置条目在手机上重排
