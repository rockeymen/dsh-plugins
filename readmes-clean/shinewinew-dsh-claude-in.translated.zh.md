...
- **Claude 资产**: `rules/**​/*.md` · **项目级（当前工作区自身的 `.claude`）**: always/无 paths 规则进基线；paths/globs 规则文件命中时注入；支持 `@import` · **用户级 `~/.claude`**: 同左（CC 语义：个人规则默认 alwaysApply）
- **Claude 资产**: `skills/**​/SKILL.md` · **项目级（当前工作区自身的 `.claude`）**: rank 450，`skill` 工具直读源文件 · **用户级 `~/.claude`**: rank 250（同名用户覆盖项目；对齐 Claude Code personal > project）
- **Claude 资产**: `agents/**​/*.md` · **项目级（当前工作区自身的 `.claude`）**: 基线目录 + `claude-agent-<name>` 技能桥 · **用户级 `~/.claude`**: 同左（同名项目覆盖）
- **Claude 资产**: `hooks`（settings.json / settings.local.json / hooks.json） · **项目级（当前工作区自身的 `.claude`）**: command hooks 映射到 DSH seams（SessionStart / UserPromptSubmit / PreToolUse / PostToolUse / Stop / SubagentStart / SubagentStop），退出码 2 阻塞、permissionDecision 折叠 · **用户级 `~/.claude`**: **全局生效**（无项目 `.claude` 时也跑）
