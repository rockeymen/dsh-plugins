...
### Var · Default · Purpose
- **Var**: `TREG_DATABASE_URL` · **Default**: `sqlite+aiosqlite:///./treg.db` · **Purpose**: DB URL (SQLite for dev, Postgres in prod)
- **Var**: `TREG_SECRET_KEY` · **Default**: *(empty)* · **Purpose**: Fernet key for secrets-at-rest; empty → an ephemeral key is minted (secrets won't survive a restart)
- **Var**: `TREG_PUBLIC_URL` · **Default**: `https://treg.to` · **Purpose**: treg's public base, used to build the OAuth callback URI
- **Var**: `TREG_SESSION_SECRET` · **Default**: *(empty)* · **Purpose**: signs the dashboard session cookie; falls back to `TREG_SECRET_KEY`. Set a real value in prod
- **Var**: `TREG_GITHUB_CLIENT_ID` / `_SECRET` · **Default**: *(empty)* · **Purpose**: GitHub OAuth sign-in (callback `/auth/github/callback`); empty hides the button
- **Var**: `TREG_GOOGLE_CLIENT_ID` / `_SECRET` · **Default**: *(empty)* · **Purpose**: Google OAuth sign-in (redirect `/auth/google/callback`); empty hides the button
- **Var**: `TREG_RESEND_API_KEY` / `TREG_EMAIL_FROM` · **Default**: *(empty)* · **Purpose**: transactional email via Resend (OTP codes + invites); From must be a Resend-verified sender
- **Var**: `TREG_ADMIN_TOKEN` · **Default**: *(empty)* · **Purpose**: cross-tenant **super-admin** bearer; authorizes every `/admin/*` endpoint. Empty disables the env path (only `is_superadmin` users reach `/admin`). Keep it long + secret.
- **Var**: `TREG_EMAIL_DEV_MODE` · **Default**: `false` · **Purpose**: when true, `/auth/email/start` returns the OTP in its response (no mail sender needed) — **dev/local only**, never in prod.
...
### Module · Role
- **Module**: `proxy.py` · **Role**: `relay()` — the whole product in one function: a faithful streaming proxy
- **Module**: `injectors.py` · **Role**: the auth-shape seam: `env`, `cli_auth`, `secret_file`, `oauth` place a secret into a header/query
- **Module**: `oauth.py` · **Role**: token freshness (single-flight refresh) + the connect flow (consent URL, code exchange)
- **Module**: `health.py` · **Role**: credential health: refresh oauth, probe tools, webhook the owner of anything broken
- **Module**: `convert.py` · **Role**: scaffold a skill directory into a registerable bundle manifest
- **Module**: `api.py` · **Role**: the API — the only brain; CLI + skill are thin clients over it
- **Module**: `cli.py` · **Role**: the `treg` CLI
- **Module**: `models.py` · **Role**: SQLModel tables: `Org`, `User`, `Membership`, `Invite`, `Secret`, `Tool`, `Bundle`, `PendingOAuth`, `CallRecord`
- **Module**: `crypto.py` `config.py` `db.py` `audit.py` · **Role**: Fernet encryption + tokens · settings · async DB · deferred audit writer
