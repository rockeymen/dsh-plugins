...
### 专业蓝亮色 · 专业蓝暗色
- **专业蓝亮色**: ![dsh-work 专业蓝亮色首页](docs/images/readme/dsh-work-home-professional-light.png) · **专业蓝暗色**: ![dsh-work 专业蓝暗色首页](docs/images/readme/dsh-work-home-professional-dark.png)
...
### 功能 · 用户可以做什么
- **功能**: DSH 对话 · **用户可以做什么**: 流式回答、思考过程、工具调用、停止、继续、重试、消息分支和重启恢复
- **功能**: 模型与权限 · **用户可以做什么**: 选择 Provider、模型和推理强度，管理凭据引用、Session 权限、工具审批和模型提问
- **功能**: Tool、Skill、MCP 与多 Agent · **用户可以做什么**: 使用当前 Profile 中的工具、技能、MCP、Hook、子 Agent 和 Workflow
- **功能**: 项目与对话 · **用户可以做什么**: 创建项目、全局或临时对话，置顶、排序、重命名、归档、恢复和删除
- **功能**: 桌面外壳与设置 · **用户可以做什么**: 使用三列工作台、左右栏折叠、全局搜索、缩放快捷键和更新检查，调整语言、网络、通知、终端与隐私选项
- **功能**: 项目上下文与记忆 · **用户可以做什么**: 设置应用指令、项目指令、授权源码目录、写入目标以及全局或项目记忆
- **功能**: 输入与引用 · **用户可以做什么**: 使用 `@` 引用文件、使用 `#` 引用对话，粘贴图片和大段文本附件
- **功能**: 编码工作区 · **用户可以做什么**: 查看 Diff、逐行评论和编辑、在外部编辑器打开、发起 AI Review，并安全撤销模型产生的文件修改
- **功能**: Git Worktree · **用户可以做什么**: 创建、启用、停用和删除隔离工作目录，让新对话在指定 Worktree 中运行
- **功能**: 文件与搜索 · **用户可以做什么**: 浏览项目文件、任务和产物，预览文本、代码、图片和 Office 内容，按文件名或正文搜索
- **功能**: Browser Workspace · **用户可以做什么**: 多标签浏览、历史、页内查找、缩放、下载、打印、开发者工具、站点权限、网页快照和“使用此页”
- **功能**: 结果与证据 · **用户可以做什么**: 直接查看当前 DSH Session 的完整轨迹、工具输入输出、耗时、Token 和最终回答
- **功能**: Canvas 与本地 Site · **用户可以做什么**: 创建和编辑 Canvas、处理行内建议与版本冲突，生成并响应式预览单文件 Site
- **功能**: Office 产物 · **用户可以做什么**: 创建、查看和定点编辑 Markdown、DOCX、XLSX、PPTX 和 PDF，并保留版本
- **功能**: 主题与外观 · **用户可以做什么**: 切换 Profile 主题，新建、导入、预览、编辑、导出和删除本地主题，调整明暗模式、背景和透明度
- **功能**: 插件中心 · **用户可以做什么**: 检查兼容性，把 DSH Profile Bundle 安装到当前 Web Profile，并查看来源、版本和加载顺序
...
### 平台 · 当前状态
- **平台**: macOS Apple Silicon · **当前状态**: 开发与目录包已验证
- **平台**: macOS Intel · **当前状态**: Rosetta 检查通过，仍需 Intel 实机验收
- **平台**: Windows x64 · **当前状态**: 已接入构建流程，仍需安装包实机验收
- **平台**: Windows arm64 · **当前状态**: 暂不支持
- **平台**: Linux · **当前状态**: 暂无桌面打包配置
