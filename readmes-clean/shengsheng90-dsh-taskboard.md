...
### Requirement · Value
- **Requirement**: Node.js · **Value**: `^22.19.0` or `>=24.0.0` (24 recommended; built-in `node:sqlite`)
- **Requirement**: pnpm · **Value**: `11` (`packageManager` is `pnpm@11.15.1`)
- **Requirement**: DeepSeek Harness · **Value**: `0.1.0-rc.5` checkout or installation, **web** profile
- **Requirement**: Network · **Value**: only needed to clone this repo and install Node dependencies
- **Requirement**: Permissions · **Value**: write access to `$DSH_HOME` (default `~/.dsh`) and the ability to restart the Harness process
...
### Name · Value
- **Name**: Package name · **Value**: `@shengsheng/dsh-taskboard`
- **Name**: Default profile · **Value**: `web`
- **Name**: Default Web port · **Value**: `3080` (detect; do not assume)
- **Name**: Profile directory · **Value**: `$DSH_HOME/profiles/` , usually `~/.dsh/profiles/web`
- **Name**: Packed tarball name · **Value**: `shengsheng-dsh-taskboard-<version>.tgz`
...
If nothing is listening on `3080`, search other common ports or ask the operator for the URL they use (`http://127.0.0.1:`).
...
### Check · Expected
- **Check**: `GET /` contains `@shengsheng/dsh-taskboard` · **Expected**: count ≥ 1
- **Check**: `GET /plugins/@shengsheng/dsh-taskboard/client.js` · **Expected**: HTTP 200
- **Check**: Harness boot log · **Expected**: no plugin import / apply error
- **Check**: Browser · **Expected**: refresh `http://127.0.0.1:`; a Taskboard control appears in the sidebar footer
...
### Symptom · Cause · Fix
- **Symptom**: `dsh: command not found` · **Cause**: CLI not on `PATH` · **Fix**: From a Harness checkout root, use `pnpm dsh ...`
- **Symptom**: `ERR_PNPM_ADDING_TO_ROOT` · **Cause**: profile is a pnpm workspace root · **Fix**: Add `-w`
- **Symptom**: git / directory install has no `lib/` · **Cause**: `lib/` is gitignored · **Fix**: `pnpm build && pnpm pack`, then add the `.tgz`
- **Symptom**: `EPERM` writing `~/.dsh` · **Cause**: sandbox · **Fix**: Ask the operator for full permissions; the write is idempotent
- **Symptom**: Manifest / `client.js` still 404 · **Cause**: no restart, or checked too early · **Fix**: Restart, then poll (step 7)
- **Symptom**: Import / apply error · **Cause**: missing peers or missing bundle entry · **Fix**: Heal fallbacks with `--dump-config`; confirm `dsh.profile.bundles`
- **Symptom**: `declares no dsh.bundle` · **Cause**: package missing bundle patch · **Fix**: Rebuild this repo; do not hand-edit the installed tree
- **Symptom**: GUI down after restart · **Cause**: Harness failed to boot · **Fix**: Read `/tmp/dsh-harness-restart.log` or the process log; confirm checkout path and `pnpm dsh`
...
### Tool · Purpose
- **Tool**: `taskboard_list` · **Purpose**: Bounded list for one exact `project_id`
- **Tool**: `taskboard_get` · **Purpose**: Full detail, version, comments, relations, claim
- **Tool**: `taskboard_claim` · **Purpose**: Claim one eligible `todo` with `expected_version`
- **Tool**: `taskboard_comment` · **Purpose**: Append a Markdown comment
- **Tool**: `taskboard_submit_review` · **Purpose**: Move owned `in_progress` work to `in_review`
- **Tool**: `taskboard_block` · **Purpose**: Block with a concrete reason
- **Tool**: `taskboard_release_claim` · **Purpose**: Release only the current Agent's claim
- **Tool**: `taskboard_relate` · **Purpose**: Add `parent`, `blocks`, or `related` in the same project
...
dsh-taskboard --database .dsh/taskboard.sqlite task create --project  --title "Ship the plugin"
...
### Key · Default · Notes
- **Key**: `databasePath` · **Default**: `.dsh/taskboard.sqlite` · **Notes**: `DSH_TASKBOARD_DATABASE`
- **Key**: `attachmentRoot` · **Default**: `.dsh/taskboard-attachments` · **Notes**: `DSH_TASKBOARD_ATTACHMENTS`
- **Key**: `pageSize` · **Default**: `100` · **Notes**: Bounded list page
- **Key**: `maxAttachmentBytes` · **Default**: `26214400` · **Notes**: Per file (25 MiB)
- **Key**: `maxTaskAttachmentBytes` · **Default**: `104857600` · **Notes**: Per task (100 MiB)
- **Key**: `minAutomationIntervalMs` · **Default**: `30000` · **Notes**: Floor for automation interval
- **Key**: `maxProjectWorkers` · **Default**: `2` · **Notes**: Concurrent claims per project
- **Key**: `maxGlobalWorkers` · **Default**: `4` · **Notes**: Concurrent claims globally
- **Key**: `allowSharedWorktrees` · **Default**: `false` · **Notes**: Exclusive development context
- **Key**: `clientRefreshIntervalMs` · **Default**: `15000` · **Notes**: Snapshot recovery interval
- **Key**: `maxChangeWaiters` · **Default**: `128` · **Notes**: Long-poll waiter cap
- **Key**: `maxChangeWatchMs` · **Default**: `30000` · **Notes**: Long-poll timeout
- **Key**: `defaultAgentPreset` · **Default**: `standard` · **Notes**: Worker preset
...
Backup both the SQLite file (and WAL, if liv