...
- Structure · **notes-style memory**: free text · **dsh-ontology**: classes, typed relations, entities, triples
- Bad input · **notes-style memory**: silently stored · **dsh-ontology**: rejected, naming the constraint it broke
- Implied facts · **notes-style memory**: restated by hand, drift apart · **dsh-ontology**: derived on read from `transitive` / `symmetric` / `inverseOf`
- Retraction · **notes-style memory**: orphaned prose lingers · **dsh-ontology**: facts cascade with their entity; derived facts vanish with their premise
- Reading it back · **notes-style memory**: grep and hope · **dsh-ontology**: query by class (subclass-aware), triple pattern, neighbourhood, or shortest path
...
### Key · Default · Meaning
- **Key**: `domain` · **Default**: `ontology` · **Meaning**: Storage domain name; one name is one isolated graph. Must match `/^[a-z][a-z0-9_]*$/`.
- **Key**: `strict` · **Default**: `true` · **Meaning**: Reject facts that violate the vocabulary. `false` stores them with the violations recorded, for exploratory modelling.
- **Key**: `inference` · **Default**: `true` · **Meaning**: Allow queries to return entailed facts.
- **Key**: `promptSection` · **Default**: `true` · **Meaning**: Contribute the vocabulary summary to the system prompt.
- **Key**: `promptMaxTerms` · **Default**: `60` · **Meaning**: Upper bound on terms in that summary.
- **Key**: `promptOrder` · **Default**: `150` · **Meaning**: Where the section lands in the assembled prompt.
- **Key**: `defaultLimit` / `maxLimit` · **Default**: `50` / `500` · **Meaning**: Query result sizes.
- **Key**: `maxEntities` / `maxFacts` · **Default**: `20000` / `100000` · **Meaning**: Capacity guards; a write past the bound fails loudly.
...
### Mode · Answers
- **Mode**: `schema` · **Answers**: What vocabulary exists? (Start here.)
- **Mode**: `stats` · **Answers**: How big is the graph, and how much is inferable?
- **Mode**: `entities` · **Answers**: Which individuals are in class X (including its subclasses), or match text Y?
- **Mode**: `facts` · **Answers**: Which triples match this pattern? Each omitted position is a wildcard.
- **Mode**: `neighbors` · **Answers**: What surrounds this entity, out to depth N?
- **Mode**: `path` · **Answers**: How are these two entities connected?
