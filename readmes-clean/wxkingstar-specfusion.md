...
### 问题 · 原因 · 解决方案
- **问题**: Claude Code 找不到 · **原因**: 安装时没加 `-g`，只装到了当前目录 · **解决方案**: 重新运行 `npx skills add wxkingstar/SpecFusion -g -y`
- **问题**: Cursor 找不到 · **原因**: skills CLI 目前不会为 Cursor 创建规则文件 · **解决方案**: 用上面的手动安装命令
- **问题**: 验证是否安装成功 · **原因**: — · **解决方案**: Claude Code: `ls ~/.claude/skills/specfusion/SKILL.md`Cursor: `ls ~/.cursor/rules/specfusion.mdc`
...
### 平台 · 文档数量 · 覆盖范围
- **平台**: 企业微信 · **文档数量**: ~2,740 · **覆盖范围**: 服务端 API、客户端 API、应用开发
- **平台**: 飞书 · **文档数量**: ~4,260 · **覆盖范围**: 服务端 API、事件订阅、小程序
- **平台**: 钉钉 · **文档数量**: ~2,750 · **覆盖范围**: 企业内部应用、服务端 API、客户端 JSAPI
- **平台**: 淘宝开放平台 · **文档数量**: ~6,940 · **覆盖范围**: 商品、交易、物流、店铺、用户等 API
- **平台**: 小红书 · **文档数量**: ~100 · **覆盖范围**: 电商开放平台 API（订单、商品、售后、物流等）
- **平台**: 抖音电商开放平台 · **文档数量**: ~1,390 · **覆盖范围**: 商品、订单、物流、售后、精选联盟、即时零售等 API
- **平台**: 微信小程序 · **文档数量**: ~470 · **覆盖范围**: 服务端 API（登录、用户信息、小程序码、客服、数据分析、安全、物流等）
- **平台**: 微信小店 · **文档数量**: ~490 · **覆盖范围**: 商品管理、订单管理、售后管理、物流发货、资金结算、营销优惠券等 API
- **平台**: 拼多多开放平台 · **文档数量**: ~280 · **覆盖范围**: 订单、商品、物流、售后、营销、店铺、虚拟类目、多多进宝等 API
- **平台**: 有赞开放平台 · **文档数量**: ~1,240 · **覆盖范围**: 用户、会员、商品、交易、物流、营销、店铺、分销、财务、美业等 API
- **平台**: 微信支付 · **文档数量**: ~550 · **覆盖范围**: JSAPI/APP/H5/Native/小程序支付、退款、分账、合单支付、代金券、商家转账等 API
- **平台**: 支付宝开放平台 · **文档数量**: ~600 · **覆盖范围**: 当面付、APP支付、手机网站支付、电脑网站支付、资金、会员、营销、安全等 API
- **平台**: 京东商家开放平台 · **文档数量**: ~6,290 · **覆盖范围**: 商品、订单、物流、售后、促销、店铺、数据、发票、供应商等 API
- **平台**: SHEIN开放平台 · **文档数量**: ~240 · **覆盖范围**: 密钥授权、商品、订单、退货退款、采购单、库存、财务、物流、Webhook 等 API
- **平台**: 得物开放平台 · **文档数量**: ~270 · **覆盖范围**: 商品、订单、售后、出价、入仓、开票、文件、对账单等 API
- **平台**: 火山引擎云服务器 · **文档数量**: ~121 · **覆盖范围**: 实例、镜像、密钥对、安全组、地域、部署集、专有宿主机、云助手等 API
- **平台**: 火山引擎 · **文档数量**: ~34,000 · **覆盖范围**: 179 个云产品文档：计算、AI、网络、存储、数据库、容器、安全、CDN、视频云、大数据等
- **平台**: 阿里云百炼 · **文档数量**: ~1,290 · **覆盖范围**: 千问大模型、DashScope SDK、OpenAI兼容接口、语音合成/识别、图像/视频生成、应用开发等
- **平台**: 泛微 e-teams 开放平台 · **文档数量**: ~500 · **覆盖范围**: 认证免登、组织架构、人员、工作流程、业务表单、任务、考勤、CRM、订单、文档、日程等 API
- **平台**: 北森 iTalent 开放平台 · **文档数量**: ~1,080 · **覆盖范围**: 招聘、组织员工、假勤、薪酬、绩效、目标、学习云、干部管理、360评估、PaaS 平台等 API
...
### 变量 · 默认值 · 说明
- **变量**: `PORT` · **默认值**: `3456` · **说明**: 服务端口
- **变量**: `DB_PATH` · **默认值**: `./data/specfusion.db` · **说明**: SQLite 数据库路径
- **变量**: `ADMIN_TOKEN` · **默认值**: `dev-token` · **说明**: Admin API 认证令牌
...
### 端点 · 说明
- **端点**: `GET /api/search?q=关键词&source=wecom&limit=5` · **说明**: 搜索文档
- **端点**: `GET /api/doc/{doc_id}` · **说明**: 获取文档全文
- **端点**: `GET /api/doc/{doc_id}?summary=true` · **说明**: 获取文档摘要
- **端点**: `GET /api/sources` · **说明**: 查看已接入文档源
- **端点**: `GET /api/categories?source=wecom` · **说明**: 浏览文档分类
- **端点**: `GET /api/recent?source=wecom&days=7` · **说明**: 最近更新的文档
- **端点**: `GET /api/health` · **说明**: 健康检查（返回 JSON）

### 搜索参数

### 参数 · 必填 · 说明
- **参数**: `q` · **必填**: 是 · **说明**: 搜索关键词（接口名、API 路径、错误码、功能概念）
- **参数**: `source` · **必填**: 否 · **说明**: 文档来源：`wecom` / `feishu` / `dingtalk` / `taobao` / `xiaohongshu` / `douyin` / `wechat-miniprogram` / `wechat-shop` / `pinduoduo` / `youzan` / `wechat-pay` / `alipay` / `jd` / `shein` / `dewu` / `volcengine-ecs` / `volcengine` / `bailian` / `weaver` / `beisen`
- **参数**: `mode` · **必填**: 否 · **说明**: 开发模式（仅企业微信）：`internal` / `third_party` / `service_provider`
- **参数**: `limit` · **必填**: 否 · **说明**: 返回数量，默认 5，最大 20
