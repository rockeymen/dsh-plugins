...
2. `eval_run` 逐条 fork `dsh --profile headless --patch <overlay> ` 子进程跑真实 agent（overlay 把会话落盘切到隔离目录，每条用例独立 workspace），解析落盘的 `session.jsonl` / `session.jsonl.zstd` trace（多帧 zstd 直读），执行断言，写 `report.json` + `report.md`；
...
子进程命令形如 `dsh --profile headless --patch <overlay> `（launcher flags 在前，
