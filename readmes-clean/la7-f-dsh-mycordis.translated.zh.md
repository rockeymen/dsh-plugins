...
### 类型 · 产物 · 说明
- **类型**: `dsh 包` · **产物**: `.tgz` · **说明**: 真实安装包（`dsh plugin add`）
- **类型**: `便携包` · **产物**: `.dshplugin.json` · **说明**: 纯 host 定义文件
- **类型**: `整包` · **产物**: `.tgz` + `.dshplugin.json` · **说明**: 同时产出两者（pkg-4 新增）
...
### 路由 · 方法 · 说明
- **路由**: `/packer2` · **方法**: GET · **说明**: Web 界面（`embed=1` 为嵌入模式）
- **路由**: `/api/plugins` · **方法**: GET · **说明**: 会话级插件清单（含默认输出目录）
- **路由**: `/api/pack` · **方法**: POST · **说明**: 单个 dsh 包打包（.tgz）
- **路由**: `/api/pack-batch` · **方法**: POST · **说明**: 批量 dsh 包打包
- **路由**: `/api/export-batch` · **方法**: POST · **说明**: 批量导出便携包
- **路由**: `/api/pack-whole` · **方法**: POST · **说明**: **打包整包**（.tgz + 便携包，一插件一子目录）
- **路由**: `/api/export` · **方法**: GET · **说明**: 下载便携包（attachment）
- **路由**: `/api/import` · **方法**: POST · **说明**: 导入便携包（注册不运行）
- **路由**: `/api/upload` · **方法**: POST · **说明**: 上传 .tgz / .dshplugin 文件
- **路由**: `/api/install` · **方法**: POST · **说明**: 安装 dsh 包（真实安装）
- **路由**: `/api/uninstall` · **方法**: POST · **说明**: 卸载 dsh 插件
- **路由**: `/api/installed` · **方法**: GET · **说明**: 指定 profile 已安装插件
- **路由**: `/api/favorites` · **方法**: GET · **说明**: 收藏列表
- **路由**: `/api/favorite` · **方法**: POST · **说明**: 收藏 / 取消 / 设常驻
- **路由**: `/api/restore-one` · **方法**: POST · **说明**: 恢复单个（注册并启动）
- **路由**: `/api/restore-favorites` · **方法**: POST · **说明**: 恢复全部收藏
- **路由**: `/api/dedupe` · **方法**: POST · **说明**: 同名插件去重
- **路由**: `/api/snapshot` · **方法**: POST · **说明**: 导出插件快照到工作区 `packer2-snapshot/`
- **路由**: `/api/browse` · **方法**: GET · **说明**: 目录选择能力探测
- **路由**: `/api/browse/pick` · **方法**: POST · **说明**: 原生目录选择
- **路由**: `/api/browse/create` · **方法**: POST · **说明**: 新建目录
...
- 真实安装（`dsh plugin add`）写入 `$DSH_HOME/profiles/`，需重启 dsh 生效
