...
在 DeepSeek Harness 设置页直接浏览 **mydsh.dev 插件大全**(catalog 5596 个,已核验可直接启用 **3656** 个)——分类浏览、本地/AI 搜索、安装、启用、停用、卸载、安全评估与**一键自动重启生效**。只展示已核验为可直接启用的 DSH 插件。非官方社区 mydsh.dev 出品。
  ...
  ![dsh-plugin-market 安装流程](docs/assets/dsh-plugin-market-install-flow.gif)

## 预览

### 分类浏览 · 本地 / AI 搜索 · 已安装状态
- **分类浏览**: ![分类浏览](docs/assets/market-browse.png) · **本地 / AI 搜索**: ![本地和 AI 搜索](docs/assets/market-search-ai.png) · **已安装状态**: ![已安装状态](docs/assets/market-installed.png)
...
### 端点 · 说明
- **端点**: `POST /api/plugin-market/browse` · **说明**: `{category, limit}` 分类浏览（limit=0 全量）
- **端点**: `POST /api/plugin-market/search` · **说明**: `{query, ai}` 本地 + 可选 AI 搜索
- **端点**: `POST /api/plugin-market/install` · **说明**: `{repo}` 安装到当前配置，但不自动启用
- **端点**: `POST /api/plugin-market/enable` · **说明**: `{repo}` 启用插件，必要时提示重启
- **端点**: `POST /api/plugin-market/disable` · **说明**: `{repo}` 停用插件，保留安装
- **端点**: `POST /api/plugin-market/uninstall` · **说明**: `{repo}` 卸载并清理启用状态
- **端点**: `POST /api/plugin-market/status` · **说明**: `{repo}` 查看单个插件生命周期状态
- **端点**: `POST /api/plugin-market/lifecycle` · **说明**: 当前配置下全部已安装插件生命周期（含运行环境与自动重启能力）
- **端点**: `POST /api/plugin-market/restart` · **说明**: 一键自动重启 DSH（PM2 托管由监督者拉起；其余环境由插件自拉起 wrapper）
- **端点**: `POST /api/plugin-market/assess` · **说明**: `{repo}` 安全评估
- **端点**: `POST /api/plugin-market/installed` · **说明**: 已安装清单
