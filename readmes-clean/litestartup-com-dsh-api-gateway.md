...
- **Method**: GET · **Path**: `/health` · **Auth**: none · **Description**: Status (reachable while disabled)
- **Method**: POST · **Path**: `/key` · **Auth**: first call only · **Description**: One-time API key bootstrap
- **Method**: POST · **Path**: `/sessions` · **Auth**: API key · **Description**: Create a session (`provider/model/maxTokens/cwd/workspace`)
- **Method**: GET · **Path**: `/sessions/discover` · **Auth**: API key · **Description**: List sessions (id/title/cwd/live/persisted) — no content
- **Method**: POST · **Path**: `/sessions/:id/adopt` · **Auth**: API key · **Description**: Adopt an existing session (`live` co-drive / `resumed` cold-resume); returns full history
- **Method**: POST · **Path**: `/sessions/:id/messages` · **Auth**: API key · **Description**: Send a message (string or block array)
- **Method**: GET · **Path**: `/sessions/:id/stream` · **Auth**: API key · **Description**: SSE: hello(replay)→chunk→message→tool_call/tool_result→turn_end
- **Method**: GET · **Path**: `/sessions/:id/history` · **Auth**: API key · **Description**: Full history of **any** session (read-only)
- **Method**: POST · **Path**: `/sessions/:id/cancel` · **Auth**: API key · **Description**: Cancel the active turn
- **Method**: POST · **Path**: `/admin/enable` · **Auth**: Admin key · **Description**: Runtime soft switch `{"enabled": bool}`
- **Method**: POST · **Path**: `/admin/rotate-key` · **Auth**: Admin key · **Description**: Rotate the provisioned key
...
- **mode**: `created` · **Meaning**: Gateway-created session · **Lifecycle**: Owned by the gateway
- **mode**: `live` · **Meaning**: **Co-driving** a GUI-open session: API messages appear in the GUI flow, turns queue from both sides · **Lifecycle**: Borrowed only — plugin stop just untracks it
- **mode**: `resumed` · **Meaning**: Cold-resume of an offline session (needs `sessionPersistence`) · **Lifecycle**: Owned by the gateway after resume
...
- Nature · **Official Python SDK**: **Embedded runtime**: `pip install deepseek-harness-sdk` ships a platform wheel and drives a bundled `dsh-jsonrpc-agent` **subprocess** over JSON-RPC stdio · **This gateway**: **A door into a running Harness**: a host-composition plugin exposing REST + SSE
- Model credentials · **Official Python SDK**: DeepSeek API keys (`DEEPSEEK_API_KEY` / `DEEPSEEK_BASE_URL`) · **This gateway**: Gateway's own API keys (independent of model credentials)
- Sessions · **Official Python SDK**: Private JSONL under `session_root`, unrelated to any deployment or GUI · **This gateway**: The deployment's shared session corpus: GUI-visible, workspace-grouped, adoptable
- Capabilities · **Official Python SDK**: Minimal default composition (local bash etc., no skills, no compaction; customizable via `cordis`) · **This gateway**: The deployment's default agent preset (tools/skills/sandbox policy)
- Platforms · **Official Python SDK**: Linux x64/arm64, macOS 14+ arm64; **no Windows** · **This gateway**: Any client language/platform, Windows PowerShell included
- Isolation · **Official Python SDK**: `danger-full-access`; run in disposable environments/containers · **This gateway**: Inherits the deployment sandbox and approval policy
- Best for · **Official Python SDK**: One-off isolated tasks from Python scripts without a long-running deployment · **This gateway**: Third parties connecting to **your running deployment**, multi-language, unified auth/limits/audit, continuing GUI sessions
...
- **Version**: **v0.1.0** · **Theme**: Baseline (current) · **Contents**: REST + SSE, settings card, reasoning/text split, workspace membership, session adopt, cross-platform docs
- **Version**: **v0.2.0** · **Theme**: Multi-tenant security ★ · **Contents**: Multi-key CRUD/revocation, per-key rate limiting (429 + `Retry-After`), **workspace model: per-key isolated + shared collaborative workspaces (`shared`/`isolated`)**, per-key approval policy, audit (requests/sessions/token usage per key), session persistence (resume after restart)
- **Version**: **v0.3.0** · **Theme**: Admin UI · **Contents**: Full admin settings page (keys/limits/workspace bindings, session monitor, usage audit, soft switch) + typert `@Remote` config surface (the admin page's foundation) + per-key agent preset selection
- **Version**: **v0.4.0** · **Theme**: Duplex streaming · **Contents**: `webServer.registerUpgrade` WebSocket full-duplex (send/stream/cancel on one connection); SSE stays as the lightweight option
- **Version**: **v0.5.0** · **Theme**: Ecosystem & ops · **Contents**: Python/Node HTTP thin clients (OpenAPI-generated — **not** the official embedded SDK, see above), deployment guide (reverse proxy + TLS, Docker Compose), metrics/telemetry export, OpenAPI generation in CI
