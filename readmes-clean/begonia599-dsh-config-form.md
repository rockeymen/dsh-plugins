...
- **Each plugin hand-rolls**: A React form (12 of 12, including the one storing correctly) · **What adopting this gives instead**: A declaration; the renderer is shared
- **Each plugin hand-rolls**: Storage — `localStorage` that is lost on cache clear, or a path that ignores `DSH_HOME` · **What adopting this gives instead**: `ctx.settings`: layered resolution, revision fencing, external-edit reload
- **Each plugin hand-rolls**: An HTTP route plus its security check · **What adopting this gives instead**: One shared route with the check written once
- **Each plugin hand-rolls**: Secret handling · **What adopting this gives instead**: `ctx.credentials`, values in `.credentials.yaml`
...
### Request · Effect
- **Request**: `GET <route>/api/forms` · **Effect**: The list pane: one row per adopting plugin
- **Request**: `GET <route>/api/forms/` · **Effect**: The detail pane: declaration, values, `base`/`user` layers, `revision`, secret states
- **Request**: `POST <route>/api/forms/` · **Effect**: `{ revision, set, unset }` → fenced path-op write; answers with the re-read state
- **Request**: `PUT <route>/api/forms//secret/<key>` · **Effect**: `{ value }` → store through the credential seam
- **Request**: `DELETE <route>/api/forms//secret/<key>` · **Effect**: Remove that credential
- **Request**: `GET <route>/api/namespaces` · **Effect**: Audit view over EVERY namespace, adopting or not, with its redaction verdict
...
### Check · Result
- **Check**: Write two fields with the current revision · **Result**: 200; `revision` 0 → 1; `user` layer holds exactly those two keys
- **Check**: Write with a stale revision · **Result**: **409 `conflict`**
- **Check**: Write an undeclared key · **Result**: **400 `undeclared-field`**
- **Check**: Write the secret key into the settings section · **Result**: **400 `undeclared-field`**
- **Check**: `PUT` a secret · **Result**: 200; `configured: true`, `source: file`
- **Check**: `PUT` a blank secret · **Result**: **400** — a blank draft is not a removal
- **Check**: `PUT`/`DELETE` an undeclared secret · **Result**: **400 `undeclared-field`**
- **Check**: Secret value present in any response · **Result**: **no**
- **Check**: `settings.yaml` after both writes · **Result**: `demo-git: { defaultBranch: develop, fetchDepth: 50 }` — no secret
- **Check**: `.credentials.yaml` after both writes · **Result**: `DEMO_GIT_TOKEN: sk-…`
- **Check**: `DELETE` the secret, then `unset` both fields · **Result**: credential removed; values back to schema defaults; `user` empty
- **Check**: `Host: evil.example.com` · **Result**: **403 `host-not-loopback`**
- **Check**: `POST` with neither `Sec-Fetch-Site` nor `Origin` · **Result**: **403 `cross-origin`**
...
### Check · Result
- **Check**: `dsh plugin add <tarball>` · **Result**: appends `dsh-config-form` to `dsh.profile.bundles`
- **Check**: `GET /api/forms` · **Result**: 200, lists the adopter
- **Check**: `GET /api/forms/` · **Result**: values resolved, secret reported unconfigured/writable
- **Check**: `index.html` · **Result**: carries the injected route
- **Check**: `/plugins/dsh-config-form/client.js` · **Result**: 200
- **Check**: `POST` a field · **Result**: lands; `user` layer holds only that key
- **Check**: `pnpm publish --dry-run` · **Result**: runs `prepare`, resolves `dsh-config-form@0.1.0`
