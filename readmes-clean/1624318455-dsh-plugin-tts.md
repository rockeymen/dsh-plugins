...
### Region · Voices
- **Region**: Simplified Chinese · **Voices**: Xiaoxuan 晓萱 · Xiaoyi 晓伊 · Yunxi 云希 · Yunyang 云扬 · Xiaoxiao 晓晓 · Yunjian 云健 · Yunxia 云夏 · liaoning-Xiaobei 晓北 · shaanxi-Xiaoni 晓妮
- **Region**: Taiwan · **Voices**: HsiaoChen 曉臻 · HsiaoYu 曉雨 · YunJhe 雲哲
- **Region**: Hong Kong · **Voices**: HiuGaai 曉佳 · HiuMaan 曉曼 · WanLung 雲龍
- **Region**: English · **Voices**: Aria · Jenny · Guy · Sonia (UK)
- **Region**: Other · **Voices**: Nanami 七海 (ja-JP) · SunHi (ko-KR) · Denise (fr-FR)
...
### Layer · Location · Role
- **Layer**: Host · **Location**: `lib/index.mjs` · **Role**: Registers `/dsh-tts-api/speak` (synthesis / chunk queue), `/dsh-tts-audio/` (audio), `/dsh-tts-api/rvc-*` (RVC inference / files / compact index / voice packs) webServer routes; runs a zero-dependency worker via `node -e`
- **Layer**: Client · **Location**: `lib/client.js` · **Role**: Hidden `<audio>` host in `shell.overlay` + the UI entries (read-aloud button / auto-read toggle / settings panel); talks to the Host through `fetch`
