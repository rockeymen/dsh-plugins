...
### Key · Default · Meaning
- **Key**: `root` · **Default**: `<cwd>/.dsh-notes` · **Meaning**: Directory where notes (`.md` files + `meta.json`) are stored.
- **Key**: `route` · **Default**: `/plugins/md-notes` · **Meaning**: HTTP API prefix served by the plugin; also hosts the icon at `<route>/icon.svg`.
...
### Symptom · Fix
- **Symptom**: Changes don't appear after install/upgrade · **Fix**: Restart dsh web — bundle layer and client metadata are cached in the process.
- **Symptom**: Icon looks stale · **Fix**: Hard-refresh the page; the icon is served with `no-cache` and reflects `assets/dsh-md-notes.svg` on every request.
- **Symptom**: Plugin doesn't load · **Fix**: Verify the layer: `dsh --profile web --dump-config` and look for the `md-notes` row.
- **Symptom**: Installed from git and `add` failed · **Fix**: pnpm ≥10 blocks build scripts by default; add the printed package key under `allowBuilds` in the profile's `pnpm-workspace.yaml`, then re-run `add`.
- **Symptom**: Notes can't be created/saved · **Fix**: Make sure the configured `root` points to an existing writable directory.
...
### Command · Purpose
- **Command**: `npm run build` · **Purpose**: Full build (tsc host → tsc client → tsdown)
- **Command**: `npm run typecheck` · **Purpose**: Type-check only (both programs)
- **Command**: `npm run link-deps` · **Purpose**: Re-link `@deepseek-ai/*` types to the checkout
- **Command**: `npm run bundle` · **Purpose**: Build only the client bundle
...
### Path · Contents
- **Path**: `src/` · **Contents**: Source code (host half + client half)
- **Path**: `src/host/` · **Contents**: Notes domain logic (`notes.ts`) + HTTP layer (`http.ts`)
- **Path**: `src/client/` · **Contents**: Browser half: entry (`index.ts`) + feature modules under `features/`
- **Path**: `src/client/features/locales/` · **Contents**: zh/en UI dictionaries (dsh locale namespace `md-notes`)
- **Path**: `assets/` · **Contents**: Plugin icon (SVG source + PNG)
- **Path**: `docs/` · **Contents**: Design docs: `features.md` (functional), `architecture.md`, `TODO.md`
- **Path**: `scripts/` · **Contents**: Dev tooling (e.g. `link-deps.mjs`)
- **Path**: `lib/` · **Contents**: Build output (gitignored; what npm publishes)