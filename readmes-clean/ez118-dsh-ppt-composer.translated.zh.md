...
### 能力 · 说明
- **能力**: 对话式生成 · **说明**: AI 主动询问主题/受众/结构 → 确认后拆解为多页 → 确认内容 → 询问配色 → 套用 → 导出
- **能力**: 长宽比与过渡 · **说明**: `16:9` / `4:3`；页间过渡 `fade` / `slide` / `zoom` / `none`
- **能力**: 媒体支持 · **说明**: `image`（URL / data URI）、`video`（控制条播放）、`chart`（CSS 柱状图，导出自包含）
- **能力**: 主题 v2 · **说明**: 文档主题（配色/双字体/圆角）+ **每页局部主题覆盖**（`slide.theme`），各页可不同配色/字体
- **能力**: SVG 装饰 · **说明**: `slide.decor` 内联 SVG（折线时间线箭头、流程图步骤、几何装饰），viewBox `0 0 1920 1080/1440` 铺满整页、位于内容下层
- **能力**: 入场动画 · **说明**: `slide.entrance`：`fade` / `rise` / `zoom` / `none`，标题与正文分段错峰
- **能力**: 页面对齐 · **说明**: `slide.align`：`center` / `top`（带 `decor` 的复杂模板页默认 `top`，标题固定在顶部不与装饰重叠）
- **能力**: 实时预览 · **说明**: 固定逻辑画布（1920×1080 / 1920×1440）`transform: scale` 缩放居中；可拖拽、右下角调整大小、翻页、缩略图跳转、导出
- **能力**: HTML 导出 · **说明**: 与预览**同画布**的自包含 HTML：键盘翻页、翻页按钮组、窗口自适应缩放、主题/装饰/动画一致
...
### 工具 · 作用
- **工具**: `ppt_create` · **作用**: 创建/重建 PPT（`title`、`slides[]`、`ratio`、`transition`）
- **工具**: `ppt_get` · **作用**: 读取完整状态（含每页 id、当前主题、可用主题）
- **工具**: `ppt_update` · **作用**: 整体修改（标题/整组幻灯片/长宽比/过渡/主题）
- **工具**: `ppt_delete` · **作用**: 删除当前会话 PPT
- **工具**: `ppt_slide_add` · **作用**: 插入一页（`slide`、`index?`）
- **工具**: `ppt_slide_update` · **作用**: 按 `slideId` 局部修改（patch 覆盖）
- **工具**: `ppt_slide_remove` · **作用**: 按 `slideId` 删除
- **工具**: `ppt_theme_create` · **作用**: 创建主题（colors/fontFamily/fonts/radius）
- **工具**: `ppt_theme_update` · **作用**: 修改主题（部分覆盖；默认主题不可改）
- **工具**: `ppt_theme_get` · **作用**: 读取主题/列表/当前主题
- **工具**: `ppt_theme_apply` · **作用**: 套用主题到当前 PPT
- **工具**: `ppt_export` · **作用**: 导出独立 HTML 到会话工作区
...
- 自包含（无外部依赖），``/`<video>` 引用外部 URL 时需联网，data URI 则完全离线；
