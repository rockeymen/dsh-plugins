...
- **Setting**: Enabled · **Default**: `true` · **Values**: on / off
- **Setting**: Mode · **Default**: `live` · **Values**: `live`, `cached`, `indexed`
- **Setting**: Context size · **Default**: `medium` · **Values**: `low`, `medium`, `high`
- **Setting**: Fallback model · **Default**: `gpt-5.4` · **Values**: Codex model ID
- **Setting**: Maximum output tokens · **Default**: `2048` · **Values**: positive integer
...
### Setting · Default · Values
- **Setting**: Enabled · **Default**: `true` · **Values**: on / off
- **Setting**: Image model · **Default**: `gpt-image-2` · **Values**: image model ID
- **Setting**: Image count · **Default**: `1` · **Values**: 1–10
- **Setting**: Size · **Default**: `auto` · **Values**: `auto`, `1024x1024`, `1536x1024`, `1024x1536`
- **Setting**: Quality · **Default**: `auto` · **Values**: `auto`, `low`, `medium`, `high`
- **Setting**: Background · **Default**: `auto` · **Values**: `auto`, `opaque`, `transparent`
...
### Row · Export · Purpose
- **Row**: `llm-codex-auth` · **Export**: `dsh-codex-auth` · **Purpose**: Shared auth coordinator and LLM route
- **Row**: `codex-search` · **Export**: `dsh-codex-auth/search` · **Purpose**: Global Search Provider
- **Row**: `codex-image` · **Export**: `dsh-codex-auth/image` · **Purpose**: Agent-scoped image tools
...
- **Field**: `llmEnabled` · **Default**: `true` · **Meaning**: Register the `openai-codex` LLM route
- **Field**: `authJsonPath` · **Default**: `''` → `$CODEX_HOME`/`~/.codex/auth.json` · **Meaning**: Codex auth file
- **Field**: `credentialRef` · **Default**: `CODEX_CHATGPT_TOKEN` · **Meaning**: Value-free reference shown by the card
- **Field**: `refreshLeadMs` · **Default**: `300000` · **Meaning**: Refresh lead time in milliseconds
- **Field**: `codexCommand` · **Default**: `codex` · **Meaning**: CLI command used for login and version probing
- **Field**: `displayName` · **Default**: `OpenAI Codex (chatgpt)` · **Meaning**: Provider label in model selectors
- **Field**: `transport` · **Default**: `sse` · **Meaning**: Streaming transport: `sse`, `websocket`, or `auto` (WebSocket first with SSE fallback). SSE is the default: the WebSocket upgrade is unreliable through common HTTP proxies, and every new conversation pays the connect timeout before `auto` falls back
- **Field**: `websocketConnectTimeoutMs` · **Default**: `5000` · **Meaning**: WebSocket connect timeout in milliseconds (used only when `transport` is not `sse`; `0` disables it)
- **Field**: `timeoutMs` · **Default**: `120000` · **Meaning**: Request timeout in milliseconds (SSE response-header phase; also the WebSocket message idle interval; `0` disables it)
