...
### 维度 · Claude Code · 本插件
- **维度**: `/commit` · **Claude Code**: 分析变更、生成消息并提交 · **本插件**: ✅ 同流程，另加 git 安全协议与预检短路
- **维度**: `/review` · **Claude Code**: 审查当前变更并输出报告 · **本插件**: ✅ 同流程，支持全部官方修饰词
- **维度**: 修饰词语法 · **Claude Code**: `/review:security`（冒号） · **本插件**: `/review security`（空格；DSH 命令解析器 `[a-z0-9_-]` 不支持冒号）
- **维度**: 提交执行 · **Claude Code**: 模型直接执行 · **本插件**: 模型经 DSH 沙箱/审批策略执行（更安全）
- **维度**: 会话语言 · **Claude Code**: 跟随用户 · **本插件**: 跟随用户（默认简体中文）
