...
![引导页（设置个人密码）](docs/assets/onboarding.png)引导页（初始密码登录后）
![登录（含 2FA 验证码）](docs/assets/login.png)登录（含 2FA 验证码）

![2FA 登录成功](docs/assets/login-success.png)2FA 登录成功
![OTP 设置（QR 码）](docs/assets/otp-setup.png)OTP 设置（QR 码）

![设置菜单（含认证设置入口）](docs/assets/settings-menu.png)设置菜单（含"认证设置"入口）
![认证设置面板](docs/assets/settings-auth.png)认证设置面板
...
### 字段 · 默认 · 含义
- **字段**: `listenHost` / `listenPort` · **默认**: `0.0.0.0` / `3080` · **含义**: 网关对外监听地址与端口
- **字段**: `upstreamHost` / `upstreamPort` · **默认**: `127.0.0.1` / `3081` · **含义**: 内部 webserver 地址与端口
- **字段**: `minPasswordLength` · **默认**: `8` · **含义**: 密码最小长度（4–128）
- **字段**: `requireMixedCase` / `requireSpecial` · **默认**: `true` / `true` · **含义**: 密码复杂度：大小写混合或特殊字符二选一满足
- **字段**: `maxLoginFailures` / `lockMinutes` · **默认**: `5` / `5` · **含义**: 密码失败锁定阈值与时长
- **字段**: `maxGlobalAuthAttemptsPerMinute` · **默认**: `60` · **含义**: 全局登录尝试速率上限
- **字段**: `maxOtpAttemptsPerMinute` · **默认**: `10` · **含义**: 单来源 OTP/备份码验证速率上限
- **字段**: `otpEnabled`（已废弃） · **默认**: `false` · **含义**: 不再作为启用开关——2FA 由用户登录后在「认证设置」中绑定激活；字段保留仅为兼容旧配置
- **字段**: `otpRequired` · **默认**: `false` · **含义**: 2FA 激活后强制每次登录验证（无需任何配置）
- **字段**: `otpIssuer` / `otpPeriod` / `otpDigits` / `otpWindow` · **默认**: `dsh-auth-gateway` / `30` / `6` / `1` · **含义**: TOTP 参数（显示名、周期、位数、窗口）
- **字段**: `backupCodeCount` / `backupCodeLength` · **默认**: `10` / `8` · **含义**: 备份代码数量与长度
...
### 文档 · 内容
- **文档**: [docs/SECURITY.md](docs/SECURITY.md) · **内容**: 威胁模型、OTP 安全设计、已知限制与恢复路径
- **文档**: [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md) · **内容**: 端口与监听、LAN 部署、HTTPS 建议、故障排查
- **文档**: [docs/TESTING.md](docs/TESTING.md) · **内容**: 单元测试、端到端（Playwright）、API/WebSocket 门禁验证
- **文档**: [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) · **内容**: 架构说明、构建、开发统计
