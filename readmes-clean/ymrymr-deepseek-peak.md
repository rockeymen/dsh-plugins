...
- **Route**: `/api/peak-hours/state` · **Method**: `GET` · **Purpose**: Current pause / phase / queue / balance / threshold (2 s polled by the browser)
- **Route**: `/api/peak-hours/state` · **Method**: `POST` · **Purpose**: Toggle the global pause (writes through the host's settings service)
- **Route**: `/api/peak-hours/balance` · **Method**: `GET` · **Purpose**: Live DeepSeek account balance (5 min host cache)
- **Route**: `/api/peak-hours/balance?fresh=1` · **Method**: `GET` · **Purpose**: Force a fresh upstream fetch (bypasses both the host and the browser cache; the in-tooltip REFRESH button calls this)
- **Route**: `/api/peak-hours/usage?rangeDays=N` · **Method**: `GET` · **Purpose**: Per-day, per-model token / cost buckets for the trailing N days (default 30, max 365; 5 min host cache)
- **Route**: `/api/peak-hours/usage?fresh=1&rangeDays=N` · **Method**: `GET` · **Purpose**: Force a fresh host walk of the session persistence log
- **Route**: `/api/peak-hours/queue/dispatch` · **Method**: `POST` · **Purpose**: Manually dispatch the front queued item (the per-row send arrow in the queue card calls this)
...
### Check · Result
- **Check**: `pnpm run constraints` · **Result**: 220/220 packages conform
- **Check**: `pnpm run verify-cordis-config` · **Result**: 120/120 config files passed
- **Check**: `pnpm run verify-package-invariants` · **Result**: 220/220 hand-owned companions conform
- **Check**: `tsc -b tsconfig.client.json` · **Result**: clean
- **Check**: `pnpm --filter ... run bundle` · **Result**: ~78 kB `client.js`
- **Check**: `pnpm run build` (host + client + web) · **Result**: clean
- **Check**: Live runtime smoke (curl `/plugins/.../client.js`) · **Result**: 200, ~78 kB, slot registered
