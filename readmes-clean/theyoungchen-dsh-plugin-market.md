...
### 类型 · 判定 · 安装方式
- **类型**: 插件 · **判定**: 根目录含 `package.json` · **安装方式**: `pnpm add` 进 profile（官方路径）
- **类型**: Skill · **判定**: 根目录含 `SKILL.md` · **安装方式**: 克隆到 `~/.dsh/skills/<名>/`
- **类型**: 预设 · **判定**: `preset.yml` / `agent.cordis.yml` · **安装方式**: 克隆到 `~/.dsh/.agent-presets/<名>/`
- **类型**: 脚本 · **判定**: `install.sh` / `install.ps1` · **安装方式**: 克隆后执行安装脚本（需确认信任）

## 工作原理

- **node half**（`src/index.ts`）：在宿主注册同源路由 `/api/plugin-market/*`。安装时在 `$DSH_HOME/profiles/web` 里 `spawn pnpm add github:<owner/repo>`，并把声明 `dsh.bundle` 的依赖 reconcile 进 `dsh.profile.bundles` 层栈；进度通过轮询 `GET /api/plugin-market/job/` 返回，`POST /api/plugin-market/job//cancel` 可终止进程树。
