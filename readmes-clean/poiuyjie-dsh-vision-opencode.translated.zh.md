...
> **DeepSeek 不认图怎么办？OpenCode 这套多模态平替方案了解一下。**
  ...
  ![dsh-vision-opencode 演示](assets/demo.png)
...
### 情况 · 行为
- **情况**: 识图调用限流/超时/5xx/传输错误 · **行为**: 退避 800ms 重试 1 次（共 2 次尝试）
- **情况**: 非可重试失败（AUTH/4xx 等） · **行为**: 不重试，立即降级
- **情况**: 重试耗尽（发图自动转换路径） · **行为**: 降级为占位文本，主模型照常回复并告知用户
- **情况**: 重试耗尽（`vision_read_image` 工具路径） · **行为**: 工具结果 `isError`（良性），主模型继续工作
- **情况**: 单次调用挂起 · **行为**: 每次尝试独立 60s 超时
- **情况**: 用户取消回合 · **行为**: 直接终止，不做无意义降级
- **情况**: 插件自身意外 bug · **行为**: 最后一道保险：图片全部降级为占位文本，不杀死回合
- **情况**: 切换到其他供应商或模型 · **行为**: 自动读取其输入能力；纯文本模型转换图片，原生多模态模型保留 DSH 原链路
