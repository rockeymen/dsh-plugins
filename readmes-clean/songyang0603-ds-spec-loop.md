...
- **Capability**: Understand the repository before editing · **What the Agent does**: Reads applicable instructions, current docs, existing decisions, relevant source, tests, generated files, and Git history when needed
- **Capability**: Maintain decision records · **What the Agent does**: Creates or updates the record that owns a decision, and keeps its lifecycle accurate as work is implemented, rejected, superseded, or archived
- **Capability**: Define verifiable acceptance criteria · **What the Agent does**: Connects each requirement to observable behavior, likely failure points, and evidence that can confirm or disprove completion
- **Capability**: Implement the complete path · **What the Agent does**: Follows the change through definitions, providers, registration or loading, consumers, persistence, and user- or model-facing behavior where applicable
- **Capability**: Keep the repository consistent · **What the Agent does**: Updates affected code, public contracts, tests, snapshots, generated artifacts, current documentation, and decision rationale together
- **Capability**: Support simplification and deletion · **What the Agent does**: Checks real consumers and removes obsolete registrations, exports, files, tests, documentation, and compatibility surfaces—not only the main source file
- **Capability**: Review without modifying · **What the Agent does**: Can perform a read-only comparison of active decisions, source, tests, generated artifacts, and current documentation
- **Capability**: Fit existing conventions · **What the Agent does**: Reuses a repository's ADR, RFC, design-doc, or Spec system instead of creating a competing documentation structure
...
### Host · Invoke
- **Host**: Codex · **Invoke**: `$ds-spec-loop`
- **Host**: Claude Code · **Invoke**: `/ds-spec-loop`
- **Host**: GitHub Copilot CLI · **Invoke**: `/ds-spec-loop`
- **Host**: Other compatible agents · **Invoke**: Use the host's documented Skill invocation or mention `ds-spec-loop` by name
...
- **Host**: Codex · **User scope**: `~/.agents/skills/ds-spec-loop` · **Repository scope**: `.agents/skills/ds-spec-loop`
- **Host**: Claude Code · **User scope**: `~/.claude/skills/ds-spec-loop` · **Repository scope**: `.claude/skills/ds-spec-loop`
- **Host**: GitHub Copilot CLI · **User scope**: `~/.copilot/skills/ds-spec-loop` or `~/.agents/skills/ds-spec-loop` · **Repository scope**: `.github/skills/ds-spec-loop` or `.agents/skills/ds-spec-loop`
