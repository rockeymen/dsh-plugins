...
- **方面**: 平台 · **状态**: 全部（宿主 answerer；可选 Web 审查面板，依赖会话投影能力）
- **方面**: 模型 · **状态**: 任意（审查器默认继承会话代理的路由；`reviewerModel` 可覆盖）
...
### 键 · 默认值 · 含义
- **键**: `enableByDefault` · **默认值**: `true` · **含义**: 会话默认开启 auto-review；`/auto-review on\ · off` 写入的持久覆盖优先于此
- **键**: `toolsPolicy.default` · **默认值**: `human` · **含义**: 未列出工具的默认策略（委派给人类 answerer）
- **键**: `toolsPolicy.overrides` · **默认值**: `{}` · **含义**: 按工具策略：`ai` / `human` / `never`
- **键**: `riskRules` · **默认值**: `[]` · **含义**: 在工具表之前匹配的 `{pattern, policy, field?}`；`field` 选择 `reason`（默认）、`toolName` 或 `arguments`
- **键**: `reviewerProvider` · **默认值**: `fork` · **含义**: 审查器的子代理 provider（进程内 fork 后端）
- **键**: `reviewerModel` · **默认值**: *(继承)* · **含义**: 审查器模型 id；不设置则继承会话代理的路由
- **键**: `reviewerTimeoutMs` · **默认值**: `60000` · **含义**: 裁决截止时间；超时后应用 fallback 策略
- **键**: `reviewerTools` · **默认值**: `[read, glob, grep]` · **含义**: 审查器子代理的工具白名单（必须非空）
- **键**: `fallbackPolicy` · **默认值**: `rejected` · **含义**: 审查器失败时的策略：`rejected`（失败关闭）/ `delegate` / `allow-once`
- **键**: `maxReviewsPerTurn` · **默认值**: `10` · **含义**: 每个开放轮次真实 AI 裁决的预算；超出后请求委派给人类
- **键**: `maxFailuresPerTurn` · **默认值**: `10` · **含义**: 每个开放轮次审查器失败的预算
- **键**: `reasonMaxChars` · **默认值**: `2000` · **含义**: 审查理由与脱敏参数预览的上限
- **键**: `reviewerGuidance` · **默认值**: *(无)* · **含义**: 追加到审查器提示词的可选指导性说明
- **键**: `reviewerPolicyText` · **默认值**: *(无)* · **含义**: 注入审查器提示词的 Markdown 裁决策略（Codex 风格）
- **键**: `denyGuidance` · **默认值**: *(反规避文本)* · **含义**: 追加到每一条注入的拒绝理由之后的指导
- **键**: `contextBudget` · **默认值**: `{turns: 0, maxChars: 4000}` · **含义**: 审查器提示词的紧凑记录预算；`turns: 0` 表示禁用
- **键**: `riskPolicy` · **默认值**: `{maxAutoAllow: high, onHighRisk: delegate}` · **含义**: 超过 `maxAutoAllow` 的 `allow` 裁决委派或拒绝
- **键**: `circuitBreaker` · **默认值**: `{consecutiveDenies: 3, windowDenies: 6, windowSize: 10, action: delegate}` · **含义**: 拒绝熔断器
- **键**: `overrideTtlMs` · **默认值**: `300000` · **含义**: `/auto-review approve` 覆盖的有效时长
- **键**: `language` · **默认值**: `en` · **含义**: `/auto-review` 命令输出的界面语言（`en` \ · `zh`）
...
### 界面 · 类型 · 说明
- **界面**: `auto-review` · **类型**: answerer · **说明**: `approval/request` 瀑布 answerer —— 认领 `ai` 策略请求，其余经 `next()` 委派
- **界面**: `/auto-review` · **类型**: 命令 · **说明**: `on\ · off\ · status\ · approve [n]` —— 持久会话覆盖、预算与累计统计
- **界面**: 拒绝理由注入 · **类型**: 监听器 · **说明**: `tools/post-execute` —— 将裁决 / fallback / `never` 理由反馈到被拒绝的工具结果
- **界面**: `autoReview` · **类型**: 会话投影 · **说明**: 由仅日志的 `autoReview/*` 事件折叠而成
- **界面**: Web 审查面板 · **类型**: 客户端 · **说明**: 会话头部操作：开关、预算、统计、最近裁决、一次性批准
- **界面**: `dsh-eval` · **类型**: CLI · **说明**: YAML 驱动的代理评估引擎（`bin/dsh-eval.mjs`）
- **界面**: invariant 配套插件 · **类型**: invariant · **说明**: `dsh-auto-review/invariant`（可选；需要 `invariants` 服务）
...
- `autoReview/state` 与 `autoReview/verdict` 以信封 `ignorable: true` 标记追加，任何 harness 构建都能加载日志 —— 不认识这些仓库外类型的读取方直接跳过这些记录。（rc.6 宿主接受并忽略该标记；pre-0.1.1 版本写入的会话可用 `dsh-permission-rules` 的 `scripts/repair-session-logs.mjs` 修复。）
...
### 插件 · 一句话简介
- **插件**: [dsh-mcp-panel](https://github.com/PerryLink/dsh-mcp-panel) · **一句话简介**: 只读 MCP 运行时面板：/mcp 命令 + 带状态、工具与错误的 Settings 标签页
- **插件**: [dsh-doublecheck](https://github.com/PerryLink/dsh-doublecheck) · **一句话简介**: 工程纪律守卫：需求质询、测试门禁、对手评审
- **插件**: [dsh-background-agents](https://github.com/PerryLink/dsh-background-agents) · **一句话简介**: 带 Web UI 侧栏、消息与中断的持久后台子代理
- **插件**: [dsh-lsp-actions](https://github.com/PerryLink/dsh-lsp-actions) · **一句话简介**: 通过语言服务器的 LSP 诊断、格式化、补全、代码操作与重命名
- **插件**: [dsh-output-styles](https://github.com/PerryLink/dsh-output-styles) · **一句话简介**: Claude Code outputStyles 等价的运行时风格切换
- **插件**: [dsh-checkpoint-rewind](https://github.com/PerryLink/dsh-checkpoint-rewind) · **一句话简介**: Claude Code /rewind 等价：快照、会话 fork、一次性恢复
- **插件**: [dsh-permission-rules](https://github.com/PerryLink/dsh-permission-rules) · **一句话简介**: Claude Code 风格声明式 allow/deny/ask 权限规则，带审计
- **插件**: **[dsh-auto-review](https://github.com/PerryLink/dsh-auto-review)** · **一句话简介**: 审批链上的第二模型自动审查，默认失败关闭
- **插件**: [dsh-memento](https://github.com/PerryLink/dsh-memento) · **一句话简介**: 审批门控的跨会话记忆：ctx.memory 接缝 + SQLite + 记忆工具
- **插件**: [dsh-skill-pack-security](https://github.com/PerryLink/dsh-skill-pack-security) · **一句话简介**: 安全审计技能包：密钥扫描、依赖与供应链审查
- **插件**: [dsh-session-pin](https://github.com/PerryLink/dsh-session-pin) · **一句话简介**: 在 Web 侧栏置顶会话，带持久排序
- **插件**: [dsh-composer-history](https://github.com/PerryLink/dsh-composer-history) · **一句话简介**: Web 输入框的终端式历史：方向键、Ctrl+R 搜索
- **插件**: [dsh-github](https://github.com/PerryLink/dsh-github) · **一句话简介**: 面向 DSH 的 GitHub PR/issues 集成，每次写入经审批门控
- **插件**: [dsh-plugin-guide](https://github.com/PerryLink/dsh-plugin-guide) · **一句话简介**: 作为按需代理技能的插件开发知识库
- **插件**: [dsh-claude-move](https://github.com/PerryLink/dsh-claude-move) · **一句话简介**: 把 Claude Code 会话、记忆、技能与 CLAUDE.md 迁入 DSH
