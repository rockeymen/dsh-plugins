# DSH Desk Pet
  ...
  ![The DeepSeek whale, breathing and blinking](docs/media/idle.gif)

  A desk pet that shows you what your agent is doing.
  It floats above every window — including fullscreen — and changes expression
  as DSH works, waits, finishes or fails.

  ![idle, working, waiting, error, happy, sleeping](docs/media/states.png)
...
### State · When
- **State**: **idle** · **When**: Nothing to do — breathes, blinks now and then
- **State**: **working** · **When**: DSH is running
- **State**: **waiting** · **When**: Blocked on a confirmation, approval, or your input
- **State**: **error** · **When**: The run failed
- **State**: **happy** · **When**: A run just finished; settles back to idle after a few seconds
- **State**: **sleeping** · **When**: Dozes when the agent is idle **and** your pointer has stopped moving. Any activity, or a poke, wakes it.
...
## Skins
  ...
  ![Five skins](docs/media/skins.png)

  <sub>DeepSeek Whale (default) · Blue Whale · Threadcore · Nautilus · Jellyfish</sub>

Right-click to cycle, or `--skin `. Every skin has all six states at three
frames each.
...
A skin is a folder of frames. Anything at `assets/web//<state>/*.png`
