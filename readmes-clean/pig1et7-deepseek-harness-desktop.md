...
### 版本 · 说明
- **版本**: 1.0.3 · **说明**: 文件上传插件（📎 上传 + docx/pdf 文本提取 + Agent 工具 + 一键安装脚本）
- **版本**: 1.0.2 · **说明**: 图标更新（DeepSeek 卡通娘 v2）
- **版本**: 1.0.1 · **说明**: 图标更新（DeepSeek 卡通娘）
- **版本**: 1.0.0 · **说明**: 初始版本：桌面套壳应用
...
### 条件 · 说明
- **条件**: Windows x64 · **说明**: 预编译的 portable exe 仅支持 Windows x64
- **条件**: Node.js 18+ · **说明**: 启动 dsh 服务需要 node（到 nodejs.org 安装 LTS 版）
- **条件**: 网络 · **说明**: 首次运行需要联网，通过 npx 获取 dsh 包
- **条件**: Edge 浏览器（仅方式 C） · **说明**: Windows 10/11 自带
...
### 键 · 说明
- **键**: `port` · **说明**: 起始端口，默认 3080；被占用时自动向后找空闲端口
- **键**: `workspaceDir` · **说明**: dsh 服务的工作目录，默认 exe 所在目录
- **键**: `dshBin` · **说明**: 手动指定 `@deepseek-ai/dsh/lib/bin.js` 的绝对路径（通常无需设置）
- **键**: `nodeBin` · **说明**: 手动指定 node 可执行文件路径（通常无需设置）
- **键**: `npmCache` · **说明**: 手动指定 npm 缓存目录（用于搜索 npx 缓存中的 dsh）
