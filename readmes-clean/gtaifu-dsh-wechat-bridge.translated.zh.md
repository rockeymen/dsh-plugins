...
### 依赖 · 支持/已验证 · 最后验证日期
- **依赖**: Node.js · **支持/已验证**: `>= 18` · **最后验证日期**: 2026-08-14
- **依赖**: DSH · **支持/已验证**: `dsh@0.1.0-rc.6`（npm） · **最后验证日期**: 2026-08-14
- **依赖**: 腾讯 iLink · **支持/已验证**: 对照官方 SDK `2.4.6` · **最后验证日期**: 2026-08-14
- **依赖**: 操作系统 · **支持/已验证**: Windows / macOS / Linux（核心为纯 Node） · **最后验证日期**: 2026-08-14
...
### 指令 · 作用
- **指令**: `/help` · **作用**: 指令列表
- **指令**: `/status` · **作用**: 连接剩余时间 + 当前会话与记忆条数
- **指令**: `/time` · **作用**: 本次连接剩余时间
- **指令**: `/sessions` · **作用**: 列出你的会话（记忆轮数 + 当前标记）
- **指令**: `/new <名字>` · **作用**: 新建命名会话并切换过去
- **指令**: `/switch <名字>` · **作用**: 切换会话（`main` 为默认会话）
- **指令**: `/clear [名字]` · **作用**: 清除当前（或指定）会话的对话记忆（工作目录文件保留）
- **指令**: `/reconnect` · **作用**: 手动重新连接
- **指令**: `/send <文件路径> [说明]` · **作用**: 把电脑上的文件发给你（绝对路径直接用；相对路径按当前会话的工作目录算；图片/视频按媒体发送，其余按文件发送）
...
### 选项 · 默认 · 说明
- **选项**: `--base-url` · **默认**: `https://ilinkai.weixin.qq.com` · **说明**: iLink 端点（测试时可指向 mock）
- **选项**: `--channel-version` · **默认**: `2.4.6` · **说明**: base_info.channel_version
- **选项**: `--bot-agent` · **默认**: `dsh-wechat-bridge/…` · **说明**: base_info.bot_agent（仅观测用途）
- **选项**: `--data-dir` · **默认**: `./data` · **说明**: 凭证/记忆/工作目录根
- **选项**: `--auth-file` · **默认**: `<data-dir>/weixin-auth.json` · **说明**: 🔒 登录凭证（含 bot token，勿外传）
- **选项**: `--allow-from` · **默认**: 全部 · **说明**: 🔒 只响应指定用户 ID（逗号分隔，强烈建议设置）
- **选项**: `--reply-max-chars` · **默认**: `3800` · **说明**: 单条回复截断上限
- **选项**: `--no-typing` · **默认**: 关 · **说明**: 不发"正在输入"状态
- **选项**: `--session-ms` / `--relogin-before-ms` · **默认**: 7 天 / 24h · **说明**: 本地会话计时 / 计时到期前提醒提前量（实际有效性由腾讯服务器决定）
- **选项**: `--dsh-bin` · **默认**: 自动解析 · **说明**: DSH 可执行文件路径（找不到 `dsh` 时用）
- **选项**: `--timeout-ms` / `--max-turns` / `--max-history-chars` · **默认**: 同 bridge · **说明**: DSH 调用超时 / 记忆条数上限 / 记忆字符上限
...
### 类别 · 内容
- **类别**: **网络** · **内容**: 仅 HTTPS 访问 `ilinkai.weixin.qq.com`（登录/收发）与腾讯 CDN（媒体上传下载）；闭环测试时指向本地 mock。不访问其他任何公网地址。
- **类别**: **文件（读写）** · **内容**: `data/`（默认，含凭证、历史、会话注册表、工作目录、媒体）；`test-data/`（测试）。
- **类别**: **文件（只读）** · **内容**: `~/.dsh/sessions/`（`sessions` 命令读取 DSH 运行轨迹）。
- **类别**: **凭据** · **内容**: `weixin-auth.json` 含 bot token——即"以你的微信身份收发消息"的凭据，🔒 切勿提交版本控制或外传（`.gitignore` 已覆盖）。
- **类别**: **用户数据** · **内容**: 所有往来消息原文 + 时间戳存于 `data/history/`（本地明文 JSON）；媒体文件解密后存于 `data/media/`。数据只在你机器与腾讯通道之间流动，不发给任何第三方。
- **类别**: **执行权限** · **内容**: 任何能给你微信发消息的人都能触发本机 DSH 执行任务（等于你本机账号的操作权）→ 务必用 `--allow-from` 只放行自己的微信号，并维持 DSH 自身的沙箱/审批配置。
...
### 现象 · 原因与处理
- **现象**: 回复 `errcode -14` / 提示 token 失效 · **原因与处理**: 凭证过期：实现会自动重新扫码续连；也可手动 `dsh-weixin login`
- **现象**: 二维码过期 / 扫了没反应 · **原因与处理**: 重新 `dsh-weixin login`，并确认手机微信打开的是**最新**打印的链接
- **现象**: 收不到消息 · **原因与处理**: 确认 `run` 进程在跑；`/status` 看连接剩余时间；看终端有无 `[bridge …]` 报错
- **现象**: `dsh: command not found` · **原因与处理**: 安装 DSH，或用 `--dsh-bin` 指定可执行文件路径
- **现象**: 媒体下载/上传失败（如 `x-encrypted-param` 缺失） · **原因与处理**: 腾讯协议变更或端点被代理改写：升级本仓库、确认 `--base-url` 为官方地址
- **现象**: `bridge.mjs serve` 端口被占用 · **原因与处理**: `--port` 换端口
- **现象**: 查看 DSH 轨迹提示 `zstd` 不存在 · **原因与处理**: 安装 zstd，或改用 DSH WebUI 查看对应 session
- **现象**: 扫码成功后无法配对（节点跳转） · **原因与处理**: 网络环境问题：换网络重试；实现已覆盖节点跳转分支
