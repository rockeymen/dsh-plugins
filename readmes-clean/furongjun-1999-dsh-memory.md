...
- **AGI 缺失的能力**: 每次对话都"失忆"，没有跨会话的自我连续性 · **这是 AGI 的什么**: **自我连续性**（我是谁） · **灵枢提供**: **时空记忆图**：五层记忆（锚点/结构/知识/情境/自我）+ 跨会话 recall/search
- **AGI 缺失的能力**: 训练后权重冻结，不能随经历自主学习 · **这是 AGI 的什么**: **终身学习**（成长） · **灵枢提供**: **知识飞轮**：验证→归纳→联想→蒸馏→推演，随使用持续演化
- **AGI 缺失的能力**: 黑箱不可审计，无法验证行为边界 · **这是 AGI 的什么**: **可验证性**（可信） · **灵枢提供**: **可审计信任**：对抗护栏五规则 + 宪章 + 全量事件留痕 + 白箱智能
- **AGI 缺失的能力**: 只处理当下 token，没有稳定世界结构 · **这是 AGI 的什么**: **世界模型**（理解） · **灵枢提供**: **条件空间 + 语义时空图**：信息差 D_norm 驱动的预测与决策
- **AGI 缺失的能力**: 无自我表征，不能反思自己的认知/情绪 · **这是 AGI 的什么**: **自我认知**（元认知） · **灵枢提供**: **P0 系列**：cognition / self_reliability / emotional_bias / 递归反思
...
### 传统定位 · AGI 能力定位
- **传统定位**: DeepSeek Harness 插件 · **AGI 能力定位**: AGI 的长期记忆基础设施
- **传统定位**: 跨会话记忆 · **AGI 能力定位**: 智能体的**自我连续性**
- **传统定位**: 知识飞轮 · **AGI 能力定位**: 智能体的**自主学习与演化**
- **传统定位**: 可审计信任 · **AGI 能力定位**: 智能体的**可验证行为约束**
- **传统定位**: 时空记忆图 · **AGI 能力定位**: 智能体的**世界模型**
...
### 模块 · 工具
- **模块**: 记忆 · **工具**: remember / recall / search / timeline / session_note / session_recall / compact_context
- **模块**: 推理 · **工具**: think / relate / reason / predict_routes
- **模块**: 认知 · **工具**: self_check / gap_trend / cognition / cognition_report / emotional_bias / self_reliability / action_log / preflight
- **模块**: 反思 · **工具**: recursive_reflect
- **模块**: 学习 · **工具**: blindspots / learn / induce
- **模块**: 飞轮 · **工具**: distill / flywheel_report / transfer_test / calibrate
- **模块**: 摄取 · **工具**: ingest_text / ingest_file / ingest_url / web_search
- **模块**: 生命 · **工具**: step / lifecycle_state
- **模块**: 长期记忆门 · **工具**: longterm_snapshot / promote_memories（v1.15 主动沉淀）
- **模块**: 服务 · **工具**: service_info
...
### 字段 · 类型 · 默认值 · 说明
- **字段**: `serverName` · **类型**: string · **默认值**: `lingshu` · **说明**: 工具命名空间前缀（工具名 `lingshu_<name>`）
- **字段**: `python` · **类型**: string · **默认值**: `python` · **说明**: Python 可执行文件
- **字段**: `moduleArgs` · **类型**: string[] · **默认值**: `['-m', 'aeis.mcp.server']` · **说明**: 灵枢 server 启动参数
- **字段**: `dbPath` · **类型**: string · **默认值**: `data/lingshu.db` · **说明**: 记忆库 SQLite 路径（自动建目录）
- **字段**: `identity` · **类型**: string · **默认值**: `灵枢` · **说明**: 灵枢身份标识
- **字段**: `env` · **类型**: object · **默认值**: `{}` · **说明**: 追加环境变量（`BOCHA_API_KEY` / `AEIS_DESIGNER_KEY`…）
- **字段**: `tools` · **类型**: `'brain' \ · **默认值**: 'core' \ · **说明**: 'all' \ · string[]` · `'brain'` · 暴露的工具集合
- **字段**: `memory.userMessage` · **类型**: boolean · **默认值**: `true` · **说明**: 用户消息 → 自动 remember
- **字段**: `memory.assistantMessage` · **类型**: boolean · **默认值**: `false` · **说明**: agent 回复 → 自动 remember
- **字段**: `memory.toolResult` · **类型**: boolean · **默认值**: `false` · **说明**: 工具结果 → 自动 remember
- **字段**: `memory.importance` · **类型**: number · **默认值**: `0.6` · **说明**: 自动记忆的重要性（0~1）
- **字段**: `toolCallTimeoutMs` · **类型**: number · **默认值**: `60000` · **说明**: 单次工具调用超时
- **字段**: `maxRetryDelayMs` · **类型**: number · **默认值**: `30000` · **说明**: 进程重启最大退避间隔
- **字段**: `failOnStartupError` · **类型**: boolean · **默认值**: `false` · **说明**: 启动失败是否让插件激活失败
